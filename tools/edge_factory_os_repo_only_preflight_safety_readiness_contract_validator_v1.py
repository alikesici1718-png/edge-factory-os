...
MODULE_NAME = "edge_factory_os_repo_only_preflight_safety_readiness_contract_validator_v1"
CURRENT_TOOL_REL = "tools/edge_factory_os_repo_only_preflight_safety_readiness_contract_validator_v1.py"
OUT_DIR = LAB_ROOT / MODULE_NAME
EXPECTED_HEAD = "e226e5f"
EXPECTED_PREVIOUS_TRACKED_PYTHON_COUNT = 595
NEXT_MODULE = "edge_factory_os_repo_only_preflight_safety_readiness_review_v1.py"
POST_CHECK_STATUS = "REPO_ONLY_PREFLIGHT_SAFETY_READINESS_CONTRACT_VALIDATOR_POST_COMMIT_CHECK_PASS"
...
CONTRACT_ARTIFACT = LAB_ROOT / "edge_factory_os_repo_only_preflight_safety_readiness_contract_v1" / "repo_only_preflight_safety_readiness_contract_v1_latest.json"
...
def git_state() -> Dict[str, Any]:
    ...
    unexpected_untracked = [path for path in untracked if path != CURRENT_TOOL_REL]
    return {
        ...
        "repo_clean_for_preflight_safety_readiness_contract_validator": len(dirty_tracked) == 0 and len(unexpected_untracked) == 0,
    }
...
def contract_valid(contract_payload: Dict[str, Any]) -> bool:
    contract = contract_payload.get("preflight_safety_readiness_contract", {})
    blocked = contract.get("blocked_until_explicit_future_approval", [])
    requirements = contract.get("future_preflight_requirements", [])
    return (
        contract_payload.get("preflight_safety_readiness_contract_status") in {"READY", "PASS"}
        and contract_payload.get("preflight_safety_readiness_contract_ready") is True
        and contract_payload.get("preflight_safety_readiness_is_repo_only_planning") is True
        and contract_payload.get("contract_requires_kill_switch_readiness") is True
        and contract_payload.get("contract_requires_evidence_chain_compliance") is True
        ...
        and contract_payload.get("runtime_preflight_implementation_performed") is False
        and contract_payload.get("runtime_touch_performed") is False
        and contract_payload.get("capital_touch_performed") is False
        and contract_payload.get("live_touch_performed") is False
        and contract_payload.get("real_order_touch_performed") is False
        and contract_payload.get("next_module") == "edge_factory_os_repo_only_preflight_safety_readiness_contract_validator_v1.py"
        ...
        and contract.get("runtime_preflight_implementation_allowed") is False
        and contract.get("requires_kill_switch_readiness") is True
        and contract.get("requires_evidence_chain_compliance") is True
        and contract.get("missing_evidence_fails_closed") is True
        and "runtime path" in blocked
        and "live path" in blocked
        and "capital path" in blocked
        and "order path" in blocked
        and "schema or config creation" in blocked
        and any("kill-switch readiness" in str(item) for item in requirements)
        and any("evidence_chain_quality" in str(item) for item in requirements)
    )
...
def build_payload() -> Dict[str, Any]:
    ...
    replacement_checks = {
        ...
        "repo_clean_for_preflight_safety_readiness_contract_validator": git["repo_clean_for_preflight_safety_readiness_contract_validator"],
        ...
        "preflight_safety_readiness_contract_found": bool(contract_payload),
        "preflight_safety_readiness_contract_validated": contract_valid(contract_payload),
        "next_module_expected": NEXT_MODULE == "edge_factory_os_repo_only_preflight_safety_readiness_review_v1.py",
    ...
    payload: Dict[str, Any] = {
        ...
        "preflight_safety_readiness_contract_validator_status": "PASS" if ready else "FAIL_CLOSED",
        "post_check_status": POST_CHECK_STATUS if ready else "REPO_ONLY_PREFLIGHT_SAFETY_READINESS_CONTRACT_VALIDATOR_POST_COMMIT_CHECK_FAIL_CLOSED",
        "final_decision": "PREFLIGHT_SAFETY_READINESS_CONTRACT_VALIDATED" if ready else "PREFLIGHT_SAFETY_READINESS_CONTRACT_VALIDATOR_FAIL_CLOSED",
        "next_action": "BUILD_PREFLIGHT_SAFETY_READINESS_REVIEW_V1" if ready else "REVIEW_PREFLIGHT_SAFETY_READINESS_CONTRACT_INPUTS",
        "next_module": NEXT_MODULE if ready else None,
        "preflight_safety_readiness_contract_found": bool(contract_payload),
        "preflight_safety_readiness_contract_validated": ready,
        "contract_is_repo_only": True,
        "contract_is_planning_only": True,
        "preflight_safety_readiness_is_repo_only_planning": True,
        "contract_requires_kill_switch_readiness": True,
        "contract_requires_evidence_chain_compliance": True,
        ...
        "derived_live_repo_post_check_reason": "Preflight safety readiness contract validator uses the contract artifact plus live repo replacement checks; evidence remains DERIVED_OVERUSED_ATTENTION and weaker than primary artifact verification.",
        "replacement_checks_all_true": ready,
        "derived_live_repo_post_check_replacement_checks": replacement_checks,
        "validation": {"git_state": git, "tracked_python_validation": py, "contract_loaded": bool(contract_payload)},
...
def write_outputs(payload: Dict[str, Any]) -> Dict[str, str]:
    ...
    latest_json = OUT_DIR / "repo_only_preflight_safety_readiness_contract_validator_v1_latest.json"
    timestamped_json = OUT_DIR / f"repo_only_preflight_safety_readiness_contract_validator_v1_{stamp}.json"
    latest_txt = OUT_DIR / "repo_only_preflight_safety_readiness_contract_validator_v1_latest.txt"
...
def main() -> int:
    ...
    return 0 if payload["preflight_safety_readiness_contract_validator_status"] in {"READY", "PASS"} else 3
