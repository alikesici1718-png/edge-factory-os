...
MODULE_NAME = (
    "edge_factory_os_repo_only_historical_data_acquisition_okx_single_symbol_small_range_1m_to_1h_build_execution_"
    "after_preview_approval_v1"
)
CURRENT_TOOL_REL = (
    "tools/edge_factory_os_repo_only_historical_data_acquisition_okx_single_symbol_small_range_1m_to_1h_build_execution_"
    "after_preview_approval_v1.py"
)
OUT_DIR = LAB_ROOT / MODULE_NAME

EXPECTED_HEAD = "6c5ba99"
EXPECTED_TRACKED_PYTHON_COUNT_PRE_COMMIT = 691
EXPECTED_TRACKED_PYTHON_COUNT_POST_COMMIT = 692
...
NEXT_MODULE_PASS = (
    "edge_factory_os_repo_only_historical_data_acquisition_okx_single_symbol_small_range_1m_to_1h_build_execution_validator_"
...
BUILD_PREVIEW_DIR = (
    LAB_ROOT
    / "edge_factory_os_repo_only_historical_data_acquisition_okx_single_symbol_small_range_1m_to_1h_build_preview_after_download_validator_v1"
)
BUILD_PREVIEW_ARTIFACT = (
    BUILD_PREVIEW_DIR
    / "repo_only_historical_data_acquisition_okx_single_symbol_small_range_1m_to_1h_build_preview_after_download_validator_v1_latest.json"
)
DOWNLOAD_VALIDATOR_DIR = (
    LAB_ROOT
    / "edge_factory_os_repo_only_historical_data_acquisition_okx_single_symbol_small_range_download_execution_validator_after_execution_v1"
)
DOWNLOAD_VALIDATOR_ARTIFACT = (
    DOWNLOAD_VALIDATOR_DIR
    / "repo_only_historical_data_acquisition_okx_single_symbol_small_range_download_execution_validator_after_execution_v1_latest.json"
)
HASH_VALIDATION_REPORT_ARTIFACT = DOWNLOAD_VALIDATOR_DIR / "historical_okx_single_symbol_small_range_hash_validation_report.json"
ZIP_SCHEMA_VALIDATION_REPORT_ARTIFACT = (
    DOWNLOAD_VALIDATOR_DIR / "historical_okx_single_symbol_small_range_zip_schema_validation_report.json"
)
...
BUILD_PREVIEW_STATUS_PASS = (
    "PASS_HISTORICAL_DATA_ACQUISITION_OKX_SINGLE_SYMBOL_SMALL_RANGE_1M_TO_1H_BUILD_PREVIEW_APPROVED_"
    "EXECUTION_NEXT_NO_BUILD_YET"
)
...
STATUS_PASS = (
    "PASS_HISTORICAL_DATA_ACQUISITION_OKX_SINGLE_SYMBOL_SMALL_RANGE_1M_TO_1H_BUILD_EXECUTED_PENDING_VALIDATOR_"
    "PIPELINE_RANGE_ONLY"
)
...
EVIDENCE_BEFORE = (
    "HISTORICAL_DATA_ACQUISITION_OKX_SINGLE_SYMBOL_SMALL_RANGE_1M_TO_1H_BUILD_PREVIEW_APPROVED_EXECUTION_NEXT_"
    "NO_BUILD_YET"
)
EVIDENCE_AFTER = (
    "HISTORICAL_DATA_ACQUISITION_OKX_SINGLE_SYMBOL_SMALL_RANGE_1M_TO_1H_BUILD_EXECUTED_PENDING_VALIDATOR_"
    "PIPELINE_RANGE_ONLY"
)
...
EXPECTED_TOTAL_SOURCE_ROWS = 10080
MAX_SOURCE_ROWS_PER_FILE = 2000
MAX_TOTAL_SOURCE_ROWS = 14000
EXPECTED_OUTPUT_ROWS = 168
MAX_OUTPUT_ROWS = 168
EXPECTED_TOTAL_COMPLETE_OUTPUT_HOURS = 168
EXPECTED_SCHEMA = [
    "instrument_name",
    "open",
    "high",
...
def validate_required_artifacts() -> Dict[str, Dict[str, Any]]:
    paths = {
        ...
        "hash_validation": HASH_VALIDATION_REPORT_ARTIFACT,
        "zip_schema_validation": ZIP_SCHEMA_VALIDATION_REPORT_ARTIFACT,
...
def validate_preflight(artifacts: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    hash_validation = artifacts["hash_validation"]
    zip_schema = artifacts["zip_schema_validation"]
    execution = artifacts["execution_report"].get("execution_scope", {})
    policy_validator = artifacts["policy_validator"]
    policy = artifacts["policy"]

    require_equal(
        preview.get("historical_data_acquisition_okx_single_symbol_small_range_1m_to_1h_build_preview_bundle_status"),
        BUILD_PREVIEW_STATUS_PASS,
        "preview.status",
    )
    require_equal(preview.get("next_module"), REQUESTED_MODULE, "preview.next_module", STATUS_BLOCKED_NEXT_MODULE)
    require_true(preview.get("approval_grants_future_small_range_build_next"), "preview.future_build")
    require_false(preview.get("approval_grants_new_download_now"), "preview.new_download")
    require_equal(preview.get("future_build_scope_file_count"), EXPECTED_FILE_COUNT, "preview.file_count")
    require_equal(preview.get("future_build_scope_symbol_count"), 1, "preview.symbol_count")
    require_equal(preview.get("future_build_scope_day_count"), 7, "preview.day_count")
    require_true(preview.get("future_build_scope_limited_to_7_files_1_symbol_7_days"), "preview.scope_limited")
    require_equal(preview.get("target_symbol"), TARGET_SYMBOL, "preview.target_symbol")
    require_equal(preview.get("date_range_start"), DATE_RANGE_START, "preview.start")
    require_equal(preview.get("date_range_end"), DATE_RANGE_END, "preview.end")
    require_equal(preview.get("expected_total_source_rows"), EXPECTED_TOTAL_SOURCE_ROWS, "preview.source_rows")
    require_equal(preview.get("expected_output_rows"), EXPECTED_OUTPUT_ROWS, "preview.output_rows")
    ...
    require_false(preview.get("source_manifest_acquisition_ready"), "preview.source_ready")
    require_false(preview.get("broad_acquisition_execution_allowed_now"), "preview.broad")
    ...
    require_equal(preview.get("dormant_repo_attention_count"), 716, "preview.dormant")
    require_true(preview.get("replacement_checks_all_true"), "preview.replacement")
    validate_no_true_dangerous_flags(preview, "preview")

    require_true(validator.get("small_range_download_execution_validated"), "validator.validated")
    require_true(validator.get("approved_url_list_exact_match"), "validator.url_exact")
    require_equal(validator.get("downloaded_file_count"), EXPECTED_FILE_COUNT, "validator.file_count")
    require_true(validator.get("all_downloaded_zip_paths_exist"), "validator.paths")
    require_true(validator.get("all_hashes_match_recorded"), "validator.hashes")
    require_true(validator.get("all_zip_open_success"), "validator.zip_open")
    require_true(validator.get("all_expected_inner_csv_present"), "validator.csv_present")
    require_false(validator.get("any_zip_path_traversal_detected"), "validator.traversal")
    require_true(validator.get("all_expected_schema_match"), "validator.schema")
    require_true(validator.get("all_observed_symbols_match_target"), "validator.symbol")
    require_true(validator.get("all_one_minute_interval_observed_from_samples"), "validator.one_minute")
    require_false(validator.get("direct_1h_interval_present"), "validator.direct_1h")
    require_true(validator.get("safe_for_small_range_build_preview"), "validator.safe_build_preview")
    require_false(validator.get("safe_for_broad_acquisition"), "validator.broad")
    require_false(validator.get("safe_for_research_backtest"), "validator.research")
    require_false(validator.get("safe_for_edge_claim"), "validator.edge")
    require_false(validator.get("data_build_performed"), "validator.data_build")
    require_false(validator.get("aggregation_performed_now"), "validator.aggregation")
    require_false(validator.get("source_manifest_acquisition_ready"), "validator.source_ready")
    require_false(validator.get("broad_acquisition_execution_allowed_now"), "validator.broad_allowed")

    require_true(hash_validation.get("all_hashes_match_recorded"), "hash_validation.hashes")
    require_equal(hash_validation.get("validated_file_count"), EXPECTED_FILE_COUNT, "hash_validation.file_count")
    require_true(zip_schema.get("zip_inventory_validation", {}).get("all_zip_open_success"), "zip_schema.zip_open")
    require_true(zip_schema.get("zip_inventory_validation", {}).get("all_expected_inner_csv_present"), "zip_schema.csv_present")
    require_false(zip_schema.get("zip_inventory_validation", {}).get("any_zip_path_traversal_detected"), "zip_schema.traversal")
    require_true(zip_schema.get("schema_sample_validation", {}).get("all_expected_schema_match"), "zip_schema.schema")

    require_true(execution.get("small_range_download_execution_performed"), "execution.download")
    require_equal(execution.get("downloaded_file_count"), EXPECTED_FILE_COUNT, "execution.file_count")
    require_true(execution.get("no_data_build"), "execution.no_build")
    require_true(execution.get("no_aggregation"), "execution.no_aggregation")
    ...
    require_equal(policy.get("input_schema_policy", {}).get("expected_columns"), EXPECTED_SCHEMA, "policy.columns")
    ...
    require_false(policy.get("completeness_policy", {}).get("synthetic_fill_allowed"), "policy.synthetic")
    require_false(policy.get("completeness_policy", {}).get("forward_fill_allowed"), "policy.forward_fill")
    require_false(policy.get("completeness_policy", {}).get("backfill_allowed"), "policy.backfill")
    ...
    return {
        ...
        "hash_validation_report_artifact": str(HASH_VALIDATION_REPORT_ARTIFACT),
        "zip_schema_validation_report_artifact": str(ZIP_SCHEMA_VALIDATION_REPORT_ARTIFACT),
...
def parse_decimal(value: str, field: str, row_number: int, source_file: str) -> Decimal:
    try:
        parsed = Decimal(value)
    except InvalidOperation as exc:
        raise RuntimeError(f"{STATUS_BLOCKED_CONTEXT}: numeric parse failure {field} {source_file} row={row_number}") from exc
    if not parsed.is_finite():
        raise RuntimeError(f"{STATUS_BLOCKED_CONTEXT}: non-finite numeric value {field} {source_file} row={row_number}")
    return parsed


def parse_open_time(value: str, row_number: int, source_file: str) -> int:
    try:
        parsed = int(value)
    except ValueError as exc:
        raise RuntimeError(f"{STATUS_BLOCKED_CONTEXT}: open_time parse failure {source_file} row={row_number}") from exc
    return parsed
...
def expected_sources(hash_validation: Dict[str, Any]) -> List[Dict[str, Any]]:
    entries = hash_validation.get("hash_file_validation")
    if not isinstance(entries, list) or len(entries) != EXPECTED_FILE_COUNT:
        raise RuntimeError(f"{STATUS_BLOCKED_CONTEXT}: hash validation must contain 7 files")
    return entries


def validate_zip_and_read_rows(entry: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    zip_path = Path(str(entry.get("downloaded_zip_path", "")))
    if not (zip_path.exists() and zip_path.is_file()):
        raise RuntimeError(f"{STATUS_BLOCKED_CONTEXT}: source ZIP missing: {zip_path}")
    if recompute_sha256(zip_path) != entry.get("downloaded_zip_sha256_recorded"):
        raise RuntimeError(f"{STATUS_BLOCKED_CONTEXT}: source ZIP SHA256 mismatch before CSV read: {zip_path}")
    expected_inner_csv = str(entry.get("expected_inner_csv"))
    source_date = str(entry.get("date"))
    source_sha = str(entry.get("downloaded_zip_sha256_recorded"))
    with zipfile.ZipFile(zip_path, "r") as zf:
        names = zf.namelist()
        if len(names) > 10:
            raise RuntimeError(f"{STATUS_BLOCKED_CONTEXT}: too many ZIP members: {zip_path}")
        if any(zip_member_has_path_traversal(name) for name in names):
            raise RuntimeError(f"{STATUS_BLOCKED_CONTEXT}: ZIP path traversal detected: {zip_path}")
        if expected_inner_csv not in names:
            raise RuntimeError(f"{STATUS_BLOCKED_CONTEXT}: expected inner CSV missing: {expected_inner_csv}")
        rows: List[Dict[str, Any]] = []
        with zf.open(expected_inner_csv, "r") as raw:
            text = io.TextIOWrapper(raw, encoding="utf-8", newline="")
            reader = csv.DictReader(text)
            if reader.fieldnames != EXPECTED_SCHEMA:
                raise RuntimeError(f"{STATUS_BLOCKED_CONTEXT}: CSV schema mismatch: {expected_inner_csv}")
            for row_number, row in enumerate(reader, start=1):
                if row_number > MAX_SOURCE_ROWS_PER_FILE:
                    raise RuntimeError(f"{STATUS_BLOCKED_CONTEXT}: source rows exceed per-file limit: {expected_inner_csv}")
                symbol = row["instrument_name"]
                open_time = parse_open_time(row["open_time"], row_number, expected_inner_csv)
                open_value = parse_decimal(row["open"], "open", row_number, expected_inner_csv)
                high_value = parse_decimal(row["high"], "high", row_number, expected_inner_csv)
                low_value = parse_decimal(row["low"], "low", row_number, expected_inner_csv)
                close_value = parse_decimal(row["close"], "close", row_number, expected_inner_csv)
                vol = parse_decimal(row["vol"], "vol", row_number, expected_inner_csv)
                vol_ccy = parse_decimal(row["vol_ccy"], "vol_ccy", row_number, expected_inner_csv)
                vol_quote = parse_decimal(row["vol_quote"], "vol_quote", row_number, expected_inner_csv)
                if high_value < max(open_value, close_value, low_value):
                    raise RuntimeError(f"{STATUS_BLOCKED_CONTEXT}: invalid high {expected_inner_csv} row={row_number}")
                if low_value > min(open_value, close_value, high_value):
                    raise RuntimeError(f"{STATUS_BLOCKED_CONTEXT}: invalid low {expected_inner_csv} row={row_number}")
                if vol < 0 or vol_ccy < 0 or vol_quote < 0:
                    raise RuntimeError(f"{STATUS_BLOCKED_CONTEXT}: negative volume {expected_inner_csv} row={row_number}")
                rows.append(
                    {
    ...
    if not all(open_times[idx] < open_times[idx + 1] for idx in range(len(open_times) - 1)):
        raise RuntimeError(f"{STATUS_BLOCKED_CONTEXT}: open_time non-monotonic within file: {expected_inner_csv}")
    duplicate_count = sum(count - 1 for count in Counter(open_times).values() if count > 1)
    if duplicate_count:
        raise RuntimeError(f"{STATUS_BLOCKED_CONTEXT}: duplicate open_time within file: {expected_inner_csv}")
    return rows, {
        "date": source_date,
        "source_csv_file": expected_inner_csv,
        "source_zip_path": str(zip_path),
        "source_url": str(entry.get("source_url")),
        "source_zip_sha256": source_sha,
        "source_zip_size_bytes": entry.get("downloaded_zip_size_bytes"),
        "source_row_count": len(rows),
        "open_time_monotonic": True,
        "duplicate_open_time_count": duplicate_count,
    }


def analyze_all_rows(rows: List[Dict[str, Any]], file_metadata: List[Dict[str, Any]]) -> Dict[str, Any]:
    if len(file_metadata) != EXPECTED_FILE_COUNT:
        raise RuntimeError(f"{STATUS_BLOCKED_CONTEXT}: wrong file count processed")
    if len(rows) > MAX_TOTAL_SOURCE_ROWS:
        raise RuntimeError(f"{STATUS_BLOCKED_CONTEXT}: total rows exceed limit")
    if len(rows) != EXPECTED_TOTAL_SOURCE_ROWS:
        raise RuntimeError(f"{STATUS_BLOCKED_CONTEXT}: source_row_count_total={len(rows)} expected {EXPECTED_TOTAL_SOURCE_ROWS}")
    ...
    rows_sorted = sorted(rows, key=lambda item: item["open_time"])
    open_times = [row["open_time"] for row in rows_sorted]
    duplicate_count = sum(count - 1 for count in Counter(open_times).values() if count > 1)
    if duplicate_count:
    ...
    bad_deltas = [delta for delta in deltas if delta != 60000]
    ...
    source_row_count_by_file = {item["source_csv_file"]: item["source_row_count"] for item in file_metadata}
    return {
        "source_row_count_total": len(rows),
        "expected_total_source_rows": EXPECTED_TOTAL_SOURCE_ROWS,
        "source_row_count_by_file": source_row_count_by_file,
        "source_row_count_by_file_valid": all(count == 1440 for count in source_row_count_by_file.values()),
        "unique_symbol_count": len(symbols),
        "observed_symbol": symbols[0],
        "open_time_monotonic_by_file": all(item["open_time_monotonic"] for item in file_metadata),
        "duplicate_open_time_count_total": duplicate_count,
        ...
        "daily_boundary_interpretation": (
            ...
            f"{iso_utc_from_ms(open_times[0])} through {iso_utc_from_ms(open_times[-1])}"
        ),
    }
...
def aggregate_rows(rows: List[Dict[str, Any]]) -> List[Dict[str, str]]:
    ...
    for hour_start in sorted(grouped):
        ...
        source_row_count = len({item["open_time"] for item in group})
        if source_row_count != 60 or len(group) != 60:
            raise RuntimeError(f"{STATUS_BLOCKED_CONTEXT}: incomplete hour encountered")
        source_dates = sorted({item["source_date"] for item in group})
        source_csvs = sorted({item["source_csv_file"] for item in group})
        source_shas = sorted({item["source_zip_sha256"] for item in group})
        output.append(
            {
                ...
                "source_zip_sha256": "|".join(source_shas),
    ...
    if len(output) != EXPECTED_OUTPUT_ROWS:
        raise RuntimeError(f"{STATUS_BLOCKED_CONTEXT}: output row count={len(output)} expected {EXPECTED_OUTPUT_ROWS}")
    return output


def write_output_csv(rows: List[Dict[str, str]]) -> Path:
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    output_path = (OUT_DIR / "historical_okx_single_symbol_small_range_1m_to_1h_output.csv").resolve()
...
def build_reports(
    ...
    aggregation = {
        ...
        "expected_total_complete_output_hours": EXPECTED_TOTAL_COMPLETE_OUTPUT_HOURS,
        "all_hours_complete": incomplete_count == 0 and complete_count == EXPECTED_TOTAL_COMPLETE_OUTPUT_HOURS,
    ...
    provenance = {
        ...
        "provenance_status": "SEVEN_FILE_SINGLE_SYMBOL_PIPELINE_RANGE_VALIDATION_BUILD_OUTPUT",
    }
    compliance = {
    ...
    execution_scope = {
        "small_range_build_execution_performed": True,
        "build_scope": BUILD_SCOPE,
        ...
        "multi_file_processing_limited_to_approved_7_files": True,
...
def build_payload(
    ...
    replacement_checks = {
        ...
        "seven_files_processed": input_validation["file_count_processed"] == EXPECTED_FILE_COUNT,
        ...
        "output_rows_expected": aggregation["output_1h_row_count"] == EXPECTED_OUTPUT_ROWS,
    ...
    payload: Dict[str, Any] = {
        ...
        "historical_data_acquisition_okx_single_symbol_small_range_1m_to_1h_build_execution_status": STATUS_PASS,
        "final_decision": EVIDENCE_AFTER,
        "next_action": "VALIDATE_SINGLE_SYMBOL_SMALL_RANGE_1M_TO_1H_BUILD_EXECUTION_PIPELINE_RANGE_ONLY",
        ...
        **aggregation,
        ...
        "derived_live_repo_post_check_reason": (
            "built exactly one BTC-USDT-SWAP seven-day 1h pipeline-range-validation output from the seven already-"
            "validated ZIP files; reconfirmed every SHA256 before reading, read exactly seven expected CSVs, validated "
            "10080 monotonic unique 1m rows, produced 168 complete UTC 1h rows with no synthetic fill, and performed "
            "no new download/API/browse, no multi-symbol work, and no research/backtest/edge/runtime/capital/live/"
            "schema/config/generic-runner action"
        ),
...
def write_artifacts(payload: Dict[str, Any], reports: Dict[str, Dict[str, Any]]) -> None:
    execution_report = {
        "generated_at_utc": utc_now(),
        "execution_scope": reports["execution_scope"],
        "input_validation": {
            key: payload[key]
            for key in (
        ...
        "gap_duplicate_report": reports["gap_duplicate"],
        "provenance_report": reports["provenance"],
        "compliance_report": reports["compliance"],
        "next_module_decision": {"next_module": payload["next_module"], "next_action": payload["next_action"]},
    }
    outputs = {
        "historical_okx_single_symbol_small_range_1m_to_1h_build_execution_report.json": execution_report,
        "historical_okx_single_symbol_small_range_1m_to_1h_gap_duplicate_report.json": reports["gap_duplicate"],
        "historical_okx_single_symbol_small_range_1m_to_1h_schema_validation_report.json": reports["schema"],
        "historical_okx_single_symbol_small_range_1m_to_1h_output_provenance_report.json": reports["provenance"],
        "historical_okx_single_symbol_small_range_1m_to_1h_build_execution_compliance_report.json": reports["compliance"],
        "historical_okx_single_symbol_small_range_1m_to_1h_build_execution_summary.json": payload,
        "repo_only_historical_data_acquisition_okx_single_symbol_small_range_1m_to_1h_build_execution_after_preview_approval_v1_latest.json": payload,
    }
...
def blocked_payload(exc: Exception) -> Dict[str, Any]:
    ...
    return {
        "module_name": MODULE_NAME,
        "generated_at_utc": utc_now(),
        "historical_data_acquisition_okx_single_symbol_small_range_1m_to_1h_build_execution_status": STATUS_BLOCKED_CONTEXT,
        "final_decision": STATUS_BLOCKED_CONTEXT,
        "next_action": "STOP_FAIL_CLOSED_NO_SMALL_RANGE_1M_TO_1H_BUILD_OUTPUT",
        ...
        "small_range_build_execution_performed": False,
        "build_scope": BUILD_SCOPE,
        "target_symbol": TARGET_SYMBOL,
        "date_range_start": DATE_RANGE_START,
        "date_range_end": DATE_RANGE_END,
        ...
        "all_source_zips_exist": False,
        "all_source_zip_sha256_match": False,
...
def main() -> int:
    try:
        ...
        file_metadata: List[Dict[str, Any]] = []
        for entry in expected_sources(artifacts["hash_validation"]):
            rows, metadata = validate_zip_and_read_rows(entry)
            all_rows.extend(rows)
            file_metadata.append(metadata)
        input_validation = analyze_all_rows(all_rows, file_metadata)
        input_validation["file_count_processed"] = len(file_metadata)
        output_rows = aggregate_rows(all_rows)
        output_path = write_output_csv(output_rows)
        reports = build_reports(preflight, input_validation, file_metadata, output_rows, output_path)
        payload = build_payload(preflight, input_validation, reports)
        ...
        write_json(
            OUT_DIR / "repo_only_historical_data_acquisition_okx_single_symbol_small_range_1m_to_1h_build_execution_after_preview_approval_v1_latest.json",
            payload,
        )
        write_json(OUT_DIR / "historical_okx_single_symbol_small_range_1m_to_1h_build_execution_summary.json", payload)
        print(json.dumps(payload, indent=2, sort_keys=True))
        return 1
