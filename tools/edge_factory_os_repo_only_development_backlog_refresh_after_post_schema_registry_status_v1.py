...
MODULE_NAME = "edge_factory_os_repo_only_development_backlog_refresh_after_post_schema_registry_status_v1"
OUT_DIR = LAB_ROOT / "edge_factory_os_repo_only_development_backlog_refresh_after_post_schema_registry_status_v1"
OUT_DIR.mkdir(parents=True, exist_ok=True)

EXPECTED_HEAD_PREFIX = "220668d"
CURRENT_TOOL_REL = "tools/edge_factory_os_repo_only_development_backlog_refresh_after_post_schema_registry_status_v1.py"
...
QUEUE_POST_JSON = LAB_ROOT / "edge_factory_os_repo_only_development_queue_selector_after_post_schema_registry_status_post_commit_check" / "repo_only_development_queue_selector_after_post_schema_registry_status_post_commit_check_latest.json"
...
SELECTED_NEXT_MODULE = "edge_factory_os_repo_only_next_action_selector_after_post_schema_registry_backlog_refresh_v1.py"
...
SAFETY_FLAGS: Dict[str, bool] = {
    "repo_only_development_backlog_refresh_after_post_schema_registry_status": True,
    "read_only_backlog_refresh_only": True,
    "development_queue_selector_after_post_schema_registry_status_required": True,
    "next_action_selector_after_post_schema_registry_backlog_refresh_allowed_next": True,
...
def validate_inputs() -> Dict[str, Any]:
    ...
    if subject != "Add repo-only development queue selector after post-schema-registry status":
        errors.append(f"unexpected HEAD commit subject: {subject}")

    paths = commit_paths("HEAD")
    checks["head_commit_paths"] = paths
    if paths != ["tools/edge_factory_os_repo_only_development_queue_selector_after_post_schema_registry_status_v1.py"]:
    ...
    inputs = {
        "development_queue_selector": (
            ...
            "edge_factory_os_repo_only_development_backlog_refresh_after_post_schema_registry_status_v1.py",
        ),
        "development_queue_selector_post_check": (
            QUEUE_POST_JSON,
            "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_STATUS_POST_COMMIT_CHECK_PASS",
            "edge_factory_os_repo_only_development_backlog_refresh_after_post_schema_registry_status_v1.py",
        ),
...
def build_backlog(validation: Dict[str, Any]) -> Dict[str, Any]:
    ...
    selector = as_dict(selector_obj.get("next_action_selector_after_post_schema_registry_status", {}))
    selector_action = as_dict(selector.get("selected_action", {}))
    ...
    prerequisites = {
        ...
        "queue_selected_this_backlog_refresh": selected_task.get("module") == "edge_factory_os_repo_only_development_backlog_refresh_after_post_schema_registry_status_v1.py",
        "queue_selected_repo_only_scope": selected_task.get("scope") == "REPO_ONLY_OS_INTELLIGENCE",
        "parent_selector_ready": selector_obj.get("next_action_selector_after_post_schema_registry_status_status") == "REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_STATUS_V1_READY",
        "parent_selector_selected_queue": selector_action.get("module") == "edge_factory_os_repo_only_development_queue_selector_after_post_schema_registry_status_v1.py",
    ...
    backlog_items = [
        {
            "rank": 1,
            "key": "POST_SCHEMA_REGISTRY_BACKLOG_NEXT_ACTION_SELECTOR",
            "module": SELECTED_NEXT_MODULE,
            "scope": "REPO_ONLY_OS_INTELLIGENCE",
            "allowed": True,
            "reason": "Backlog refresh is complete; select the next repo-only governance/intelligence action without schema apply.",
            "blocked_actions": [
        ...
        {
            "rank": 3,
            "key": "FRAMEWORK_SCHEMA_REGISTRY_APPLY_APPROVAL_REQUEST",
            "module": None,
            "scope": "BLOCKED_USER_MANUAL_APPROVAL_REQUIRED",
            "allowed": False,
            "reason": "No automatic approval request or apply path is selected; explicit future approval text is required.",
            "blocked_actions": ["schema_file_creation", "schema_file_edit", "schema_apply"],
        },
        {
            "rank": 4,
    ...
    return {
        "development_backlog_status": "DEVELOPMENT_BACKLOG_AFTER_POST_SCHEMA_REGISTRY_STATUS_ACTIVE" if selected else "DEVELOPMENT_BACKLOG_AFTER_POST_SCHEMA_REGISTRY_STATUS_BLOCKED",
        ...
        "selected_next_action": "BUILD_REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_BACKLOG_REFRESH_V1" if selected else None,
        "selected_scope": selected["scope"] if selected else None,
        "prerequisites": prerequisites,
        "context_summary": {
            "queue_selected_module": selected_task.get("module"),
            "queue_selected_scope": selected_task.get("scope"),
            "parent_selector_selected_module": selector_action.get("module"),
            ...
            "approval_state_source_order": "status_summary.approval_state -> status_summary flat -> queue_context -> manual approval record",
            "schema_state_source_order": "status_summary.schema_action_state -> status_summary flat -> queue_context -> manual approval record",
        ...
        "invariants": {
            ...
            "selected_module_is_next_action_selector_after_post_schema_registry_backlog_refresh": selected is not None and selected["module"] == SELECTED_NEXT_MODULE,
...
def main() -> int:
    ...
    payload = {
        "module": MODULE_NAME,
        "development_backlog_refresh_after_post_schema_registry_status_status": "REPO_ONLY_DEVELOPMENT_BACKLOG_REFRESH_AFTER_POST_SCHEMA_REGISTRY_STATUS_V1_READY" if passed else "REPO_ONLY_DEVELOPMENT_BACKLOG_REFRESH_AFTER_POST_SCHEMA_REGISTRY_STATUS_V1_BLOCKED",
        "severity": "OK" if passed else "BLOCKED",
        "allowed_scope": "REPO_ONLY_OS_INTELLIGENCE",
        "final_decision": "DEVELOPMENT_BACKLOG_REFRESH_READY_FOR_NEXT_ACTION_SELECTOR" if passed else "KEEP_FREEZE_REVIEW_DEVELOPMENT_BACKLOG_AFTER_POST_SCHEMA_REGISTRY_STATUS_ERRORS",
        "next_action": "BUILD_REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_BACKLOG_REFRESH_V1" if passed else "REVIEW_DEVELOPMENT_BACKLOG_AFTER_POST_SCHEMA_REGISTRY_STATUS_ERRORS",
        "next_module": SELECTED_NEXT_MODULE if passed else None,
        "reason": "Development backlog refreshed; next-action selector selected, schema apply/create/edit remain blocked and physically absent." if passed else "Development backlog refresh after post-schema-registry status validation failed.",
        ...
        "development_backlog_refresh_after_post_schema_registry_status": backlog,
    ...
    latest_json = OUT_DIR / "repo_only_development_backlog_refresh_after_post_schema_registry_status_v1_latest.json"
    timestamped_json = OUT_DIR / f"repo_only_development_backlog_refresh_after_post_schema_registry_status_v1_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json"
    latest_txt = OUT_DIR / "repo_only_development_backlog_refresh_after_post_schema_registry_status_v1_latest.txt"

    latest_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    timestamped_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")

    lines = [
        "EDGE FACTORY OS REPO-ONLY DEVELOPMENT BACKLOG REFRESH AFTER POST-SCHEMA-REGISTRY STATUS v1",
        "=" * 100,
        f"development_backlog_refresh_after_post_schema_registry_status_status: {payload['development_backlog_refresh_after_post_schema_registry_status_status']}",
