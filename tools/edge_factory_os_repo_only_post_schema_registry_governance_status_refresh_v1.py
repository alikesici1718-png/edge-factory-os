...
MODULE_NAME = "edge_factory_os_repo_only_post_schema_registry_governance_status_refresh_v1"
OUT_DIR = LAB_ROOT / "edge_factory_os_repo_only_post_schema_registry_governance_status_refresh_v1"
OUT_DIR.mkdir(parents=True, exist_ok=True)

EXPECTED_HEAD_PREFIX = "a4181b7"
CURRENT_TOOL_REL = "tools/edge_factory_os_repo_only_post_schema_registry_governance_status_refresh_v1.py"

QUEUE_JSON = LAB_ROOT / "edge_factory_os_repo_only_development_queue_selector_after_schema_registry_closure_record_v1" / "repo_only_development_queue_selector_after_schema_registry_closure_record_v1_latest.json"
QUEUE_POST_JSON = LAB_ROOT / "edge_factory_os_repo_only_development_queue_selector_after_schema_registry_closure_record_post_commit_check" / "repo_only_development_queue_selector_after_schema_registry_closure_record_post_commit_check_latest.json"
SELECTOR_JSON = LAB_ROOT / "edge_factory_os_repo_only_next_action_selector_after_schema_registry_closure_record_v1" / "repo_only_next_action_selector_after_schema_registry_closure_record_v1_latest.json"
...
CLOSURE_POST_JSON = LAB_ROOT / "edge_factory_os_repo_only_schema_registry_governance_chain_closure_record_post_commit_check" / "repo_only_schema_registry_governance_chain_closure_record_post_commit_check_latest.json"
...
SELECTED_NEXT_MODULE = "edge_factory_os_repo_only_next_action_selector_after_post_schema_registry_governance_status_refresh_v1.py"
...
SAFETY_FLAGS: Dict[str, bool] = {
    "repo_only_post_schema_registry_governance_status_refresh": True,
    "read_only_status_refresh_only": True,
    "development_queue_selector_after_schema_registry_closure_record_required": True,
    "next_action_selector_after_post_schema_registry_governance_status_refresh_allowed_next": True,
...
def first_status(obj: Dict[str, Any]) -> Optional[str]:
    for key in [
        "development_queue_selector_after_schema_registry_closure_record_status",
        "audit_status",
        "next_action_selector_after_schema_registry_closure_record_status",
...
def validate_inputs() -> Dict[str, Any]:
    ...
    if git_state["dirty_tracked_count"] != 0:
        errors.append(f"dirty tracked paths present before governance status refresh: {git_state['dirty_tracked_paths']}")

    if actual_untracked != expected_untracked:
        errors.append(f"unexpected untracked set before governance status refresh: expected={sorted(expected_untracked)} actual={sorted(actual_untracked)}")

    if physical_before["planned_schema_file_existing_count"] != 0:
        errors.append(f"planned schema files existed before governance status refresh: {physical_before['existing_planned_schema_files']}")

    subject = commit_subject("HEAD")
    checks["head_commit_subject"] = subject
    if subject != "Add repo-only development queue selector after schema registry closure record":
        errors.append(f"unexpected HEAD commit subject: {subject}")

    paths = commit_paths("HEAD")
    checks["head_commit_paths"] = paths
    if paths != ["tools/edge_factory_os_repo_only_development_queue_selector_after_schema_registry_closure_record_v1.py"]:
        errors.append(f"unexpected HEAD commit paths: {paths}")

    inputs = {
        "development_queue_selector_after_closure": (
            QUEUE_JSON,
            "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_SCHEMA_REGISTRY_CLOSURE_RECORD_V1_READY",
            "edge_factory_os_repo_only_post_schema_registry_governance_status_refresh_v1.py",
        ),
        "development_queue_selector_after_closure_post_check": (
            QUEUE_POST_JSON,
            "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_SCHEMA_REGISTRY_CLOSURE_RECORD_POST_COMMIT_CHECK_PASS",
            "edge_factory_os_repo_only_post_schema_registry_governance_status_refresh_v1.py",
        ),
        "next_action_selector_after_closure": (
            SELECTOR_JSON,
            "REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_SCHEMA_REGISTRY_CLOSURE_RECORD_V1_READY",
            None,
        ),
        ...
        "schema_registry_closure_record_post_check": (
            CLOSURE_POST_JSON,
            "REPO_ONLY_SCHEMA_REGISTRY_GOVERNANCE_CHAIN_CLOSURE_RECORD_POST_COMMIT_CHECK_PASS",
            None,
        ),
...
def build_status_refresh(validation: Dict[str, Any]) -> Dict[str, Any]:
    queue_obj = validation["loaded"]["development_queue_selector_after_closure"]
    queue = as_dict(queue_obj.get("development_queue_selector_after_schema_registry_closure_record", {}))
    selected_task = as_dict(queue.get("selected_development_task", {}))
    queue_context = as_dict(queue.get("context_summary", {}))

    selector_obj = validation["loaded"]["next_action_selector_after_closure"]
    selector = as_dict(selector_obj.get("next_action_selector_after_schema_registry_closure_record", {}))
    ...
    closure_policy = as_dict(closure.get("closure_policy", {}))
    closure_next_step = as_dict(closure.get("next_step", {}))
    ...
    prerequisites = {
        "queue_ready": queue_obj.get("development_queue_selector_after_schema_registry_closure_record_status") == "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_SCHEMA_REGISTRY_CLOSURE_RECORD_V1_READY",
        "queue_selected_this_refresh": selected_task.get("module") == "edge_factory_os_repo_only_post_schema_registry_governance_status_refresh_v1.py",
        "queue_selected_repo_only_scope": selected_task.get("scope") == "REPO_ONLY_OS_INTELLIGENCE",
        "selector_ready": selector_obj.get("next_action_selector_after_schema_registry_closure_record_status") == "REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_SCHEMA_REGISTRY_CLOSURE_RECORD_V1_READY",
        "selector_selected_queue": selected_action.get("module") == "edge_factory_os_repo_only_development_queue_selector_after_schema_registry_closure_record_v1.py",
        "closure_ready": closure_obj.get("schema_registry_governance_chain_closure_record_status") == "REPO_ONLY_SCHEMA_REGISTRY_GOVERNANCE_CHAIN_CLOSURE_RECORD_V1_READY",
        "closure_next_step_selected_selector": closure_next_step.get("module") == "edge_factory_os_repo_only_next_action_selector_after_schema_registry_closure_record_v1.py",
        "schema_registry_chain_closed_no_apply": closure_decision.get("schema_registry_apply_closed_now") is True and closure_policy.get("schema_apply_stays_blocked_without_exact_manual_approval") is True,
    ...
    governance_status = {
        "schema_registry_governance_chain_closed": True,
        "schema_registry_apply_allowed_now": effective_schema_state.get("schema_apply_allowed_now"),
        ...
        "schema_registry_apply_performed_now": False,
        "schema_file_creation_performed_now": False,
        "schema_file_edit_performed_now": False,
        ...
        "general_repo_only_flow_resumed": True,
        "next_general_repo_only_selector_required": True,
        "runtime_capital_live_remain_blocked": True,
        "strategy_research_candidate_family_remain_blocked": True,
    }

    return {
        "status_refresh_status": "POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_READY" if all(prerequisites.values()) else "POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_BLOCKED",
        "governance_status": governance_status,
        "selected_next_step": {
            "step": "BUILD_REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_V1",
            "module": SELECTED_NEXT_MODULE,
            "scope": "REPO_ONLY_OS_INTELLIGENCE",
            "reason": "Governance status has been refreshed after schema-registry chain closure; select next repo-only task.",
        },
        "context_summary": {
            "queue_selected_module": selected_task.get("module"),
            "selector_selected_module": selected_action.get("module"),
            "closure_status": closure_obj.get("schema_registry_governance_chain_closure_record_status"),
            "closure_decision": closure_decision,
            "closure_policy": closure_policy,
            "manual_approval_record": manual_record,
            "effective_approval_state": effective_approval_state,
            "effective_schema_state": effective_schema_state,
            "queue_context": queue_context,
            "selector_context": selector_context,
            "closure_chain_summary": closure_chain_summary,
            "approval_state_source_order": "queue_context -> selector_context -> closure_chain_summary -> manual approval record",
            "schema_state_source_order": "queue_context -> selector_context -> closure_decision -> closure_chain_summary -> manual approval record",
        ...
        "invariants": {
            ...
            "selected_next_module_is_next_action_selector": SELECTED_NEXT_MODULE == "edge_factory_os_repo_only_next_action_selector_after_post_schema_registry_governance_status_refresh_v1.py",
            "schema_registry_chain_closed_no_apply": governance_status["schema_registry_governance_chain_closed"] is True,
            "schema_apply_allowed_false": governance_status["schema_registry_apply_allowed_now"] is False,
            "schema_file_creation_allowed_false": governance_status["schema_file_creation_allowed_now"] is False,
            "schema_file_edit_allowed_false": governance_status["schema_file_edit_allowed_now"] is False,
            "manual_approval_absent": governance_status["manual_approval_present"] is False,
            "manual_approval_invalid": governance_status["manual_approval_valid"] is False,
            "approval_granted_false": governance_status["approval_granted_now"] is False,
            "planned_schema_files_absent_before": validation["physical_before"]["planned_schema_file_existing_count"] == 0,
            "planned_schema_files_absent_after": physical_after["planned_schema_file_existing_count"] == 0,
            "schema_apply_not_performed": physical_after["schema_apply_performed_count"] == 0,
            "schema_file_creation_not_performed": physical_after["schema_file_creation_performed_count"] == 0,
            ...
            "no_repo_file_creation_or_modification_performed_by_status_refresh": True,
        },
    }
...
def main() -> int:
    ...
    status_refresh = build_status_refresh(validation)

    for key, value in status_refresh["invariants"].items():
    ...
    payload = {
        "module": MODULE_NAME,
        "post_schema_registry_governance_status_refresh_status": "REPO_ONLY_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_V1_READY" if passed else "REPO_ONLY_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_V1_BLOCKED",
        "severity": "OK" if passed else "BLOCKED",
        "allowed_scope": "REPO_ONLY_OS_INTELLIGENCE",
        "final_decision": "POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_READY_FOR_NEXT_ACTION_SELECTOR" if passed else "KEEP_FREEZE_REVIEW_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_ERRORS",
        "next_action": "BUILD_REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_V1" if passed else "REVIEW_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_ERRORS",
        "next_module": SELECTED_NEXT_MODULE if passed else None,
        "reason": "Post-schema-registry governance status refreshed; schema apply/create/edit remain blocked and physically absent." if passed else "Post-schema-registry governance status refresh validation failed.",
        ...
        "post_schema_registry_governance_status_refresh": status_refresh,
        ...
        "physical_planned_schema_file_existing_count_before": status_refresh["physical_guards"]["before"]["planned_schema_file_existing_count"],
        "physical_planned_schema_file_existing_count_after": status_refresh["physical_guards"]["after"]["planned_schema_file_existing_count"],
    }

    latest_json = OUT_DIR / "repo_only_post_schema_registry_governance_status_refresh_v1_latest.json"
    timestamped_json = OUT_DIR / f"repo_only_post_schema_registry_governance_status_refresh_v1_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json"
    latest_txt = OUT_DIR / "repo_only_post_schema_registry_governance_status_refresh_v1_latest.txt"

    latest_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    timestamped_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")

    lines = [
        "EDGE FACTORY OS REPO-ONLY POST-SCHEMA-REGISTRY GOVERNANCE STATUS REFRESH v1",
        "=" * 100,
        f"post_schema_registry_governance_status_refresh_status: {payload['post_schema_registry_governance_status_refresh_status']}",
        ...
        json.dumps(
            {
                "governance_status": status_refresh["governance_status"],
                "selected_next_step": status_refresh["selected_next_step"],
                "context_summary": status_refresh["context_summary"],
                "physical_guards": status_refresh["physical_guards"],
                "prerequisites": status_refresh["prerequisites"],
                "status_policy": status_refresh["status_policy"],
                "invariants": status_refresh["invariants"],
            },
            indent=2,
            sort_keys=True,
        ),
