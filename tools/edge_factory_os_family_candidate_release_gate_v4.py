...
MODULE = "edge_factory_os_family_candidate_release_gate_v4"
...
LESSON_CHECKER_LATEST = (
    BASE_DIR / "edge_factory_os_lesson_memory_checker_v1" / "lesson_memory_checker_latest.json"
)
...
RUN_DIR = OUT_ROOT / f"family_candidate_release_gate_v4_{NOW.strftime('%Y%m%d_%H%M%S')}"
LATEST_JSON = OUT_ROOT / "family_candidate_release_gate_v4_latest.json"
LATEST_MD = OUT_ROOT / "family_candidate_release_gate_v4_latest.md"
...
def make_check(pass_value: bool, status: str, evidence: Dict[str, Any], block_if_fail: bool = True) -> Dict[str, Any]:
    return {
        "pass": bool(pass_value),
        "status": status,
        "evidence": evidence,
        "block_if_fail": bool(block_if_fail),
    }
...
def main() -> int:
    ...
    if lesson_checker is None:
        attention.append("lesson_memory_checker_latest_missing")
        lesson_checker = {}
    ...
    checks["FULL_UNIVERSE_OFFLINE_BACKTEST_PASS"] = make_check(
        ...
        str(full_feed.get("status") or full_eval.get("evaluator_status")),
        {
            ...
            "interpretation": "Full-universe test ran on 285-symbol 1Y panel and produced zero release-quality candidates.",
        },
    )
    ...
    checks["OOS_OR_ROLLING_VALIDATION_PASS"] = make_check(
        ...
        "NOT_PASSED_BECAUSE_FULL_UNIVERSE_CANDIDATE_FAILED",
        {
            ...
            "note": "No candidate survived full-universe release criteria; rolling/OOS cannot rescue this route without a surviving candidate.",
        },
    )
    ...
    checks["COST_SLIPPAGE_SENSITIVITY_PASS"] = make_check(
        ...
        str(cost_feed.get("status") or cost_eval.get("evaluator_status")),
        {
            ...
            "interpretation": "Cost sensitivity ran but edge was not recovered.",
        },
    )
    ...
    checks["REGIME_BUCKET_DIAGNOSTIC_PASS"] = make_check(
        ...
        str(regime_feed.get("status") or regime_eval.get("evaluator_status")),
        {
            ...
            "interpretation": "Regime diagnostic was promising but not release-pass.",
        },
    )
    ...
    checks["SYMBOL_CONCENTRATION_RISK_PASS"] = make_check(
        ...
        "SYMBOL_CONCENTRATION_RISK_NOT_CLEARED",
        {
            ...
            "interpretation": "Symbol concentration and route instability are not cleared.",
        },
    )
    ...
    sample_pass = bool((best_all_count or 0) >= 300 and (best_oos_count or 0) >= 75)

    checks["SAMPLE_SIZE_MINIMUM_PASS"] = make_check(
        sample_pass,
        "PASS_FOR_BEST_CANDIDATE_SAMPLE_ONLY" if sample_pass else "FAIL_SAMPLE_MINIMUM",
        {
            "best_candidate_all_trade_count": best_all_count,
            "best_candidate_oos_trade_count": best_oos_count,
            "note": "Sample count passes for the best candidate, but the candidate still fails release criteria.",
        },
    )

    lesson_feed = lesson_checker.get("release_gate_feed") if isinstance(lesson_checker.get("release_gate_feed"), dict) else {}
    lesson_pass = lesson_feed.get("LESSON_MEMORY_NO_REPEAT_FAILURE_PASS") is True

    checks["LESSON_MEMORY_NO_REPEAT_FAILURE_PASS"] = make_check(
        lesson_pass,
        str(lesson_feed.get("status") or lesson_checker.get("checker_status")),
        {
            "release_gate_feed": lesson_feed,
            ...
            "matched_lesson": lesson_checker.get("matched_lesson"),
            "interpretation": "Lesson memory says this exact route is a known full-universe failure unless genuinely new evidence exists.",
        },
    )
    ...
    checks["MANUAL_OR_AI_REVIEW_PASS"] = make_check(
        False,
        "MANUAL_OR_AI_REVIEW_NOT_COMPLETED_AND_NOT_MEANINGFUL_AFTER_FAILURE",
        {
            "note": "Manual/AI review cannot override failed full-universe and lesson-memory gates.",
        },
    )
    ...
    if critical:
        release_status = "FAMILY_CANDIDATE_RELEASE_GATE_V4_CRITICAL_BLOCKED"
        severity = "CRITICAL"
        ...
        release_status = "FAMILY_CANDIDATE_RELEASE_GATE_V4_PASS"
        severity = "ATTENTION"
        ...
        release_status = "FAMILY_CANDIDATE_RELEASE_GATE_V4_BLOCKED_KNOWN_FAILURE_ROUTE"
        severity = "ATTENTION"
        allowed_scope = "READ_ONLY_RESEARCH"
        next_action = "QUEUE_NEW_RESEARCH_DIRECTION_DO_NOT_REPEAT_BLOCKED_ROUTE"
        reason = f"failed_required_checks={len(failed_checks)}; lesson_memory_blocks_route={lesson_checker.get('route_blocked_by_lesson_memory')}"
    ...
    result = {
        ...
        "rule": {
            ...
            "lesson_memory_known_failure_blocks_repeat_route": True,
        },
        ...
        "sources": {
            ...
            "lesson_memory_checker_v1": str(LESSON_CHECKER_LATEST),
        ...
        "current_route_blocked_by_lesson_memory": lesson_checker.get("route_blocked_by_lesson_memory"),
        "current_route_hash": lesson_checker.get("route_hash"),
        "repeat_same_route_recommended": False,

        "next_work_order": [
            "queue_new_research_direction",
            "do_not_repeat_ret3_threshold_hold_route_without_new_evidence",
            "design_new_feature_or_family_archetype_instead_of_rescuing_failed_route",
        ],
    ...
    out_json = RUN_DIR / "family_candidate_release_gate_v4_state.json"
    out_md = RUN_DIR / "family_candidate_release_gate_v4_report.md"

    dump_json(out_json, result)
    dump_json(LATEST_JSON, result)

    md = f"""# EDGE FACTORY OS FAMILY CANDIDATE RELEASE GATE v4
...
{json.dumps(checks, indent=2, default=str)[:32000]}
...
"""
    ...
    print("=" * 100)
    print("EDGE FACTORY OS FAMILY CANDIDATE RELEASE GATE v4")
    print("=" * 100)
    ...
    print(f"repeat_same_route_recommended: False")
    print(f"current_route_blocked_by_lesson_memory: {lesson_checker.get('route_blocked_by_lesson_memory')}")
    print(f"current_route_hash: {lesson_checker.get('route_hash')}")
    print()
    print("CHECK SUMMARY")
    print("-" * 100)
