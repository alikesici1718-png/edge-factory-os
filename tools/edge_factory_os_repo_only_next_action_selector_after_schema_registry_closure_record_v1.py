...
MODULE_NAME = "edge_factory_os_repo_only_next_action_selector_after_schema_registry_closure_record_v1"
OUT_DIR = LAB_ROOT / "edge_factory_os_repo_only_next_action_selector_after_schema_registry_closure_record_v1"
OUT_DIR.mkdir(parents=True, exist_ok=True)

EXPECTED_HEAD_PREFIX = "a1dc4d8"
CURRENT_TOOL_REL = "tools/edge_factory_os_repo_only_next_action_selector_after_schema_registry_closure_record_v1.py"
...
SELECTED_NEXT_MODULE = "edge_factory_os_repo_only_development_queue_selector_after_schema_registry_closure_record_v1.py"
...
SAFETY_FLAGS: Dict[str, bool] = {
    "repo_only_next_action_selector_after_schema_registry_closure_record": True,
    "read_only_next_action_selection_only": True,
    "schema_registry_governance_chain_closure_record_required": True,
    "development_queue_selector_after_schema_registry_closure_record_allowed_next": True,
...
def validate_inputs() -> Dict[str, Any]:
    ...
    if git_state["dirty_tracked_count"] != 0:
        errors.append(f"dirty tracked paths present before closure next-action selector: {git_state['dirty_tracked_paths']}")

    if actual_untracked != expected_untracked:
        errors.append(f"unexpected untracked set before closure next-action selector: expected={sorted(expected_untracked)} actual={sorted(actual_untracked)}")

    if physical_before["planned_schema_file_existing_count"] != 0:
        errors.append(f"planned schema files existed before closure next-action selector: {physical_before['existing_planned_schema_files']}")

    subject = commit_subject("HEAD")
    checks["head_commit_subject"] = subject
    if subject != "Add repo-only schema registry governance chain closure record":
        errors.append(f"unexpected HEAD commit subject: {subject}")

    paths = commit_paths("HEAD")
    checks["head_commit_paths"] = paths
    if paths != ["tools/edge_factory_os_repo_only_schema_registry_governance_chain_closure_record_v1.py"]:
    ...
    inputs = {
        "closure_record": (
            ...
            "edge_factory_os_repo_only_next_action_selector_after_schema_registry_closure_record_v1.py",
        ),
        "closure_record_post_check": (
            CLOSURE_POST_JSON,
...
def build_selector(validation: Dict[str, Any]) -> Dict[str, Any]:
    closure_obj = validation["loaded"]["closure_record"]
    ...
    chain_summary = as_dict(closure.get("chain_summary", {}))
    next_step = as_dict(closure.get("next_step", {}))

    queue_obj = validation["loaded"]["queue_selector_after_backlog"]
    queue = as_dict(queue_obj.get("development_queue_selector_after_post_schema_registry_backlog_refresh", {}))
    selected_queue_task = as_dict(queue.get("selected_development_task", {}))
    ...
    effective_approval_state = {
        "manual_approval_present": first_not_none(
            ...
            chain_summary.get("manual_approval_present"),
        ...
        "manual_approval_valid": first_not_none(
            ...
            chain_summary.get("manual_approval_valid"),
        ...
        "approval_granted_now": first_not_none(
            ...
            chain_summary.get("approval_granted_now"),
    ...
    effective_schema_state = {
        "schema_apply_allowed_now": first_not_none(
            ...
            chain_summary.get("schema_apply_allowed_now"),
        ...
        "schema_file_creation_allowed_now": first_not_none(
            ...
            chain_summary.get("schema_file_creation_allowed_now"),
        ...
        "schema_file_edit_allowed_now": first_not_none(
            ...
            chain_summary.get("schema_file_edit_allowed_now"),
    ...
    prerequisites = {
        ...
        "closure_selected_this_selector": closure_obj.get("next_module") == "edge_factory_os_repo_only_next_action_selector_after_schema_registry_closure_record_v1.py",
        "closure_next_step_selected_this_selector": next_step.get("module") == "edge_factory_os_repo_only_next_action_selector_after_schema_registry_closure_record_v1.py",
        ...
        "queue_selected_closure_record": selected_queue_task.get("module") == "edge_factory_os_repo_only_schema_registry_governance_chain_closure_record_v1.py",
    ...
    candidate_actions = [
        {
            "rank": 1,
            "key": "DEVELOPMENT_QUEUE_SELECTOR_AFTER_SCHEMA_REGISTRY_CLOSURE_RECORD",
            "module": SELECTED_NEXT_MODULE,
            "scope": "REPO_ONLY_OS_INTELLIGENCE",
            "allowed": True,
            "reason": "Schema-registry chain is closed with no apply; select the next general repo-only governance/intelligence queue.",
            "blocked_actions": [
        ...
        },
        {
            ...
            "allowed": False,
            "reason": "Blocked because schema governance chain explicitly closed without approval/apply.",
    ...
    return {
        "selector_status": "NEXT_ACTION_SELECTOR_AFTER_SCHEMA_REGISTRY_CLOSURE_RECORD_ACTIVE" if selected else "NEXT_ACTION_SELECTOR_AFTER_SCHEMA_REGISTRY_CLOSURE_RECORD_BLOCKED",
        "candidate_actions": candidate_actions,
        "candidate_action_count": len(candidate_actions),
        "selected_action": selected,
        "selected_next_module": selected["module"] if selected else None,
        "selected_next_action": "BUILD_REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_SCHEMA_REGISTRY_CLOSURE_RECORD_V1" if selected else None,
        "selected_scope": selected["scope"] if selected else None,
        "prerequisites": prerequisites,
        "context_summary": {
            "closure_record_status": closure_obj.get("schema_registry_governance_chain_closure_record_status"),
            ...
            "approval_state_source_order": "status_summary.approval_state -> status_summary flat -> closure chain_summary -> queue context -> manual approval record",
            "schema_state_source_order": "status_summary.schema_action_state -> status_summary flat -> closure decision -> closure chain_summary -> queue context -> manual approval record",
        ...
        "invariants": {
            ...
            "selected_module_is_development_queue_selector_after_schema_registry_closure_record": selected is not None and selected["module"] == SELECTED_NEXT_MODULE,
...
def main() -> int:
    ...
    payload = {
        "module": MODULE_NAME,
        "next_action_selector_after_schema_registry_closure_record_status": "REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_SCHEMA_REGISTRY_CLOSURE_RECORD_V1_READY" if passed else "REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_SCHEMA_REGISTRY_CLOSURE_RECORD_V1_BLOCKED",
        "severity": "OK" if passed else "BLOCKED",
        "allowed_scope": "REPO_ONLY_OS_INTELLIGENCE",
        "final_decision": "NEXT_ACTION_SELECTED_DEVELOPMENT_QUEUE_SELECTOR_AFTER_SCHEMA_REGISTRY_CLOSURE_RECORD" if passed else "KEEP_FREEZE_REVIEW_NEXT_ACTION_SELECTOR_AFTER_SCHEMA_REGISTRY_CLOSURE_RECORD_ERRORS",
        "next_action": "BUILD_REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_SCHEMA_REGISTRY_CLOSURE_RECORD_V1" if passed else "REVIEW_NEXT_ACTION_SELECTOR_AFTER_SCHEMA_REGISTRY_CLOSURE_RECORD_ERRORS",
        "next_module": SELECTED_NEXT_MODULE if passed else None,
        "reason": "Selected repo-only development queue selector after schema-registry closure record; schema apply/create/edit remain blocked and physically absent." if passed else "Next-action selector after schema-registry closure record validation failed.",
        ...
        "next_action_selector_after_schema_registry_closure_record": selector,
    ...
    latest_json = OUT_DIR / "repo_only_next_action_selector_after_schema_registry_closure_record_v1_latest.json"
    timestamped_json = OUT_DIR / f"repo_only_next_action_selector_after_schema_registry_closure_record_v1_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json"
    latest_txt = OUT_DIR / "repo_only_next_action_selector_after_schema_registry_closure_record_v1_latest.txt"

    latest_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    timestamped_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")

    lines = [
        "EDGE FACTORY OS REPO-ONLY NEXT ACTION SELECTOR AFTER SCHEMA REGISTRY CLOSURE RECORD v1",
        "=" * 100,
        f"next_action_selector_after_schema_registry_closure_record_status: {payload['next_action_selector_after_schema_registry_closure_record_status']}",
