...
MODULE_NAME = "edge_factory_os_repo_only_framework_schema_registry_manual_approval_record_v1"
OUT_DIR = LAB_ROOT / "edge_factory_os_repo_only_framework_schema_registry_manual_approval_record_v1"
OUT_DIR.mkdir(parents=True, exist_ok=True)

EXPECTED_HEAD_PREFIX = "8ed3587"
CURRENT_TOOL_REL = "tools/edge_factory_os_repo_only_framework_schema_registry_manual_approval_record_v1.py"
...
SELECTED_NEXT_MODULE = "edge_factory_os_repo_only_development_queue_selector_after_schema_registry_approval_record_v1.py"
...
SAFETY_FLAGS: Dict[str, bool] = {
    "repo_only_framework_schema_registry_manual_approval_record": True,
    "manual_approval_record_only": True,
    "schema_registry_approval_gate_required": True,
    "development_queue_selector_after_schema_registry_approval_record_allowed_next": True,
...
def validate_inputs() -> Dict[str, Any]:
    ...
    if subject != "Add repo-only framework schema registry approval gate":
        errors.append(f"unexpected HEAD commit subject: {subject}")

    paths = commit_paths("HEAD")
    checks["head_commit_paths"] = paths
    if paths != ["tools/edge_factory_os_repo_only_framework_schema_registry_approval_gate_v1.py"]:
        errors.append(f"unexpected HEAD commit paths: {paths}")

    existing_schema_files_before_record = get_existing_planned_schema_files()
    if existing_schema_files_before_record:
        errors.append(f"planned schema files unexpectedly exist before manual approval record: {existing_schema_files_before_record}")

    inputs = {
        "framework_schema_registry_approval_gate": (
            GATE_JSON,
            "REPO_ONLY_FRAMEWORK_SCHEMA_REGISTRY_APPROVAL_GATE_V1_READY",
            "edge_factory_os_repo_only_framework_schema_registry_manual_approval_record_v1.py",
        ),
        "framework_schema_registry_approval_gate_post_check": (
            GATE_POST_JSON,
    ...
    return {
        ...
        "existing_schema_files_before_record": existing_schema_files_before_record,
        "clean_baseline": not errors,
    }


def build_manual_approval_record(validation: Dict[str, Any]) -> Dict[str, Any]:
    gate_obj = validation["loaded"]["framework_schema_registry_approval_gate"]
    gate = gate_obj.get("framework_schema_registry_approval_gate", {})
    decision = gate.get("approval_decision", {})
    policy = gate.get("approval_policy", {})
    preview_summary = gate.get("preview_summary", {})
    ...
    existing_schema_files_after_record = get_existing_planned_schema_files()

    record = {
        "approval_status": "SCHEMA_REGISTRY_MANUAL_APPROVAL_NOT_GRANTED",
        ...
        "previewed_schema_count": preview_summary.get("previewed_schema_count"),
        "record_reason": "No explicit manual approval was provided; schema registry apply remains blocked.",
    }
    ...
    prerequisites = {
        ...
        "approval_gate_blocks_apply": decision.get("apply_allowed_now") is False,
        "approval_gate_blocks_schema_creation": decision.get("schema_file_creation_allowed_now") is False,
        "approval_gate_blocks_schema_edit": decision.get("schema_file_edit_allowed_now") is False,
        "approval_gate_blocks_schema_apply": decision.get("schema_apply_allowed_now") is False,
        "approval_gate_manual_absent": decision.get("manual_approval_present") is False,
        "approval_gate_manual_invalid": decision.get("manual_approval_valid") is False,
        "approval_policy_requires_exact_text": policy.get("exact_required_approval_text") == EXPECTED_APPROVAL_TEXT,
        "approval_policy_blocks_without_approval": policy.get("schema_apply_blocked_without_approval") is True,
        ...
        "preview_existing_repo_schema_file_count_zero": preview.get("existing_repo_schema_file_count") == 0,
        "preview_repo_creation_zero": preview.get("repo_schema_creation_performed_count") == 0,
        "preview_repo_edit_zero": preview.get("repo_schema_edit_performed_count") == 0,
        "preview_repo_apply_zero": preview.get("repo_schema_apply_performed_count") == 0,
        ...
        "contract_requires_approval_before_schema_apply": contract.get("contract_policy", {}).get("approval_required_before_schema_apply") is True,
        "no_existing_planned_schema_files_before_record": not validation["existing_schema_files_before_record"],
        "no_existing_planned_schema_files_after_record": not existing_schema_files_after_record,
        "record_manual_approval_absent": record["manual_approval_present"] is False,
        "record_manual_approval_invalid": record["manual_approval_valid"] is False,
        "record_apply_blocked": record["apply_allowed_now"] is False,
        "tracked_python_clean": validation["tracked_python_validation"]["pass"] is True,
    }

    return {
        "manual_approval_record_status": "SCHEMA_REGISTRY_MANUAL_APPROVAL_RECORD_ACTIVE" if all(prerequisites.values()) else "SCHEMA_REGISTRY_MANUAL_APPROVAL_RECORD_BLOCKED",
        "manual_approval_record": record,
        "record_policy": {
            "record_absence_only": True,
            "schema_apply_blocked_without_exact_manual_approval": True,
            "exact_required_approval_text": EXPECTED_APPROVAL_TEXT,
            "development_queue_selector_required_next": True,
            "no_schema_apply_now": True,
            "no_schema_file_creation_now": True,
            "no_schema_file_edit_now": True,
        ...
        "schema_file_guard": {
            ...
            "existing_schema_files_before_record": validation["existing_schema_files_before_record"],
            "existing_schema_files_after_record": existing_schema_files_after_record,
            "planned_schema_file_existing_count_after_record": len(existing_schema_files_after_record),
        },
        "recommended_next_step": {
            "step": "BUILD_REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_SCHEMA_REGISTRY_APPROVAL_RECORD_V1",
            "module": SELECTED_NEXT_MODULE,
            "scope": "REPO_ONLY_OS_INTELLIGENCE",
            "reason": "Manual approval absence has been recorded; resume repo-only development queue selection with schema apply still blocked.",
        },
        "invariants": {
            "manual_record_prerequisites_all_true": all(prerequisites.values()),
            "manual_approval_present_false": record["manual_approval_present"] is False,
            "manual_approval_valid_false": record["manual_approval_valid"] is False,
            "approval_granted_false": record["approval_granted_now"] is False,
            "apply_allowed_false": record["apply_allowed_now"] is False,
            "schema_file_creation_allowed_false": record["schema_file_creation_allowed_now"] is False,
            "schema_file_edit_allowed_false": record["schema_file_edit_allowed_now"] is False,
            "schema_apply_allowed_false": record["schema_apply_allowed_now"] is False,
            "schema_file_creation_performed_false": record["schema_file_creation_performed_now"] is False,
            "schema_file_edit_performed_false": record["schema_file_edit_performed_now"] is False,
            "schema_apply_performed_false": record["schema_apply_performed_now"] is False,
            "no_existing_planned_schema_files_before_record": not validation["existing_schema_files_before_record"],
            "no_existing_planned_schema_files_after_record": not existing_schema_files_after_record,
            "development_queue_selector_required_next": True,
            "no_runtime_strategy_or_capital_authorized": True,
            "no_repo_file_creation_or_modification_performed_by_manual_record": True,
        },
    }
...
def main() -> int:
    ...
    record_payload = build_manual_approval_record(validation)

    for key, value in record_payload["invariants"].items():
    ...
    payload = {
        "module": MODULE_NAME,
        "framework_schema_registry_manual_approval_record_status": "REPO_ONLY_FRAMEWORK_SCHEMA_REGISTRY_MANUAL_APPROVAL_RECORD_V1_READY" if passed else "REPO_ONLY_FRAMEWORK_SCHEMA_REGISTRY_MANUAL_APPROVAL_RECORD_V1_BLOCKED",
        "severity": "ATTENTION" if passed else "BLOCKED",
        "allowed_scope": "REPO_ONLY_SCHEMA_APPROVAL_RECORD",
        "final_decision": "SCHEMA_REGISTRY_MANUAL_APPROVAL_ABSENT_RECORDED_APPLY_BLOCKED" if passed else "KEEP_FREEZE_REVIEW_SCHEMA_REGISTRY_MANUAL_APPROVAL_RECORD_ERRORS",
        "next_action": "BUILD_REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_SCHEMA_REGISTRY_APPROVAL_RECORD_V1" if passed else "REVIEW_SCHEMA_REGISTRY_MANUAL_APPROVAL_RECORD_ERRORS",
        "next_module": SELECTED_NEXT_MODULE if passed else None,
        "reason": "Manual approval absence recorded; schema apply/create/edit remain blocked." if passed else "Framework schema registry manual approval record validation failed.",
        ...
        "framework_schema_registry_manual_approval_record": record_payload,
    ...
    latest_json = OUT_DIR / "repo_only_framework_schema_registry_manual_approval_record_v1_latest.json"
    timestamped_json = OUT_DIR / f"repo_only_framework_schema_registry_manual_approval_record_v1_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json"
    latest_txt = OUT_DIR / "repo_only_framework_schema_registry_manual_approval_record_v1_latest.txt"

    latest_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    timestamped_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")

    lines = [
        "EDGE FACTORY OS REPO-ONLY FRAMEWORK SCHEMA REGISTRY MANUAL APPROVAL RECORD v1",
        "=" * 100,
        f"framework_schema_registry_manual_approval_record_status: {payload['framework_schema_registry_manual_approval_record_status']}",
        ...
        json.dumps(
            {
                "manual_approval_record": record_payload["manual_approval_record"],
                "record_policy": record_payload["record_policy"],
                "schema_file_guard": record_payload["schema_file_guard"],
                "prerequisites": record_payload["prerequisites"],
                "invariants": record_payload["invariants"],
                "recommended_next_step": record_payload["recommended_next_step"],
            },
            indent=2,
            sort_keys=True,
        ),
