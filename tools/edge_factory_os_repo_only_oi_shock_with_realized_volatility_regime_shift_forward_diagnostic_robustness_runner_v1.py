#!/usr/bin/env python
"""Robustness diagnostic for OI shock / realized-volatility regime findings."""
...
MODULE = "OI_SHOCK_WITH_REALIZED_VOLATILITY_REGIME_SHIFT_FORWARD_DIAGNOSTIC_ROBUSTNESS_RUNNER_V1"
MODULE_RELATIVE_PATH = "tools/edge_factory_os_repo_only_oi_shock_with_realized_volatility_regime_shift_forward_diagnostic_robustness_runner_v1.py"
ARTIFACT_RELATIVE_PATH = "artifacts/research/oi_shock_with_realized_volatility_regime_shift_forward_diagnostic_robustness_runner_v1.json"
ARTIFACT_PATH = REPO_ROOT / ARTIFACT_RELATIVE_PATH

EXPECTED_HEAD = "fea6e348fce93ae9e522d5f482f06bdd29042cb6"
...
ROBUSTNESS_STATUS_PASS = "PASS_REPO_ONLY_OI_SHOCK_WITH_REALIZED_VOLATILITY_REGIME_SHIFT_FORWARD_DIAGNOSTIC_ROBUSTNESS_RUNNER_CREATED"
ROBUSTNESS_STATUS_BLOCKED = "BLOCKED_OI_SHOCK_WITH_REALIZED_VOLATILITY_REGIME_SHIFT_FORWARD_DIAGNOSTIC_ROBUSTNESS_RUNNER"
ARTIFACT_KIND = "OI_SHOCK_WITH_REALIZED_VOLATILITY_REGIME_SHIFT_FORWARD_DIAGNOSTIC_ROBUSTNESS_RUNNER"

RESULT_PROMISING = "OI_SHOCK_WITH_REALIZED_VOLATILITY_REGIME_SHIFT_ROBUSTNESS_PROMISING_VOLATILITY_DIAGNOSTIC_ONLY"
RESULT_WEAK = "OI_SHOCK_WITH_REALIZED_VOLATILITY_REGIME_SHIFT_ROBUSTNESS_WEAK_OR_NOT_ROBUST"
RESULT_ATTENTION = "OI_SHOCK_WITH_REALIZED_VOLATILITY_REGIME_SHIFT_ROBUSTNESS_DATA_QUALITY_ATTENTION"
RESULT_FAILED = "OI_SHOCK_WITH_REALIZED_VOLATILITY_REGIME_SHIFT_ROBUSTNESS_FAILED_STOP"

THEORY_ID = "OI_SHOCK_WITH_REALIZED_VOLATILITY_REGIME_SHIFT"
NEXT_EVALUATOR = "OI_SHOCK_WITH_REALIZED_VOLATILITY_REGIME_SHIFT_ROBUSTNESS_EVALUATOR_V1"
...
PRIMARY_TARGETS = [
    ("best_oi_expansion_volatility_expansion_definition", "15m"),
    ("best_oi_expansion_volatility_compression_break_definition", "15m"),
]
SECONDARY_SIGNED_TARGETS = [
    ("best_oi_expansion_volatility_expansion_definition", "24h"),
    ("best_oi_expansion_volatility_compression_break_definition", "24h"),
    ("best_oi_contraction_volatility_expansion_definition", "24h"),
]
...
def load_forward_diagnostic_module() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("oi_shock_vol_forward_diagnostic_for_robustness_v1", FORWARD_DIAGNOSTIC_TOOL)
    if spec is None or spec.loader is None:
        raise RobustnessBlocked("could not load forward diagnostic module")
...
def load_inputs() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], dict[str, Any], dict[str, Any], dict[str, str]]:
    ...
    payload_hashes = {
        SOURCE_DIAGNOSTIC_RELATIVE_PATH: verify_payload_hash(diagnostic, "OI shock forward diagnostic"),
    ...
    if diagnostic.get("result_classification") != "OI_SHOCK_WITH_REALIZED_VOLATILITY_REGIME_SHIFT_FORWARD_DIAGNOSTIC_PROMISING_VOLATILITY_DIAGNOSTIC_ONLY":
    ...
    observed_top = {
        (item.get("selection_slot"), item.get("horizon"))
        for item in diagnostic.get("top_volatility_findings", [])
        if isinstance(item, dict)
    }
    if not required_top.issubset(observed_top):
        raise RobustnessBlocked("prior diagnostic missing required primary volatility top findings")
    ...
    for slot in MAIN_SELECTION_SLOTS:
        for horizon in HORIZONS:
            ...
            output[slot][horizon] = {
                ...
                "mean_abs_return": quantile_stats(abs_values, "abs_return")["abs_return_mean"],
                "median_abs_return": quantile_stats(abs_values, "abs_return")["abs_return_median"],
                "std_abs_return": quantile_stats(abs_values, "abs_return")["abs_return_std"],
                "q50_abs_return": quantile_stats(abs_values, "abs_return")["abs_return_q50"],
                "q75_abs_return": quantile_stats(abs_values, "abs_return")["abs_return_q75"],
                "q90_abs_return": quantile_stats(abs_values, "abs_return")["abs_return_q90"],
                "q95_abs_return": quantile_stats(abs_values, "abs_return")["abs_return_q95"],
                "q99_abs_return": quantile_stats(abs_values, "abs_return")["abs_return_q99"],
    ...
    for slot in MAIN_SELECTION_SLOTS:
        for horizon in HORIZONS:
            ...
            if obs is None:
                ...
                p_high = safe_float((1 + np.sum(null_means >= float(obs))) / (1 + PERMUTATION_COUNT))
            ...
            p_values[slot][horizon] = {
                ...
                "primary_p_value_for_multiple_comparison": p_high,
            }
...
def arbusdt_sensitivity(
    ...
    for offset, (slot, horizon) in enumerate(PRIMARY_TARGETS, start=100):
        ...
        ratio = (mean_value / full_mean) if mean_value is not None and full_mean not in {None, 0.0} else None
        subset_events = {key: [] for key in MAIN_SELECTION_SLOTS}
        subset_events[slot] = kept_events
        subset_pool_events = {key: events[key] for key in MAIN_SELECTION_SLOTS}
        subset_pools = build_month_aware_pools(subset_pool_events, kline_by_symbol, "abs")
...
def secondary_signed_tracking(diagnostic: dict[str, Any]) -> dict[str, Any]:
    rows = []
    for slot, horizon in SECONDARY_SIGNED_TARGETS:
        key = f"{slot}__{horizon}"
        rows.append(
            {
                "selection_slot": slot,
                "horizon": horizon,
                "prior_signed_fdr_q": diagnostic.get("signed_return_fdr_q_values", {}).get(key),
                "prior_signed_bonferroni_p": diagnostic.get("signed_return_bonferroni_p_values", {}).get(key),
                "tracked_only": True,
                "promoted": False,
            }
        )
    return {
        "secondary_signed_return_targets": rows,
...
def primary_gates(
    ...
    for slot, horizon in PRIMARY_TARGETS:
        key = f"{slot}__{horizon}"
        symbol_key = "any_single_symbol_necessary"
        month_key = "any_single_month_necessary"
        gate = {
            ...
            "no_single_symbol_dependence": leave_symbol.get(key, {}).get(symbol_key) is False,
            "no_single_month_dependence": leave_month.get(key, {}).get(month_key) is False,
            "arbusdt_exclusion_preserves_effect": arbusdt.get(key, {}).get("arbusdt_exclusion_pass") is True,
            "alternate_volatility_proxy_supports_direction": alt_metrics.get(key, {}).get("alternate_proxy_supports_direction") is True,
...
def base_artifact(
    ...
    return {
        ...
        "observed_volatility_stats_by_definition_and_horizon": {},
        "null_volatility_stats_by_definition_and_horizon": {},
        "p_values_by_definition_and_horizon": {},
        "fdr_q_values": {},
        "bonferroni_p_values": {},
...
def build_artifact() -> dict[str, Any]:
    ...
    shock = diag.load_discovery_module()
    events, kline_by_symbol, reconstruction = diag.reconstruct_events(dataset, kline, discovery, shock)
    ...
    gates, failed_gates = primary_gates(null_stats, fdr, bonferroni, leave_symbol, leave_month, arbusdt, alt_metrics, missing_forward)
    hashes_after = input_artifact_hashes()
    if hashes_before != hashes_after:
        raise RobustnessBlocked("input artifact hash changed during robustness run")
    any_primary_pass = any(item.get("primary_target_passes_all_gates") is True for item in gates.values())
    ...
    data_quality_warnings = [
        ...
        "Missing rows are zero for primary 15m robustness targets.",
    ]
    if any_primary_pass:
    ...
    prior_summary = {
        ...
        "top_signed_return_findings_tracked_only": diagnostic.get("top_signed_return_findings", [])[:3],
    ...
    validation_checks = {
        ...
        == "OI_SHOCK_WITH_REALIZED_VOLATILITY_REGIME_SHIFT_FORWARD_DIAGNOSTIC_PROMISING_VOLATILITY_DIAGNOSTIC_ONLY",
    ...
    artifact.update(
        {
            ...
            "observed_volatility_stats_by_definition_and_horizon": observed,
            "null_volatility_stats_by_definition_and_horizon": null_stats,
            "p_values_by_definition_and_horizon": p_values,
            "fdr_q_values": fdr,
