...
MODULE_NAME = "edge_factory_os_repo_only_development_queue_selector_after_post_schema_registry_governance_backlog_refresh_v1"
OUT_DIR = LAB_ROOT / "edge_factory_os_repo_only_development_queue_selector_after_post_schema_registry_governance_backlog_refresh_v1"
OUT_DIR.mkdir(parents=True, exist_ok=True)

EXPECTED_HEAD_PREFIX = "49477b9"
CURRENT_TOOL_REL = "tools/edge_factory_os_repo_only_development_queue_selector_after_post_schema_registry_governance_backlog_refresh_v1.py"
...
SELECTED_NEXT_MODULE = "edge_factory_os_repo_only_standard_os_status_refresh_after_post_schema_registry_governance_backlog_refresh_v1.py"
...
SAFETY_FLAGS: Dict[str, bool] = {
    "repo_only_development_queue_selector_after_post_schema_registry_governance_backlog_refresh": True,
    "read_only_development_queue_selection_only": True,
    "next_action_selector_after_post_schema_registry_governance_backlog_refresh_required": True,
    "standard_os_status_refresh_after_post_schema_registry_governance_backlog_refresh_allowed_next": True,
...
def validate_inputs() -> Dict[str, Any]:
    ...
    if subject != "Add repo-only next-action selector after post schema registry governance backlog refresh":
        errors.append(f"unexpected HEAD commit subject: {subject}")

    paths = commit_paths("HEAD")
    checks["head_commit_paths"] = paths
    if paths != ["tools/edge_factory_os_repo_only_next_action_selector_after_post_schema_registry_governance_backlog_refresh_v1.py"]:
    ...
    inputs = {
        "next_action_selector_after_governance_backlog_refresh": (
            ...
            "edge_factory_os_repo_only_development_queue_selector_after_post_schema_registry_governance_backlog_refresh_v1.py",
        ),
...
def build_queue(validation: Dict[str, Any]) -> Dict[str, Any]:
    ...
    prerequisites = {
        "parent_selector_ready": selector_obj.get("next_action_selector_after_post_schema_registry_governance_backlog_refresh_status") == "REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_BACKLOG_REFRESH_V1_READY",
        "parent_selected_this_queue_selector": selected_action.get("module") == "edge_factory_os_repo_only_development_queue_selector_after_post_schema_registry_governance_backlog_refresh_v1.py",
    ...
    queue_items = [
        {
            "rank": 1,
            "key": "STANDARD_OS_STATUS_REFRESH_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_BACKLOG_REFRESH",
            "module": SELECTED_NEXT_MODULE,
            "scope": "REPO_ONLY_OS_INTELLIGENCE",
            "allowed": True,
            "reason": "Governance backlog selector is clean; refresh standard OS status before selecting broader repo-only work.",
            "blocked_actions": [
    ...
    return {
        "development_queue_status": "DEVELOPMENT_QUEUE_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_BACKLOG_REFRESH_ACTIVE" if selected else "DEVELOPMENT_QUEUE_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_BACKLOG_REFRESH_BLOCKED",
        ...
        "selected_next_action": "BUILD_REPO_ONLY_STANDARD_OS_STATUS_REFRESH_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_BACKLOG_REFRESH_V1" if selected else None,
        ...
        "invariants": {
            ...
            "selected_module_is_standard_os_status_refresh": selected is not None and selected["module"] == SELECTED_NEXT_MODULE,
...
def main() -> int:
    ...
    payload = {
        "module": MODULE_NAME,
        "development_queue_selector_after_post_schema_registry_governance_backlog_refresh_status": "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_BACKLOG_REFRESH_V1_READY" if passed else "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_BACKLOG_REFRESH_V1_BLOCKED",
        "severity": "OK" if passed else "BLOCKED",
        "allowed_scope": "REPO_ONLY_OS_INTELLIGENCE",
        "final_decision": "DEVELOPMENT_QUEUE_SELECTED_STANDARD_OS_STATUS_REFRESH_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_BACKLOG_REFRESH" if passed else "KEEP_FREEZE_REVIEW_DEVELOPMENT_QUEUE_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_BACKLOG_REFRESH_ERRORS",
        "next_action": "BUILD_REPO_ONLY_STANDARD_OS_STATUS_REFRESH_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_BACKLOG_REFRESH_V1" if passed else "REVIEW_DEVELOPMENT_QUEUE_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_BACKLOG_REFRESH_ERRORS",
        "next_module": SELECTED_NEXT_MODULE if passed else None,
        "reason": "Selected repo-only standard OS status refresh after governance backlog refresh; schema apply/create/edit remain blocked and physically absent." if passed else "Development queue selector after governance backlog refresh validation failed.",
        ...
        "development_queue_selector_after_post_schema_registry_governance_backlog_refresh": queue,
    ...
    latest_json = OUT_DIR / "repo_only_development_queue_selector_after_post_schema_registry_governance_backlog_refresh_v1_latest.json"
    timestamped_json = OUT_DIR / f"repo_only_development_queue_selector_after_post_schema_registry_governance_backlog_refresh_v1_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json"
    latest_txt = OUT_DIR / "repo_only_development_queue_selector_after_post_schema_registry_governance_backlog_refresh_v1_latest.txt"

    latest_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    timestamped_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")

    lines = [
        "EDGE FACTORY OS REPO-ONLY DEVELOPMENT QUEUE SELECTOR AFTER POST-SCHEMA-REGISTRY GOVERNANCE BACKLOG REFRESH v1",
        "=" * 100,
        f"development_queue_selector_after_post_schema_registry_governance_backlog_refresh_status: {payload['development_queue_selector_after_post_schema_registry_governance_backlog_refresh_status']}",
