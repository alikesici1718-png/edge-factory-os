...
MODULE_NAME = "edge_factory_os_repo_only_standard_os_status_refresh_after_standard_os_status_backlog_refresh_v1"
OUT_DIR = LAB_ROOT / "edge_factory_os_repo_only_standard_os_status_refresh_after_standard_os_status_backlog_refresh_v1"
OUT_DIR.mkdir(parents=True, exist_ok=True)

EXPECTED_HEAD_PREFIX = "b3769d7"
CURRENT_TOOL_REL = "tools/edge_factory_os_repo_only_standard_os_status_refresh_after_standard_os_status_backlog_refresh_v1.py"

QUEUE_JSON = LAB_ROOT / "edge_factory_os_repo_only_development_queue_selector_after_standard_os_status_backlog_refresh_v1" / "repo_only_development_queue_selector_after_standard_os_status_backlog_refresh_v1_latest.json"
QUEUE_POST_JSON = LAB_ROOT / "edge_factory_os_repo_only_development_queue_selector_after_standard_os_status_backlog_refresh_post_commit_check" / "repo_only_development_queue_selector_after_standard_os_status_backlog_refresh_post_commit_check_latest.json"
SELECTOR_JSON = LAB_ROOT / "edge_factory_os_repo_only_next_action_selector_after_standard_os_status_backlog_refresh_v1" / "repo_only_next_action_selector_after_standard_os_status_backlog_refresh_v1_latest.json"
SELECTOR_POST_JSON = LAB_ROOT / "edge_factory_os_repo_only_next_action_selector_after_standard_os_status_backlog_refresh_post_commit_check" / "repo_only_next_action_selector_after_standard_os_status_backlog_refresh_post_commit_check_latest.json"

SELECTED_NEXT_MODULE = "edge_factory_os_repo_only_next_action_selector_after_standard_os_status_backlog_status_refresh_v1.py"
...
SAFETY_FLAGS: Dict[str, bool] = {
    "repo_only_standard_os_status_refresh_after_standard_os_status_backlog_refresh": True,
    "read_only_standard_status_refresh_only": True,
    "development_queue_selector_after_standard_os_status_backlog_refresh_required": True,
    "next_action_selector_after_standard_os_status_backlog_status_refresh_allowed_next": True,
...
def first_status(obj: Dict[str, Any]) -> Optional[str]:
    for key in [
        "development_queue_selector_after_standard_os_status_backlog_refresh_status",
        "audit_status",
        "next_action_selector_after_standard_os_status_backlog_refresh_status",
    ]:
        value = obj.get(key)
        if value:
            return str(value)
...
def validate_inputs() -> Dict[str, Any]:
    ...
    if git_state["dirty_tracked_count"] != 0:
        errors.append(f"dirty tracked paths present before standard OS status refresh: {git_state['dirty_tracked_paths']}")

    if actual_untracked != expected_untracked:
        errors.append(f"unexpected untracked set before standard OS status refresh: expected={sorted(expected_untracked)} actual={sorted(actual_untracked)}")

    if physical_before["planned_schema_file_existing_count"] != 0:
        errors.append(f"planned schema files existed before standard OS status refresh: {physical_before['existing_planned_schema_files']}")

    subject = commit_subject("HEAD")
    checks["head_commit_subject"] = subject
    if subject != "Add repo-only development queue selector after standard OS status backlog refresh":
        errors.append(f"unexpected HEAD commit subject: {subject}")

    paths = commit_paths("HEAD")
    checks["head_commit_paths"] = paths
    if paths != ["tools/edge_factory_os_repo_only_development_queue_selector_after_standard_os_status_backlog_refresh_v1.py"]:
        errors.append(f"unexpected HEAD commit paths: {paths}")

    inputs = {
        "development_queue_selector": (
            QUEUE_JSON,
            "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_V1_READY",
            "edge_factory_os_repo_only_standard_os_status_refresh_after_standard_os_status_backlog_refresh_v1.py",
        ),
        "development_queue_selector_post_check": (
            QUEUE_POST_JSON,
            "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_POST_COMMIT_CHECK_PASS",
            "edge_factory_os_repo_only_standard_os_status_refresh_after_standard_os_status_backlog_refresh_v1.py",
        ),
        "next_action_selector_after_backlog_refresh": (
            SELECTOR_JSON,
            "REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_V1_READY",
            None,
        ),
        "next_action_selector_after_backlog_refresh_post_check": (
            SELECTOR_POST_JSON,
            "REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_POST_COMMIT_CHECK_PASS",
            None,
        ),
    }
...
def build_status_refresh(validation: Dict[str, Any]) -> Dict[str, Any]:
    ...
    queue = queue_obj.get("development_queue_selector_after_standard_os_status_backlog_refresh", {})
    ...
    selector_obj = validation["loaded"]["next_action_selector_after_backlog_refresh"]
    selector = selector_obj.get("next_action_selector_after_standard_os_status_backlog_refresh", {})
    selected_action = selector.get("selected_action", {})

    physical_after = physical_guard_snapshot()
    git_state = validation["git_state"]
    tracked_python = validation["tracked_python_validation"]

    prerequisites = {
        "queue_ready": queue_obj.get("development_queue_selector_after_standard_os_status_backlog_refresh_status") == "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_V1_READY",
        "queue_selected_this_refresh": selected_task.get("module") == "edge_factory_os_repo_only_standard_os_status_refresh_after_standard_os_status_backlog_refresh_v1.py",
        "queue_selected_repo_only_scope": selected_task.get("scope") == "REPO_ONLY_OS_INTELLIGENCE",
        "selector_ready": selector_obj.get("next_action_selector_after_standard_os_status_backlog_refresh_status") == "REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_V1_READY",
        "selector_selected_queue": selected_action.get("module") == "edge_factory_os_repo_only_development_queue_selector_after_standard_os_status_backlog_refresh_v1.py",
        ...
        "tracked_python_clean": tracked_python["pass"] is True,
    }

    standard_status = {
        "repo_clean_before_wrapper_commit": git_state["dirty_tracked_count"] == 0 and set(git_state["untracked_paths"]) == {CURRENT_TOOL_REL},
        ...
        "schema_registry_apply_allowed_now": False,
        "schema_file_creation_allowed_now": False,
        "schema_file_edit_allowed_now": False,
        "schema_apply_performed_now": False,
        "schema_file_creation_performed_now": False,
        ...
        "general_repo_only_flow_active": True,
        "standard_os_status_backlog_refresh_chain_active": True,
        "next_standard_selector_required": True,
    }

    return {
        "standard_status_refresh_status": "STANDARD_OS_STATUS_REFRESH_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_READY" if all(prerequisites.values()) else "STANDARD_OS_STATUS_REFRESH_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_BLOCKED",
        "standard_os_status": standard_status,
        "selected_next_step": {
            "step": "BUILD_REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_STATUS_REFRESH_V1",
            "module": SELECTED_NEXT_MODULE,
            "scope": "REPO_ONLY_OS_INTELLIGENCE",
            "reason": "Standard OS status refreshed after standard OS status backlog refresh; select next repo-only task.",
        },
        "context_summary": {
        ...
        "status_policy": {
            "standard_status_refresh_only": True,
        ...
        "invariants": {
            ...
            "selected_next_module_is_next_action_selector": SELECTED_NEXT_MODULE == "edge_factory_os_repo_only_next_action_selector_after_standard_os_status_backlog_status_refresh_v1.py",
            "schema_apply_allowed_false": standard_status["schema_registry_apply_allowed_now"] is False,
            "schema_file_creation_allowed_false": standard_status["schema_file_creation_allowed_now"] is False,
            "schema_file_edit_allowed_false": standard_status["schema_file_edit_allowed_now"] is False,
            "manual_approval_absent": standard_status["manual_approval_present"] is False,
            "manual_approval_invalid": standard_status["manual_approval_valid"] is False,
            "approval_granted_false": standard_status["approval_granted_now"] is False,
            "planned_schema_files_absent_before": validation["physical_before"]["planned_schema_file_existing_count"] == 0,
            "planned_schema_files_absent_after": physical_after["planned_schema_file_existing_count"] == 0,
            "schema_apply_not_performed": physical_after["schema_apply_performed_count"] == 0,
            "schema_file_creation_not_performed": physical_after["schema_file_creation_performed_count"] == 0,
...
def main() -> int:
    ...
    payload = {
        "module": MODULE_NAME,
        "standard_os_status_refresh_after_standard_os_status_backlog_refresh_status": "REPO_ONLY_STANDARD_OS_STATUS_REFRESH_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_V1_READY" if passed else "REPO_ONLY_STANDARD_OS_STATUS_REFRESH_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_V1_BLOCKED",
        "severity": "OK" if passed else "BLOCKED",
        "allowed_scope": "REPO_ONLY_OS_INTELLIGENCE",
        "final_decision": "STANDARD_OS_STATUS_REFRESH_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_READY_FOR_NEXT_ACTION_SELECTOR" if passed else "KEEP_FREEZE_REVIEW_STANDARD_OS_STATUS_REFRESH_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_ERRORS",
        "next_action": "BUILD_REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_STATUS_REFRESH_V1" if passed else "REVIEW_STANDARD_OS_STATUS_REFRESH_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_ERRORS",
        "next_module": SELECTED_NEXT_MODULE if passed else None,
        "reason": "Standard OS status refreshed after standard OS status backlog refresh; schema apply/create/edit remain blocked and physically absent." if passed else "Standard OS status refresh after standard OS status backlog refresh validation failed.",
        ...
        "standard_os_status_refresh_after_standard_os_status_backlog_refresh": refresh,
    ...
    latest_json = OUT_DIR / "repo_only_standard_os_status_refresh_after_standard_os_status_backlog_refresh_v1_latest.json"
    timestamped_json = OUT_DIR / f"repo_only_standard_os_status_refresh_after_standard_os_status_backlog_refresh_v1_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json"
    latest_txt = OUT_DIR / "repo_only_standard_os_status_refresh_after_standard_os_status_backlog_refresh_v1_latest.txt"

    latest_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    timestamped_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")

    lines = [
        "EDGE FACTORY OS REPO-ONLY STANDARD OS STATUS REFRESH AFTER STANDARD OS STATUS BACKLOG REFRESH v1",
        "=" * 100,
        f"standard_os_status_refresh_after_standard_os_status_backlog_refresh_status: {payload['standard_os_status_refresh_after_standard_os_status_backlog_refresh_status']}",
        ...
        json.dumps(
            {
                "standard_os_status": refresh["standard_os_status"],
                "selected_next_step": refresh["selected_next_step"],
                "context_summary": refresh["context_summary"],
                "physical_guards": refresh["physical_guards"],
                "prerequisites": refresh["prerequisites"],
                "status_policy": refresh["status_policy"],
