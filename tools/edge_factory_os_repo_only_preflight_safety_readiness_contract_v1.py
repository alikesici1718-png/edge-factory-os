...
MODULE_NAME = "edge_factory_os_repo_only_preflight_safety_readiness_contract_v1"
CURRENT_TOOL_REL = "tools/edge_factory_os_repo_only_preflight_safety_readiness_contract_v1.py"
OUT_DIR = LAB_ROOT / MODULE_NAME
EXPECTED_HEAD = "c925fe0"
EXPECTED_PREVIOUS_TRACKED_PYTHON_COUNT = 594
NEXT_MODULE = "edge_factory_os_repo_only_preflight_safety_readiness_contract_validator_v1.py"
POST_CHECK_STATUS = "REPO_ONLY_PREFLIGHT_SAFETY_READINESS_CONTRACT_POST_COMMIT_CHECK_PASS"
...
def git_state() -> Dict[str, Any]:
    ...
    return {
        ...
        "repo_clean_for_preflight_safety_readiness_contract": len(dirty_tracked) == 0 and len(unexpected_untracked) == 0,
    }
...
def kill_switch_adoption_ok(adoption: Dict[str, Any]) -> bool:
    return (
        ...
        and adoption.get("next_module") == "edge_factory_os_repo_only_preflight_safety_readiness_contract_v1.py"
...
def contract_definition() -> Dict[str, Any]:
    return {
        "scope": "repo_only_preflight_safety_readiness_contract",
        "contract_is_repo_only": True,
        "contract_is_planning_only": True,
        "runtime_preflight_implementation_allowed": False,
        "requires_kill_switch_readiness": True,
        "requires_evidence_chain_compliance": True,
        "primary_artifact_or_exact_marker_preferred": True,
        "derived_evidence_is_weaker_than_primary": True,
        "missing_evidence_fails_closed": True,
        "blocked_until_explicit_future_approval": [
            "runtime path",
            "paper path",
            "live path",
            "capital path",
            ...
            "generic runner approval or implementation",
        ],
        "future_preflight_requirements": [
            "kill-switch readiness adoption must be respected",
            "evidence_chain_quality must use the exact allowed value set",
            "plain PASS must not be treated as strong evidence without exact marker or artifact",
            "derived_live_repo_post_check must carry explicit reason and complete replacement checks",
            "missing primary evidence must fail closed unless explicit replacement checks are complete",
            "runtime, launcher, capital, live, order, candidate, family, schema, and config paths stay blocked until future explicit approval",
        ],
        "false_confidence_controls": [
            "do not infer executable readiness from repo-only planning",
            "do not treat repeated derived checks as progress",
            "do not treat derived evidence as primary artifact verification",
        ],
    }
...
def build_payload() -> Dict[str, Any]:
    ...
    adoption = load_json(KILL_SWITCH_ADOPTION_ARTIFACT)
    ...
    replacement_checks = {
        ...
        "repo_clean_for_preflight_safety_readiness_contract": git["repo_clean_for_preflight_safety_readiness_contract"],
        ...
        "kill_switch_adoption_artifact_exists": bool(adoption),
        "kill_switch_adoption_ok": kill_switch_adoption_ok(adoption),
        "contract_requires_kill_switch_readiness": contract["requires_kill_switch_readiness"] is True,
        "contract_requires_evidence_chain_compliance": contract["requires_evidence_chain_compliance"] is True,
        "runtime_preflight_not_implemented": contract["runtime_preflight_implementation_allowed"] is False,
        "next_module_expected": NEXT_MODULE == "edge_factory_os_repo_only_preflight_safety_readiness_contract_validator_v1.py",
    ...
    payload: Dict[str, Any] = {
        ...
        "preflight_safety_readiness_contract_status": "PASS" if ready else "FAIL_CLOSED",
        "post_check_status": POST_CHECK_STATUS if ready else "REPO_ONLY_PREFLIGHT_SAFETY_READINESS_CONTRACT_POST_COMMIT_CHECK_FAIL_CLOSED",
        "final_decision": "PREFLIGHT_SAFETY_READINESS_CONTRACT_READY" if ready else "PREFLIGHT_SAFETY_READINESS_CONTRACT_FAIL_CLOSED",
        "next_action": "BUILD_PREFLIGHT_SAFETY_READINESS_CONTRACT_VALIDATOR_V1" if ready else "REVIEW_PREFLIGHT_SAFETY_READINESS_CONTRACT_INPUTS",
        "next_module": NEXT_MODULE if ready else None,
        "preflight_safety_readiness_contract_ready": ready,
        "preflight_safety_readiness_contract": contract,
        ...
        "derived_live_repo_post_check_reason": "Preflight safety readiness contract uses the kill-switch readiness adoption artifact plus live repo replacement checks; evidence remains DERIVED_OVERUSED_ATTENTION and weaker than primary artifact verification.",
        "replacement_checks_all_true": ready,
        "derived_live_repo_post_check_replacement_checks": replacement_checks,
        "validation": {"git_state": git, "tracked_python_validation": py, "kill_switch_adoption_loaded": bool(adoption)},
...
def write_outputs(payload: Dict[str, Any]) -> Dict[str, str]:
    ...
    latest_json = OUT_DIR / "repo_only_preflight_safety_readiness_contract_v1_latest.json"
    timestamped_json = OUT_DIR / f"repo_only_preflight_safety_readiness_contract_v1_{stamp}.json"
    latest_txt = OUT_DIR / "repo_only_preflight_safety_readiness_contract_v1_latest.txt"
...
def main() -> int:
    ...
    return 0 if payload["preflight_safety_readiness_contract_status"] in {"READY", "PASS"} else 3
