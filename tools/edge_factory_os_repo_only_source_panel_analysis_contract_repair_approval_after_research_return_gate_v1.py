...
MODULE_NAME = "edge_factory_os_repo_only_source_panel_analysis_contract_repair_approval_after_research_return_gate_v1"
CURRENT_TOOL_REL = "tools/edge_factory_os_repo_only_source_panel_analysis_contract_repair_approval_after_research_return_gate_v1.py"
OUT_DIR = LAB_ROOT / MODULE_NAME
EXPECTED_PARENT_HEAD = "f8f5a28"
EXPECTED_PREVIOUS_TRACKED_PYTHON_COUNT = 618
EXPECTED_TRACKED_PYTHON_COUNT = 619
...
NEXT_MODULE_REPAIR_APPLY_PREVIEW = "edge_factory_os_repo_only_source_panel_analysis_contract_repair_apply_preview_after_research_return_gate_v1.py"
...
POST_CHECK_STATUS = "REPO_ONLY_SOURCE_PANEL_ANALYSIS_CONTRACT_REPAIR_APPROVAL_AFTER_RESEARCH_RETURN_GATE_POST_COMMIT_CHECK_PASS"
...
USER_REPAIR_APPROVAL_SCOPE = "APPROVAL_RECORD_CREATION_ONLY_NO_REPAIR_APPLY_NO_CONTRACT_EDIT"
...
def prior_repair_preview_respected(preview: Dict[str, Any]) -> bool:
    return (
        preview.get("source_panel_analysis_contract_repair_preview_status") == "PASS"
        and preview.get("repair_preview_completed") is True
        and preview.get("source_panel_contract_repairable") is True
        and preview.get("repair_preview_only") is True
        and preview.get("repair_apply_performed") is False
        and preview.get("repair_apply_allowed_now") is False
        and preview.get("repair_requires_human_approval") is True
        and preview.get("repair_would_touch_prior_contract_artifact") is True
        and preview.get("repair_would_touch_runtime_capital_live") is False
        and preview.get("repair_would_create_schema_or_config") is False
        and preview.get("repair_would_approve_generic_runner") is False
        and preview.get("repair_would_reopen_old_source_panel_anomaly_route") is False
        and preview.get("repair_would_claim_source_panel_result_primary_strength_now") is False
...
def approval_scope() -> Dict[str, bool]:
    return {
        "approval_grants_repair_approval_record_only": True,
...
def build_payload() -> Dict[str, Any]:
    ...
    preview, preview_valid_json, preview_json_error = load_json_checked(REPAIR_PREVIEW_ARTIFACT_PATH)
    ...
    prior_preview_ok = prior_repair_preview_respected(preview)
    next_module = NEXT_MODULE_REPAIR_APPLY_PREVIEW if prior_preview_ok else NEXT_MODULE_ROUTE_RESELECTOR
    ...
    replacement_checks = {
        ...
        "tracked_python_count_increases_from_618_to_619": py["tracked_python_count"] == EXPECTED_TRACKED_PYTHON_COUNT,
        ...
        "repair_preview_artifact_valid_json": preview_valid_json,
        "prior_repair_preview_respected": prior_preview_ok,
        "repair_approval_record_created": True,
        "user_repair_approval_present": True,
        "approval_record_only": scope["approval_grants_repair_approval_record_only"],
        ...
        "repair_apply_preview_allowed_next": next_module == NEXT_MODULE_REPAIR_APPLY_PREVIEW,
        ...
        "next_module_exact": next_module == NEXT_MODULE_REPAIR_APPLY_PREVIEW,
    ...
    approval_context = {
        "prior_repair_preview_passed": preview.get("source_panel_analysis_contract_repair_preview_status") == "PASS",
        "contract_is_repairable": True,
        "repair_would_touch_prior_contract_artifact": True,
        "repair_requires_human_approval": True,
        "user_gave_approval_for_approval_record_only": True,
        "repair_apply_is_not_performed_now": True,
        "contract_artifact_sha256_observed_not_modified": contract_hash,
    }
    repair_boundary = {
    ...
    next_module_decision = {
        ...
        "repair_path_remains_safe": prior_preview_ok,
        "next_module": next_module if ready else NEXT_MODULE_ROUTE_RESELECTOR,
        "repair_apply_directly_selected": False,
    ...
    payload: Dict[str, Any] = {
        ...
        "source_panel_analysis_contract_repair_approval_status": "PASS" if ready else "FAIL_CLOSED",
        "post_check_status": POST_CHECK_STATUS if ready else "REPO_ONLY_SOURCE_PANEL_ANALYSIS_CONTRACT_REPAIR_APPROVAL_AFTER_RESEARCH_RETURN_GATE_POST_COMMIT_CHECK_FAIL_CLOSED",
        "final_decision": "SOURCE_PANEL_CONTRACT_REPAIR_APPROVAL_RECORDED_APPLY_PREVIEW_GATE_NEXT" if ready else "SOURCE_PANEL_CONTRACT_REPAIR_APPROVAL_RECORD_FAIL_CLOSED",
        "next_action": "BUILD_SOURCE_PANEL_ANALYSIS_CONTRACT_REPAIR_APPLY_PREVIEW_AFTER_RESEARCH_RETURN_GATE" if ready else "RESELECT_RESEARCH_RETURN_ROUTE_AFTER_SOURCE_PANEL_CONTRACT_BLOCK",
        "next_module": next_module if ready else NEXT_MODULE_ROUTE_RESELECTOR,
        "prior_repair_preview_respected": prior_preview_ok,
        "repair_approval_record_created": True,
        "user_repair_approval_present": True,
        "user_repair_approval_scope": USER_REPAIR_APPROVAL_SCOPE,
        "approval_grants_repair_approval_record_only": scope["approval_grants_repair_approval_record_only"],
        ...
        "derived_live_repo_post_check_reason": (
            "This repair approval record uses live repo checks plus the prior repair-preview artifact only to record manual approval for creating an approval record. "
        ...
        "repair_preview_artifact_snapshot": {
            ...
            "status": preview.get("source_panel_analysis_contract_repair_preview_status"),
        ...
        "validation": {
            ...
            "allowed_next_modules": [
                NEXT_MODULE_REPAIR_APPLY_PREVIEW,
                NEXT_MODULE_ROUTE_RESELECTOR,
            ],
        },
        "safety_flags": {
            "repo_only": True,
            "repair_approval_record_only": True,
            "repair_apply_performed": False,
...
def write_outputs(payload: Dict[str, Any]) -> Dict[str, str]:
    ...
    latest_json = OUT_DIR / "repo_only_source_panel_analysis_contract_repair_approval_after_research_return_gate_v1_latest.json"
    timestamped_json = OUT_DIR / f"repo_only_source_panel_analysis_contract_repair_approval_after_research_return_gate_v1_{stamp}.json"
    latest_txt = OUT_DIR / "repo_only_source_panel_analysis_contract_repair_approval_after_research_return_gate_v1_latest.txt"
...
def main() -> int:
    ...
    print(rendered)
    return 0 if payload["source_panel_analysis_contract_repair_approval_status"] == "PASS" else 3
