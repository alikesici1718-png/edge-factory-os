...
MODULE_NAME = "edge_factory_os_repo_only_development_queue_selector_after_standard_os_status_backlog_refresh_v1"
OUT_DIR = LAB_ROOT / "edge_factory_os_repo_only_development_queue_selector_after_standard_os_status_backlog_refresh_v1"
OUT_DIR.mkdir(parents=True, exist_ok=True)

EXPECTED_HEAD_PREFIX = "7c728ea"
CURRENT_TOOL_REL = "tools/edge_factory_os_repo_only_development_queue_selector_after_standard_os_status_backlog_refresh_v1.py"
...
BACKLOG_JSON = LAB_ROOT / "edge_factory_os_repo_only_development_backlog_refresh_after_standard_os_status_refresh_v1" / "repo_only_development_backlog_refresh_after_standard_os_status_refresh_v1_latest.json"
BACKLOG_POST_JSON = LAB_ROOT / "edge_factory_os_repo_only_development_backlog_refresh_after_standard_os_status_refresh_post_commit_check" / "repo_only_development_backlog_refresh_after_standard_os_status_refresh_post_commit_check_latest.json"

SELECTED_NEXT_MODULE = "edge_factory_os_repo_only_standard_os_status_refresh_after_standard_os_status_backlog_refresh_v1.py"
...
SAFETY_FLAGS: Dict[str, bool] = {
    "repo_only_development_queue_selector_after_standard_os_status_backlog_refresh": True,
    "read_only_development_queue_selection_only": True,
    "next_action_selector_after_standard_os_status_backlog_refresh_required": True,
    "standard_os_status_refresh_after_standard_os_status_backlog_refresh_allowed_next": True,
...
def first_status(obj: Dict[str, Any]) -> Optional[str]:
    for key in [
        ...
        "audit_status",
        "development_backlog_refresh_after_standard_os_status_refresh_status",
    ]:
        value = obj.get(key)
        if value:
            return str(value)
...
def validate_inputs() -> Dict[str, Any]:
    ...
    if subject != "Add repo-only next-action selector after standard OS status backlog refresh":
        errors.append(f"unexpected HEAD commit subject: {subject}")

    paths = commit_paths("HEAD")
    checks["head_commit_paths"] = paths
    if paths != ["tools/edge_factory_os_repo_only_next_action_selector_after_standard_os_status_backlog_refresh_v1.py"]:
        errors.append(f"unexpected HEAD commit paths: {paths}")

    inputs = {
        "next_action_selector_after_standard_os_status_backlog_refresh": (
            SELECTOR_JSON,
            "REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_V1_READY",
            "edge_factory_os_repo_only_development_queue_selector_after_standard_os_status_backlog_refresh_v1.py",
        ),
        "next_action_selector_after_standard_os_status_backlog_refresh_post_check": (
            SELECTOR_POST_JSON,
            "REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_POST_COMMIT_CHECK_PASS",
            "edge_factory_os_repo_only_development_queue_selector_after_standard_os_status_backlog_refresh_v1.py",
        ),
        "development_backlog_refresh_after_standard_os_status_refresh": (
            BACKLOG_JSON,
            "REPO_ONLY_DEVELOPMENT_BACKLOG_REFRESH_AFTER_STANDARD_OS_STATUS_REFRESH_V1_READY",
            None,
        ),
        "development_backlog_refresh_after_standard_os_status_refresh_post_check": (
            BACKLOG_POST_JSON,
            "REPO_ONLY_DEVELOPMENT_BACKLOG_REFRESH_AFTER_STANDARD_OS_STATUS_REFRESH_POST_COMMIT_CHECK_PASS",
            None,
        ),
    }
...
def build_queue(validation: Dict[str, Any]) -> Dict[str, Any]:
    selector_obj = validation["loaded"]["next_action_selector_after_standard_os_status_backlog_refresh"]
    ...
    backlog_obj = validation["loaded"]["development_backlog_refresh_after_standard_os_status_refresh"]
    backlog = backlog_obj.get("development_backlog_refresh_after_standard_os_status_refresh", {})
    selected_backlog_item = backlog.get("selected_next_backlog_item", {})

    physical_after = physical_guard_snapshot()

    prerequisites = {
        "parent_selector_ready": selector_obj.get("next_action_selector_after_standard_os_status_backlog_refresh_status") == "REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_V1_READY",
        "parent_selected_this_queue_selector": selected_action.get("module") == "edge_factory_os_repo_only_development_queue_selector_after_standard_os_status_backlog_refresh_v1.py",
        "parent_selected_repo_only_scope": selected_action.get("scope") == "REPO_ONLY_OS_INTELLIGENCE",
        "backlog_ready": backlog_obj.get("development_backlog_refresh_after_standard_os_status_refresh_status") == "REPO_ONLY_DEVELOPMENT_BACKLOG_REFRESH_AFTER_STANDARD_OS_STATUS_REFRESH_V1_READY",
        "backlog_selected_parent_selector": selected_backlog_item.get("module") == "edge_factory_os_repo_only_next_action_selector_after_standard_os_status_backlog_refresh_v1.py",
    ...
    queue_items = [
        {
            "rank": 1,
            "key": "STANDARD_OS_STATUS_REFRESH_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH",
            "module": SELECTED_NEXT_MODULE,
            "scope": "REPO_ONLY_OS_INTELLIGENCE",
            "allowed": True,
            "reason": "Standard OS status backlog selector is clean; refresh standard OS status again before broader repo-only work.",
            "blocked_actions": [
    ...
    return {
        "development_queue_status": "DEVELOPMENT_QUEUE_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_ACTIVE" if selected else "DEVELOPMENT_QUEUE_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_BLOCKED",
        ...
        "selected_next_action": "BUILD_REPO_ONLY_STANDARD_OS_STATUS_REFRESH_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_V1" if selected else None,
        ...
        "queue_policy": {
            "standard_os_status_refresh_selected": True,
        ...
        "invariants": {
            ...
            "selected_module_is_standard_os_status_refresh_after_standard_os_status_backlog_refresh": selected is not None and selected["module"] == SELECTED_NEXT_MODULE,
...
def main() -> int:
    ...
    payload = {
        "module": MODULE_NAME,
        "development_queue_selector_after_standard_os_status_backlog_refresh_status": "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_V1_READY" if passed else "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_V1_BLOCKED",
        "severity": "OK" if passed else "BLOCKED",
        "allowed_scope": "REPO_ONLY_OS_INTELLIGENCE",
        "final_decision": "DEVELOPMENT_QUEUE_SELECTED_STANDARD_OS_STATUS_REFRESH_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH" if passed else "KEEP_FREEZE_REVIEW_DEVELOPMENT_QUEUE_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_ERRORS",
        "next_action": "BUILD_REPO_ONLY_STANDARD_OS_STATUS_REFRESH_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_V1" if passed else "REVIEW_DEVELOPMENT_QUEUE_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_ERRORS",
        "next_module": SELECTED_NEXT_MODULE if passed else None,
        "reason": "Selected repo-only standard OS status refresh after standard OS status backlog refresh; schema apply/create/edit remain blocked and physically absent." if passed else "Development queue selector after standard OS status backlog refresh validation failed.",
        ...
        "development_queue_selector_after_standard_os_status_backlog_refresh": queue,
    ...
    latest_json = OUT_DIR / "repo_only_development_queue_selector_after_standard_os_status_backlog_refresh_v1_latest.json"
    timestamped_json = OUT_DIR / f"repo_only_development_queue_selector_after_standard_os_status_backlog_refresh_v1_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json"
    latest_txt = OUT_DIR / "repo_only_development_queue_selector_after_standard_os_status_backlog_refresh_v1_latest.txt"

    latest_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    timestamped_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")

    lines = [
        "EDGE FACTORY OS REPO-ONLY DEVELOPMENT QUEUE SELECTOR AFTER STANDARD OS STATUS BACKLOG REFRESH v1",
        "=" * 100,
        f"development_queue_selector_after_standard_os_status_backlog_refresh_status: {payload['development_queue_selector_after_standard_os_status_backlog_refresh_status']}",
