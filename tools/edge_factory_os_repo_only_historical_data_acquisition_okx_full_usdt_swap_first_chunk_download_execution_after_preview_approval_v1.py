...
MODULE_NAME = "edge_factory_os_repo_only_historical_data_acquisition_okx_full_usdt_swap_first_chunk_download_execution_after_preview_approval_v1"
REQUESTED_MODULE = f"{MODULE_NAME}.py"
EXPECTED_HEAD = "558338d"
PREVIOUS_STATUS = "PASS_HISTORICAL_DATA_ACQUISITION_OKX_FULL_USDT_SWAP_FIRST_CHUNK_DOWNLOAD_PREVIEW_APPROVED_EXECUTION_READY"
PASS_STATUS = "PASS_HISTORICAL_DATA_ACQUISITION_OKX_FULL_USDT_SWAP_FIRST_CHUNK_DOWNLOAD_EXECUTED_PENDING_VALIDATOR_NO_BUILD"
GAP_STATUS = "PASS_HISTORICAL_DATA_ACQUISITION_OKX_FULL_USDT_SWAP_FIRST_CHUNK_DOWNLOAD_EXECUTED_WITH_COVERAGE_GAPS_PENDING_VALIDATOR_NO_BUILD"
BLOCKED_STATUS = "BLOCKED_HISTORICAL_DATA_ACQUISITION_OKX_FULL_USDT_SWAP_FIRST_CHUNK_DOWNLOAD_EXECUTION_FAILED"
NEXT_MODULE = "edge_factory_os_repo_only_historical_data_acquisition_okx_full_usdt_swap_first_chunk_download_execution_validator_after_execution_v1.py"
FAILED_NEXT_MODULE = "edge_factory_os_repo_only_historical_data_acquisition_okx_full_usdt_swap_first_chunk_download_execution_blocked_record_v1.py"
AFTER_QUALITY_PASS = "HISTORICAL_DATA_ACQUISITION_OKX_FULL_USDT_SWAP_FIRST_CHUNK_DOWNLOAD_EXECUTED_PENDING_VALIDATOR_NO_BUILD"
AFTER_QUALITY_GAP = "HISTORICAL_DATA_ACQUISITION_OKX_FULL_USDT_SWAP_FIRST_CHUNK_DOWNLOAD_EXECUTED_WITH_COVERAGE_GAPS_PENDING_VALIDATOR_NO_BUILD"

REPO_ROOT = Path(r"C:\Users\alike\OneDrive\Desktop\edge_lab_new\edge_factory_os_repo")
EDGE_LAB_ROOT = REPO_ROOT.parent
APPROVED_TOOL = REPO_ROOT / "tools" / REQUESTED_MODULE
OUTPUT_DIR = EDGE_LAB_ROOT / MODULE_NAME
DOWNLOAD_DIR = OUTPUT_DIR / "downloaded_first_chunk_approved_quarantine"

PREVIEW_DIR = EDGE_LAB_ROOT / "edge_factory_os_repo_only_historical_data_acquisition_okx_full_usdt_swap_first_chunk_download_preview_after_manifest_preview_v1"
...
PREVIEW_SUMMARY = PREVIEW_DIR / "historical_okx_full_usdt_swap_first_chunk_download_preview_summary.json"
PREVIEW_MANIFEST = PREVIEW_DIR / "historical_okx_full_usdt_swap_first_chunk_planned_file_manifest_preview.json"
PREVIEW_APPROVAL = PREVIEW_DIR / "historical_okx_full_usdt_swap_first_chunk_download_execution_approval_record.json"
PREVIEW_RESOURCE_LIMITS = PREVIEW_DIR / "historical_okx_full_usdt_swap_first_chunk_download_resource_limits.json"
...
REQUIRED_OUTPUTS = [
    "historical_okx_full_usdt_swap_first_chunk_download_execution_report.json",
    "historical_okx_full_usdt_swap_first_chunk_download_file_manifest_after_execution.json",
    "historical_okx_full_usdt_swap_first_chunk_download_gap_report.json",
    "historical_okx_full_usdt_swap_first_chunk_reuse_validation_report.json",
    "historical_okx_full_usdt_swap_first_chunk_sha256_report.json",
    "historical_okx_full_usdt_swap_first_chunk_zip_inventory_report.json",
    "historical_okx_full_usdt_swap_first_chunk_schema_sample_report.json",
    "historical_okx_full_usdt_swap_first_chunk_download_execution_compliance_report.json",
    "historical_okx_full_usdt_swap_first_chunk_download_execution_summary.json",
]

APPROVED_CHUNK_ID = "chunk_01"
APPROVED_SYMBOLS = [
    "0G-USDT-SWAP",
    "1INCH-USDT-SWAP",
    "2Z-USDT-SWAP",
...
def load_and_validate_preview() -> dict[str, Any]:
    summary = load_json(PREVIEW_SUMMARY, "first_chunk_preview_summary")
    manifest = load_json(PREVIEW_MANIFEST, "first_chunk_planned_manifest_preview")
    approval = load_json(PREVIEW_APPROVAL, "first_chunk_download_execution_approval")
    resource_limits = load_json(PREVIEW_RESOURCE_LIMITS, "first_chunk_download_resource_limits")
    entries = manifest.get("planned_entries", [])
    require(isinstance(entries, list), "planned_entries must be a list")
    checks = {
        "previous_status_passed": summary.get("historical_data_acquisition_okx_full_usdt_swap_first_chunk_download_preview_status") == PREVIOUS_STATUS,
        "current_next_module_matches": summary.get("next_module") == REQUESTED_MODULE,
        "preview_created": summary.get("first_chunk_download_preview_created") is True,
        "chunk_id_valid": summary.get("chunk_id") == APPROVED_CHUNK_ID,
        "chunk_symbols_valid": summary.get("first_chunk_symbols") == APPROVED_SYMBOLS,
        "symbol_count_valid": summary.get("first_chunk_symbol_count") == len(APPROVED_SYMBOLS),
        "expected_file_count_valid": summary.get("expected_first_chunk_file_count") == EXPECTED_FIRST_CHUNK_FILE_COUNT,
        "planned_file_count_valid": len(entries) == EXPECTED_FIRST_CHUNK_FILE_COUNT and summary.get("planned_file_count") == EXPECTED_FIRST_CHUNK_FILE_COUNT,
        "reuse_candidate_count_valid": summary.get("reuse_candidate_file_count") == EXPECTED_REUSE_CANDIDATE_FILE_COUNT,
        "approval_available": approval.get("approval_grants_future_first_chunk_download_execution_next") is True,
...
def validate_manifest_entries(entries: list[dict[str, Any]]) -> None:
    ...
    for entry in entries:
        ...
        seen.add(key)
    require(len(seen) == EXPECTED_FIRST_CHUNK_FILE_COUNT, "manifest unique planned entry count mismatch")
...
def load_pilot_reuse_index() -> dict[str, dict[str, Any]]:
    ...
    require(len(reuse) == EXPECTED_REUSE_CANDIDATE_FILE_COUNT, "ADA reuse candidate count mismatch")
    return reuse
        ...
        if symbol == REUSE_SYMBOL:
            ...
            plan.append(
                PlannedFile(
                    ...
                    expected_schema_key=str(entry["expected_schema_key"]),
                    local_zip_path=path,
                    source_kind="NEW_APPROVED_FIRST_CHUNK_DOWNLOAD",
                )
            )
    require(len(plan) == EXPECTED_FIRST_CHUNK_FILE_COUNT, "planned execution file count mismatch")
...
def download_one(item: PlannedFile) -> dict[str, Any]:
    ...
    if item.local_zip_path.exists():
        return {
            ...
            "source_kind": "EXISTING_APPROVED_FIRST_CHUNK_FILE",
            "coverage_gap": False,
        }
    request = urllib.request.Request(item.url, headers={"User-Agent": "edge-factory-os-first-chunk-downloader/1.0"})
    last_error = ""
    ...
    return {
        ...
        "file_size_bytes": 0,
        "source_kind": "MISSING_OR_FAILED_APPROVED_FIRST_CHUNK_FILE",
        ...
        manifest.append(
            {
                ...
                "expected_inner_csv": item.expected_inner_csv,
                "local_zip_path": str(item.local_zip_path),
                "source_kind": item.source_kind if item.symbol == REUSE_SYMBOL else (download or {}).get("source_kind", "NEW_APPROVED_FIRST_CHUNK_DOWNLOAD"),
...
def write_reports(
    ...
    replacement_checks = {
        ...
        **preview["checks"],
        "approved_plan_count_valid": len(plan) == EXPECTED_FIRST_CHUNK_FILE_COUNT,
    ...
    base = {
        "historical_data_acquisition_okx_full_usdt_swap_first_chunk_download_execution_status": status,
        ...
        "first_chunk_download_valid_for_validator": replacement_checks_all_true,
...
def blocked_payload(message: str) -> dict[str, Any]:
    return {
        "historical_data_acquisition_okx_full_usdt_swap_first_chunk_download_execution_status": BLOCKED_STATUS,
        ...
        "source_manifest_acquisition_ready": False,
        "near_3y_eligible_symbol_count_now": 0,
        "first_chunk_download_valid_for_validator": False,
        "full_universe_acquisition_allowed_now": False,
        "data_build_allowed_now": False,
        "strategy_backtest_edge_allowed_now": False,
        ...
        "dormant_repo_attention_count": 716,
        "current_evidence_chain_quality_after_execution": "FIRST_CHUNK_DOWNLOAD_EXECUTION_FAILED_CLOSED",
...
def main() -> int:
    try:
        ...
        write_json(OUTPUT_DIR / "historical_okx_full_usdt_swap_first_chunk_download_execution_summary.json", blocked)
