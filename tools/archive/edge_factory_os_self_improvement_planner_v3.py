...
OUT_ROOT = WORKSPACE / "edge_factory_os_self_improvement_planner_v3"
...
def main() -> int:
    out_dir = OUT_ROOT / f"os_self_improvement_planner_v3_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    add_task(
        tasks, 900, "build_repo_source_map", "edge_factory_os_repo_source_map_v1.py",
        source_map_done, repo_only_allowed,
        f"Source map evidence: {source_map_evidence}",
        "" if repo_only_allowed else "standard_stack_not_clean_or_repo_dirty",
    )

    add_task(
        tasks, 875, "build_artifact_freshness_index", "edge_factory_os_artifact_freshness_index_v1.py",
        artifact_freshness_done, repo_only_allowed and source_map_done,
        f"Artifact freshness evidence: {artifact_freshness_evidence}",
        "" if repo_only_allowed and source_map_done else "source_map_required_or_repo_not_clean",
    )

    add_task(
        tasks, 850, "build_test_harness_manifest", "edge_factory_os_test_harness_manifest_v1.py",
        test_harness_done, repo_only_allowed and source_map_done and artifact_freshness_done,
        f"Test harness evidence: {test_harness_evidence}",
        "" if repo_only_allowed and source_map_done and artifact_freshness_done else "artifact_freshness_required",
    )

    add_task(
        tasks, 825, "build_module_dependency_graph", "edge_factory_os_dependency_graph_v1.py",
        dependency_graph_done, repo_only_allowed and source_map_done and artifact_freshness_done and test_harness_done,
        f"Dependency graph evidence: {dependency_graph_evidence}",
        "" if repo_only_allowed and source_map_done and artifact_freshness_done and test_harness_done else "test_harness_required",
    )

    add_task(
        tasks, 810, "review_dangerous_markers", "edge_factory_os_dangerous_marker_review_index_v1.py",
        dangerous_marker_review_done, repo_only_allowed and dependency_graph_done,
        f"Dangerous marker review evidence: {dangerous_marker_review_evidence}",
        "" if repo_only_allowed and dependency_graph_done else "dependency_graph_required",
    )

    add_task(
        tasks, 790, "build_repo_cleanup_plan", "edge_factory_os_repo_cleanup_plan_v1.py",
        repo_cleanup_plan_done,
        repo_only_allowed and dangerous_marker_review_done,
        f"Repo cleanup plan evidence: {repo_cleanup_plan_evidence}. Plan only; no delete/move.",
        "" if repo_only_allowed and dangerous_marker_review_done else "dangerous_marker_review_required",
    )

    add_task(
        tasks, 780, "build_runtime_apply_gate", "edge_factory_os_runtime_apply_gate_v1.py",
    ...
    add_task(
        tasks, 770, "build_candidate_generation_guard", "edge_factory_os_candidate_generation_guard_v1.py",
    ...
    state_path = out_dir / "os_self_improvement_planner_v3_state.json"
    latest_path = OUT_ROOT / "os_self_improvement_planner_latest.json"
    tasks_csv = out_dir / "os_self_improvement_planner_v3_tasks.csv"
    report_path = out_dir / "os_self_improvement_planner_v3_report.md"
    ...
    md = []
    md.append("# Edge Factory OS Self-Improvement Planner v3")
    md.append("")
    ...
    print("EDGE FACTORY OS SELF-IMPROVEMENT PLANNER v3")
    print("=" * 100)
