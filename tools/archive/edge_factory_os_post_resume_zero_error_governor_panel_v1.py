...
MODULE_NAME = "edge_factory_os_post_resume_zero_error_governor_panel_v1"
OUT_DIR = LAB_ROOT / "edge_factory_os_post_resume_zero_error_governor_panel_v1"
OUT_DIR.mkdir(parents=True, exist_ok=True)

EXPECTED_HEAD_PREFIX = "b39f256"
CURRENT_TOOL_REL = "tools/edge_factory_os_post_resume_zero_error_governor_panel_v1.py"
...
SELECTOR_POST_CHECK_JSON = LAB_ROOT / "edge_factory_os_repo_only_development_queue_selector_post_commit_check" / "repo_only_development_queue_selector_post_commit_check_latest.json"
STATUS_REFRESH_JSON = LAB_ROOT / "edge_factory_os_post_resume_repo_only_status_refresh_v1" / "post_resume_repo_only_status_refresh_v1_latest.json"
CONTROLLED_RESUME_JSON = LAB_ROOT / "edge_factory_os_controlled_resume_approval_v1" / "controlled_resume_approval_v1_latest.json"
...
WHOLE_SYSTEM_AUDIT_JSON = LAB_ROOT / "edge_factory_os_whole_system_external_audit_no_repo_file" / "whole_system_external_audit_latest.json"

SAFETY_FLAGS: Dict[str, bool] = {
    "post_resume_zero_error_governor_panel": True,
    "read_only_governor_panel_only": True,
...
def validate_status_record(
    ...
    if record["critical_issue_count"] != 0:
        errors.append(f"{key} critical_issue_count not 0: {record['critical_issue_count']}")

    if record["warning_count"] != 0:
        errors.append(f"{key} warning_count not 0: {record['warning_count']}")
...
def validate_inputs() -> Dict[str, Any]:
    ...
    if head_subject != "Add repo-only development queue selector":
        errors.append(f"unexpected HEAD commit subject: {head_subject}")

    inputs = {
        "selector": (SELECTOR_JSON, "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_V1_READY", "edge_factory_os_post_resume_zero_error_governor_panel_v1.py"),
        "selector_post_check": (SELECTOR_POST_CHECK_JSON, "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_POST_COMMIT_CHECK_PASS", "edge_factory_os_post_resume_zero_error_governor_panel_v1.py"),
        "status_refresh": (STATUS_REFRESH_JSON, "POST_RESUME_REPO_ONLY_STATUS_REFRESH_V1_READY", None),
        "controlled_resume": (CONTROLLED_RESUME_JSON, "CONTROLLED_RESUME_APPROVAL_V1_READY", None),
        "zero_error_protocol": (ZERO_ERROR_PROTOCOL_JSON, "ZERO_ERROR_RESUME_PROTOCOL_V1_READY", None),
        "whole_system_audit": (WHOLE_SYSTEM_AUDIT_JSON, "WHOLE_SYSTEM_EXTERNAL_AUDIT_PASS", None),
    }
...
def build_governor_panel(validation: Dict[str, Any]) -> Dict[str, Any]:
    ...
    clean_baseline = (
        validation["pass"]
        and validation["current_step_untracked_ok"]
    ...
    return {
        "panel_status": "POST_RESUME_ZERO_ERROR_GOVERNOR_PANEL_ACTIVE" if clean_baseline else "POST_RESUME_ZERO_ERROR_GOVERNOR_PANEL_BLOCKED",
        "panel_scope": "REPO_ONLY_GOVERNANCE_STATUS_PANEL",
        "current_repo_state": {
            "head": git_state["head"],
        ...
        "recommended_next_step": {
            "step": "BUILD_ZERO_ERROR_WORK_ORDER_TEMPLATE",
            "module": "edge_factory_os_zero_error_work_order_template_v1.py",
            "scope": "REPO_ONLY_GOVERNANCE_TEMPLATE",
            "reason": "The governor panel is now the shared state source; next create the reusable work-order template for future repo-only changes.",
        },
    }
...
def main() -> int:
    ...
    governor_panel = build_governor_panel(validation)

    if validation["pass"] and governor_panel["panel_status"] != "POST_RESUME_ZERO_ERROR_GOVERNOR_PANEL_ACTIVE":
        errors.append(f"governor panel did not become active: {governor_panel['panel_status']}")
    ...
    payload = {
        "module": MODULE_NAME,
        "governor_status": (
            "POST_RESUME_ZERO_ERROR_GOVERNOR_PANEL_V1_READY"
            if passed
            else "POST_RESUME_ZERO_ERROR_GOVERNOR_PANEL_V1_BLOCKED"
        ),
        "severity": "OK" if passed else "BLOCKED",
        "allowed_scope": "REPO_ONLY_GOVERNANCE_STATUS_PANEL",
        "final_decision": (
            "ZERO_ERROR_GOVERNOR_PANEL_READY_FOR_WORK_ORDER_TEMPLATE"
            if passed
            else "KEEP_FREEZE_REVIEW_GOVERNOR_PANEL_ERRORS"
        ),
        "next_action": (
            "BUILD_ZERO_ERROR_WORK_ORDER_TEMPLATE_V1"
            if passed
            else "REVIEW_GOVERNOR_PANEL_ERRORS"
        ),
        "next_module": (
            "edge_factory_os_zero_error_work_order_template_v1.py"
            if passed
            else None
        ),
        "reason": (
            "Created repo-only zero-error governor panel; future steps must consume this panel before work."
            if passed
            else "Governor panel validation failed."
        ),
    ...
    latest_json = OUT_DIR / "post_resume_zero_error_governor_panel_v1_latest.json"
    timestamped_json = OUT_DIR / f"post_resume_zero_error_governor_panel_v1_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json"
    latest_txt = OUT_DIR / "post_resume_zero_error_governor_panel_v1_latest.txt"

    latest_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    timestamped_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")

    lines = [
        "EDGE FACTORY OS POST-RESUME ZERO-ERROR GOVERNOR PANEL v1",
        "=" * 100,
        f"governor_status: {payload['governor_status']}",
        ...
        "GOVERNOR PANEL",
        "-" * 100,
        json.dumps(governor_panel, indent=2, sort_keys=True),
        "",
        "SAFETY",
        "-" * 100,
