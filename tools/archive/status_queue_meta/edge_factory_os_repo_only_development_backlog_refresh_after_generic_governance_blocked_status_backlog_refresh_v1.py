...
MODULE_NAME = "edge_factory_os_repo_only_development_backlog_refresh_after_generic_governance_blocked_status_backlog_refresh_v1"
OUT_DIR = LAB_ROOT / MODULE_NAME

EXPECTED_HEAD = "04e6427"
EXPECTED_TRACKED_PYTHON_COUNT = 541
CURRENT_TOOL_REL = "tools/edge_factory_os_repo_only_development_backlog_refresh_after_generic_governance_blocked_status_backlog_refresh_v1.py"
APPROVED_CURRENT_MODULE = "edge_factory_os_repo_only_development_backlog_refresh_after_generic_governance_blocked_status_backlog_refresh_v1.py"

READY_STATUS = "REPO_ONLY_DEVELOPMENT_BACKLOG_REFRESH_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_REFRESH_V1_READY"
FINAL_DECISION = "GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_REFRESH_BACKLOG_READY_FOR_NEXT_ACTION_SELECTOR"
NEXT_ACTION = "BUILD_REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_STATUS_BACKLOG_REFRESH_V1"
NEXT_MODULE = "edge_factory_os_repo_only_next_action_selector_after_generic_governance_blocked_status_backlog_status_backlog_refresh_v1.py"

QUEUE_JSON = (
    LAB_ROOT
    / "edge_factory_os_repo_only_development_queue_selector_after_generic_governance_blocked_status_backlog_refresh_v1"
    / "repo_only_development_queue_selector_after_generic_governance_blocked_status_backlog_refresh_v1_latest.json"
)

REQUIRED_QUEUE_STATUS = "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_REFRESH_V1_READY"
REQUIRED_QUEUE_POST_CHECK_STATUS = "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_REFRESH_POST_COMMIT_CHECK_PASS"
REQUIRED_QUEUE_NEXT_ACTION = "BUILD_REPO_ONLY_DEVELOPMENT_BACKLOG_REFRESH_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_REFRESH_V1"
REQUIRED_QUEUE_FINAL_DECISION = "GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_REFRESH_QUEUE_SELECTED_READY_FOR_BACKLOG_REFRESH"
...
SAFETY_FLAGS: Dict[str, bool] = {
    "repo_only_development_backlog_refresh_after_generic_governance_blocked_status_backlog_refresh": True,
    "development_backlog_refresh_only": True,
    "read_only_queue_evidence_validation_only": True,
    "next_action_selector_after_generic_governance_blocked_status_backlog_status_backlog_refresh_allowed_next": True,
...
def queue_route_object(queue_obj: Dict[str, Any]) -> Dict[str, Any]:
    route = queue_obj.get("development_queue_selector_after_generic_governance_blocked_status_backlog_refresh", {})
    return route if isinstance(route, dict) else {}


def validate_queue_record(errors: List[str], queue_obj: Dict[str, Any]) -> None:
    if queue_obj.get("development_queue_selector_after_generic_governance_blocked_status_backlog_refresh_status") != REQUIRED_QUEUE_STATUS:
        errors.append(
            "development queue selector status mismatch: "
            f"{queue_obj.get('development_queue_selector_after_generic_governance_blocked_status_backlog_refresh_status')}"
        )
    ...
    if queue_obj.get("next_module") != APPROVED_CURRENT_MODULE:
...
def validate_queue_route(errors: List[str], queue_obj: Dict[str, Any]) -> None:
    ...
    if not route:
        ...
        return
    selected = route.get("selected_development_task", {})
    if not isinstance(selected, dict):
        errors.append("development queue selector selected_development_task missing")
        return
    if selected.get("key") != "DEVELOPMENT_BACKLOG_REFRESH_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_REFRESH":
        errors.append(f"development queue selector selected key mismatch: {selected.get('key')}")
    if selected.get("module") != APPROVED_CURRENT_MODULE:
...
def derive_queue_post_check(git_state: Dict[str, Any], tracked_python: Dict[str, Any]) -> Dict[str, Any]:
    commit_paths_head = commit_paths("HEAD")
    ...
    if git_state["head"] != EXPECTED_HEAD:
        errors.append(f"latest commit mismatch: {git_state['head']}")
    if git_state["dirty_tracked_count"] != 0:
        errors.append(f"dirty tracked paths: {git_state['dirty_tracked_paths']}")
    if git_state["untracked_paths"] != [CURRENT_TOOL_REL]:
        errors.append(f"unexpected untracked paths during run: {git_state['untracked_paths']}")
    if commit_paths_head != ["tools/edge_factory_os_repo_only_development_queue_selector_after_generic_governance_blocked_status_backlog_refresh_v1.py"]:
        errors.append(f"latest commit paths mismatch: {commit_paths_head}")
    if not tracked_python["pass"]:
        errors.append("tracked Python syntax/BOM validation failed")
    if tracked_python["tracked_python_file_count"] != EXPECTED_TRACKED_PYTHON_COUNT:
        errors.append(f"tracked Python count mismatch: {tracked_python['tracked_python_file_count']}")
    if planned_existing:
        errors.append(f"planned schema files exist: {planned_existing}")
    if generic_runner_exists:
        errors.append("generic runner target file exists")
    return {
        "audit_status": REQUIRED_QUEUE_POST_CHECK_STATUS if not errors else "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_REFRESH_POST_COMMIT_CHECK_FAIL",
        "critical_issue_count": len(errors),
        "warning_count": 0,
        "errors": errors,
        "latest_commit": git_state["head"],
        "commit_paths": commit_paths_head,
        "next_module": APPROVED_CURRENT_MODULE,
        ...
        "derived_from_live_repo_state": True,
    }
...
def validate_inputs() -> Dict[str, Any]:
    ...
    if subject != "Add repo-only development queue selector after generic governance blocked status backlog refresh":
        errors.append(f"unexpected HEAD commit subject: {subject}")
    if paths != ["tools/edge_factory_os_repo_only_development_queue_selector_after_generic_governance_blocked_status_backlog_refresh_v1.py"]:
    ...
    post_check = derive_queue_post_check(git_state, tracked_python)
    loaded["development_queue_selector_post_check"] = post_check
    source_statuses["development_queue_selector_post_check"] = source_record(Path("<derived-live-repo-post-check>"), post_check)
    if post_check["audit_status"] != REQUIRED_QUEUE_POST_CHECK_STATUS:
        errors.append(f"development queue selector post-check status mismatch: {post_check['audit_status']}")
    if post_check["critical_issue_count"] != 0:
        errors.append(f"development queue selector post-check critical_issue_count not zero: {post_check['critical_issue_count']}")
    if post_check["latest_commit"] != EXPECTED_HEAD:
        errors.append(f"development queue selector post-check latest_commit mismatch: {post_check['latest_commit']}")
    if post_check["next_module"] != APPROVED_CURRENT_MODULE:
        errors.append(f"development queue selector post-check next_module mismatch: {post_check['next_module']}")
...
def build_backlog(validation: Dict[str, Any]) -> Dict[str, Any]:
    ...
    selected_from_parent = route.get("selected_development_task", {}) if isinstance(route.get("selected_development_task"), dict) else {}
    ...
    prerequisites = {
        ...
        "parent_queue_selector_ready": queue_obj.get("development_queue_selector_after_generic_governance_blocked_status_backlog_refresh_status") == REQUIRED_QUEUE_STATUS,
        "parent_selected_backlog_refresh": selected_from_parent.get("module") == APPROVED_CURRENT_MODULE,
        "parent_selected_backlog_refresh_key": selected_from_parent.get("key") == "DEVELOPMENT_BACKLOG_REFRESH_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_REFRESH",
    ...
    backlog_items = [
        {
            "rank": 1,
            "key": "NEXT_ACTION_SELECTOR_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_STATUS_BACKLOG_REFRESH",
            "module": NEXT_MODULE,
    ...
    return {
        "backlog_refresh_status": "DEVELOPMENT_BACKLOG_REFRESH_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_REFRESH_READY" if selected else "DEVELOPMENT_BACKLOG_REFRESH_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_REFRESH_BLOCKED",
        ...
        "confirmed_proofs": {
            ...
            "queue_selector_selected_repo_only_development_backlog_refresh_route": selected_from_parent.get("key") == "DEVELOPMENT_BACKLOG_REFRESH_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_REFRESH",
        },
        "physical_guards": {
        ...
        "invariants": {
            ...
            "selected_module_is_next_action_selector_after_generic_governance_blocked_status_backlog_status_backlog_refresh": selected is not None and selected["module"] == NEXT_MODULE,
            "selected_action_is_next_action_selector": selected is not None and selected["key"] == "NEXT_ACTION_SELECTOR_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_STATUS_BACKLOG_REFRESH",
...
def build_payload(validation: Dict[str, Any], backlog: Dict[str, Any], errors: List[str]) -> Dict[str, Any]:
    ...
    payload: Dict[str, Any] = {
        "module": MODULE_NAME,
        "development_backlog_refresh_after_generic_governance_blocked_status_backlog_refresh_status": (
            READY_STATUS
            if passed
            else "REPO_ONLY_DEVELOPMENT_BACKLOG_REFRESH_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_REFRESH_V1_BLOCKED"
        ),
        "severity": "OK" if passed else "BLOCKED",
        "allowed_scope": "REPO_ONLY_OS_INTELLIGENCE",
        "final_decision": FINAL_DECISION if passed else "KEEP_GENERIC_GOVERNANCE_BLOCKED_REVIEW_DEVELOPMENT_BACKLOG_REFRESH_AFTER_BACKLOG_REFRESH_ERRORS",
        "next_action": NEXT_ACTION if passed else "REVIEW_DEVELOPMENT_BACKLOG_REFRESH_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_REFRESH_ERRORS",
        "next_module": NEXT_MODULE if passed else None,
        "reason": (
            "Refreshed the repo-only development backlog after the generic governance blocked-status backlog-refresh queue selector and selected the next action selector."
            if passed
        ...
        "development_backlog_refresh_after_generic_governance_blocked_status_backlog_refresh": backlog,
...
def write_outputs(payload: Dict[str, Any]) -> Dict[str, str]:
    ...
    latest_json = OUT_DIR / "repo_only_development_backlog_refresh_after_generic_governance_blocked_status_backlog_refresh_v1_latest.json"
    timestamped_json = OUT_DIR / f"repo_only_development_backlog_refresh_after_generic_governance_blocked_status_backlog_refresh_v1_{stamp}.json"
    latest_txt = OUT_DIR / "repo_only_development_backlog_refresh_after_generic_governance_blocked_status_backlog_refresh_v1_latest.txt"
    ...
    lines = [
        "EDGE FACTORY OS REPO-ONLY DEVELOPMENT BACKLOG REFRESH AFTER GENERIC GOVERNANCE BLOCKED STATUS BACKLOG REFRESH v1",
        "=" * 100,
        f"development_backlog_refresh_after_generic_governance_blocked_status_backlog_refresh_status: {payload['development_backlog_refresh_after_generic_governance_blocked_status_backlog_refresh_status']}",
        ...
        json.dumps(
            {
                "selected_next_backlog_item": payload["development_backlog_refresh_after_generic_governance_blocked_status_backlog_refresh"]["selected_next_backlog_item"],
                "confirmed_proofs": payload["development_backlog_refresh_after_generic_governance_blocked_status_backlog_refresh"]["confirmed_proofs"],
                "prerequisites": payload["development_backlog_refresh_after_generic_governance_blocked_status_backlog_refresh"]["prerequisites"],
                "invariants": payload["development_backlog_refresh_after_generic_governance_blocked_status_backlog_refresh"]["invariants"],
