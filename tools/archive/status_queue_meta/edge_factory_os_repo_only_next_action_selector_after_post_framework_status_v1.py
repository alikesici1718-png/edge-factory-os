...
MODULE_NAME = "edge_factory_os_repo_only_next_action_selector_after_post_framework_status_v1"
OUT_DIR = LAB_ROOT / "edge_factory_os_repo_only_next_action_selector_after_post_framework_status_v1"
OUT_DIR.mkdir(parents=True, exist_ok=True)

EXPECTED_HEAD_PREFIX = "0482642"
CURRENT_TOOL_REL = "tools/edge_factory_os_repo_only_next_action_selector_after_post_framework_status_v1.py"
...
SELECTED_NEXT_MODULE = "edge_factory_os_repo_only_development_queue_selector_after_post_framework_status_v1.py"

SAFETY_FLAGS: Dict[str, bool] = {
    "repo_only_next_action_selector_after_post_framework_status": True,
    "read_only_next_action_selection_only": True,
    "post_framework_status_refresh_required": True,
    ...
    "development_queue_selector_after_post_framework_status_allowed_next": True,
...
def validate_inputs() -> Dict[str, Any]:
    ...
    if subject != "Add repo-only post-framework status refresh":
        errors.append(f"unexpected HEAD commit subject: {subject}")

    paths = commit_paths("HEAD")
    checks["head_commit_paths"] = paths
    if paths != ["tools/edge_factory_os_repo_only_post_framework_status_refresh_v1.py"]:
    ...
    inputs = {
        "post_framework_status_refresh": (
            ...
            "edge_factory_os_repo_only_next_action_selector_after_post_framework_status_v1.py",
        ),
...
def build_selector(validation: Dict[str, Any]) -> Dict[str, Any]:
    ...
    parent_next_task = status.get("next_task", {})
    parent_next_module = parent_next_task.get("module") or status_obj.get("next_module")
    ...
    prerequisites = {
        ...
        "parent_selected_this_selector": parent_next_module == "edge_factory_os_repo_only_next_action_selector_after_post_framework_status_v1.py",
        "readme_apply_blocked": backlog_context.get("readme_apply_blocked") is True,
        "manual_approval_present_false": backlog_context.get("manual_approval_present") is False,
        "manual_approval_valid_false": backlog_context.get("manual_approval_valid") is False,
    ...
    candidate_queue = [
        {
            "rank": 1,
            "key": "DEVELOPMENT_QUEUE_SELECTOR_AFTER_POST_FRAMEWORK_STATUS",
            "module": SELECTED_NEXT_MODULE,
            "scope": "REPO_ONLY_OS_INTELLIGENCE",
            "allowed": True,
            "reason": "Post-framework status is clean; choose the next repo-only development task with a queue selector.",
            "blocked_actions": [
                "runtime_touch",
                "launcher_execution",
                "capital_change",
        ...
        },
        {
            ...
            "allowed": False,
            "reason": "Strategy research remains blocked until a future explicit research approval and governance path exists.",
            "blocked_actions": ["strategy_research", "candidate_generation", "holdout_access", "family_release"],
        },
    ]
    ...
    return {
        "selector_status": "NEXT_ACTION_SELECTOR_AFTER_POST_FRAMEWORK_STATUS_ACTIVE" if selected else "NEXT_ACTION_SELECTOR_AFTER_POST_FRAMEWORK_STATUS_BLOCKED",
        "candidate_queue": candidate_queue,
        "selected_action": selected,
        "selected_next_module": selected["module"] if selected else None,
        "selected_next_action": "BUILD_REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_POST_FRAMEWORK_STATUS_V1" if selected else None,
        ...
        "context_summary": {
            ...
            "parent_next_module_detected": parent_next_module,
        },
        "selector_policy": {
            "prefer_queue_selector_after_status_refresh": True,
        ...
        "invariants": {
            ...
            "selected_module_is_development_queue_selector_after_post_framework_status": selected is not None and selected["module"] == SELECTED_NEXT_MODULE,
...
def main() -> int:
    ...
    payload = {
        "module": MODULE_NAME,
        "next_action_selector_after_post_framework_status_status": "REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_POST_FRAMEWORK_STATUS_V1_READY" if passed else "REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_POST_FRAMEWORK_STATUS_V1_BLOCKED",
        "severity": "OK" if passed else "BLOCKED",
        "allowed_scope": "REPO_ONLY_OS_INTELLIGENCE",
        "final_decision": "NEXT_ACTION_SELECTED_DEVELOPMENT_QUEUE_SELECTOR_AFTER_POST_FRAMEWORK_STATUS" if passed else "KEEP_FREEZE_REVIEW_NEXT_ACTION_SELECTOR_AFTER_POST_FRAMEWORK_STATUS_ERRORS",
        "next_action": "BUILD_REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_POST_FRAMEWORK_STATUS_V1" if passed else "REVIEW_NEXT_ACTION_SELECTOR_AFTER_POST_FRAMEWORK_STATUS_ERRORS",
        "next_module": SELECTED_NEXT_MODULE if passed else None,
        "reason": "Selected repo-only development queue selector after clean post-framework status refresh." if passed else "Next-action selector after post-framework status validation failed.",
        ...
        "next_action_selector_after_post_framework_status": selector,
    ...
    latest_json = OUT_DIR / "repo_only_next_action_selector_after_post_framework_status_v1_latest.json"
    timestamped_json = OUT_DIR / f"repo_only_next_action_selector_after_post_framework_status_v1_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json"
    latest_txt = OUT_DIR / "repo_only_next_action_selector_after_post_framework_status_v1_latest.txt"

    latest_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    timestamped_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")

    lines = [
        "EDGE FACTORY OS REPO-ONLY NEXT ACTION SELECTOR AFTER POST-FRAMEWORK STATUS v1",
        "=" * 100,
        f"next_action_selector_after_post_framework_status_status: {payload['next_action_selector_after_post_framework_status_status']}",
