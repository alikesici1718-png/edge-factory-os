...
MODULE_NAME = "edge_factory_os_repo_only_next_action_selector_after_post_schema_registry_governance_status_refresh_v1"
OUT_DIR = LAB_ROOT / "edge_factory_os_repo_only_next_action_selector_after_post_schema_registry_governance_status_refresh_v1"
OUT_DIR.mkdir(parents=True, exist_ok=True)

EXPECTED_HEAD_PREFIX = "40be394"
CURRENT_TOOL_REL = "tools/edge_factory_os_repo_only_next_action_selector_after_post_schema_registry_governance_status_refresh_v1.py"
...
SELECTED_NEXT_MODULE = "edge_factory_os_repo_only_development_queue_selector_after_post_schema_registry_governance_status_refresh_v1.py"
...
SAFETY_FLAGS: Dict[str, bool] = {
    "repo_only_next_action_selector_after_post_schema_registry_governance_status_refresh": True,
    "read_only_next_action_selection_only": True,
    "post_schema_registry_governance_status_refresh_required": True,
    "development_queue_selector_after_post_schema_registry_governance_status_refresh_allowed_next": True,
...
def validate_inputs() -> Dict[str, Any]:
    ...
    if physical_before["planned_schema_file_existing_count"] != 0:
        errors.append(f"planned schema files existed before next-action selector: {physical_before['existing_planned_schema_files']}")

    subject = commit_subject("HEAD")
    checks["head_commit_subject"] = subject
    if subject != "Add repo-only post schema registry governance status refresh":
        errors.append(f"unexpected HEAD commit subject: {subject}")

    paths = commit_paths("HEAD")
    checks["head_commit_paths"] = paths
    if paths != ["tools/edge_factory_os_repo_only_post_schema_registry_governance_status_refresh_v1.py"]:
        errors.append(f"unexpected HEAD commit paths: {paths}")

    inputs = {
        "post_schema_registry_governance_status_refresh": (
            REFRESH_JSON,
            "REPO_ONLY_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_V1_READY",
            "edge_factory_os_repo_only_next_action_selector_after_post_schema_registry_governance_status_refresh_v1.py",
        ),
        "post_schema_registry_governance_status_refresh_post_check": (
            REFRESH_POST_JSON,
            "REPO_ONLY_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_POST_COMMIT_CHECK_PASS",
            "edge_factory_os_repo_only_next_action_selector_after_post_schema_registry_governance_status_refresh_v1.py",
        ),
        "queue_selector_after_closure": (
            QUEUE_JSON,
...
def build_selector(validation: Dict[str, Any]) -> Dict[str, Any]:
    refresh_obj = validation["loaded"]["post_schema_registry_governance_status_refresh"]
    ...
    selected_next_step = as_dict(refresh.get("selected_next_step", {}))
    ...
    queue_obj = validation["loaded"]["queue_selector_after_closure"]
    queue = as_dict(queue_obj.get("development_queue_selector_after_schema_registry_closure_record", {}))
    queue_selected = as_dict(queue.get("selected_development_task", {}))
    queue_context = as_dict(queue.get("context_summary", {}))

    selector_obj = validation["loaded"]["next_action_selector_after_closure"]
    previous_selector = as_dict(selector_obj.get("next_action_selector_after_schema_registry_closure_record", {}))
    previous_selector_context = as_dict(previous_selector.get("context_summary", {}))
    ...
    effective_approval_state = {
        "manual_approval_present": first_not_none(
            ...
            previous_selector_context.get("manual_approval_present"),
        ...
        "manual_approval_valid": first_not_none(
            ...
            previous_selector_context.get("manual_approval_valid"),
        ...
        "approval_granted_now": first_not_none(
            ...
            previous_selector_context.get("approval_granted_now"),
    ...
    effective_schema_state = {
        "schema_apply_allowed_now": first_not_none(
            ...
            previous_selector_context.get("schema_apply_allowed_now"),
        ...
        "schema_file_creation_allowed_now": first_not_none(
            ...
            previous_selector_context.get("schema_file_creation_allowed_now"),
        ...
        "schema_file_edit_allowed_now": first_not_none(
            ...
            previous_selector_context.get("schema_file_edit_allowed_now"),
    ...
    prerequisites = {
        "refresh_ready": refresh_obj.get("post_schema_registry_governance_status_refresh_status") == "REPO_ONLY_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_V1_READY",
        "refresh_selected_this_selector": refresh_obj.get("next_module") == "edge_factory_os_repo_only_next_action_selector_after_post_schema_registry_governance_status_refresh_v1.py",
        "refresh_next_step_selected_this_selector": selected_next_step.get("module") == "edge_factory_os_repo_only_next_action_selector_after_post_schema_registry_governance_status_refresh_v1.py",
        "governance_chain_closed": governance_status.get("schema_registry_governance_chain_closed") is True,
        ...
        "queue_selected_refresh": queue_selected.get("module") == "edge_factory_os_repo_only_post_schema_registry_governance_status_refresh_v1.py",
        ...
        "closure_schema_registry_apply_closed": closure_decision.get("schema_registry_apply_closed_now") is True,
        "manual_record_ready": manual_obj.get("framework_schema_registry_manual_approval_record_status") == "REPO_ONLY_FRAMEWORK_SCHEMA_REGISTRY_MANUAL_APPROVAL_RECORD_V1_READY",
    ...
    candidate_actions = [
        {
            "rank": 1,
            "key": "DEVELOPMENT_QUEUE_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH",
            "module": SELECTED_NEXT_MODULE,
            "scope": "REPO_ONLY_OS_INTELLIGENCE",
            "allowed": True,
            "reason": "Post-schema-registry governance status is clean; select next repo-only development queue.",
            "blocked_actions": [
        ...
        },
        {
            ...
            "allowed": False,
            "reason": "Blocked because schema registry governance closed without exact manual approval.",
    ...
    return {
        "selector_status": "NEXT_ACTION_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_ACTIVE" if selected else "NEXT_ACTION_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_BLOCKED",
        "candidate_actions": candidate_actions,
        "candidate_action_count": len(candidate_actions),
        "selected_action": selected,
        "selected_next_module": selected["module"] if selected else None,
        "selected_next_action": "BUILD_REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_V1" if selected else None,
        "selected_scope": selected["scope"] if selected else None,
        "prerequisites": prerequisites,
        "context_summary": {
            "refresh_status": refresh_obj.get("post_schema_registry_governance_status_refresh_status"),
            "refresh_selected_next_module": selected_next_step.get("module"),
            "governance_status": governance_status,
            "queue_selected_module": queue_selected.get("module"),
            "closure_apply_closed": closure_decision.get("schema_registry_apply_closed_now"),
            ...
            "state_source_order": "refresh effective state -> governance_status -> refresh_context -> queue_context -> previous selector context -> closure decision/summary -> manual approval record",
        ...
        "selector_policy": {
            "post_schema_registry_governance_status_refresh_clean": True,
        ...
        "invariants": {
            ...
            "selected_module_is_development_queue_selector_after_post_schema_registry_governance_status_refresh": selected is not None and selected["module"] == SELECTED_NEXT_MODULE,
...
def main() -> int:
    ...
    payload = {
        "module": MODULE_NAME,
        "next_action_selector_after_post_schema_registry_governance_status_refresh_status": "REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_V1_READY" if passed else "REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_V1_BLOCKED",
        "severity": "OK" if passed else "BLOCKED",
        "allowed_scope": "REPO_ONLY_OS_INTELLIGENCE",
        "final_decision": "NEXT_ACTION_SELECTED_DEVELOPMENT_QUEUE_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH" if passed else "KEEP_FREEZE_REVIEW_NEXT_ACTION_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_ERRORS",
        "next_action": "BUILD_REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_V1" if passed else "REVIEW_NEXT_ACTION_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_ERRORS",
        "next_module": SELECTED_NEXT_MODULE if passed else None,
        "reason": "Selected repo-only development queue selector after post-schema-registry governance status refresh; schema apply/create/edit remain blocked and physically absent." if passed else "Next-action selector after post-schema-registry governance status refresh validation failed.",
        ...
        "next_action_selector_after_post_schema_registry_governance_status_refresh": selector,
    ...
    latest_json = OUT_DIR / "repo_only_next_action_selector_after_post_schema_registry_governance_status_refresh_v1_latest.json"
    timestamped_json = OUT_DIR / f"repo_only_next_action_selector_after_post_schema_registry_governance_status_refresh_v1_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json"
    latest_txt = OUT_DIR / "repo_only_next_action_selector_after_post_schema_registry_governance_status_refresh_v1_latest.txt"

    latest_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    timestamped_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")

    lines = [
        "EDGE FACTORY OS REPO-ONLY NEXT ACTION SELECTOR AFTER POST-SCHEMA-REGISTRY GOVERNANCE STATUS REFRESH v1",
        "=" * 100,
        f"next_action_selector_after_post_schema_registry_governance_status_refresh_status: {payload['next_action_selector_after_post_schema_registry_governance_status_refresh_status']}",
