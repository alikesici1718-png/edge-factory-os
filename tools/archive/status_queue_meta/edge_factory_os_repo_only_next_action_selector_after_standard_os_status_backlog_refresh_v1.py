...
MODULE_NAME = "edge_factory_os_repo_only_next_action_selector_after_standard_os_status_backlog_refresh_v1"
OUT_DIR = LAB_ROOT / "edge_factory_os_repo_only_next_action_selector_after_standard_os_status_backlog_refresh_v1"
OUT_DIR.mkdir(parents=True, exist_ok=True)

EXPECTED_HEAD_PREFIX = "7c3389b"
CURRENT_TOOL_REL = "tools/edge_factory_os_repo_only_next_action_selector_after_standard_os_status_backlog_refresh_v1.py"
...
QUEUE_JSON = LAB_ROOT / "edge_factory_os_repo_only_development_queue_selector_after_standard_os_status_refresh_v1" / "repo_only_development_queue_selector_after_standard_os_status_refresh_v1_latest.json"
QUEUE_POST_JSON = LAB_ROOT / "edge_factory_os_repo_only_development_queue_selector_after_standard_os_status_refresh_post_commit_check" / "repo_only_development_queue_selector_after_standard_os_status_refresh_post_commit_check_latest.json"

SELECTED_NEXT_MODULE = "edge_factory_os_repo_only_development_queue_selector_after_standard_os_status_backlog_refresh_v1.py"
...
SAFETY_FLAGS: Dict[str, bool] = {
    "repo_only_next_action_selector_after_standard_os_status_backlog_refresh": True,
    "read_only_next_action_selection_only": True,
    "development_backlog_refresh_after_standard_os_status_refresh_required": True,
    "development_queue_selector_after_standard_os_status_backlog_refresh_allowed_next": True,
...
def first_status(obj: Dict[str, Any]) -> Optional[str]:
    for key in [
        ...
        "audit_status",
        "development_queue_selector_after_standard_os_status_refresh_status",
    ]:
        value = obj.get(key)
        if value:
            return str(value)
...
def validate_inputs() -> Dict[str, Any]:
    ...
    if subject != "Add repo-only development backlog refresh after standard OS status refresh":
        errors.append(f"unexpected HEAD commit subject: {subject}")

    paths = commit_paths("HEAD")
    checks["head_commit_paths"] = paths
    if paths != ["tools/edge_factory_os_repo_only_development_backlog_refresh_after_standard_os_status_refresh_v1.py"]:
    ...
    inputs = {
        "development_backlog_refresh": (
            ...
            "edge_factory_os_repo_only_next_action_selector_after_standard_os_status_backlog_refresh_v1.py",
        ),
        ...
        "development_queue_selector": (
            QUEUE_JSON,
            "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_STANDARD_OS_STATUS_REFRESH_V1_READY",
            None,
        ),
        "development_queue_selector_post_check": (
            QUEUE_POST_JSON,
            "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_STANDARD_OS_STATUS_REFRESH_POST_COMMIT_CHECK_PASS",
            None,
        ),
    }
...
def build_selector(validation: Dict[str, Any]) -> Dict[str, Any]:
    ...
    selected_item = backlog.get("selected_next_backlog_item", {})
    context = backlog.get("context_summary", {})
    ...
    queue = queue_obj.get("development_queue_selector_after_standard_os_status_refresh", {})
    ...
    prerequisites = {
        ...
        "backlog_selected_this_selector": backlog_obj.get("next_module") == "edge_factory_os_repo_only_next_action_selector_after_standard_os_status_backlog_refresh_v1.py",
        "backlog_selected_this_selector_item": selected_item.get("module") == "edge_factory_os_repo_only_next_action_selector_after_standard_os_status_backlog_refresh_v1.py",
        ...
        "queue_ready": queue_obj.get("development_queue_selector_after_standard_os_status_refresh_status") == "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_STANDARD_OS_STATUS_REFRESH_V1_READY",
        "queue_selected_backlog_refresh": selected_queue_task.get("module") == "edge_factory_os_repo_only_development_backlog_refresh_after_standard_os_status_refresh_v1.py",
    ...
    candidate_actions = [
        {
            "rank": 1,
            "key": "DEVELOPMENT_QUEUE_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH",
    ...
    return {
        "selector_status": "NEXT_ACTION_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_ACTIVE" if selected else "NEXT_ACTION_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_BLOCKED",
        "candidate_actions": candidate_actions,
        "candidate_action_count": len(candidate_actions),
        "selected_action": selected,
        "selected_next_module": selected["module"] if selected else None,
        "selected_next_action": "BUILD_REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_V1" if selected else None,
        "selected_scope": selected["scope"] if selected else None,
        "prerequisites": prerequisites,
        "context_summary": {
            "backlog_status": backlog_obj.get("development_backlog_refresh_after_standard_os_status_refresh_status"),
        ...
        "invariants": {
            ...
            "selected_module_is_development_queue_selector_after_standard_os_status_backlog_refresh": selected is not None and selected["module"] == SELECTED_NEXT_MODULE,
...
def main() -> int:
    ...
    payload = {
        "module": MODULE_NAME,
        "next_action_selector_after_standard_os_status_backlog_refresh_status": "REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_V1_READY" if passed else "REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_V1_BLOCKED",
        "severity": "OK" if passed else "BLOCKED",
        "allowed_scope": "REPO_ONLY_OS_INTELLIGENCE",
        "final_decision": "NEXT_ACTION_SELECTED_DEVELOPMENT_QUEUE_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH" if passed else "KEEP_FREEZE_REVIEW_NEXT_ACTION_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_ERRORS",
        "next_action": "BUILD_REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_V1" if passed else "REVIEW_NEXT_ACTION_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_ERRORS",
        "next_module": SELECTED_NEXT_MODULE if passed else None,
        "reason": "Selected repo-only development queue selector after standard OS status backlog refresh; schema apply/create/edit remain blocked and physically absent." if passed else "Next-action selector after standard OS status backlog refresh validation failed.",
        ...
        "next_action_selector_after_standard_os_status_backlog_refresh": selector,
    ...
    latest_json = OUT_DIR / "repo_only_next_action_selector_after_standard_os_status_backlog_refresh_v1_latest.json"
    timestamped_json = OUT_DIR / f"repo_only_next_action_selector_after_standard_os_status_backlog_refresh_v1_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json"
    latest_txt = OUT_DIR / "repo_only_next_action_selector_after_standard_os_status_backlog_refresh_v1_latest.txt"

    latest_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    timestamped_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")

    lines = [
        "EDGE FACTORY OS REPO-ONLY NEXT ACTION SELECTOR AFTER STANDARD OS STATUS BACKLOG REFRESH v1",
        "=" * 100,
        f"next_action_selector_after_standard_os_status_backlog_refresh_status: {payload['next_action_selector_after_standard_os_status_backlog_refresh_status']}",
