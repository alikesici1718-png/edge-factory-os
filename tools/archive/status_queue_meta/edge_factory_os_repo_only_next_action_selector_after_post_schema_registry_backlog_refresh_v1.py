...
MODULE_NAME = "edge_factory_os_repo_only_next_action_selector_after_post_schema_registry_backlog_refresh_v1"
OUT_DIR = LAB_ROOT / "edge_factory_os_repo_only_next_action_selector_after_post_schema_registry_backlog_refresh_v1"
OUT_DIR.mkdir(parents=True, exist_ok=True)

EXPECTED_HEAD_PREFIX = "6c909e8"
CURRENT_TOOL_REL = "tools/edge_factory_os_repo_only_next_action_selector_after_post_schema_registry_backlog_refresh_v1.py"
...
SELECTOR_JSON = LAB_ROOT / "edge_factory_os_repo_only_next_action_selector_after_post_schema_registry_status_v1" / "repo_only_next_action_selector_after_post_schema_registry_status_v1_latest.json"
...
SELECTED_NEXT_MODULE = "edge_factory_os_repo_only_development_queue_selector_after_post_schema_registry_backlog_refresh_v1.py"
...
SAFETY_FLAGS: Dict[str, bool] = {
    "repo_only_next_action_selector_after_post_schema_registry_backlog_refresh": True,
    "read_only_next_action_selection_only": True,
    "development_backlog_refresh_after_post_schema_registry_status_required": True,
    "development_queue_selector_after_post_schema_registry_backlog_refresh_allowed_next": True,
...
def first_status(obj: Dict[str, Any]) -> Optional[str]:
    for key in [
        ...
        "next_action_selector_after_post_schema_registry_status_status",
...
def validate_inputs() -> Dict[str, Any]:
    ...
    if git_state["dirty_tracked_count"] != 0:
        errors.append(f"dirty tracked paths present before backlog next-action selector: {git_state['dirty_tracked_paths']}")

    if actual_untracked != expected_untracked:
        errors.append(f"unexpected untracked set before backlog next-action selector: expected={sorted(expected_untracked)} actual={sorted(actual_untracked)}")

    if physical_before["planned_schema_file_existing_count"] != 0:
        errors.append(f"planned schema files existed before backlog next-action selector: {physical_before['existing_planned_schema_files']}")

    subject = commit_subject("HEAD")
    checks["head_commit_subject"] = subject
    if subject != "Add repo-only development backlog refresh after post-schema-registry status":
        errors.append(f"unexpected HEAD commit subject: {subject}")

    paths = commit_paths("HEAD")
    checks["head_commit_paths"] = paths
    if paths != ["tools/edge_factory_os_repo_only_development_backlog_refresh_after_post_schema_registry_status_v1.py"]:
    ...
    inputs = {
        "development_backlog_refresh": (
            ...
            "edge_factory_os_repo_only_next_action_selector_after_post_schema_registry_backlog_refresh_v1.py",
        ),
        ...
        "previous_next_action_selector": (
            SELECTOR_JSON,
            "REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_STATUS_V1_READY",
            None,
        ),
...
def build_selector(validation: Dict[str, Any]) -> Dict[str, Any]:
    ...
    queue = as_dict(queue_obj.get("development_queue_selector_after_post_schema_registry_status", {}))
    ...
    prerequisites = {
        ...
        "backlog_selected_this_selector": selected_backlog_item.get("module") == "edge_factory_os_repo_only_next_action_selector_after_post_schema_registry_backlog_refresh_v1.py",
        "backlog_selected_repo_only_scope": selected_backlog_item.get("scope") == "REPO_ONLY_OS_INTELLIGENCE",
        "queue_ready": queue_obj.get("development_queue_selector_after_post_schema_registry_status_status") == "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_STATUS_V1_READY",
        "queue_selected_backlog_refresh": queue_selected.get("module") == "edge_factory_os_repo_only_development_backlog_refresh_after_post_schema_registry_status_v1.py",
    ...
    candidate_actions = [
        {
            "rank": 1,
            "key": "DEVELOPMENT_QUEUE_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_BACKLOG_REFRESH",
            "module": SELECTED_NEXT_MODULE,
            "scope": "REPO_ONLY_OS_INTELLIGENCE",
            "allowed": True,
            "reason": "Backlog refresh is clean; select repo-only queue after backlog refresh while schema apply remains blocked.",
            "blocked_actions": [
    ...
    return {
        "selector_status": "NEXT_ACTION_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_BACKLOG_REFRESH_ACTIVE" if selected else "NEXT_ACTION_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_BACKLOG_REFRESH_BLOCKED",
        "candidate_actions": candidate_actions,
        "candidate_action_count": len(candidate_actions),
        "selected_action": selected,
        "selected_next_module": selected["module"] if selected else None,
        "selected_next_action": "BUILD_REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_BACKLOG_REFRESH_V1" if selected else None,
        "selected_scope": selected["scope"] if selected else None,
        "prerequisites": prerequisites,
        "context_summary": {
            "backlog_selected_module": selected_backlog_item.get("module"),
            "backlog_selected_scope": selected_backlog_item.get("scope"),
            ...
            "approval_state_source_order": "status_summary.approval_state -> status_summary flat -> backlog_context -> queue_context -> manual approval record",
            "schema_state_source_order": "status_summary.schema_action_state -> status_summary flat -> backlog_context -> queue_context -> manual approval record",
        ...
        "invariants": {
            ...
            "selected_module_is_development_queue_selector_after_post_schema_registry_backlog_refresh": selected is not None and selected["module"] == SELECTED_NEXT_MODULE,
...
def main() -> int:
    ...
    payload = {
        "module": MODULE_NAME,
        "next_action_selector_after_post_schema_registry_backlog_refresh_status": "REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_BACKLOG_REFRESH_V1_READY" if passed else "REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_BACKLOG_REFRESH_V1_BLOCKED",
        "severity": "OK" if passed else "BLOCKED",
        "allowed_scope": "REPO_ONLY_OS_INTELLIGENCE",
        "final_decision": "NEXT_ACTION_SELECTED_DEVELOPMENT_QUEUE_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_BACKLOG_REFRESH" if passed else "KEEP_FREEZE_REVIEW_NEXT_ACTION_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_BACKLOG_REFRESH_ERRORS",
        "next_action": "BUILD_REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_BACKLOG_REFRESH_V1" if passed else "REVIEW_NEXT_ACTION_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_BACKLOG_REFRESH_ERRORS",
        "next_module": SELECTED_NEXT_MODULE if passed else None,
        "reason": "Selected repo-only development queue selector after backlog refresh; schema apply/create/edit remain blocked and physically absent." if passed else "Next-action selector after post-schema-registry backlog refresh validation failed.",
        ...
        "next_action_selector_after_post_schema_registry_backlog_refresh": selector,
    ...
    latest_json = OUT_DIR / "repo_only_next_action_selector_after_post_schema_registry_backlog_refresh_v1_latest.json"
    timestamped_json = OUT_DIR / f"repo_only_next_action_selector_after_post_schema_registry_backlog_refresh_v1_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json"
    latest_txt = OUT_DIR / "repo_only_next_action_selector_after_post_schema_registry_backlog_refresh_v1_latest.txt"

    latest_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    timestamped_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")

    lines = [
        "EDGE FACTORY OS REPO-ONLY NEXT ACTION SELECTOR AFTER POST-SCHEMA-REGISTRY BACKLOG REFRESH v1",
        "=" * 100,
        f"next_action_selector_after_post_schema_registry_backlog_refresh_status: {payload['next_action_selector_after_post_schema_registry_backlog_refresh_status']}",
