...
MODULE_NAME = "edge_factory_os_repo_only_development_queue_selector_after_schema_registry_closure_record_v1"
OUT_DIR = LAB_ROOT / "edge_factory_os_repo_only_development_queue_selector_after_schema_registry_closure_record_v1"
OUT_DIR.mkdir(parents=True, exist_ok=True)

EXPECTED_HEAD_PREFIX = "1bc85b8"
CURRENT_TOOL_REL = "tools/edge_factory_os_repo_only_development_queue_selector_after_schema_registry_closure_record_v1.py"
...
SELECTOR_POST_JSON = LAB_ROOT / "edge_factory_os_repo_only_next_action_selector_after_schema_registry_closure_record_post_commit_check" / "repo_only_next_action_selector_after_schema_registry_closure_record_post_commit_check_latest.json"
...
SELECTED_NEXT_MODULE = "edge_factory_os_repo_only_post_schema_registry_governance_status_refresh_v1.py"
...
SAFETY_FLAGS: Dict[str, bool] = {
    "repo_only_development_queue_selector_after_schema_registry_closure_record": True,
    "read_only_development_queue_selection_only": True,
    "next_action_selector_after_schema_registry_closure_record_required": True,
    "post_schema_registry_governance_status_refresh_allowed_next": True,
...
def validate_inputs() -> Dict[str, Any]:
    ...
    if physical_before["planned_schema_file_existing_count"] != 0:
        errors.append(f"planned schema files existed before development queue selector: {physical_before['existing_planned_schema_files']}")

    subject = commit_subject("HEAD")
    checks["head_commit_subject"] = subject
    if subject != "Add repo-only next-action selector after schema registry closure record":
        errors.append(f"unexpected HEAD commit subject: {subject}")

    paths = commit_paths("HEAD")
    checks["head_commit_paths"] = paths
    if paths != ["tools/edge_factory_os_repo_only_next_action_selector_after_schema_registry_closure_record_v1.py"]:
    ...
    inputs = {
        "next_action_selector_after_closure": (
            ...
            "edge_factory_os_repo_only_development_queue_selector_after_schema_registry_closure_record_v1.py",
        ),
        "next_action_selector_after_closure_post_check": (
            SELECTOR_POST_JSON,
            "REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_SCHEMA_REGISTRY_CLOSURE_RECORD_POST_COMMIT_CHECK_PASS",
            "edge_factory_os_repo_only_development_queue_selector_after_schema_registry_closure_record_v1.py",
        ),
...
def build_queue(validation: Dict[str, Any]) -> Dict[str, Any]:
    ...
    prerequisites = {
        "parent_selector_ready": selector_obj.get("next_action_selector_after_schema_registry_closure_record_status") == "REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_SCHEMA_REGISTRY_CLOSURE_RECORD_V1_READY",
        "parent_selected_this_queue_selector": selected_action.get("module") == "edge_factory_os_repo_only_development_queue_selector_after_schema_registry_closure_record_v1.py",
        ...
        "closure_selected_parent_selector": closure_next_step.get("module") == "edge_factory_os_repo_only_next_action_selector_after_schema_registry_closure_record_v1.py",
        "schema_registry_apply_closed": closure_decision.get("schema_registry_apply_closed_now") is True,
        "closure_schema_apply_allowed_false": closure_decision.get("schema_apply_allowed_now") is False,
        "closure_schema_file_creation_allowed_false": closure_decision.get("schema_file_creation_allowed_now") is False,
        "closure_schema_file_edit_allowed_false": closure_decision.get("schema_file_edit_allowed_now") is False,
    ...
    queue_items = [
        {
            "rank": 1,
            "key": "POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH",
            "module": SELECTED_NEXT_MODULE,
            "scope": "REPO_ONLY_OS_INTELLIGENCE",
            "allowed": True,
            "reason": "Schema-registry governance chain is closed without apply; refresh general governance status before selecting future repo-only work.",
            "blocked_actions": [
    ...
    return {
        "development_queue_status": "DEVELOPMENT_QUEUE_AFTER_SCHEMA_REGISTRY_CLOSURE_RECORD_ACTIVE" if selected else "DEVELOPMENT_QUEUE_AFTER_SCHEMA_REGISTRY_CLOSURE_RECORD_BLOCKED",
        ...
        "selected_next_action": "BUILD_REPO_ONLY_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_V1" if selected else None,
        ...
        "context_summary": {
            ...
            "approval_state_source_order": "status_summary.approval_state -> status_summary flat -> parent selector context -> closure chain_summary -> manual approval record",
            "schema_state_source_order": "status_summary.schema_action_state -> status_summary flat -> parent selector context -> closure decision -> closure chain_summary -> manual approval record",
        ...
        "queue_policy": {
            ...
            "post_schema_registry_governance_status_refresh_selected": True,
        ...
        "invariants": {
            ...
            "selected_module_is_post_schema_registry_governance_status_refresh": selected is not None and selected["module"] == SELECTED_NEXT_MODULE,
...
def main() -> int:
    ...
    payload = {
        "module": MODULE_NAME,
        "development_queue_selector_after_schema_registry_closure_record_status": "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_SCHEMA_REGISTRY_CLOSURE_RECORD_V1_READY" if passed else "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_SCHEMA_REGISTRY_CLOSURE_RECORD_V1_BLOCKED",
        "severity": "OK" if passed else "BLOCKED",
        "allowed_scope": "REPO_ONLY_OS_INTELLIGENCE",
        "final_decision": "DEVELOPMENT_QUEUE_SELECTED_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH" if passed else "KEEP_FREEZE_REVIEW_DEVELOPMENT_QUEUE_AFTER_SCHEMA_REGISTRY_CLOSURE_RECORD_ERRORS",
        "next_action": "BUILD_REPO_ONLY_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_V1" if passed else "REVIEW_DEVELOPMENT_QUEUE_AFTER_SCHEMA_REGISTRY_CLOSURE_RECORD_ERRORS",
        "next_module": SELECTED_NEXT_MODULE if passed else None,
        "reason": "Selected repo-only post-schema-registry governance status refresh; schema apply/create/edit remain blocked and physically absent." if passed else "Development queue selector after schema-registry closure record validation failed.",
        ...
        "development_queue_selector_after_schema_registry_closure_record": queue,
    ...
    latest_json = OUT_DIR / "repo_only_development_queue_selector_after_schema_registry_closure_record_v1_latest.json"
    timestamped_json = OUT_DIR / f"repo_only_development_queue_selector_after_schema_registry_closure_record_v1_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json"
    latest_txt = OUT_DIR / "repo_only_development_queue_selector_after_schema_registry_closure_record_v1_latest.txt"

    latest_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    timestamped_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")

    lines = [
        "EDGE FACTORY OS REPO-ONLY DEVELOPMENT QUEUE SELECTOR AFTER SCHEMA REGISTRY CLOSURE RECORD v1",
        "=" * 100,
        f"development_queue_selector_after_schema_registry_closure_record_status: {payload['development_queue_selector_after_schema_registry_closure_record_status']}",
