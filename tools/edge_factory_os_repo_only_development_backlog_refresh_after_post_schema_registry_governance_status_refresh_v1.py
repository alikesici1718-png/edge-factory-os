...
MODULE_NAME = "edge_factory_os_repo_only_development_backlog_refresh_after_post_schema_registry_governance_status_refresh_v1"
OUT_DIR = LAB_ROOT / "edge_factory_os_repo_only_development_backlog_refresh_after_post_schema_registry_governance_status_refresh_v1"
OUT_DIR.mkdir(parents=True, exist_ok=True)

EXPECTED_HEAD_PREFIX = "6c2052c"
CURRENT_TOOL_REL = "tools/edge_factory_os_repo_only_development_backlog_refresh_after_post_schema_registry_governance_status_refresh_v1.py"

QUEUE_JSON = LAB_ROOT / "edge_factory_os_repo_only_development_queue_selector_after_post_schema_registry_governance_status_refresh_v1" / "repo_only_development_queue_selector_after_post_schema_registry_governance_status_refresh_v1_latest.json"
QUEUE_POST_JSON = LAB_ROOT / "edge_factory_os_repo_only_development_queue_selector_after_post_schema_registry_governance_status_refresh_post_commit_check" / "repo_only_development_queue_selector_after_post_schema_registry_governance_status_refresh_post_commit_check_latest.json"
...
SELECTED_NEXT_MODULE = "edge_factory_os_repo_only_next_action_selector_after_post_schema_registry_governance_backlog_refresh_v1.py"
...
SAFETY_FLAGS: Dict[str, bool] = {
    "repo_only_development_backlog_refresh_after_post_schema_registry_governance_status_refresh": True,
    "read_only_backlog_refresh_only": True,
    "development_queue_selector_after_post_schema_registry_governance_status_refresh_required": True,
    "next_action_selector_after_post_schema_registry_governance_backlog_refresh_allowed_next": True,
...
def first_status(obj: Dict[str, Any]) -> Optional[str]:
    for key in [
        "development_queue_selector_after_post_schema_registry_governance_status_refresh_status",
        "audit_status",
...
def validate_inputs() -> Dict[str, Any]:
    ...
    if subject != "Add repo-only development queue selector after post schema registry governance status refresh":
        errors.append(f"unexpected HEAD commit subject: {subject}")

    paths = commit_paths("HEAD")
    checks["head_commit_paths"] = paths
    if paths != ["tools/edge_factory_os_repo_only_development_queue_selector_after_post_schema_registry_governance_status_refresh_v1.py"]:
        errors.append(f"unexpected HEAD commit paths: {paths}")

    inputs = {
        "development_queue_selector": (
            QUEUE_JSON,
            "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_V1_READY",
            "edge_factory_os_repo_only_development_backlog_refresh_after_post_schema_registry_governance_status_refresh_v1.py",
        ),
        "development_queue_selector_post_check": (
            QUEUE_POST_JSON,
            "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_POST_COMMIT_CHECK_PASS",
            "edge_factory_os_repo_only_development_backlog_refresh_after_post_schema_registry_governance_status_refresh_v1.py",
        ),
...
def build_backlog(validation: Dict[str, Any]) -> Dict[str, Any]:
    ...
    queue = as_dict(queue_obj.get("development_queue_selector_after_post_schema_registry_governance_status_refresh", {}))
    selected_task = as_dict(queue.get("selected_development_task", {}))
    queue_context = as_dict(queue.get("context_summary", {}))
    queue_effective_approval = as_dict(queue_context.get("effective_approval_state", {}))
    queue_effective_schema = as_dict(queue_context.get("effective_schema_state", {}))
    ...
    effective_approval_state = {
        "manual_approval_present": first_not_none(
            queue_effective_approval.get("manual_approval_present"),
        ...
        "manual_approval_valid": first_not_none(
            queue_effective_approval.get("manual_approval_valid"),
        ...
        "approval_granted_now": first_not_none(
            queue_effective_approval.get("approval_granted_now"),
    ...
    effective_schema_state = {
        "schema_apply_allowed_now": first_not_none(
            queue_effective_schema.get("schema_apply_allowed_now"),
        ...
        "schema_file_creation_allowed_now": first_not_none(
            queue_effective_schema.get("schema_file_creation_allowed_now"),
        ...
        "schema_file_edit_allowed_now": first_not_none(
            queue_effective_schema.get("schema_file_edit_allowed_now"),
    ...
    prerequisites = {
        "queue_ready": queue_obj.get("development_queue_selector_after_post_schema_registry_governance_status_refresh_status") == "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_V1_READY",
        "queue_selected_this_backlog_refresh": selected_task.get("module") == "edge_factory_os_repo_only_development_backlog_refresh_after_post_schema_registry_governance_status_refresh_v1.py",
        "queue_selected_repo_only_scope": selected_task.get("scope") == "REPO_ONLY_OS_INTELLIGENCE",
        "selector_ready": selector_obj.get("next_action_selector_after_post_schema_registry_governance_status_refresh_status") == "REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_V1_READY",
        "selector_selected_queue": selected_action.get("module") == "edge_factory_os_repo_only_development_queue_selector_after_post_schema_registry_governance_status_refresh_v1.py",
    ...
    backlog_items = [
        {
            "rank": 1,
            "key": "NEXT_ACTION_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_BACKLOG_REFRESH",
            "module": SELECTED_NEXT_MODULE,
            "scope": "REPO_ONLY_OS_INTELLIGENCE",
            "allowed": True,
            "reason": "Backlog refreshed after governance status refresh; select next repo-only development task.",
            "blocked_actions": [
    ...
    return {
        "backlog_refresh_status": "DEVELOPMENT_BACKLOG_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_READY" if selected else "DEVELOPMENT_BACKLOG_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_BLOCKED",
        ...
        "selected_next_action": "BUILD_REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_BACKLOG_REFRESH_V1" if selected else None,
        ...
        "context_summary": {
            ...
            "state_source_order": "queue effective state -> selector effective state -> refresh effective state -> queue context -> selector context -> governance_status -> refresh context -> closure decision/summary -> manual approval record",
        ...
        "invariants": {
            ...
            "selected_module_is_next_action_selector_after_governance_backlog_refresh": selected is not None and selected["module"] == SELECTED_NEXT_MODULE,
...
def main() -> int:
    ...
    payload = {
        "module": MODULE_NAME,
        "development_backlog_refresh_after_post_schema_registry_governance_status_refresh_status": "REPO_ONLY_DEVELOPMENT_BACKLOG_REFRESH_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_V1_READY" if passed else "REPO_ONLY_DEVELOPMENT_BACKLOG_REFRESH_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_V1_BLOCKED",
        "severity": "OK" if passed else "BLOCKED",
        "allowed_scope": "REPO_ONLY_OS_INTELLIGENCE",
        "final_decision": "DEVELOPMENT_BACKLOG_REFRESH_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_READY_FOR_NEXT_ACTION_SELECTOR" if passed else "KEEP_FREEZE_REVIEW_DEVELOPMENT_BACKLOG_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_ERRORS",
        "next_action": "BUILD_REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_BACKLOG_REFRESH_V1" if passed else "REVIEW_DEVELOPMENT_BACKLOG_AFTER_POST_SCHEMA_REGISTRY_GOVERNANCE_STATUS_REFRESH_ERRORS",
        "next_module": SELECTED_NEXT_MODULE if passed else None,
        "reason": "Development backlog refreshed after post-schema-registry governance status refresh; schema apply/create/edit remain blocked and physically absent." if passed else "Development backlog refresh after post-schema-registry governance status refresh validation failed.",
        ...
        "development_backlog_refresh_after_post_schema_registry_governance_status_refresh": backlog,
    ...
    latest_json = OUT_DIR / "repo_only_development_backlog_refresh_after_post_schema_registry_governance_status_refresh_v1_latest.json"
    timestamped_json = OUT_DIR / f"repo_only_development_backlog_refresh_after_post_schema_registry_governance_status_refresh_v1_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json"
    latest_txt = OUT_DIR / "repo_only_development_backlog_refresh_after_post_schema_registry_governance_status_refresh_v1_latest.txt"

    latest_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    timestamped_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")

    lines = [
        "EDGE FACTORY OS REPO-ONLY DEVELOPMENT BACKLOG REFRESH AFTER POST-SCHEMA-REGISTRY GOVERNANCE STATUS REFRESH v1",
        "=" * 100,
        f"development_backlog_refresh_after_post_schema_registry_governance_status_refresh_status: {payload['development_backlog_refresh_after_post_schema_registry_governance_status_refresh_status']}",
