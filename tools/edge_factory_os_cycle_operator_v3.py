...
OUT_ROOT = WORKSPACE / "edge_factory_os_cycle_operator_v3"
...
def derive_operator_decision(
    ...
    # v3 priority fix:
    # New errors / journal attention must outrank performance watch.
    # Performance watch is important, but it is not allowed to hide fresh error/journal attention.
    cycle_new_errors = cycle.get("new_errors_since_ack") is True
    cycle_errors = as_int(cycle.get("errors"))

    if cycle_status == "CONTROL_TOWER_CYCLE_JOURNAL_ATTENTION" or cycle_new_errors:
        return (
            "CYCLE_OPERATOR_JOURNAL_OR_ERROR_ATTENTION",
            "ATTENTION",
            "READ_ONLY_REVIEW",
            "REVIEW_CONTROL_TOWER_DECISION_CHANGE_AND_ERROR_STATE",
            f"cycle_status={cycle_status}; new_errors_since_ack={cycle_new_errors}; errors={cycle_errors}; performance_status={perf_status}; negative_watch_families={','.join(map(str, negative_watch_families))}",
        )
...
def main() -> int:
    out_dir = OUT_ROOT / f"cycle_operator_v3_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    state_path = out_dir / "cycle_operator_v3_state.json"
    latest_path = OUT_ROOT / "cycle_operator_latest.json"
    steps_csv = out_dir / "cycle_operator_v3_steps.csv"
    report_path = out_dir / "cycle_operator_v3_report.md"
    ...
    print("EDGE FACTORY OS CYCLE OPERATOR v3")
    print("=" * 100)
    ...
    print(f"error_priority_note={'NEW_ERRORS_OR_JOURNAL_ATTENTION_OUTRANKS_PERFORMANCE_WATCH' if state['new_errors_since_ack'] or str(state.get('cycle_status')) == 'CONTROL_TOWER_CYCLE_JOURNAL_ATTENTION' else 'NONE'}")
