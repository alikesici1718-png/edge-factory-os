...
MODULE_NAME = "edge_factory_os_repo_only_final_form_gap_analysis_contract_v1"
CURRENT_TOOL_REL = "tools/edge_factory_os_repo_only_final_form_gap_analysis_contract_v1.py"
OUT_DIR = LAB_ROOT / MODULE_NAME

EXPECTED_HEAD = "8288eb6"
EXPECTED_PREVIOUS_TRACKED_PYTHON_COUNT = 585
NEXT_MODULE = "edge_factory_os_repo_only_final_form_gap_analysis_runner_v1.py"
POST_CHECK_STATUS = "REPO_ONLY_FINAL_FORM_GAP_ANALYSIS_CONTRACT_POST_COMMIT_CHECK_PASS"
...
BACKLOG_PRIORITIZATION_ARTIFACT = LAB_ROOT / "edge_factory_os_repo_only_development_backlog_prioritization_after_evidence_chain_rollout_v1" / "repo_only_development_backlog_prioritization_after_evidence_chain_rollout_v1_latest.json"
...
FINAL_FORM_DIMENSIONS = [
    ...
    "generic_runner_safe_mode_reopening_path_without_implementation",
]
...
def git_state() -> Dict[str, Any]:
    ...
    return {
        ...
        "repo_clean_for_final_form_gap_analysis_contract": len(dirty_tracked) == 0 and len(unexpected_untracked) == 0,
    }
...
def backlog_prioritization_ok(artifact: Dict[str, Any]) -> bool:
    return (
        artifact.get("development_backlog_prioritization_status") in {"READY", "PASS"}
        and artifact.get("next_module") == "edge_factory_os_repo_only_final_form_gap_analysis_contract_v1.py"
        and artifact.get("selected_next_item_is_material") is True
        and artifact.get("selected_next_item_is_repo_only") is True
        and artifact.get("selected_next_item_does_not_reopen_generic_governance_loop") is True
        and artifact.get("selected_next_item_does_not_approve_generic_runner") is True
        and artifact.get("evidence_chain_policy_level") == POLICY_LEVEL
...
def contract_definition() -> Dict[str, Any]:
    return {
        "scope": "repo_only_final_form_gap_analysis_contract",
        "dimensions": FINAL_FORM_DIMENSIONS,
        "required_gap_states": ["satisfied", "partially_satisfied", "blocked", "missing"],
        "required_severity_levels": ["P0", "P1", "P2", "P3", "INFO"],
        "must_read_repo_state_where_feasible": True,
        "must_read_latest_status_audit_policy_artifacts_where_feasible": True,
        "must_produce_machine_readable_gap_analysis": True,
        "must_not_perform_trading_research": True,
        "must_not_implement_generic_runner": True,
        "generic_runner_safe_mode_reopening_path_is_contract_only": True,
        "must_not_create_schema_config_runtime_live_capital_order_candidate_family_work": True,
        "must_preserve_evidence_chain_policy": True,
        "must_preserve_generic_runner_blocked_state": True,
        "must_preserve_closed_loop_state": True,
    }
...
def build_payload() -> Dict[str, Any]:
    ...
    backlog = load_json(BACKLOG_PRIORITIZATION_ARTIFACT)
    ...
    replacement_checks = {
        ...
        "repo_clean_for_final_form_gap_analysis_contract": git["repo_clean_for_final_form_gap_analysis_contract"],
        ...
        "backlog_prioritization_artifact_exists": bool(backlog),
        "backlog_prioritization_ok": backlog_prioritization_ok(backlog),
        "contract_dimensions_complete": set(FINAL_FORM_DIMENSIONS) == set(contract["dimensions"]),
        "contract_forbids_trading_research_and_generic_runner_implementation": contract["must_not_perform_trading_research"] and contract["must_not_implement_generic_runner"],
        "contract_preserves_policy_and_loop": contract["must_preserve_evidence_chain_policy"] and contract["must_preserve_generic_runner_blocked_state"] and contract["must_preserve_closed_loop_state"],
        "next_module_expected": NEXT_MODULE == "edge_factory_os_repo_only_final_form_gap_analysis_runner_v1.py",
    }
    ...
    payload: Dict[str, Any] = {
        ...
        "final_form_gap_analysis_contract_status": "PASS" if ready else "FAIL_CLOSED",
        "post_check_status": POST_CHECK_STATUS if ready else "REPO_ONLY_FINAL_FORM_GAP_ANALYSIS_CONTRACT_POST_COMMIT_CHECK_FAIL_CLOSED",
        "final_decision": "FINAL_FORM_GAP_ANALYSIS_CONTRACT_READY" if ready else "FINAL_FORM_GAP_ANALYSIS_CONTRACT_FAIL_CLOSED",
        "next_action": "BUILD_FINAL_FORM_GAP_ANALYSIS_RUNNER_V1" if ready else "REVIEW_FINAL_FORM_GAP_ANALYSIS_CONTRACT_INPUTS",
        "next_module": NEXT_MODULE if ready else None,
        "final_form_gap_analysis_contract_ready": ready,
        "final_form_gap_analysis_contract": contract,
        ...
        "derived_live_repo_post_check_reason": "Final-form gap analysis contract uses the backlog prioritization artifact plus live repo replacement checks; evidence remains DERIVED_OVERUSED_ATTENTION and weaker than primary artifact verification.",
        "replacement_checks_all_true": ready,
        "derived_live_repo_post_check_replacement_checks": replacement_checks,
        "validation": {"git_state": git, "tracked_python_validation": py, "backlog_prioritization_loaded": bool(backlog)},
        "safety_flags": {
            "final_form_gap_analysis_contract": True,
            "repo_only": True,
            "auto_fix_performed": False,
            "direct_apply_performed": False,
            "repair_preview_required": False,
...
def write_outputs(payload: Dict[str, Any]) -> Dict[str, str]:
    ...
    latest_json = OUT_DIR / "repo_only_final_form_gap_analysis_contract_v1_latest.json"
    timestamped_json = OUT_DIR / f"repo_only_final_form_gap_analysis_contract_v1_{stamp}.json"
    latest_txt = OUT_DIR / "repo_only_final_form_gap_analysis_contract_v1_latest.txt"
...
def main() -> int:
    ...
    return 0 if payload["final_form_gap_analysis_contract_status"] in {"READY", "PASS"} else 3
