...
BUG_LOCALIZATION_PATH = REPO_ROOT / "artifacts" / "strategy_reviews" / "crypto_15m_liquidity_sweep_execution_bug_localization_v1.json"
ARTIFACT_PATH = REPO_ROOT / "artifacts" / "strategy_executions" / "crypto_15m_liquidity_sweep_reversal_execution_repaired_v1.json"

STATUS = "PASS_REPO_CODE_ONLY_CRYPTO_15M_LIQUIDITY_SWEEP_REVERSAL_REPAIRED_EXECUTED"
ARTIFACT_KIND = "CRYPTO_15M_LIQUIDITY_SWEEP_REVERSAL_REPAIRED_EXECUTION"
...
def main() -> int:
    ...
    allowed_status = {
        "?? tools/edge_factory_os_repo_only_crypto_15m_liquidity_sweep_reversal_execution_repaired_v1.py",
        "?? artifacts/strategy_executions/crypto_15m_liquidity_sweep_reversal_execution_repaired_v1.json",
    }
    ...
    bug_localization = load_json(BUG_LOCALIZATION_PATH)
    ...
    actual_pre_capacity = Counter(candidate["side"] for values in candidates_by_idx.values() for candidate in values)
    ...
    repaired_accepted_total = metrics["accepted_long_trades"] + metrics["accepted_short_trades"]
    filter_count_sanity = {
        "audit_raw_long_sweep_count": audit_long.get("raw_sweep_count"),
        "audit_raw_short_sweep_count": audit_short.get("raw_sweep_count"),
        "audit_after_candle_rejection_count": {
            "long": audit_long.get("after_candle_rejection_count"),
            "short": audit_short.get("after_candle_rejection_count"),
            "total": audit_long.get("after_candle_rejection_count", 0) + audit_short.get("after_candle_rejection_count", 0),
        },
        "audit_after_volume_filter_count": {
            "long": audit_long.get("after_volume_filter_count"),
            "short": audit_short.get("after_volume_filter_count"),
            "total": audit_long.get("after_volume_filter_count", 0) + audit_short.get("after_volume_filter_count", 0),
        },
        "audit_after_rank_filter_count": {
            "long": audit_long.get("after_cross_sectional_rank_count"),
            "short": audit_short.get("after_cross_sectional_rank_count"),
            "total": audit_long.get("after_cross_sectional_rank_count", 0) + audit_short.get("after_cross_sectional_rank_count", 0),
        },
        "audit_after_btc_disaster_filter_count": {
            "long": audit_long.get("after_btc_disaster_filter_count"),
            "short": audit_short.get("after_btc_disaster_filter_count"),
            "total": audit_long.get("after_btc_disaster_filter_count", 0) + audit_short.get("after_btc_disaster_filter_count", 0),
        },
        "audit_after_atr_risk_count": {
            "long": audit_long.get("after_atr_risk_validity_count"),
            "short": audit_short.get("after_atr_risk_validity_count"),
            "total": audit_long.get("after_atr_risk_validity_count", 0) + audit_short.get("after_atr_risk_validity_count", 0),
        },
        "audit_final_long_candidate_count": audit_long.get("final_candidate_count_before_position_capacity"),
        "audit_final_short_candidate_count": audit_short.get("final_candidate_count_before_position_capacity"),
        "repaired_actual_pre_capacity_long_candidates": actual_pre_capacity.get("long", 0),
        "repaired_actual_pre_capacity_short_candidates": actual_pre_capacity.get("short", 0),
    }
    metrics.update(
        {
            "raw_long_sweep_count": filter_count_sanity["audit_raw_long_sweep_count"],
            "raw_short_sweep_count": filter_count_sanity["audit_raw_short_sweep_count"],
            "after_candle_rejection_count": filter_count_sanity["audit_after_candle_rejection_count"],
            "after_volume_filter_count": filter_count_sanity["audit_after_volume_filter_count"],
            "after_rank_filter_count": filter_count_sanity["audit_after_rank_filter_count"],
            "after_btc_disaster_filter_count": filter_count_sanity["audit_after_btc_disaster_filter_count"],
            "after_atr_risk_count": filter_count_sanity["audit_after_atr_risk_count"],
            "final_long_candidate_count": filter_count_sanity["audit_final_long_candidate_count"],
            "final_short_candidate_count": filter_count_sanity["audit_final_short_candidate_count"],
            "repaired_actual_pre_capacity_long_candidates": actual_pre_capacity.get("long", 0),
            "repaired_actual_pre_capacity_short_candidates": actual_pre_capacity.get("short", 0),
        }
    )
    integrity_checks = {
        ...
        "filter_audit_final_candidates_positive": audit_final_total > 0,
        "repaired_accepts_trades_when_audit_has_final_candidates": not (audit_final_total > 0 and repaired_accepted_total == 0),
        "same_preregistered_config_preserved": bug_localization.get("config_id") == CONFIG_ID and prereg.get("config_id") == CONFIG_ID,
    ...
    artifact = {
        ...
        "module": "edge_factory_os_repo_only_crypto_15m_liquidity_sweep_reversal_execution_repaired_v1",
        "strategy": STRATEGY,
        ...
        "source_artifacts": {
            ...
            "bug_localization": str(BUG_LOCALIZATION_PATH),
            "panel_directory": str(PANEL_DIR),
        },
        "repair_summary": {
            "suspected_bug_category": bug_localization.get("suspected_bug_category"),
            "implementation_repair": "Replaced monotonic high/low deque length checks with existence checks after idx-based 192-bar lookback gating. The deques track extrema candidates, not lookback row counts.",
        ...
        "metrics": metrics,
        "filter_count_sanity": filter_count_sanity,
