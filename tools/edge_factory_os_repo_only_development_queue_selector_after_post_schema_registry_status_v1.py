...
MODULE_NAME = "edge_factory_os_repo_only_development_queue_selector_after_post_schema_registry_status_v1"
OUT_DIR = LAB_ROOT / "edge_factory_os_repo_only_development_queue_selector_after_post_schema_registry_status_v1"
OUT_DIR.mkdir(parents=True, exist_ok=True)

EXPECTED_HEAD_PREFIX = "773194a"
CURRENT_TOOL_REL = "tools/edge_factory_os_repo_only_development_queue_selector_after_post_schema_registry_status_v1.py"
...
SELECTOR_POST_JSON = LAB_ROOT / "edge_factory_os_repo_only_next_action_selector_after_post_schema_registry_status_post_commit_check" / "repo_only_next_action_selector_after_post_schema_registry_status_post_commit_check_latest.json"
...
SELECTED_NEXT_MODULE = "edge_factory_os_repo_only_development_backlog_refresh_after_post_schema_registry_status_v1.py"
...
SAFETY_FLAGS: Dict[str, bool] = {
    "repo_only_development_queue_selector_after_post_schema_registry_status": True,
    "read_only_development_queue_selection_only": True,
    "next_action_selector_after_post_schema_registry_status_required": True,
    "development_backlog_refresh_after_post_schema_registry_status_allowed_next": True,
...
def validate_inputs() -> Dict[str, Any]:
    ...
    if subject != "Add repo-only next-action selector after post-schema-registry status":
        errors.append(f"unexpected HEAD commit subject: {subject}")

    paths = commit_paths("HEAD")
    checks["head_commit_paths"] = paths
    if paths != ["tools/edge_factory_os_repo_only_next_action_selector_after_post_schema_registry_status_v1.py"]:
    ...
    inputs = {
        "next_action_selector": (
            ...
            "edge_factory_os_repo_only_development_queue_selector_after_post_schema_registry_status_v1.py",
        ),
        "next_action_selector_post_check": (
            SELECTOR_POST_JSON,
            "REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_STATUS_POST_COMMIT_CHECK_PASS",
            "edge_factory_os_repo_only_development_queue_selector_after_post_schema_registry_status_v1.py",
        ),
...
def build_development_queue(validation: Dict[str, Any]) -> Dict[str, Any]:
    ...
    prerequisites = {
        ...
        "parent_selected_this_queue_selector": selected_action.get("module") == "edge_factory_os_repo_only_development_queue_selector_after_post_schema_registry_status_v1.py",
    ...
    queue_items = [
        {
            "rank": 1,
            "key": "DEVELOPMENT_BACKLOG_REFRESH_AFTER_POST_SCHEMA_REGISTRY_STATUS",
            "module": SELECTED_NEXT_MODULE,
            "scope": "REPO_ONLY_OS_INTELLIGENCE",
            "allowed": True,
            "reason": "Schema registry chain is safely blocked at approval; refresh repo-only backlog to choose the next non-apply governance/intelligence task.",
            "blocked_actions": [
    ...
    return {
        "development_queue_status": "DEVELOPMENT_QUEUE_AFTER_POST_SCHEMA_REGISTRY_STATUS_ACTIVE" if selected else "DEVELOPMENT_QUEUE_AFTER_POST_SCHEMA_REGISTRY_STATUS_BLOCKED",
        ...
        "selected_next_action": "BUILD_REPO_ONLY_DEVELOPMENT_BACKLOG_REFRESH_AFTER_POST_SCHEMA_REGISTRY_STATUS_V1" if selected else None,
        ...
        "context_summary": {
            ...
            "approval_state_source_order": "status_summary.approval_state -> status_summary flat -> parent selector context -> manual approval record",
            "schema_state_source_order": "status_summary.schema_action_state -> status_summary flat -> parent selector context -> manual approval record",
...
def main() -> int:
    ...
    payload = {
        "module": MODULE_NAME,
        "development_queue_selector_after_post_schema_registry_status_status": "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_STATUS_V1_READY" if passed else "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_POST_SCHEMA_REGISTRY_STATUS_V1_BLOCKED",
        "severity": "OK" if passed else "BLOCKED",
        "allowed_scope": "REPO_ONLY_OS_INTELLIGENCE",
        "final_decision": "DEVELOPMENT_QUEUE_SELECTED_BACKLOG_REFRESH_AFTER_POST_SCHEMA_REGISTRY_STATUS" if passed else "KEEP_FREEZE_REVIEW_DEVELOPMENT_QUEUE_AFTER_POST_SCHEMA_REGISTRY_STATUS_ERRORS",
        "next_action": "BUILD_REPO_ONLY_DEVELOPMENT_BACKLOG_REFRESH_AFTER_POST_SCHEMA_REGISTRY_STATUS_V1" if passed else "REVIEW_DEVELOPMENT_QUEUE_AFTER_POST_SCHEMA_REGISTRY_STATUS_ERRORS",
        "next_module": SELECTED_NEXT_MODULE if passed else None,
        "reason": "Selected repo-only development backlog refresh; schema apply/create/edit remain blocked and physically absent." if passed else "Development queue selector after post-schema-registry status validation failed.",
        ...
        "development_queue_selector_after_post_schema_registry_status": queue,
    ...
    latest_json = OUT_DIR / "repo_only_development_queue_selector_after_post_schema_registry_status_v1_latest.json"
    timestamped_json = OUT_DIR / f"repo_only_development_queue_selector_after_post_schema_registry_status_v1_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json"
    latest_txt = OUT_DIR / "repo_only_development_queue_selector_after_post_schema_registry_status_v1_latest.txt"

    latest_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    timestamped_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")

    lines = [
        "EDGE FACTORY OS REPO-ONLY DEVELOPMENT QUEUE SELECTOR AFTER POST-SCHEMA-REGISTRY STATUS v1",
        "=" * 100,
        f"development_queue_selector_after_post_schema_registry_status_status: {payload['development_queue_selector_after_post_schema_registry_status_status']}",
