...
MODULE_NAME = "edge_factory_os_repo_only_historical_data_acquisition_okx_full_usdt_swap_first_chunk_download_execution_validator_after_execution_v1"
REQUESTED_MODULE = f"{MODULE_NAME}.py"
EXPECTED_HEAD = "face25f"
PREVIOUS_STATUS = "PASS_HISTORICAL_DATA_ACQUISITION_OKX_FULL_USDT_SWAP_FIRST_CHUNK_DOWNLOAD_EXECUTED_WITH_COVERAGE_GAPS_PENDING_VALIDATOR_NO_BUILD"
PASS_GAP_STATUS = "PASS_HISTORICAL_DATA_ACQUISITION_OKX_FULL_USDT_SWAP_FIRST_CHUNK_DOWNLOAD_VALIDATED_WITH_COVERAGE_GAPS_COVERAGE_SUMMARY_READY_NO_BUILD"
PASS_COMPLETE_STATUS = "PASS_HISTORICAL_DATA_ACQUISITION_OKX_FULL_USDT_SWAP_FIRST_CHUNK_DOWNLOAD_VALIDATED_COMPLETE_COVERAGE_SUMMARY_READY_NO_BUILD"
BLOCKED_STATUS = "BLOCKED_HISTORICAL_DATA_ACQUISITION_OKX_FULL_USDT_SWAP_FIRST_CHUNK_DOWNLOAD_VALIDATION_FAILED"
NEXT_MODULE = "edge_factory_os_repo_only_historical_data_acquisition_okx_full_usdt_swap_first_chunk_download_coverage_summary_after_validator_v1.py"
FAILED_NEXT_MODULE = "edge_factory_os_repo_only_historical_data_acquisition_okx_full_usdt_swap_first_chunk_download_validation_blocked_record_v1.py"
AFTER_QUALITY_GAP = "HISTORICAL_DATA_ACQUISITION_OKX_FULL_USDT_SWAP_FIRST_CHUNK_DOWNLOAD_VALIDATED_WITH_COVERAGE_GAPS_COVERAGE_SUMMARY_READY_NO_BUILD"
AFTER_QUALITY_COMPLETE = "HISTORICAL_DATA_ACQUISITION_OKX_FULL_USDT_SWAP_FIRST_CHUNK_DOWNLOAD_VALIDATED_COMPLETE_COVERAGE_SUMMARY_READY_NO_BUILD"
...
ARTIFACTS = {
    "execution_report": EXECUTION_DIR / "historical_okx_full_usdt_swap_first_chunk_download_execution_report.json",
    "file_manifest": EXECUTION_DIR / "historical_okx_full_usdt_swap_first_chunk_download_file_manifest_after_execution.json",
    "gap_report": EXECUTION_DIR / "historical_okx_full_usdt_swap_first_chunk_download_gap_report.json",
    "reuse_report": EXECUTION_DIR / "historical_okx_full_usdt_swap_first_chunk_reuse_validation_report.json",
    "sha256_report": EXECUTION_DIR / "historical_okx_full_usdt_swap_first_chunk_sha256_report.json",
    "zip_inventory_report": EXECUTION_DIR / "historical_okx_full_usdt_swap_first_chunk_zip_inventory_report.json",
    "schema_sample_report": EXECUTION_DIR / "historical_okx_full_usdt_swap_first_chunk_schema_sample_report.json",
    "compliance_report": EXECUTION_DIR / "historical_okx_full_usdt_swap_first_chunk_download_execution_compliance_report.json",
    "execution_summary": EXECUTION_DIR / "historical_okx_full_usdt_swap_first_chunk_download_execution_summary.json",
}

REQUIRED_OUTPUTS = [
    "historical_okx_full_usdt_swap_first_chunk_download_execution_validator.json",
    "historical_okx_full_usdt_swap_first_chunk_download_validation_report.json",
    "historical_okx_full_usdt_swap_first_chunk_per_symbol_coverage_validation_report.json",
    "historical_okx_full_usdt_swap_first_chunk_gap_validation_report.json",
    "historical_okx_full_usdt_swap_first_chunk_reuse_validation_validator_report.json",
    "historical_okx_full_usdt_swap_first_chunk_sha256_validation_report.json",
    "historical_okx_full_usdt_swap_first_chunk_zip_schema_sample_validation_report.json",
    "historical_okx_full_usdt_swap_first_chunk_download_execution_validator_summary.json",
]
...
EXPECTED_FINAL_AVAILABLE_FILE_COUNT = 10_106
EXPECTED_MISSING_OR_FAILED_FILE_COUNT = 10_954
EXPECTED_REUSED_FILE_COUNT = 1_053
EXPECTED_SUCCESSFUL_NEW_DOWNLOAD_COUNT = 9_053
...
def validate_execution_summary(summary: dict[str, Any]) -> dict[str, bool]:
    checks = {
        "previous_status_passed": summary.get("historical_data_acquisition_okx_full_usdt_swap_first_chunk_download_execution_status") == PREVIOUS_STATUS,
        ...
        "expected_file_count_valid": summary.get("expected_first_chunk_file_count") == EXPECTED_FIRST_CHUNK_FILE_COUNT,
        "planned_file_count_valid": summary.get("planned_file_count") == EXPECTED_FIRST_CHUNK_FILE_COUNT,
        ...
        "full_symbol_count_valid": summary.get("symbols_with_full_file_coverage_count") == 5,
        "gap_symbol_count_valid": summary.get("symbols_with_coverage_gaps_count") == 15,
...
def validate_manifest_and_counts(lists: dict[str, list[dict[str, Any]]], execution_summary: dict[str, Any]) -> dict[str, Any]:
    ...
    require(len(manifest) == EXPECTED_FIRST_CHUNK_FILE_COUNT, "file manifest row count mismatch")
    manifest_keys = [key(row) for row in manifest]
    require(len(set(manifest_keys)) == EXPECTED_FIRST_CHUNK_FILE_COUNT, "duplicate symbol/date rows in file manifest")
    ...
    require(len(sha_records) == EXPECTED_FIRST_CHUNK_FILE_COUNT, "sha256 record count mismatch")
    require(len(zip_records) == EXPECTED_FIRST_CHUNK_FILE_COUNT, "zip inventory count mismatch")
    require(len(schema_records) == EXPECTED_FIRST_CHUNK_FILE_COUNT, "schema sample count mismatch")
    ...
    successful_new_download_count = sum(
        1
        for row in manifest
        if row.get("source_kind") == "EXISTING_APPROVED_FIRST_CHUNK_FILE" and row.get("available_for_validator") is True
    )
    ...
    require(new_download_attempt_count == failed_download_attempt_count, "top-level new download attempt count and failed-attempt manifest count mismatch")
    require(download_result_row_count == failed_download_attempt_count + successful_new_download_count, "download result rows do not reconcile with successful existing/downloaded plus failed attempts")
...
def classify_symbols(lists: dict[str, list[dict[str, Any]]]) -> tuple[list[dict[str, Any]], list[str], list[str]]:
    ...
    require(len(full_symbols) == 5, "full coverage symbol count mismatch")
    require(len(gap_symbols) == 15, "gap symbol count mismatch")
...
def build_common_summary(
    ...
    return {
        "historical_data_acquisition_okx_full_usdt_swap_first_chunk_download_execution_validator_status": status,
        ...
        "planned_file_count": EXPECTED_FIRST_CHUNK_FILE_COUNT,
        ...
        "first_chunk_download_validated_for_coverage_summary": True,
...
def run_validator() -> dict[str, Any]:
    ...
    replacement_checks = {
        ...
        **summary_checks,
        ...
        "near_3y_complete_symbols_are_full_coverage_only": len(full_symbols) == 5,
        **compliance_checks,
    ...
    outputs = {
        "historical_okx_full_usdt_swap_first_chunk_download_execution_validator.json": validator_bundle,
        "historical_okx_full_usdt_swap_first_chunk_download_validation_report.json": validation_report,
        "historical_okx_full_usdt_swap_first_chunk_per_symbol_coverage_validation_report.json": per_symbol_report,
        "historical_okx_full_usdt_swap_first_chunk_gap_validation_report.json": gap_report,
        "historical_okx_full_usdt_swap_first_chunk_reuse_validation_validator_report.json": reuse_report,
        "historical_okx_full_usdt_swap_first_chunk_sha256_validation_report.json": sha_report,
        "historical_okx_full_usdt_swap_first_chunk_zip_schema_sample_validation_report.json": zip_schema_report,
        "historical_okx_full_usdt_swap_first_chunk_download_execution_validator_summary.json": summary,
    }
...
def blocked_payload(message: str) -> dict[str, Any]:
    return {
        "historical_data_acquisition_okx_full_usdt_swap_first_chunk_download_execution_validator_status": BLOCKED_STATUS,
        "download_execution_validated": False,
        "blocked_reason": message,
        "chunk_id": CHUNK_ID,
        "planned_file_count": 0,
        "final_available_file_count": 0,
        "missing_or_failed_file_count": EXPECTED_FIRST_CHUNK_FILE_COUNT,
        ...
        "first_chunk_download_validated_for_coverage_summary": False,
        "active_p0_blocker_count": 1,
        "active_p1_attention_count": 505,
        "current_evidence_chain_quality_after_validator": "FIRST_CHUNK_DOWNLOAD_VALIDATION_FAILED_CLOSED",
...
def main() -> int:
    try:
        ...
        write_json(OUTPUT_DIR / "historical_okx_full_usdt_swap_first_chunk_download_execution_validator_summary.json", blocked)
