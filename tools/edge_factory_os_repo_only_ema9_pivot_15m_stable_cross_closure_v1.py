...
def build_payload() -> Dict[str, Any]:
    ...
    if unexpected_dirty_paths:
        raise RuntimeError(f"unexpected dirty paths during closure: {unexpected_dirty_paths}")
    if actual_head != EXPECTED_PRE_CLOSURE_HEAD:
        raise RuntimeError(f"HEAD moved before closure: {actual_head} != {EXPECTED_PRE_CLOSURE_HEAD}")
    if actual_tracked_python_count != EXPECTED_PRE_CLOSURE_TRACKED_PYTHON_COUNT:
        raise RuntimeError(
            "tracked Python count mismatch before closure: "
    ...
    if evaluator.get("strategy_name") != STRATEGY_NAME:
        raise RuntimeError("strategy name mismatch")
    if evaluator.get("route_family") != ROUTE_FAMILY:
    ...
    payload: Dict[str, Any] = {
        ...
        "closure_reason": (
            "Route closed after evaluator. The evaluator result is preserved exactly; this closure grants "
            "no candidate, edge claim, family release, runtime, live, or capital permission."
        ),
        "permissions": {
    ...
    if not all(payload["validation_checks"].values()):
        raise RuntimeError(f"closure validation checks failed: {payload['validation_checks']}")
