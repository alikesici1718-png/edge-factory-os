...
OUT_ROOT = WORKSPACE / "edge_factory_os_monitoring_stack_runner_v4"
...
def main() -> int:
    out_dir = OUT_ROOT / f"monitoring_stack_runner_v4_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    state_path = out_dir / "monitoring_stack_runner_v4_state.json"
    latest_path = OUT_ROOT / "monitoring_stack_latest.json"
    steps_csv = out_dir / "monitoring_stack_runner_v4_steps.csv"
    alerts_csv = out_dir / "monitoring_stack_runner_v4_alerts.csv"
    report_path = out_dir / "monitoring_stack_runner_v4_report.md"
    ...
    print("EDGE FACTORY OS MONITORING STACK RUNNER v4")
    print("=" * 100)
