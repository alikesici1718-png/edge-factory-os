...
MODULE_NAME = "edge_factory_os_repo_only_next_action_selector_after_generic_governance_blocked_status_backlog_refresh_v1"
CURRENT_TOOL_REL = "tools/edge_factory_os_repo_only_next_action_selector_after_generic_governance_blocked_status_backlog_refresh_v1.py"
OUT_DIR = LAB_ROOT / MODULE_NAME

EXPECTED_HEAD = "776aa22"
EXPECTED_TRACKED_PYTHON_COUNT = 539

BACKLOG_JSON = (
    LAB_ROOT
    / "edge_factory_os_repo_only_development_backlog_refresh_after_generic_governance_blocked_status_v1"
    / "repo_only_development_backlog_refresh_after_generic_governance_blocked_status_v1_latest.json"
)
BACKLOG_POST_CHECK_JSON = (
    LAB_ROOT
    / "edge_factory_os_repo_only_development_backlog_refresh_after_generic_governance_blocked_status_post_commit_check"
    / "repo_only_development_backlog_refresh_after_generic_governance_blocked_status_post_commit_check_latest.json"
)

REQUIRED_BACKLOG_STATUS = "REPO_ONLY_DEVELOPMENT_BACKLOG_REFRESH_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_V1_READY"
REQUIRED_BACKLOG_POST_CHECK_STATUS = "REPO_ONLY_DEVELOPMENT_BACKLOG_REFRESH_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_POST_COMMIT_CHECK_PASS"
REQUIRED_BACKLOG_NEXT_ACTION = "BUILD_REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_REFRESH_V1"
REQUIRED_BACKLOG_NEXT_MODULE = "edge_factory_os_repo_only_next_action_selector_after_generic_governance_blocked_status_backlog_refresh_v1.py"

READY_STATUS = "REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_REFRESH_V1_READY"
FINAL_DECISION = "GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_REFRESH_EVALUATED_READY_FOR_DEVELOPMENT_QUEUE_SELECTOR"
NEXT_ACTION = "BUILD_REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_REFRESH_V1"
NEXT_MODULE = "edge_factory_os_repo_only_development_queue_selector_after_generic_governance_blocked_status_backlog_refresh_v1.py"
...
SAFETY_FLAGS: Dict[str, bool] = {
    "repo_only_next_action_selector_after_generic_governance_blocked_status_backlog_refresh": True,
    "next_action_selector_only": True,
    "read_only_backlog_evidence_validation_only": True,
    "development_queue_selector_after_generic_governance_blocked_status_backlog_refresh_allowed_next": True,
}
...
def first_status(obj: Dict[str, Any]) -> Optional[str]:
    for key in [
        "next_action_selector_after_generic_governance_blocked_status_backlog_refresh_status",
        "development_backlog_refresh_after_generic_governance_blocked_status_status",
        "audit_status",
        "status",
    ]:
        value = obj.get(key)
...
def validate_source_false_fields(errors: List[str], backlog_obj: Dict[str, Any]) -> None:
    for field in SOURCE_FALSE_FIELDS:
        if backlog_obj.get(field) is not False:
            errors.append(f"backlog refresh field not false: {field}={backlog_obj.get(field)}")
    for flag in DANGEROUS_FLAGS:
        if backlog_obj.get(flag) is not False:
            errors.append(f"backlog refresh dangerous flag not false: {flag}={backlog_obj.get(flag)}")
        safety_flags = backlog_obj.get("safety_flags", {})
        if isinstance(safety_flags, dict) and safety_flags.get(flag) is not False:
            errors.append(f"backlog refresh safety flag not false: {flag}={safety_flags.get(flag)}")


def validate_source_physical_guards(errors: List[str], backlog_obj: Dict[str, Any], post_check: Dict[str, Any]) -> None:
    guards = backlog_obj.get("physical_guards", {})
    if not isinstance(guards, dict):
        errors.append("backlog refresh physical_guards missing")
        return
    if guards.get("planned_schema_file_existing_count") != 0:
        errors.append(f"backlog refresh planned schema count not zero: {guards.get('planned_schema_file_existing_count')}")
    if guards.get("generic_runner_target_file_exists_now") is not False:
        errors.append(f"backlog refresh generic runner target exists not false: {guards.get('generic_runner_target_file_exists_now')}")
    for key, expected in PHYSICAL_FALSE_FIELDS.items():
        if guards.get(key) is not expected:
            errors.append(f"backlog refresh physical guard mismatch: {key}={guards.get(key)}")
    ...
    if isinstance(counts, dict):
        ...
        errors.append("backlog refresh post-check counts missing")
...
def backlog_route_object(backlog_obj: Dict[str, Any]) -> Dict[str, Any]:
    route = backlog_obj.get("development_backlog_refresh_after_generic_governance_blocked_status", {})
...
def validate_backlog_route(errors: List[str], backlog_obj: Dict[str, Any]) -> None:
    ...
    if not route:
        errors.append("backlog refresh route object missing")
        return
    selected = route.get("selected_next_backlog_item", {})
    if not isinstance(selected, dict):
        errors.append("backlog refresh selected_next_backlog_item missing")
        return
    if selected.get("key") != "NEXT_ACTION_SELECTOR_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_REFRESH":
        errors.append(f"backlog refresh selected key mismatch: {selected.get('key')}")
    if selected.get("module") != REQUIRED_BACKLOG_NEXT_MODULE:
        errors.append(f"backlog refresh selected module mismatch: {selected.get('module')}")
    if selected.get("allowed") is not True:
        errors.append(f"backlog refresh selected item not allowed true: {selected.get('allowed')}")
    proofs = route.get("confirmed_proofs", {})
    if not isinstance(proofs, dict):
        errors.append("backlog refresh confirmed proofs missing")
    else:
        for proof in [
            ...
            if proofs.get(proof) is not True:
                errors.append(f"backlog refresh proof not true: {proof}={proofs.get(proof)}")
...
def validate_inputs() -> Dict[str, Any]:
    ...
    if subject != "Add repo-only development backlog refresh after generic governance blocked status":
        errors.append(f"unexpected HEAD commit subject: {subject}")
    if paths != ["tools/edge_factory_os_repo_only_development_backlog_refresh_after_generic_governance_blocked_status_v1.py"]:
        errors.append(f"unexpected HEAD commit paths: {paths}")

    inputs = {
        "development_backlog_refresh": BACKLOG_JSON,
        "development_backlog_refresh_post_check": BACKLOG_POST_CHECK_JSON,
    }
    ...
    backlog_obj = loaded.get("development_backlog_refresh", {})
    post_check = loaded.get("development_backlog_refresh_post_check", {})
    if backlog_obj:
        if backlog_obj.get("development_backlog_refresh_after_generic_governance_blocked_status_status") != REQUIRED_BACKLOG_STATUS:
            errors.append(f"backlog refresh status mismatch: {backlog_obj.get('development_backlog_refresh_after_generic_governance_blocked_status_status')}")
        if backlog_obj.get("critical_issue_count") != 0:
            errors.append(f"backlog refresh critical_issue_count not zero: {backlog_obj.get('critical_issue_count')}")
        if backlog_obj.get("next_action") != REQUIRED_BACKLOG_NEXT_ACTION:
            errors.append(f"backlog refresh next_action mismatch: {backlog_obj.get('next_action')}")
        if backlog_obj.get("next_module") != REQUIRED_BACKLOG_NEXT_MODULE:
            errors.append(f"backlog refresh next_module mismatch: {backlog_obj.get('next_module')}")
        if backlog_obj.get("final_decision") != "GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_REFRESH_READY_FOR_NEXT_ACTION_SELECTOR":
            errors.append(f"backlog refresh final_decision mismatch: {backlog_obj.get('final_decision')}")
        validate_source_false_fields(errors, backlog_obj)
        validate_backlog_route(errors, backlog_obj)

    if post_check:
        if post_check.get("audit_status") != REQUIRED_BACKLOG_POST_CHECK_STATUS:
            errors.append(f"backlog refresh post-check status mismatch: {post_check.get('audit_status')}")
        if post_check.get("critical_issue_count") != 0:
            errors.append(f"backlog refresh post-check critical_issue_count not zero: {post_check.get('critical_issue_count')}")
        if post_check.get("latest_commit") != EXPECTED_HEAD:
            errors.append(f"backlog refresh post-check latest_commit mismatch: {post_check.get('latest_commit')}")
        if post_check.get("next_module") != REQUIRED_BACKLOG_NEXT_MODULE:
            errors.append(f"backlog refresh post-check next_module mismatch: {post_check.get('next_module')}")
        validate_source_physical_guards(errors, backlog_obj, post_check)
...
def build_selector(validation: Dict[str, Any]) -> Dict[str, Any]:
    ...
    prerequisites = {
        ...
        "parent_backlog_refresh_ready": backlog_obj.get("development_backlog_refresh_after_generic_governance_blocked_status_status") == REQUIRED_BACKLOG_STATUS,
        "parent_selected_this_selector": selected_from_parent.get("module") == REQUIRED_BACKLOG_NEXT_MODULE,
        "parent_selected_next_action_selector_key": selected_from_parent.get("key") == "NEXT_ACTION_SELECTOR_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_REFRESH",
    ...
    candidate_actions = [
        {
            "rank": 1,
            "key": "DEVELOPMENT_QUEUE_SELECTOR_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_REFRESH",
            "module": NEXT_MODULE,
            "scope": "REPO_ONLY_OS_INTELLIGENCE",
            "allowed": True,
            "reason": "Backlog refresh is verified; route to the repo-only development queue selector without unblocking generic runner work.",
            "blocked_actions": [
    ...
    return {
        "selector_status": "NEXT_ACTION_SELECTOR_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_REFRESH_ACTIVE" if selected else "NEXT_ACTION_SELECTOR_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_REFRESH_BLOCKED",
        ...
        "confirmed_proofs": {
            ...
            "backlog_refresh_selected_repo_only_next_action_selector_route": selected_from_parent.get("key") == "NEXT_ACTION_SELECTOR_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_REFRESH",
        },
        "physical_guards": {
        ...
        "invariants": {
            ...
            "selected_module_is_development_queue_selector_after_generic_governance_blocked_status_backlog_refresh": selected is not None and selected["module"] == NEXT_MODULE,
            "selected_action_is_development_queue_selector": selected is not None and selected["key"] == "DEVELOPMENT_QUEUE_SELECTOR_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_REFRESH",
...
def build_payload(validation: Dict[str, Any], selector: Dict[str, Any], errors: List[str]) -> Dict[str, Any]:
    ...
    payload: Dict[str, Any] = {
        "module": MODULE_NAME,
        "next_action_selector_after_generic_governance_blocked_status_backlog_refresh_status": READY_STATUS if passed else "REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_REFRESH_V1_BLOCKED",
        ...
        "next_action": NEXT_ACTION if passed else "REVIEW_NEXT_ACTION_SELECTOR_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_REFRESH_ERRORS",
        "next_module": NEXT_MODULE if passed else None,
        "reason": (
            "Selected the repo-only development queue selector after evaluating the generic governance blocked-status backlog refresh."
            if passed
        ...
        "next_action_selector_after_generic_governance_blocked_status_backlog_refresh": selector,
...
def write_outputs(payload: Dict[str, Any]) -> Dict[str, str]:
    ...
    latest_json = OUT_DIR / "repo_only_next_action_selector_after_generic_governance_blocked_status_backlog_refresh_v1_latest.json"
    timestamped_json = OUT_DIR / f"repo_only_next_action_selector_after_generic_governance_blocked_status_backlog_refresh_v1_{stamp}.json"
    latest_txt = OUT_DIR / "repo_only_next_action_selector_after_generic_governance_blocked_status_backlog_refresh_v1_latest.txt"
    ...
    lines = [
        "EDGE FACTORY OS REPO-ONLY NEXT ACTION SELECTOR AFTER GENERIC GOVERNANCE BLOCKED STATUS BACKLOG REFRESH v1",
        "=" * 100,
        f"next_action_selector_after_generic_governance_blocked_status_backlog_refresh_status: {payload['next_action_selector_after_generic_governance_blocked_status_backlog_refresh_status']}",
        ...
        json.dumps(
            {
                "selected_action": payload["next_action_selector_after_generic_governance_blocked_status_backlog_refresh"]["selected_action"],
                "confirmed_proofs": payload["next_action_selector_after_generic_governance_blocked_status_backlog_refresh"]["confirmed_proofs"],
                "prerequisites": payload["next_action_selector_after_generic_governance_blocked_status_backlog_refresh"]["prerequisites"],
                "invariants": payload["next_action_selector_after_generic_governance_blocked_status_backlog_refresh"]["invariants"],
