#!/usr/bin/env python3
"""Create the old_short clean-room bounded behavioral validation V2 preview.

This module is preview-only. It defines class and function contracts for a
future bounded behavioral validation V2 dry-run and writes a JSON preview
artifact. It does not execute validation, compare full datasets, run a backtest,
compute PnL, run the runner, generate signals, touch runtime, use network/API,
allocate capital, generate candidates, or claim edge.
"""
...
STATUS = "PASS_REPO_ONLY_OLD_SHORT_CLEAN_ROOM_BOUNDED_BEHAVIORAL_VALIDATION_V2_PREVIEW_CREATED"
ARTIFACT_KIND = "OLD_SHORT_CLEAN_ROOM_BOUNDED_BEHAVIORAL_VALIDATION_V2_PREVIEW"
ROUTE_KEY = "old_short_clean_room_v1"
EXPECTED_HEAD = "7dc4631bcc52b94608564f1e8c542901c3b47925"
EXPECTED_TRACKED_PYTHON_COUNT = 969
NEXT_ALLOWED_STEP = "OLD_SHORT_CLEAN_ROOM_BOUNDED_BEHAVIORAL_VALIDATION_V2_DRY_RUN_V1"

MODULE = "tools/edge_factory_os_repo_only_old_short_clean_room_bounded_behavioral_validation_v2_preview_v1.py"
ARTIFACT = "artifacts/old_short_clean_room/old_short_clean_room_bounded_behavioral_validation_v2_preview_v1.json"
...
NO_LIVE_GUARD_CONSTANTS = {
    "BEHAVIORAL_VALIDATION_V2_ALLOWED": False,
...
MASTER_AND_CLEAN_ROOM_EXPECTED_FILES = [
    "signals.csv",
    "pending_entries.csv",
    "open_positions.csv",
    "closed_trades.csv",
...
class OldShortV2BehavioralValidationConfigPreview:
    """Preview configuration for future bounded V2 validation."""

    route_key: str = ROUTE_KEY
    preview_only: bool = True
    behavioral_validation_v2_allowed_now: bool = False
    default_sample_limit_rows_per_file: int = 100

    def schema(self) -> dict[str, Any]:
        return dataclasses.asdict(self)


class OldShortV2BoundedSampleLoaderPreview:
    """Contract preview for bounded sample loading."""

    def input_roots(self) -> dict[str, str]:
        return {
            "master_upper_system_proxy_output": MASTER_PROXY_ROOT,
            "clean_room_v2_realistic_fixture_runner_output": CLEAN_ROOM_V2_OUTPUT_ROOT,
            "accepted_lifecycle_fixture_package": ACCEPTED_LIFECYCLE_ROOT,
        }

    def preview_return_shape(self) -> dict[str, Any]:
        return {
            "bounded_samples_loaded": False,
            "sample_limit_rows_per_file": 100,
            "full_dataset_comparison_allowed": False,
        }


class OldShortV2MasterProxySamplePreview:
    """Contract preview for MASTER proxy sample fields."""

    def expected_files(self) -> list[str]:
        return MASTER_AND_CLEAN_ROOM_EXPECTED_FILES


class OldShortV2CleanRoomOutputSamplePreview:
    """Contract preview for clean-room V2 output sample fields."""

    def expected_files(self) -> list[str]:
        return MASTER_AND_CLEAN_ROOM_EXPECTED_FILES


class OldShortV2AcceptedLifecycleFixturePreview:
    """Contract preview for accepted lifecycle fixture inputs."""

    def expected_files(self) -> list[str]:
        return ACCEPTED_LIFECYCLE_EXPECTED_FILES

    def limitation_contract(self) -> dict[str, Any]:
        return {
...
class OldShortV2BehaviorDimensionCheckerPreview:
    """Contract preview for future behavior dimension checks."""

    def dimensions(self) -> list[str]:
        return BEHAVIOR_DIMENSIONS_V2


class OldShortV2SimilarityMetricPreview:
    """Contract preview for future similarity metric outputs."""

    def metrics(self) -> list[str]:
        return SIMILARITY_METRICS_V2


class OldShortV2FailClosedPolicyPreview:
    """Contract preview for fail-closed checks."""

    def conditions(self) -> list[str]:
        return FAIL_CLOSED_CONDITIONS_V2


class OldShortV2ValidationReportPreview:
    """Contract preview for future report shape."""
...
class OldShortBoundedBehavioralValidationV2Preview:
    """Top-level preview contract for future bounded validation V2."""

    def preview_summary(self) -> dict[str, Any]:
        return {
            "preview_only": True,
            "behavioral_validation_execution": False,
            "next_allowed_step": NEXT_ALLOWED_STEP,
            "no_edge_no_live": True,
        }
...
def build_artifact() -> dict[str, Any]:
    ...
    design_identity = design.get("v2_validation_design_identity", {})
    ...
    preview_module_structure = {
        "class_count": 10,
        "classes": [
            "OldShortV2BehavioralValidationConfigPreview",
            "OldShortV2BoundedSampleLoaderPreview",
            "OldShortV2MasterProxySamplePreview",
            "OldShortV2CleanRoomOutputSamplePreview",
            "OldShortV2AcceptedLifecycleFixturePreview",
            "OldShortV2BehaviorDimensionCheckerPreview",
            "OldShortV2SimilarityMetricPreview",
            "OldShortV2FailClosedPolicyPreview",
            "OldShortV2ValidationReportPreview",
            "OldShortBoundedBehavioralValidationV2Preview",
        ],
        "contracts_only": True,
        "behavioral_validation_executed": False,
        "full_dataset_comparison_executed": False,
    ...
    validation_checks = {
        ...
        "bounded_behavioral_validation_v2_design_loaded": design.get("status") == "PASS_REPO_ONLY_OLD_SHORT_CLEAN_ROOM_BOUNDED_BEHAVIORAL_VALIDATION_V2_DESIGN_CREATED",
        "prior_next_allowed_step_verified": design.get("next_allowed_step") == "OLD_SHORT_CLEAN_ROOM_BOUNDED_BEHAVIORAL_VALIDATION_V2_PREVIEW_V1",
        ...
        "exact_gate_replay_false_preserved": exact_gate_replay_recovered is False and design_identity.get("exact_gate_replay_recovered") is False,
        ...
        "behavior_dimensions_v2_preserved": len(BEHAVIOR_DIMENSIONS_V2) == 16 and design.get("behavior_dimensions_v2", {}).get("dimensions") == BEHAVIOR_DIMENSIONS_V2,
        "similarity_metrics_v2_preserved": len(SIMILARITY_METRICS_V2) == 18 and design.get("similarity_metrics_v2", {}).get("metrics") == SIMILARITY_METRICS_V2,
        "fail_closed_conditions_v2_preserved": len(FAIL_CLOSED_CONDITIONS_V2) == 13 and design.get("fail_closed_conditions_v2", {}).get("conditions") == FAIL_CLOSED_CONDITIONS_V2,
        "expected_limitations_preserved": set(EXPECTED_LIMITATIONS).issubset(set(design.get("expected_limitations", {}).get("limitations", []))),
    ...
    artifact = {
        ...
        "v2_input_source_preview": {
            "future_v2_dry_run_may_read_bounded_samples_only": True,
            "default_sample_limit_rows_per_file": 100,
            "input_roots": OldShortV2BoundedSampleLoaderPreview().input_roots(),
            "forbidden": [
                "raw market data",
                "raw OKX 1m data",
            ...
            "preview_reads_external_rows": False,
            "behavioral_validation_execution_allowed_now": False,
        },
        "expected_files": {
            "master_and_clean_room_output": MASTER_AND_CLEAN_ROOM_EXPECTED_FILES,
            "accepted_lifecycle_fixture_package": ACCEPTED_LIFECYCLE_EXPECTED_FILES,
        },
        "behavior_dimensions_v2": {
        ...
        "threshold_policy_v2": {
            ...
            "gate_allowed_may_be_inferred_only_from_accepted_lifecycle_fixtures": True,
        },
        "fail_closed_conditions_v2": {
        ...
        "future_result_classes": {
            "result_class_count": len(FUTURE_RESULT_CLASSES),
            "allowed_result_classes": FUTURE_RESULT_CLASSES,
        },
        "report_preview": {
            "required_fields": OldShortV2ValidationReportPreview().required_fields(),
            "payload_hash_required": True,
            "must_include_no_edge_no_live_permissions": True,
        ...
        "unresolved_fields_preserved": {
            ...
            "exact_gate_replay_recovered": False,
            "accepted_lifecycle_gate_allow_inferred": True,
            "accepted_lifecycle_sample_count": accepted_lifecycle_candidate_count,
            "gate_allowed_inferred_count": gate_allowed_inferred_count,
            "clean_room_behavioral_reconstruction": True,
        ...
        "limitations": [
            "Preview only; bounded behavioral validation V2 is not executed.",
            "Future dry-run remains bounded-sample only and must not compare full datasets.",
            "Exact gate replay remains unavailable and must stay false.",
            "Gate allowed may be inferred only from accepted lifecycle and closed trade fixtures.",
        ...
        "safety_permissions": {
            "bounded_behavioral_validation_v2_preview_created": True,
            "behavioral_validation_v2_allowed_now": False,
            "full_dataset_comparison_allowed_now": False,
...
def main() -> int:
    ...
    stdout_fields = {
        ...
        "behavioral_validation_v2_allowed_now": artifact["preview_identity"]["behavioral_validation_v2_allowed_now"],
        ...
        "result_class_count": artifact["future_result_classes"]["result_class_count"],
