...
EXPECTED_HEAD = "f2ebbc4"
EXPECTED_PREVIOUS_TRACKED_PYTHON_COUNT = 657
EXPECTED_TRACKED_PYTHON_COUNT = 658

REQUESTED_MODULE = (
    "edge_factory_os_repo_only_historical_data_acquisition_external_or_additional_source_"
    "approval_after_preview_v1.py"
)
NEXT_MODULE_VERIFICATION = (
    "edge_factory_os_repo_only_historical_data_acquisition_external_or_additional_source_"
...
EVIDENCE_BEFORE = (
    "HISTORICAL_DATA_ACQUISITION_EXTERNAL_OR_ADDITIONAL_SOURCE_PREVIEW_READY_APPROVAL_REQUIRED_NO_EXECUTION"
)
EVIDENCE_AFTER = "HISTORICAL_DATA_ACQUISITION_EXTERNAL_OR_ADDITIONAL_SOURCE_VERIFICATION_APPROVED_NEXT_NO_EXECUTION"
...
FUTURE_REQUIRED_ARTIFACTS = [
    ...
    "historical_external_source_candidate_requirements.json",
    "historical_okx_source_identity_preview.json",
    "historical_external_source_gap_decision.json",
    "historical_external_source_contract_compliance_report.json",
]

FAIL_CLOSED_RULES = [
    "source_identity_is_unclear",
    "source_is_not_official_or_cannot_be_verified",
    "source_license_restrictions_unclear",
    "one_hour_timeframe_cannot_be_confirmed",
    "three_to_four_year_coverage_cannot_be_confirmed_as_requirement",
    "timestamp_timezone_requirements_unclear",
    "symbol_universe_rule_missing",
    "holdout_survivorship_requirements_missing",
    "fake_or_synthetic_data_used_as_real",
    "data_download_fetch_or_api_occurs",
    "data_build_occurs",
...
def require_no_forbidden_source_preview_actions(preview: Dict[str, Any]) -> None:
    for key in [
        ...
        require_false(preview.get(key), f"source_preview.{key}")
...
def validate_preflight(
    ...
    require_equal(
        source_preview.get("next_module"),
        REQUESTED_MODULE,
        "source_preview.next_module",
    ...
    require_true(
        source_preview.get("okx_official_historical_archive_or_export_preferred"),
        "source_preview.okx_official_historical_archive_or_export_preferred",
    )
    require_true(source_preview.get("manual_archive_import_preferred"), "source_preview.manual_archive_import_preferred")
    require_true(
        source_preview.get("external_api_or_download_requires_separate_future_chain"),
        "source_preview.external_api_or_download_requires_separate_future_chain",
    )
    require_true(source_preview.get("source_verification_required_next"), "source_preview.source_verification_required_next")
    require_true(source_preview.get("source_approval_required_next"), "source_preview.source_approval_required_next")
    ...
    require_true(source_preview.get("p1_attention_carried_forward"), "source_preview.p1_attention_carried_forward")
    require_true(
    ...
    require_equal(source_preview.get("dormant_repo_attention_count"), 716, "source_preview.dormant_repo_attention_count")
    require_no_forbidden_source_preview_actions(source_preview)
    ...
    require_true(
        local_manual_validator.get("external_or_additional_acquisition_still_required"),
        "local_manual_validator.external_or_additional_acquisition_still_required",
    )
    ...
    require_equal(
        local_manual_validator.get("dormant_repo_attention_count"),
        716,
        "local_manual_validator.dormant_repo_attention_count",
    )
    ...
    require_true(
        contract_validator.get("dormant_repo_attention_count_carried_forward"),
        "contract_validator.dormant_repo_attention_count_carried_forward",
    )
...
def approval_sections(source_preview: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "preview_context": {
            "external_or_additional_source_preview_completed": True,
            "local_manual_source_gap_remains_open": True,
            "okx_official_historical_archive_or_export_manual_import_is_preferred_source_candidate_route": True,
            ...
            "prior_preview_status": source_preview.get(
                "historical_data_acquisition_external_or_additional_source_preview_status"
            ),
        },
        "approval_scope": {
            "approval_grants_approval_record_only": True,
            "approval_grants_source_verification_now": False,
            "approval_grants_future_source_verification_next": True,
            "approval_grants_okx_browse_now": False,
        ...
        "future_source_verification_scope": {
            "future_module_may": [
                "verify_okx_official_historical_archive_export_manual_download_source_from_user_provided_static_identity_or_approved_metadata",
                "record_source_identity",
                "record_source_type",
                "record_instrument_type",
                "record_expected_file_format",
                "record_timeframe",
                "record_symbol_universe",
                "record_date_coverage_requirements",
                "record_timezone_requirements",
                "record_provenance_requirements",
                "decide_whether_okx_official_archive_export_can_be_used_through_manual_import",
                "decide_whether_separate_external_api_download_preview_chain_is_required",
        ...
        "next_module_decision": {
            "if_approval_is_valid": NEXT_MODULE_VERIFICATION,
...
def replacement_checks(payload: Dict[str, Any]) -> Dict[str, bool]:
    return {
        ...
        "approval_record_created": payload.get("external_or_additional_source_approval_record_created") is True,
        "approval_record_only": payload.get("approval_grants_approval_record_only") is True,
        "no_source_verification_now": payload.get("approval_grants_source_verification_now") is False,
        "future_source_verification_next": payload.get("approval_grants_future_source_verification_next") is True,
        "source_verification_eligible_next": payload.get("source_verification_eligible_next") is True,
        "okx_unverified_no_browse_download_api": (
            payload.get("okx_source_verified_now") is False
            and payload.get("approval_grants_okx_browse_now") is False
        ...
        "source_manifest_provenance_controls_preserved": (
            payload.get("source_manifest_required") is True and payload.get("provenance_report_required") is True
        ),
        "fail_closed_rules_preserved": payload.get("fail_closed_conditions_preserved") is True,
        "loop_closed": payload.get("loop_remains_closed") is True,
        "next_module_allowed": payload.get("next_module") in {NEXT_MODULE_VERIFICATION, NEXT_MODULE_BLOCKED},
    }


def build_payload(preflight: Dict[str, Any], source_preview: Dict[str, Any], sections: Dict[str, Any]) -> Dict[str, Any]:
    flags = dangerous_flags()
    payload: Dict[str, Any] = {
        "module_name": MODULE_NAME,
        "generated_at_utc": utc_now(),
        "historical_data_acquisition_external_or_additional_source_approval_status": STATUS_PASS,
        "final_decision": "HISTORICAL_DATA_ACQUISITION_EXTERNAL_OR_ADDITIONAL_SOURCE_VERIFICATION_APPROVED_NEXT_NO_EXECUTION",
        "next_action": "RUN_SEPARATE_EXTERNAL_OR_ADDITIONAL_SOURCE_VERIFICATION_MODULE_NO_BROWSE_NO_DOWNLOAD_NO_API_NO_DATA_BUILD",
        "next_module": NEXT_MODULE_VERIFICATION,
        "whole_system_preflight_completed": True,
        "live_next_module_matches_requested_module": True,
        "artifact_chain_consistent": True,
        "stale_or_contradictory_artifact_detected": False,
        ...
        "external_or_additional_source_approval_record_created": True,
        "user_source_approval_present": True,
        "user_source_approval_scope": USER_APPROVAL_SCOPE,
        "recommended_source_route": RECOMMENDED_SOURCE_ROUTE,
        ...
        "approval_grants_generic_runner_now": False,
        "approval_grants_schema_config_now": False,
        "source_verification_eligible_next": True,
        "okx_source_verified_now": False,
        "okx_download_performed": False,
        "okx_api_call_performed": False,
        ...
        "derived_live_repo_post_check": "PASS_EXTERNAL_OR_ADDITIONAL_SOURCE_APPROVAL_READY_FOR_SOURCE_VERIFICATION_NO_EXECUTION",
        "derived_live_repo_post_check_reason": (
            "approval record permits only the next separate source-verification module; this module performed no OKX "
            "verification, browsing, download, fetch, API call, data build, strategy, backtest, candidate generation, "
            "runtime, capital, live, generic-runner, schema, config, or old-route action"
        ),
        ...
        "preflight": preflight,
        "prior_source_preview_snapshot": {
            "historical_data_acquisition_external_or_additional_source_preview_status": source_preview.get(
                "historical_data_acquisition_external_or_additional_source_preview_status"
            ),
            "recommended_source_route": source_preview.get("recommended_source_route"),
            "current_evidence_chain_quality_after_preview": source_preview.get(
                "current_evidence_chain_quality_after_preview"
            ),
            "next_module": source_preview.get("next_module"),
        },
    }
...
def validate_payload(payload: Dict[str, Any]) -> None:
    required_true = [
        ...
        "external_or_additional_source_approval_record_created",
        "user_source_approval_present",
        "okx_official_historical_archive_or_export_preferred",
        "approval_grants_approval_record_only",
        "approval_grants_future_source_verification_next",
        "source_verification_eligible_next",
    ...
    require_equal(
        payload.get("historical_data_acquisition_external_or_additional_source_approval_status"),
        STATUS_PASS,
        "approval status",
    )
    require_equal(payload.get("final_decision"), EVIDENCE_AFTER, "final_decision")
    require_equal(payload.get("user_source_approval_scope"), USER_APPROVAL_SCOPE, "user_source_approval_scope")
    require_equal(payload.get("recommended_source_route"), RECOMMENDED_SOURCE_ROUTE, "recommended_source_route")
    require_equal(payload.get("next_module"), NEXT_MODULE_VERIFICATION, "next_module")
...
def main() -> None:
    ...
    preflight = validate_preflight(source_preview, local_manual_validator, contract_validator, hardening)
    sections = approval_sections(source_preview)
    payload = build_payload(preflight, source_preview, sections)
    validate_payload(payload)
    write_json(OUT_DIR / "historical_external_or_additional_source_approval_after_preview_v1.json", sections)
    write_json(
        OUT_DIR
