...
MODULE_NAME = "edge_factory_os_repo_only_development_backlog_refresh_after_generic_governance_blocked_status_v1"
CURRENT_TOOL_REL = "tools/edge_factory_os_repo_only_development_backlog_refresh_after_generic_governance_blocked_status_v1.py"
OUT_DIR = LAB_ROOT / MODULE_NAME

EXPECTED_HEAD = "e38020a"
EXPECTED_TRACKED_PYTHON_COUNT = 538

QUEUE_JSON = (
    LAB_ROOT
    / "edge_factory_os_repo_only_development_queue_selector_after_generic_governance_blocked_status_v1"
    / "repo_only_development_queue_selector_after_generic_governance_blocked_status_v1_latest.json"
)
QUEUE_POST_CHECK_JSON = (
    LAB_ROOT
    / "edge_factory_os_repo_only_development_queue_selector_after_generic_governance_blocked_status_post_commit_check"
    / "repo_only_development_queue_selector_after_generic_governance_blocked_status_post_commit_check_latest.json"
)

REQUIRED_QUEUE_STATUS = "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_V1_READY"
REQUIRED_QUEUE_POST_CHECK_STATUS = "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_POST_COMMIT_CHECK_PASS"
REQUIRED_QUEUE_NEXT_ACTION = "BUILD_REPO_ONLY_DEVELOPMENT_BACKLOG_REFRESH_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_V1"
REQUIRED_QUEUE_NEXT_MODULE = "edge_factory_os_repo_only_development_backlog_refresh_after_generic_governance_blocked_status_v1.py"

READY_STATUS = "REPO_ONLY_DEVELOPMENT_BACKLOG_REFRESH_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_V1_READY"
FINAL_DECISION = "GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_REFRESH_READY_FOR_NEXT_ACTION_SELECTOR"
NEXT_ACTION = "BUILD_REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_REFRESH_V1"
NEXT_MODULE = "edge_factory_os_repo_only_next_action_selector_after_generic_governance_blocked_status_backlog_refresh_v1.py"
...
SAFETY_FLAGS: Dict[str, bool] = {
    "repo_only_development_backlog_refresh_after_generic_governance_blocked_status": True,
    "development_backlog_refresh_only": True,
    "read_only_queue_evidence_validation_only": True,
    "next_action_selector_after_generic_governance_blocked_status_backlog_refresh_allowed_next": True,
}
...
def first_status(obj: Dict[str, Any]) -> Optional[str]:
    for key in [
        ...
        "development_queue_selector_after_generic_governance_blocked_status_status",
        "audit_status",
        "status",
    ]:
        value = obj.get(key)
...
def validate_source_false_fields(errors: List[str], queue_obj: Dict[str, Any]) -> None:
    ...
    for flag in DANGEROUS_FLAGS:
        if queue_obj.get(flag) is not False:
            errors.append(f"development queue selector dangerous flag not false: {flag}={queue_obj.get(flag)}")
        safety_flags = queue_obj.get("safety_flags", {})
        if isinstance(safety_flags, dict) and safety_flags.get(flag) is not False:
            errors.append(f"development queue selector safety flag not false: {flag}={safety_flags.get(flag)}")


def validate_source_physical_guards(errors: List[str], queue_obj: Dict[str, Any], post_check: Dict[str, Any]) -> None:
    guards = queue_obj.get("physical_guards", {})
    if not isinstance(guards, dict):
        errors.append("development queue selector physical_guards missing")
        return
    if guards.get("planned_schema_file_existing_count") != 0:
        errors.append(f"development queue selector planned schema count not zero: {guards.get('planned_schema_file_existing_count')}")
    if guards.get("generic_runner_target_file_exists_now") is not False:
        errors.append(f"development queue selector generic runner target exists not false: {guards.get('generic_runner_target_file_exists_now')}")
    for key, expected in PHYSICAL_FALSE_FIELDS.items():
        if guards.get(key) is not expected:
            errors.append(f"development queue selector physical guard mismatch: {key}={guards.get(key)}")
    ...
    if isinstance(counts, dict):
        ...
        errors.append("development queue selector post-check counts missing")
...
def queue_route_object(queue_obj: Dict[str, Any]) -> Dict[str, Any]:
    route = queue_obj.get("development_queue_selector_after_generic_governance_blocked_status", {})
...
def validate_queue_route(errors: List[str], queue_obj: Dict[str, Any]) -> None:
    ...
    if not isinstance(selected, dict):
        ...
        return
    if selected.get("key") != "DEVELOPMENT_BACKLOG_REFRESH_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS":
    ...
    if not isinstance(proofs, dict):
        ...
        for proof in [
            ...
            "selector_selected_repo_only_development_queue_route",
        ]:
...
def validate_inputs() -> Dict[str, Any]:
    ...
    if subject != "Add repo-only development queue selector after generic governance blocked status":
        errors.append(f"unexpected HEAD commit subject: {subject}")
    if paths != ["tools/edge_factory_os_repo_only_development_queue_selector_after_generic_governance_blocked_status_v1.py"]:
        errors.append(f"unexpected HEAD commit paths: {paths}")

    inputs = {
        "development_queue_selector": QUEUE_JSON,
        "development_queue_selector_post_check": QUEUE_POST_CHECK_JSON,
    }
    ...
    post_check = loaded.get("development_queue_selector_post_check", {})
    if queue_obj:
        if queue_obj.get("development_queue_selector_after_generic_governance_blocked_status_status") != REQUIRED_QUEUE_STATUS:
            errors.append(f"development queue selector status mismatch: {queue_obj.get('development_queue_selector_after_generic_governance_blocked_status_status')}")
        ...
        if queue_obj.get("final_decision") != "GENERIC_GOVERNANCE_BLOCKED_STATUS_QUEUE_SELECTED_READY_FOR_BACKLOG_REFRESH":
            errors.append(f"development queue selector final_decision mismatch: {queue_obj.get('final_decision')}")
        validate_source_false_fields(errors, queue_obj)
    ...
    if post_check:
        ...
        validate_source_physical_guards(errors, queue_obj, post_check)
...
def build_backlog(validation: Dict[str, Any]) -> Dict[str, Any]:
    ...
    prerequisites = {
        ...
        "parent_queue_selector_ready": queue_obj.get("development_queue_selector_after_generic_governance_blocked_status_status") == REQUIRED_QUEUE_STATUS,
        "parent_selected_backlog_refresh": selected_from_parent.get("module") == REQUIRED_QUEUE_NEXT_MODULE,
        "parent_selected_backlog_refresh_key": selected_from_parent.get("key") == "DEVELOPMENT_BACKLOG_REFRESH_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS",
    ...
    backlog_items = [
        {
            "rank": 1,
            "key": "NEXT_ACTION_SELECTOR_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_REFRESH",
            "module": NEXT_MODULE,
            "scope": "REPO_ONLY_OS_INTELLIGENCE",
            "allowed": True,
            "reason": "Backlog refresh is complete; select the next safe repo-only action without unblocking generic runner work.",
            "blocked_actions": [
    ...
    return {
        "backlog_refresh_status": "DEVELOPMENT_BACKLOG_REFRESH_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_READY" if selected else "DEVELOPMENT_BACKLOG_REFRESH_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_BLOCKED",
        ...
        "confirmed_proofs": {
            ...
            "queue_selector_selected_repo_only_development_backlog_refresh_route": selected_from_parent.get("key") == "DEVELOPMENT_BACKLOG_REFRESH_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS",
        },
        "physical_guards": {
        ...
        "invariants": {
            ...
            "selected_module_is_next_action_selector_after_generic_governance_blocked_status_backlog_refresh": selected is not None and selected["module"] == NEXT_MODULE,
            "selected_action_is_next_action_selector": selected is not None and selected["key"] == "NEXT_ACTION_SELECTOR_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_BACKLOG_REFRESH",
...
def build_payload(validation: Dict[str, Any], backlog: Dict[str, Any], errors: List[str]) -> Dict[str, Any]:
    ...
    payload: Dict[str, Any] = {
        "module": MODULE_NAME,
        "development_backlog_refresh_after_generic_governance_blocked_status_status": READY_STATUS if passed else "REPO_ONLY_DEVELOPMENT_BACKLOG_REFRESH_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_V1_BLOCKED",
        "severity": "OK" if passed else "BLOCKED",
        "allowed_scope": "REPO_ONLY_OS_INTELLIGENCE",
        "final_decision": FINAL_DECISION if passed else "KEEP_GENERIC_GOVERNANCE_BLOCKED_REVIEW_DEVELOPMENT_BACKLOG_REFRESH_ERRORS",
        "next_action": NEXT_ACTION if passed else "REVIEW_DEVELOPMENT_BACKLOG_REFRESH_AFTER_GENERIC_GOVERNANCE_BLOCKED_STATUS_ERRORS",
        "next_module": NEXT_MODULE if passed else None,
        "reason": (
            "Refreshed the repo-only development backlog and selected the next-action selector after backlog refresh."
            if passed
        ...
        "development_backlog_refresh_after_generic_governance_blocked_status": backlog,
...
def write_outputs(payload: Dict[str, Any]) -> Dict[str, str]:
    ...
    latest_json = OUT_DIR / "repo_only_development_backlog_refresh_after_generic_governance_blocked_status_v1_latest.json"
    timestamped_json = OUT_DIR / f"repo_only_development_backlog_refresh_after_generic_governance_blocked_status_v1_{stamp}.json"
    latest_txt = OUT_DIR / "repo_only_development_backlog_refresh_after_generic_governance_blocked_status_v1_latest.txt"
    ...
    lines = [
        "EDGE FACTORY OS REPO-ONLY DEVELOPMENT BACKLOG REFRESH AFTER GENERIC GOVERNANCE BLOCKED STATUS v1",
        "=" * 100,
        f"development_backlog_refresh_after_generic_governance_blocked_status_status: {payload['development_backlog_refresh_after_generic_governance_blocked_status_status']}",
        ...
        json.dumps(
            {
                "selected_next_backlog_item": payload["development_backlog_refresh_after_generic_governance_blocked_status"]["selected_next_backlog_item"],
                "confirmed_proofs": payload["development_backlog_refresh_after_generic_governance_blocked_status"]["confirmed_proofs"],
                "prerequisites": payload["development_backlog_refresh_after_generic_governance_blocked_status"]["prerequisites"],
                "invariants": payload["development_backlog_refresh_after_generic_governance_blocked_status"]["invariants"],
