...
PRIOR_EXECUTION_PATH = REPO_ROOT / "artifacts" / "strategy_executions" / "crypto_15m_residual_sweep_confirmation_execution_v1.json"
POSTMORTEM_PATH = REPO_ROOT / "artifacts" / "strategy_reviews" / "crypto_15m_residual_sweep_confirmation_postmortem_diagnostic_v1.json"
ARTIFACT_PATH = REPO_ROOT / "artifacts" / "strategy_reviews" / "crypto_15m_residual_sweep_confirmation_trade_detail_recovery_v1.json"

STATUS = "PASS_REPO_ONLY_CRYPTO_15M_RESIDUAL_SWEEP_CONFIRMATION_TRADE_DETAIL_RECOVERY_CREATED"
ARTIFACT_KIND = "CRYPTO_15M_RESIDUAL_SWEEP_CONFIRMATION_TRADE_DETAIL_RECOVERY"
...
MATCH_CLASS = "TRADE_DETAIL_RECOVERY_COMPLETE_AGGREGATES_MATCH_NO_EDGE_NO_LIVE"
ATTENTION_CLASS = "TRADE_DETAIL_RECOVERY_COMPLETE_AGGREGATES_DIFFER_ATTENTION_NO_EDGE_NO_LIVE"
INCOMPLETE_CLASS = "TRADE_DETAIL_RECOVERY_INCOMPLETE_NO_EDGE_NO_LIVE"
INVALID_CLASS = "TRADE_DETAIL_RECOVERY_INVALIDATED_BY_INTEGRITY_FAILURE_NO_EDGE_NO_LIVE"
...
def process_signal_candidate(
    ...
    if z_residual >= Z_THRESHOLD:
        ...
        if prior_high > 0 and highs[idx] > prior_high and closes[idx] < prior_high and closes[idx] < opens[idx] and volumes[idx] > volume_sma:
            ...
            candidate, reason = build_candidate("short", symbol, idx, master_idx, timestamps, opens, highs, lows, closes, atr, z_residual, residual_4h, sweep_depth, volume_ratio, prior_high, prior_low, btc_24h_return)
        ...
        if prior_low > 0 and lows[idx] < prior_low and closes[idx] > prior_low and closes[idx] > opens[idx] and volumes[idx] > volume_sma:
            ...
            candidate, reason = build_candidate("long", symbol, idx, master_idx, timestamps, opens, highs, lows, closes, atr, z_residual, residual_4h, sweep_depth, volume_ratio, prior_high, prior_low, btc_24h_return)
...
def trade_from_candidate(candidate: dict[str, Any], trade_id: int) -> dict[str, Any]:
    ...
    trade.update(
        {
            "trade_id": f"RSCR-{trade_id:06d}",
            "strategy": STRATEGY,
            "route_family": ROUTE_FAMILY,
            "config_id": CONFIG_ID,
            "family": "residual_sweep_confirmation_reversal",
            "gross_pnl_usdt": gross,
            "cost_pnl_usdt": cost,
            "net_pnl_usdt": net,
            "portfolio_net_bps_contribution": net / BASE_EQUITY * 10000.0,
    ...
    for master_idx, _timestamp in enumerate(master_timestamps):
        ...
        for candidate in candidates:
            ...
            if len(open_positions) >= MAX_CONCURRENT_POSITIONS or new_count >= MAX_NEW_PER_TIMESTAMP:
                ...
                continue
            trade = trade_from_candidate(candidate, len(trades) + 1)
            trades.append(trade)
            ...
            new_count += 1
            counters[f"accepted_{candidate['side']}_trades"] += 1
...
def compare_value(name: str, prior: Any, recovered: Any, diffs: list[dict[str, Any]]) -> None:
    if isinstance(prior, (int, float)) and isinstance(recovered, (int, float)):
        if abs(float(prior) - float(recovered)) > 1e-6:
...
def aggregate_reproduction(prior: dict[str, Any], metrics: dict[str, Any], split_metrics: dict[str, Any], null: dict[str, Any], integrity: dict[str, Any]) -> dict[str, Any]:
    prior_metrics = prior.get("metrics", {})
    prior_split = prior.get("split_metrics", {})
    prior_null = prior.get("null_baseline", {})
    ...
    for field in [
        ...
        "time_stop_exit_count",
    ]:
        compare_value(f"metrics.{field}", prior_metrics.get(field), metrics.get(field), diffs)
    for split_name_key in ["validation", "holdout"]:
        for field in ["portfolio_net_bps", "monthly_positive_rate"]:
            compare_value(f"split_metrics.{split_name_key}.{field}", prior_split.get(split_name_key, {}).get(field), split_metrics.get(split_name_key, {}).get(field), diffs)
    for field in ["feasible", "null_pass", "validation_percentile"]:
        compare_value(f"null_baseline.{field}", prior_null.get(field), null.get(field), diffs)
    compare_value("metric_integrity_result.passed", prior.get("metric_integrity_result", {}).get("passed"), integrity.get("passed"), diffs)
    return {"aggregate_match": not diffs, "material_difference_count": len(diffs), "differences": diffs}


def rounded_trade_rows(trades: list[dict[str, Any]]) -> list[dict[str, Any]]:
    numeric_fields = {
        "entry_price",
        "exit_price",
        "notional",
        ...
        "cost_pnl_usdt",
        "net_pnl_usdt",
        "portfolio_net_bps_contribution",
        "z_residual",
        "abs_z_residual",
        "residual_4h",
        "sweep_depth",
    ...
    for trade in trades:
        ...
        for key, value in trade.items():
            if key in {"signal_master_idx", "entry_master_idx", "exit_master_idx"}:
                continue
...
def main() -> int:
    ...
    allowed_status = {
        "?? tools/edge_factory_os_repo_only_crypto_15m_residual_sweep_confirmation_trade_detail_recovery_v1.py",
        "?? artifacts/strategy_reviews/crypto_15m_residual_sweep_confirmation_trade_detail_recovery_v1.json",
    }
    ...
    split_metrics = {split_key: summarize_split(trades, split_key) for split_key in ("train", "validation", "holdout")}
    ...
    aggregate_check = aggregate_reproduction(prior, metrics, split_metrics, null, integrity)
    trade_rows = rounded_trade_rows(trades)
    available_features = [
        "z_residual",
        "abs_z_residual",
        "residual_4h",
    ...
    if not integrity["passed"]:
        result_classification = INVALID_CLASS
        next_allowed_step = "CRYPTO_15M_RESIDUAL_SWEEP_CONFIRMATION_TRADE_DETAIL_RECOVERY_REPAIR_V1"
    elif not trade_rows:
        result_classification = INCOMPLETE_CLASS
        next_allowed_step = "CRYPTO_15M_RESIDUAL_SWEEP_CONFIRMATION_TRADE_DETAIL_RECOVERY_REPAIR_V1"
    elif aggregate_check["aggregate_match"]:
        result_classification = MATCH_CLASS
        next_allowed_step = "CRYPTO_15M_RESIDUAL_SWEEP_CONFIRMATION_POSTMORTEM_DIAGNOSTIC_V2"
    else:
        result_classification = ATTENTION_CLASS
        next_allowed_step = "CRYPTO_15M_RESIDUAL_SWEEP_CONFIRMATION_EXECUTION_REPRODUCTION_REVIEW_V1"

    safety_permissions = {
        "trade_detail_recovery_created": True,
    ...
    validation_checks = {
        ...
        "prior_artifacts_loaded": bool(prereg) and bool(prior) and bool(postmortem),
        "panel_review_loaded": bool(panel_review),
        "same_preregistered_config_used": prereg.get("config_id") == CONFIG_ID,
    ...
    artifact = {
        ...
        "module": "edge_factory_os_repo_only_crypto_15m_residual_sweep_confirmation_trade_detail_recovery_v1",
        "source_checkpoint": {
            "actual_head": git(["rev-parse", "HEAD"]),
            "tracked_python_count": tracked_python_count(),
            "repo_clean_before_run": not unexpected_status,
            "git_status_at_recovery": status_lines,
            "allowed_new_paths_at_recovery": sorted(allowed_status),
            "unexpected_dirty_paths_at_recovery": unexpected_status,
        },
        "source_artifacts": {
        ...
        "prior_result_summary": {
            "accepted_long_trades": prior.get("metrics", {}).get("accepted_long_trades"),
            "accepted_short_trades": prior.get("metrics", {}).get("accepted_short_trades"),
            "closed_trades": prior.get("metrics", {}).get("closed_trades"),
            "validation_net_bps": prior.get("split_metrics", {}).get("validation", {}).get("portfolio_net_bps"),
            "holdout_net_bps": prior.get("split_metrics", {}).get("holdout", {}).get("portfolio_net_bps"),
            "validation_monthly_positive_rate": prior.get("split_metrics", {}).get("validation", {}).get("monthly_positive_rate"),
            "holdout_monthly_positive_rate": prior.get("split_metrics", {}).get("holdout", {}).get("monthly_positive_rate"),
            "null_pass": prior.get("null_baseline", {}).get("null_pass"),
            "validation_percentile": prior.get("null_baseline", {}).get("validation_percentile"),
        },
        "aggregate_reproduction_check": aggregate_check,
        "trade_level_rows": trade_rows,
        "trade_level_summary": {
            "trade_level_data_available": bool(trade_rows),
            "recovered_trade_count": len(trade_rows),
            "long_trade_count": metrics["accepted_long_trades"],
            "short_trade_count": metrics["accepted_short_trades"],
            "closed_trade_count": metrics["closed_trades"],
            "side_split": metrics["side_split"],
            "exit_counts": {
                "stop": metrics["stop_exit_count"],
                "take": metrics["take_profit_exit_count"],
                "time": metrics["time_stop_exit_count"],
            },
        },
        "candidate_level_summary": {
            "raw_residual_extremes_long": metrics["raw_residual_extremes_long"],
            "raw_residual_extremes_short": metrics["raw_residual_extremes_short"],
            "raw_sweep_candidates_long": metrics["raw_sweep_candidates_long"],
            "raw_sweep_candidates_short": metrics["raw_sweep_candidates_short"],
            "confirmation_passed_long": metrics["confirmation_passed_long"],
            "confirmation_passed_short": metrics["confirmation_passed_short"],
            "cost_aware_gate_passed_long": metrics["cost_aware_gate_passed_long"],
            "cost_aware_gate_passed_short": metrics["cost_aware_gate_passed_short"],
            "accepted_long_trades": metrics["accepted_long_trades"],
            "accepted_short_trades": metrics["accepted_short_trades"],
            "skipped_due_no_confirmation": metrics["skipped_due_no_confirmation"],
            "skipped_due_cost_gate": metrics["skipped_due_cost_gate"],
            "skipped_due_capacity": metrics["skipped_due_capacity"],
            "skipped_due_cooldown": metrics["skipped_due_cooldown"],
            "skipped_due_invalid_risk": metrics["skipped_due_invalid_risk"],
            "skipped_due_missing_next_bar": metrics["skipped_due_missing_next_bar"],
            "skipped_due_same_symbol_open": metrics["skipped_due_same_symbol_open"],
        },
        "available_feature_fields": available_features,
        "missing_feature_fields": missing_features,
        "result_classification": result_classification,
        "next_allowed_step": next_allowed_step,
        "limitations": [
            "This recovery reruns the same preregistered diagnostic only to recover trade-level rows.",
            "No parameter, threshold, split, cost, stop, take, time stop, universe, ranking, position limit, or cooldown was changed.",
            "No V2 execution, candidate generation, edge claim, runtime, live, or capital permission is created.",
        ],
        "safety_permissions": safety_permissions,
        "validation_checks": validation_checks,
        "replacement_checks_all_true": all(validation_checks.values())
        and safety_permissions["trade_detail_recovery_created"] is True
        and all(value is False for key, value in safety_permissions.items() if key != "trade_detail_recovery_created"),
    ...
    print(f"trade_level_data_available: {str(bool(trade_rows)).lower()}")
    print(f"recovered_trade_count: {len(trade_rows)}")
    print(f"prior_closed_trade_count: {prior.get('metrics', {}).get('closed_trades')}")
    ...
    print(f"available_feature_field_count: {len(available_features)}")
    print(f"missing_feature_field_count: {len(missing_features)}")
