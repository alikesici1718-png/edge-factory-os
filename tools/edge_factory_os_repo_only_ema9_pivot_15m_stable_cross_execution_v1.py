...
ARTIFACT_PATH = "artifacts/strategy_executions/ema9_pivot_15m_ema9_pivot_stable_cross_execution_v1.json"
PREREG_PATH = "artifacts/research_preregistrations/ema9_pivot_15m_ema9_pivot_stable_cross_preregistration_contract_v1.json"
...
STATUS = "PASS_REPO_CODE_ONLY_EMA9_PIVOT_15M_EMA9_PIVOT_STABLE_CROSS_EXECUTED"
ARTIFACT_KIND = "EMA9_PIVOT_15M_EMA9_PIVOT_STABLE_CROSS_EXECUTION"
PREREG_STATUS = "PASS_REPO_ONLY_EMA9_PIVOT_15M_EMA9_PIVOT_STABLE_CROSS_PREREGISTRATION_CONTRACT_CREATED"
...
EXPECTED_PRE_EXECUTION_HEAD = "eef5b5b8847f453ba5fa8d69342452dce439edbb"
EXPECTED_PRE_EXECUTION_TRACKED_PYTHON_COUNT = 933
...
def symbol_seed(symbol: str, run_index: int) -> int:
    digest = hashlib.sha256(f"{symbol}:{run_index}:ema9_pivot-null".encode("utf-8")).hexdigest()
...
def compute_ema(closes: Sequence[float]) -> List[float]:
    if not closes:
        return []
    ema = [float(closes[0])]
    for close in closes[1:]:
        ema.append(EMA_ALPHA * float(close) + (1.0 - EMA_ALPHA) * ema[-1])
    return ema
...
def process_symbol(symbol: str, expected_path: str) -> Dict[str, Any]:
    ...
    counts: Dict[str, int] = {
        **load_counts,
        ...
        "accepted_signals": 0,
        "skipped_signals": 0,
        "fake_pivot_jump_blocked_count": 0,
    ...
    for i in range(n):
        ...
        if support_level is not None and support_confirm_i is not None and support_pivot_i is not None:
            if ema[i - 1] < support_level and ema[i] >= support_level:
                raw_signals.append(
                    {
                        "side": "long",
                        "level_type": "support",
        ...
        if resistance_level is not None and resistance_confirm_i is not None and resistance_pivot_i is not None:
            if ema[i - 1] > resistance_level and ema[i] <= resistance_level:
                raw_signals.append(
                    {
                        "side": "short",
        ...
        for signal in raw_signals:
            ...
            stable_delay = i - int(signal["level_confirmed_bar_index"])
            if stable_delay < MIN_STABLE_BARS:
                counts["fake_pivot_jump_blocked_count"] += 1
                counts["skipped_signals"] += 1
                continue
    ...
    counts["skipped_signals"] = counts["total_signals"] - counts["accepted_signals"]
    if counts["skipped_signals"] < 0:
        raise RuntimeError(f"negative skipped signal count for {symbol}")
...
def summarize_closed_trades(trades: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    profit_factor_reason: Optional[str]
    if wins and losses:
    ...
    return {
        ...
        "open_unresolved_trades": len(open_trades),
...
def build_null_summary(
    ...
    return {
        ...
        "limitation": None,
        "method_note": (
            "Within each symbol, accepted validation signal timestamps were grouped into deterministic "
            "weekly blocks, blocks were shuffled, and original signal side labels were replayed at the "
            "shuffled timestamps with the same 1% stop and 2% take-profit rules."
        ),
    }
...
def validate_source_artifacts() -> Tuple[Dict[str, Any], Dict[str, Any], List[Tuple[str, str]], Dict[str, Any]]:
    ...
    if prereg.get("strategy_name") != STRATEGY_NAME:
        raise RuntimeError("strategy name mismatch")
...
def build_execution() -> Dict[str, Any]:
    ...
    if unexpected_dirty_paths:
        raise RuntimeError(f"unexpected dirty paths during execution: {unexpected_dirty_paths}")
    if actual_head != EXPECTED_PRE_EXECUTION_HEAD:
        raise RuntimeError(f"HEAD moved before execution: {actual_head} != {EXPECTED_PRE_EXECUTION_HEAD}")
    if actual_tracked_python_count != EXPECTED_PRE_EXECUTION_TRACKED_PYTHON_COUNT:
        raise RuntimeError(
            "tracked Python count mismatch before execution: "
            f"{actual_tracked_python_count} != {EXPECTED_PRE_EXECUTION_TRACKED_PYTHON_COUNT}"
        )

    prereg, panel_review, symbols_and_paths, source_summary = validate_source_artifacts()
    ...
    if all_counts["total_signals"] != all_counts["accepted_signals"] + all_counts["skipped_signals"]:
        metric_issues.append("signal_accounting_mismatch")
    ...
    safety_review_passed = (
        ...
        and safety_review["filters_enabled"] is False
        and safety_review["score_filters_enabled"] is False
        and safety_review["seven_of_seven_filters_used"] is False
    )
    ...
    payload: Dict[str, Any] = {
        ...
        "input_data_validation": {
            ...
            "only_preregistered_panel_partitions_read": True,
        ...
        "signal_logic": {
            "long_signal": "support stable and EMA9 previous < support and EMA9 current >= support",
            "short_signal": "resistance stable and EMA9 previous > resistance and EMA9 current <= resistance",
            "pivot_high_confirmation": "candidate high at bar p is confirmed at p+rightBars if it is the max over p-leftBars..p+rightBars",
            "pivot_low_confirmation": "candidate low at bar p is confirmed at p+rightBars if it is the min over p-leftBars..p+rightBars",
            "stable_level_logic": "current_bar_index - level_confirmed_bar_index >= minStableBars",
            "fake_pivot_jump_block": "raw crosses before level stability are skipped",
            "cooldown_policy": "after accepted signal, same-symbol raw stable signals are skipped through signal_bar_index+cooldown",
        ...
        "signal_accounting": {
            "total_signals": all_counts["total_signals"],
            "accepted_signals": all_counts["accepted_signals"],
            "skipped_signals": all_counts["skipped_signals"],
            "accepted_signals_by_split": accepted_by_split,
            "fake_pivot_jump_blocked_count": all_counts["fake_pivot_jump_blocked_count"],
        ...
        "metric_integrity_summary": {
            ...
            "signal_accounting_balanced": all_counts["total_signals"]
            == all_counts["accepted_signals"] + all_counts["skipped_signals"],
    ...
    if not all(payload["validation_checks"].values()):
        raise RuntimeError(f"execution validation checks failed: {payload['validation_checks']}")
...
def main() -> None:
    ...
    summary = {
        ...
        "total_signals": payload["signal_accounting"]["total_signals"],
        ...
        "open_unresolved_trades": payload["overall_summary"]["trade_summary"]["open_unresolved_trades"],
