...
MODULE_NAME = "edge_factory_os_repo_only_development_backlog_refresh_after_framework_readme_gate_v1"
OUT_DIR = LAB_ROOT / "edge_factory_os_repo_only_development_backlog_refresh_after_framework_readme_gate_v1"
OUT_DIR.mkdir(parents=True, exist_ok=True)

EXPECTED_HEAD_PREFIX = "ffb401a"
CURRENT_TOOL_REL = "tools/edge_factory_os_repo_only_development_backlog_refresh_after_framework_readme_gate_v1.py"
...
QUEUE_POST_JSON = LAB_ROOT / "edge_factory_os_repo_only_development_queue_selector_after_framework_readme_approval_record_post_commit_check" / "repo_only_development_queue_selector_after_framework_readme_approval_record_post_commit_check_latest.json"
...
SELECTED_NEXT_MODULE = "edge_factory_os_repo_only_post_framework_status_refresh_v1.py"

SAFETY_FLAGS: Dict[str, bool] = {
    "repo_only_development_backlog_refresh_after_framework_readme_gate": True,
    "read_only_backlog_refresh_only": True,
    "readme_apply_block_respected": True,
    "manual_approval_absence_required": True,

    "repo_only_governance_allowed": True,
    "repo_only_audit_allowed": True,
    "read_only_validation_allowed": True,

    "post_framework_status_refresh_allowed_next": True,
...
def validate_inputs() -> Dict[str, Any]:
    ...
    if subject != "Add repo-only development queue selector after framework README approval record":
        errors.append(f"unexpected HEAD commit subject: {subject}")

    paths = commit_paths("HEAD")
    checks["head_commit_paths"] = paths
    if paths != ["tools/edge_factory_os_repo_only_development_queue_selector_after_framework_readme_approval_record_v1.py"]:
        errors.append(f"unexpected HEAD commit paths: {paths}")

    inputs = {
        "queue_after_readme_approval_record": (
            QUEUE_JSON,
            "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_FRAMEWORK_README_APPROVAL_RECORD_V1_READY",
            "edge_factory_os_repo_only_development_backlog_refresh_after_framework_readme_gate_v1.py",
        ),
        "queue_after_readme_approval_record_post_check": (
            QUEUE_POST_JSON,
            "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_FRAMEWORK_README_APPROVAL_RECORD_POST_COMMIT_CHECK_PASS",
            "edge_factory_os_repo_only_development_backlog_refresh_after_framework_readme_gate_v1.py",
        ),
...
def build_backlog(validation: Dict[str, Any]) -> Dict[str, Any]:
    queue_obj = validation["loaded"]["queue_after_readme_approval_record"]
    queue = queue_obj.get("development_queue_selector_after_framework_readme_approval_record", {})
    ...
    prerequisites = {
        "parent_queue_ready": queue_obj.get("development_queue_selector_after_framework_readme_approval_record_status") == "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_FRAMEWORK_README_APPROVAL_RECORD_V1_READY",
        "parent_selected_backlog_refresh": selected.get("module") == "edge_factory_os_repo_only_development_backlog_refresh_after_framework_readme_gate_v1.py",
        "readme_apply_block_respected": queue_policy.get("readme_apply_requires_explicit_manual_approval") is True,
        ...
        "approval_record_not_granted": approval_record.get("approval_status") == "FRAMEWORK_README_CONTENT_MANUAL_APPROVAL_NOT_GRANTED",
    ...
    backlog_items = [
        {
            "rank": 1,
            "key": "POST_FRAMEWORK_STATUS_REFRESH",
            "module": SELECTED_NEXT_MODULE,
            "scope": "REPO_ONLY_OS_INTELLIGENCE",
            "status": "READY",
            "reason": "After framework skeleton, README preview, and approval-blocked README apply chain, refresh the compact OS status before selecting further work.",
            "blocked_actions": [
                "runtime_touch",
                "launcher_execution",
                "capital_change",
        ...
        },
        {
            ...
            "status": "BLOCKED_MANUAL_APPROVAL_ABSENT",
            "reason": "Manual approval is absent/invalid, so README apply remains blocked.",
            "blocked_actions": ["readme_apply", "readme_edit"],
        },
        {
            "rank": 3,
            "key": "FRAMEWORK_SCHEMA_REGISTRY_CONTRACT",
            "module": "edge_factory_os_repo_only_framework_schema_registry_contract_v1.py",
            "scope": "REPO_ONLY_SCHEMA_CONTRACT",
            "status": "QUEUED_NOT_SELECTED",
            "reason": "Useful later after post-framework status refresh confirms current state.",
            "blocked_actions": [],
        },
        {
            "rank": 4,
            ...
            "status": "QUEUED_NOT_SELECTED",
            "reason": "Defer until compact status refresh confirms whether risk-review approval work is needed.",
            "blocked_actions": [],
        },
    ]

    status_counts = dict(Counter(item["status"] for item in backlog_items))
    selected_next = backlog_items[0] if all(prerequisites.values()) else None

    return {
        "backlog_refresh_status": "DEVELOPMENT_BACKLOG_AFTER_FRAMEWORK_README_GATE_ACTIVE" if selected_next else "DEVELOPMENT_BACKLOG_AFTER_FRAMEWORK_README_GATE_BLOCKED",
        "backlog_items": backlog_items,
        "backlog_item_count": len(backlog_items),
        "status_counts": status_counts,
        "selected_next_item": selected_next,
        "selected_next_module": selected_next["module"] if selected_next else None,
        "selected_next_action": "BUILD_REPO_ONLY_POST_FRAMEWORK_STATUS_REFRESH_V1" if selected_next else None,
        "selected_scope": selected_next["scope"] if selected_next else None,
        "context_summary": {
            "readme_apply_blocked": True,
            "manual_approval_present": False,
            "manual_approval_valid": False,
        ...
        "backlog_policy": {
            "prefer_status_refresh_after_closed_apply_gate": True,
            "readme_apply_stays_blocked_without_explicit_manual_approval": True,
            "no_strategy_research_until_future_explicit_research_approval": True,
        ...
        "invariants": {
            ...
            "backlog_item_count_positive": len(backlog_items) > 0,
            "selected_next_module_is_post_framework_status_refresh": selected_next is not None and selected_next["module"] == SELECTED_NEXT_MODULE,
            "readme_apply_item_blocked": backlog_items[1]["status"] == "BLOCKED_MANUAL_APPROVAL_ABSENT",
            ...
            "no_file_creation_or_modification_performed_by_backlog_refresh": True,
        },
    }
...
def main() -> int:
    ...
    payload = {
        "module": MODULE_NAME,
        "development_backlog_refresh_after_framework_readme_gate_status": "REPO_ONLY_DEVELOPMENT_BACKLOG_REFRESH_AFTER_FRAMEWORK_README_GATE_V1_READY" if passed else "REPO_ONLY_DEVELOPMENT_BACKLOG_REFRESH_AFTER_FRAMEWORK_README_GATE_V1_BLOCKED",
        "severity": "OK" if passed else "BLOCKED",
        "allowed_scope": "REPO_ONLY_OS_INTELLIGENCE",
        "final_decision": "DEVELOPMENT_BACKLOG_REFRESH_READY_POST_FRAMEWORK_STATUS_REFRESH_SELECTED" if passed else "KEEP_FREEZE_REVIEW_DEVELOPMENT_BACKLOG_REFRESH_ERRORS",
        "next_action": "BUILD_REPO_ONLY_POST_FRAMEWORK_STATUS_REFRESH_V1" if passed else "REVIEW_DEVELOPMENT_BACKLOG_REFRESH_ERRORS",
        "next_module": SELECTED_NEXT_MODULE if passed else None,
        "reason": "Refreshed repo-only development backlog after README apply gate; selected post-framework status refresh." if passed else "Development backlog refresh validation failed.",
        ...
        "development_backlog_refresh_after_framework_readme_gate": backlog,
    ...
    latest_json = OUT_DIR / "repo_only_development_backlog_refresh_after_framework_readme_gate_v1_latest.json"
    timestamped_json = OUT_DIR / f"repo_only_development_backlog_refresh_after_framework_readme_gate_v1_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json"
    latest_txt = OUT_DIR / "repo_only_development_backlog_refresh_after_framework_readme_gate_v1_latest.txt"

    latest_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    timestamped_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")

    lines = [
        "EDGE FACTORY OS REPO-ONLY DEVELOPMENT BACKLOG REFRESH AFTER FRAMEWORK README GATE v1",
        "=" * 100,
        f"development_backlog_refresh_after_framework_readme_gate_status: {payload['development_backlog_refresh_after_framework_readme_gate_status']}",
        ...
        json.dumps(
            {
                "selected_next_item": backlog["selected_next_item"],
                ...
                "blocked_runtime_or_trading_actions": backlog["blocked_runtime_or_trading_actions"],
            },
            indent=2,
            sort_keys=True,
        ),
