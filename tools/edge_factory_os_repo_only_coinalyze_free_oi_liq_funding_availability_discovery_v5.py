#!/usr/bin/env python3
"""Coinalyze free OI/liquidation/funding availability discovery V5.

Data discovery only. This module does not run a strategy, generate signals,
compute PnL, create candidates, claim edge, or grant runtime/live/capital
permission. The Coinalyze API key is read only from the temporary process
environment variable COINALYZE_API_KEY and is never printed or written.
"""
...
MODULE = "edge_factory_os_repo_only_coinalyze_free_oi_liq_funding_availability_discovery_v5"
ARTIFACT_KIND = "COINALYZE_FREE_OI_LIQ_FUNDING_AVAILABILITY_DISCOVERY_V5"
PASS_STATUS = "PASS_REPO_ONLY_COINALYZE_FREE_OI_LIQ_FUNDING_AVAILABILITY_DISCOVERY_V5_CREATED"
MISSING_KEY_STATUS = "BLOCKED_COINALYZE_API_KEY_MISSING_V5"
INVALID_KEY_STATUS = "BLOCKED_COINALYZE_API_KEY_INVALID_V5"
API_ERROR_STATUS = "BLOCKED_COINALYZE_RATE_LIMIT_OR_API_ERROR_V5"
REPO_ROOT = Path(__file__).resolve().parents[1]
TOOL_PATH = REPO_ROOT / "tools" / "edge_factory_os_repo_only_coinalyze_free_oi_liq_funding_availability_discovery_v5.py"
ARTIFACT_PATH = REPO_ROOT / "artifacts" / "data_discovery" / "coinalyze_free_oi_liq_funding_availability_discovery_v5.json"
V4_PARTIAL_PATHS = [
    REPO_ROOT / "artifacts" / "data_discovery" / "coinalyze_free_oi_liq_funding_availability_discovery_v4.json",
    REPO_ROOT / "tools" / "edge_factory_os_repo_only_coinalyze_free_oi_liq_funding_availability_discovery_v4.py",
]
PRIOR_ARTIFACTS = [
    REPO_ROOT / "artifacts" / "data_discovery" / "coinalyze_free_oi_liq_funding_availability_discovery_v1.json",
    REPO_ROOT / "artifacts" / "data_discovery" / "coinalyze_free_oi_liq_funding_availability_discovery_v2.json",
    REPO_ROOT / "artifacts" / "data_discovery" / "coinalyze_free_oi_liq_funding_availability_discovery_v3.json",
]
...
PRIOR_BLOCKED_ATTEMPTS = [
    ...
    {
        "version": "V3",
        "commit": "b65639977301e52d5ca1ba68f5ed221e6b63a949",
        "status": "BLOCKED_COINALYZE_API_KEY_MISSING_AFTER_SECURE_INPUT",
        "exact_blocker": "COINALYZE_API_KEY unavailable to the V3 process after secure input",
    },
]
...
def prior_blocked_artifacts_modified() -> bool:
    prior_paths = {str(path.relative_to(REPO_ROOT)).replace("\\", "/") for path in PRIOR_ARTIFACTS}
    return any(path in prior_paths for _status, path in git_status_entries())


def v4_partial_files_present() -> list[str]:
    present = []
    for path in V4_PARTIAL_PATHS:
        if path.exists():
            present.append(str(path.relative_to(REPO_ROOT)).replace("\\", "/"))
    return present
...
def symbol_matches_target(value: str, target_norm: str) -> bool:
    normalized = normalize_symbol(value)
    return normalized == target_norm or normalized.startswith(target_norm) or target_norm in normalized


def market_exchange(market: dict[str, Any]) -> str | None:
    raw_code = str(market.get("exchange", "")).upper()
    if raw_code in EXCHANGE_CODE_MAP:
        return EXCHANGE_CODE_MAP[raw_code]
    text = " ".join(
        str(market.get(key, ""))
...
def coinalyze_get(endpoint: str, params: dict[str, Any], api_key: str) -> tuple[bool, int | None, Any, str | None]:
    ...
    request = urllib.request.Request(
        ...
        headers={"User-Agent": "edge-factory-os-coinalyze-v5-discovery/1.0"},
        method="GET",
    )
    try:
...
def classify_results(results: list[dict[str, Any]]) -> tuple[str, str, dict[str, Any]]:
    ...
    if blocked_count and not (oi_symbols or liquidation_symbols or funding_symbols):
        return "COINALYZE_FREE_DATA_BLOCKED_API_OR_RATE_LIMIT", "COINALYZE_API_OR_RATE_LIMIT_REVIEW_V5", summary
...
def base_payload(status: str, replacement_checks_all_true: bool) -> dict[str, Any]:
    ...
    prior_unmodified = not prior_blocked_artifacts_modified()
    v4_present = v4_partial_files_present()
    validation_checks = {
        "repo_clean_before_run_after_v4_cleanup": clean and not v4_present,
        "v4_partial_files_removed_if_present": not v4_present,
        "api_key_loaded_from_one_time_env_only": status != MISSING_KEY_STATUS,
    ...
    return {
        ...
        "v4_cleanup_summary": {
            "v4_partial_files_present_after_cleanup": v4_present,
            "v4_partial_files_removed_if_present": not v4_present,
            "cleanup_action_in_this_tool": "none_required" if not v4_present else "blocked_v4_files_still_present",
        },
        "api_key_handling": {
            "env_var_name": API_KEY_ENV,
            "api_key_source": "one_time_env",
            "api_key_loaded": status != MISSING_KEY_STATUS,
        ...
        "next_allowed_step": "COINALYZE_ONE_TIME_ENV_API_KEY_REVIEW_V5",
        "next_module": "COINALYZE_ONE_TIME_ENV_API_KEY_REVIEW_V5",
        "limitations": [],
        "safety_permissions": {
...
def missing_key_payload() -> dict[str, Any]:
    payload = base_payload(MISSING_KEY_STATUS, False)
    payload["limitations"] = [
        "COINALYZE_API_KEY was not available in the one-time process environment.",
...
def discover(api_key: str) -> dict[str, Any]:
        ...
        for exchange in ("BINANCE", "OKX"):
            ...
            for market in selected:
                if market.get("_mapped_exchange") != exchange:
                    continue
                if any(symbol_matches_target(value, target_norm) for value in market_symbol_fields(market)):
                    match = market
                    break
    ...
    if not market_ok:
        if market_status == 401:
            payload["status"] = INVALID_KEY_STATUS
        else:
            payload["status"] = API_ERROR_STATUS
        payload["replacement_checks_all_true"] = False
        payload["validation_checks"]["replacement_checks_all_true"] = False
        payload["limitations"].append("Coinalyze future-markets request failed before endpoint probing.")
        return payload
    ...
    if classification == "COINALYZE_FREE_DATA_BLOCKED_API_OR_RATE_LIMIT":
        if any(result.get("http_status") == 401 for result in endpoint_results):
            payload["status"] = INVALID_KEY_STATUS
        else:
            payload["status"] = API_ERROR_STATUS
        payload["replacement_checks_all_true"] = False
        payload["validation_checks"]["replacement_checks_all_true"] = False
        payload["limitations"].append("Coinalyze endpoint probing was blocked by authorization, rate limit, or API error before usable coverage was found.")
        ...
        payload["limitations"].append("Partial recent Coinalyze coverage was discovered; no strategy, signal, or PnL was computed.")
    else:
    ...
    prior_unmodified = not prior_blocked_artifacts_modified()
    v4_present = v4_partial_files_present()
    payload["validation_checks"]["repo_clean_before_run_after_v4_cleanup"] = clean and not v4_present
    payload["validation_checks"]["v4_partial_files_removed_if_present"] = not v4_present
    ...
    if not clean or not prior_unmodified or v4_present:
        payload["status"] = "BLOCKED_UNEXPECTED_DIRTY_REPO_BEFORE_V5"
        payload["result_classification"] = "COINALYZE_FREE_DATA_BLOCKED_API_OR_RATE_LIMIT"
        payload["next_allowed_step"] = "REPO_STATE_REVIEW_BEFORE_V5"
        payload["next_module"] = "REPO_STATE_REVIEW_BEFORE_V5"
        payload["replacement_checks_all_true"] = False
        payload["validation_checks"]["replacement_checks_all_true"] = False
        payload["limitations"].append("Repo state validation failed outside expected V5 files or prior/V4 artifacts were present or modified.")
    return payload
...
def print_required_stdout(payload: dict[str, Any]) -> None:
    ...
    print("api_key_source: one_time_env")
