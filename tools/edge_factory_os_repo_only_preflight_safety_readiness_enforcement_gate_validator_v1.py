...
MODULE_NAME = "edge_factory_os_repo_only_preflight_safety_readiness_enforcement_gate_validator_v1"
CURRENT_TOOL_REL = "tools/edge_factory_os_repo_only_preflight_safety_readiness_enforcement_gate_validator_v1.py"
OUT_DIR = LAB_ROOT / MODULE_NAME

EXPECTED_PARENT_HEAD = "a3f562c"
EXPECTED_TRACKED_PYTHON_COUNT = 600
EXPECTED_PREVIOUS_TRACKED_PYTHON_COUNT = 599
NEXT_MODULE = "edge_factory_os_repo_only_preflight_safety_readiness_rollout_status_record_v1.py"
POST_CHECK_STATUS = "REPO_ONLY_PREFLIGHT_SAFETY_READINESS_ENFORCEMENT_GATE_VALIDATOR_POST_COMMIT_CHECK_PASS"
...
ENFORCEMENT_GATE_ARTIFACT = (
    LAB_ROOT
    / "edge_factory_os_repo_only_preflight_safety_readiness_enforcement_gate_v1"
    / "repo_only_preflight_safety_readiness_enforcement_gate_v1_latest.json"
)
...
def validate_gate(gate: Dict[str, Any]) -> Dict[str, bool]:
    dangerous = gate.get("dangerous_flags", {})
    gate_body = gate.get("preflight_safety_readiness_enforcement_gate", {})
    preflight_paths = gate_body.get("preflight_required_future_paths", [])
    kill_switch_paths = gate_body.get("kill_switch_required_future_paths", [])
    return {
        "gate_found": bool(gate),
        "gate_ready": gate.get("preflight_safety_enforcement_gate_ready") is True
        and gate.get("preflight_safety_readiness_enforcement_gate_status") in {"READY", "PASS"},
        "gate_next_module_points_here": gate.get("next_module") == "edge_factory_os_repo_only_preflight_safety_readiness_enforcement_gate_validator_v1.py",
        "repo_only": gate.get("enforcement_gate_is_repo_only") is True and gate_body.get("enforcement_gate_is_repo_only") is True,
        "planning_only": gate.get("enforcement_gate_is_planning_only") is True and gate_body.get("enforcement_gate_is_planning_only") is True,
        "not_runtime_implementation": gate_body.get("runtime_enforcement_implemented") is False
        and gate.get("runtime_preflight_implementation_performed") is False,
        "does_not_touch_runtime_capital_live": gate.get("enforcement_gate_does_not_touch_runtime_capital_live") is True
        and gate.get("runtime_touch_performed") is False
        and gate.get("capital_touch_performed") is False
        and gate.get("live_touch_performed") is False,
        "does_not_touch_order_candidate_family_schema_config": gate.get("real_order_touch_performed") is False
        and gate.get("planned_schema_files_existing_count") == 0
        ...
        and gate.get("generic_runner_implementation_remains_blocked") is True,
        ...
        "requires_preflight_safety_readiness": gate.get("enforcement_gate_requires_preflight_safety_readiness") is True
        and all(path in preflight_paths for path in ["runtime path", "paper path", "live path", "capital path", "candidate path", "family path", "execution path"]),
        "requires_kill_switch_readiness": gate.get("enforcement_gate_requires_kill_switch_readiness") is True
        and all(path in kill_switch_paths for path in ["runtime path", "paper path", "live path", "capital path"]),
        "requires_evidence_chain_compliance": gate.get("enforcement_gate_requires_evidence_chain_compliance") is True
        and gate.get("evidence_chain_policy_level") == POLICY_LEVEL
        and gate.get("current_evidence_chain_quality") == CURRENT_EVIDENCE_CHAIN_QUALITY,
        "missing_evidence_fails_closed": gate.get("missing_evidence_fails_closed") is True,
        "derived_evidence_weaker_than_primary": gate.get("derived_evidence_weaker_than_primary_artifact_verification") is True,
        "dangerous_flags_all_false": isinstance(dangerous, dict) and all(value is False for value in dangerous.values()),
...
def build_payload() -> Dict[str, Any]:
    ...
    gate_validated = all(value is True for value in gate_checks.values())
    ...
    replacement_checks = {
        ...
        "enforcement_gate_artifact_exists": bool(gate),
        "enforcement_gate_validated": gate_validated,
        ...
        "future_runtime_paper_live_capital_requires_preflight": gate_checks.get("requires_preflight_safety_readiness") is True,
        "future_runtime_live_capital_requires_kill_switch": gate_checks.get("requires_kill_switch_readiness") is True,
        "evidence_chain_policy_active_and_mandatory": gate_checks.get("requires_evidence_chain_compliance") is True,
        "next_module_expected": NEXT_MODULE == "edge_factory_os_repo_only_preflight_safety_readiness_rollout_status_record_v1.py",
    }
    ...
    payload: Dict[str, Any] = {
        ...
        "preflight_safety_readiness_enforcement_gate_validator_status": "READY" if ready else "FAIL_CLOSED",
        "post_check_status": POST_CHECK_STATUS if ready else "REPO_ONLY_PREFLIGHT_SAFETY_READINESS_ENFORCEMENT_GATE_VALIDATOR_POST_COMMIT_CHECK_FAIL_CLOSED",
        "final_decision": "PREFLIGHT_SAFETY_READINESS_ENFORCEMENT_GATE_VALIDATED" if ready else "PREFLIGHT_SAFETY_READINESS_ENFORCEMENT_GATE_VALIDATOR_FAIL_CLOSED",
        "next_action": "BUILD_PREFLIGHT_SAFETY_READINESS_ROLLOUT_STATUS_RECORD_V1" if ready else "REVIEW_PREFLIGHT_SAFETY_READINESS_ENFORCEMENT_GATE_VALIDATOR_INPUTS",
        "next_module": NEXT_MODULE if ready else None,
        "preflight_safety_enforcement_gate_found": bool(gate),
        "preflight_safety_enforcement_gate_validated": ready,
        "preflight_safety_readiness_adoption_record_respected": adoption_record_ok(adoption),
        ...
        "enforcement_gate_does_not_touch_order_candidate_family_schema_config": True,
        ...
        "future_runtime_paper_live_capital_requires_preflight_safety_readiness": True,
        ...
        "derived_live_repo_post_check_reason": "Gate validator uses the adoption record, enforcement gate, kill-switch adoption artifact, and post-commit live repo replacement checks; evidence remains DERIVED_OVERUSED_ATTENTION and weaker than primary artifact verification.",
...
def write_outputs(payload: Dict[str, Any]) -> Dict[str, str]:
    ...
    latest_json = OUT_DIR / "repo_only_preflight_safety_readiness_enforcement_gate_validator_v1_latest.json"
    timestamped_json = OUT_DIR / f"repo_only_preflight_safety_readiness_enforcement_gate_validator_v1_{stamp}.json"
    latest_txt = OUT_DIR / "repo_only_preflight_safety_readiness_enforcement_gate_validator_v1_latest.txt"
...
def main() -> int:
    ...
    return 0 if payload["preflight_safety_readiness_enforcement_gate_validator_status"] in {"READY", "PASS"} else 3
