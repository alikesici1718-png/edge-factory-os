...
CURRENT_TOOL_REL = (
    "tools/edge_factory_os_repo_only_historical_data_acquisition_okx_single_symbol_small_range_1m_to_1h_build_preview_"
    "after_download_validator_v1.py"
)
OUT_DIR = LAB_ROOT / MODULE_NAME

EXPECTED_HEAD = "4926340"
EXPECTED_TRACKED_PYTHON_COUNT_PRE_COMMIT = 690
EXPECTED_TRACKED_PYTHON_COUNT_POST_COMMIT = 691
...
DOWNLOAD_VALIDATOR_BUNDLE_ARTIFACT = (
    DOWNLOAD_VALIDATOR_DIR / "historical_okx_single_symbol_small_range_download_execution_validator.json"
)
...
PROVENANCE_VALIDATION_REPORT_ARTIFACT = (
    DOWNLOAD_VALIDATOR_DIR / "historical_okx_single_symbol_small_range_provenance_validation_report.json"
)
...
SINGLE_FILE_BUILD_VALIDATOR_ARTIFACT = (
    LAB_ROOT
...
SINGLE_FILE_BUILD_VALIDATOR_STATUS_PASS = (
...
def validate_required_artifacts() -> Dict[str, Dict[str, Any]]:
    paths = {
        ...
        "download_validator_bundle": DOWNLOAD_VALIDATOR_BUNDLE_ARTIFACT,
        "hash_validation_report": HASH_VALIDATION_REPORT_ARTIFACT,
        "provenance_validation_report": PROVENANCE_VALIDATION_REPORT_ARTIFACT,
        "zip_schema_validation_report": ZIP_SCHEMA_VALIDATION_REPORT_ARTIFACT,
        "execution_report": EXECUTION_REPORT_ARTIFACT,
        "preview_approval": PREVIEW_APPROVAL_ARTIFACT,
        "single_file_build_validator": SINGLE_FILE_BUILD_VALIDATOR_ARTIFACT,
...
def validate_preflight(artifacts: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    bundle = artifacts["download_validator_bundle"]
    hash_report = artifacts["hash_validation_report"]
    provenance_report = artifacts["provenance_validation_report"]
    zip_schema = artifacts["zip_schema_validation_report"]
    execution_scope = artifacts["execution_report"].get("execution_scope", {})
    preview = artifacts["preview_approval"]
    single_file_build_validator = artifacts["single_file_build_validator"]
    smoke_summary = artifacts["smoke_summary"]
    policy_validator = artifacts["policy_validator"]
    policy = artifacts["policy"]

    require_equal(
        validator.get("historical_data_acquisition_okx_single_symbol_small_range_download_execution_validator_status"),
        DOWNLOAD_VALIDATOR_STATUS_PASS,
    ...
    require_equal(validator.get("current_evidence_chain_quality_after_validator"), EVIDENCE_BEFORE, "download_validator.evidence")
    require_true(validator.get("small_range_download_execution_validated"), "download_validator.validated")
    require_equal(validator.get("approved_url_count"), EXPECTED_FILE_COUNT, "download_validator.approved_url_count")
    require_equal(validator.get("downloaded_file_count"), EXPECTED_FILE_COUNT, "download_validator.downloaded_file_count")
    require_equal(validator.get("target_symbol"), TARGET_SYMBOL, "download_validator.target_symbol")
    require_equal(validator.get("date_range_start"), DATE_RANGE_START, "download_validator.start")
    require_equal(validator.get("date_range_end"), DATE_RANGE_END, "download_validator.end")
    require_true(validator.get("approved_url_list_exact_match"), "download_validator.url_exact")
    require_equal(validator.get("extra_url_count"), 0, "download_validator.extra_url_count")
    require_equal(validator.get("missing_url_count"), 0, "download_validator.missing_url_count")
    require_equal(validator.get("duplicate_url_count"), 0, "download_validator.duplicate_url_count")
    require_true(validator.get("all_downloaded_zip_paths_exist"), "download_validator.paths_exist")
    require_true(validator.get("all_hashes_recomputed"), "download_validator.hashes_recomputed")
    require_true(validator.get("all_hashes_match_recorded"), "download_validator.hashes_match")
    require_true(validator.get("all_zip_open_success"), "download_validator.zip_open")
    require_true(validator.get("all_expected_inner_csv_present"), "download_validator.expected_csv")
    require_false(validator.get("any_zip_path_traversal_detected"), "download_validator.traversal")
    require_true(validator.get("all_expected_schema_match"), "download_validator.schema")
    require_true(validator.get("all_observed_symbols_match_target"), "download_validator.symbols")
    require_true(validator.get("all_one_minute_interval_observed_from_samples"), "download_validator.one_minute")
    require_false(validator.get("direct_1h_interval_present"), "download_validator.direct_1h")
    require_true(validator.get("safe_for_small_range_build_preview"), "download_validator.safe_build_preview")
    require_false(validator.get("safe_for_broad_acquisition"), "download_validator.broad")
    require_false(validator.get("safe_for_research_backtest"), "download_validator.research")
    require_false(validator.get("safe_for_edge_claim"), "download_validator.edge")
    require_false(validator.get("files_marked_build_ready"), "download_validator.build_ready")
    require_false(validator.get("source_manifest_acquisition_ready"), "download_validator.source_ready")
    require_false(validator.get("broad_acquisition_execution_allowed_now"), "download_validator.broad_allowed")
    ...
    require_true(int(validator.get("active_p1_attention_count", 0)) >= 8, "download_validator.active_p1")
    ...
    require_true(bundle.get("risk_decision", {}).get("safe_for_small_range_build_preview"), "validator_bundle.safe_build_preview")
    require_true(hash_report.get("all_hashes_recomputed"), "hash_report.hashes_recomputed")
    require_true(hash_report.get("all_hashes_match_recorded"), "hash_report.hashes_match")
    require_equal(hash_report.get("validated_file_count"), EXPECTED_FILE_COUNT, "hash_report.file_count")
    require_true(provenance_report.get("seven_file_provenance_validated"), "provenance_report.validated")
    require_false(provenance_report.get("new_download_performed_by_validator"), "provenance_report.new_download")
    require_false(provenance_report.get("data_build_performed"), "provenance_report.data_build")
    require_false(provenance_report.get("aggregation_performed_now"), "provenance_report.aggregation")
    require_true(zip_schema.get("zip_inventory_validation", {}).get("all_zip_open_success"), "zip_schema.zip_open")
    require_true(zip_schema.get("zip_inventory_validation", {}).get("all_expected_inner_csv_present"), "zip_schema.expected_csv")
    ...
    require_true(zip_schema.get("schema_sample_validation", {}).get("all_observed_symbols_match_target"), "zip_schema.symbols")
    require_true(zip_schema.get("schema_sample_validation", {}).get("all_one_minute_interval_observed_from_samples"), "zip_schema.one_minute")
    require_false(zip_schema.get("schema_sample_validation", {}).get("direct_1h_interval_present"), "zip_schema.direct_1h")

    require_true(execution_scope.get("small_range_download_execution_performed"), "execution_scope.execution")
    require_equal(execution_scope.get("downloaded_file_count"), EXPECTED_FILE_COUNT, "execution_scope.file_count")
    require_equal(execution_scope.get("target_symbol"), TARGET_SYMBOL, "execution_scope.target_symbol")
    require_true(execution_scope.get("no_data_build"), "execution_scope.no_data_build")
    require_true(execution_scope.get("no_aggregation"), "execution_scope.no_aggregation")
    ...
    require_equal(preview.get("planned_url_count"), EXPECTED_FILE_COUNT, "preview.planned_url_count")
    require_false(preview.get("approval_grants_broad_acquisition_now"), "preview.broad")

    require_equal(
        single_file_build_validator.get("historical_data_acquisition_okx_single_symbol_1m_to_1h_build_execution_validator_status"),
        SINGLE_FILE_BUILD_VALIDATOR_STATUS_PASS,
        "single_file_build_validator.status",
    )
    require_true(single_file_build_validator.get("build_execution_validated"), "single_file_build_validator.validated")
    require_false(single_file_build_validator.get("data_build_performed_by_validator"), "single_file_build_validator.data_build")
    require_false(single_file_build_validator.get("aggregation_performed_by_validator"), "single_file_build_validator.aggregation")
    require_true(single_file_build_validator.get("replacement_checks_all_true"), "single_file_build_validator.replacement")
    ...
    require_false(smoke_summary.get("data_build_performed"), "smoke_summary.data_build")
    ...
    require_true(policy_validator.get("replacement_checks_all_true"), "policy_validator.replacement")
    ...
    return {
        ...
        "download_validator_bundle_artifact": str(DOWNLOAD_VALIDATOR_BUNDLE_ARTIFACT),
        "hash_validation_report_artifact": str(HASH_VALIDATION_REPORT_ARTIFACT),
        "provenance_validation_report_artifact": str(PROVENANCE_VALIDATION_REPORT_ARTIFACT),
        ...
        "single_file_build_validator_artifact": str(SINGLE_FILE_BUILD_VALIDATOR_ARTIFACT),
...
def build_preview() -> Dict[str, Any]:
    return {
        "small_range_build_preview_created": True,
        ...
        "expected_range_days": EXPECTED_RANGE_DAYS,
        ...
        "expected_complete_output_hours_per_file": EXPECTED_COMPLETE_OUTPUT_HOURS_PER_FILE,
...
def build_scope() -> Dict[str, Any]:
    return {
        "future_build_scope_file_count": EXPECTED_FILE_COUNT,
        "future_build_scope_symbol_count": EXPECTED_SYMBOL_COUNT,
        "future_build_scope_day_count": EXPECTED_RANGE_DAYS,
        "future_build_scope_limited_to_7_files_1_symbol_7_days": True,
        "target_symbol": TARGET_SYMBOL,
        "date_range_start": DATE_RANGE_START,
        "date_range_end": DATE_RANGE_END,
        "expected_source_rows_per_file": EXPECTED_SOURCE_ROWS_PER_FILE,
...
def build_approval_record() -> Dict[str, Any]:
    return {
        "small_range_build_execution_approval_record_created": True,
        "user_small_range_build_execution_approval_present": True,
        "approval_scope": "APPROVAL_RECORD_FOR_NEXT_SEVEN_FILE_SINGLE_SYMBOL_1M_TO_1H_BUILD_EXECUTION_ONLY",
        "approval_grants_build_now": False,
        "approval_grants_future_small_range_build_next": True,
...
def build_future_execution_rules(policy: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "future_module_may": [
            "read full CSVs from the 7 already-validated ZIPs",
            "aggregate to 1h using validated policy",
            "write single-symbol seven-day 1h output under output directory only",
            "write build gap schema and provenance reports",
            "mark output as pipeline-range-validation only",
        ],
        "future_module_must_not": [
            "download anything",
            "call API",
            "browse",
            "process more than 7 files",
        ...
        "validated_policy_reference": {
            ...
            "forward_fill_allowed": policy["completeness_policy"]["forward_fill_allowed"],
            "backfill_allowed": policy["completeness_policy"]["backfill_allowed"],
...
def build_fail_closed_rules() -> Dict[str, Any]:
    return {
        "future_build_execution_fail_closed_if": [
            "any ZIP SHA256 differs from validator record",
            "any expected CSV missing",
            "any schema differs",
            "any file has more than 2000 source rows",
            "total rows exceed 14000",
            "unique symbol count greater than 1",
            "observed symbol is not BTC-USDT-SWAP",
            "open_time is non-monotonic within file",
            "duplicate open_time exists",
            "missing minutes exist and are marked complete",
            "any hour has fewer than 60 rows and is marked complete",
            "synthetic fill attempted",
            "forward fill or backfill attempted",
            "output row count exceeds 168",
            "output written inside tracked repo code paths",
...
def build_self_validator() -> Dict[str, Any]:
    return {
        ...
        "all_preview_artifacts_exist": True,
        "all_preview_artifacts_valid_json": True,
        "future_build_scope_limited_to_7_files_1_symbol_7_days": True,
        ...
        "small_range_build_preview_self_validated": True,
        "replacement_checks_all_true": True,
    }


def build_payload(preflight: Dict[str, Any], self_validator: Dict[str, Any]) -> Dict[str, Any]:
    ...
    replacement_checks = {
        ...
        "scope_seven_files": EXPECTED_FILE_COUNT == 7,
        "scope_one_symbol": EXPECTED_SYMBOL_COUNT == 1,
        "scope_seven_days": EXPECTED_RANGE_DAYS == 7,
        ...
        "self_validator_passed": self_validator.get("small_range_build_preview_self_validated") is True,
    }
    payload: Dict[str, Any] = {
        "module_name": MODULE_NAME,
        "generated_at_utc": utc_now(),
        "historical_data_acquisition_okx_single_symbol_small_range_1m_to_1h_build_preview_bundle_status": STATUS_PASS,
        "final_decision": EVIDENCE_AFTER,
        "next_action": "RUN_NEXT_SEPARATE_SMALL_RANGE_1M_TO_1H_BUILD_EXECUTION_ONLY",
        ...
        **preflight,
        ...
        "derived_live_repo_post_check_reason": (
            "repo-only bounded bundle created a seven-file BTC-USDT-SWAP 1m-to-1h build preview and approval record "
            "for the next separate execution module only; it read JSON artifacts only, performed no new download/API/"
            "browse, no data fetch, no full CSV read, no data build, no aggregation, no 1h output creation, and made no "
            "broad acquisition, research, backtest, edge, runtime, capital, live, schema/config, or generic-runner claim"
        ),
...
def write_bundle_artifacts(payload: Dict[str, Any], bundle: Dict[str, Dict[str, Any]]) -> None:
    outputs = {
        "historical_okx_single_symbol_small_range_1m_to_1h_build_preview.json": bundle["preview"],
        "historical_okx_single_symbol_small_range_1m_to_1h_build_scope.json": bundle["scope"],
        "historical_okx_single_symbol_small_range_1m_to_1h_build_approval_record.json": bundle["approval"],
        "historical_okx_single_symbol_small_range_1m_to_1h_build_fail_closed_rules.json": bundle["fail_closed"],
        "historical_okx_single_symbol_small_range_1m_to_1h_build_self_validator.json": bundle["self_validator"],
        "historical_okx_single_symbol_small_range_1m_to_1h_build_preview_bundle_summary.json": payload,
        "repo_only_historical_data_acquisition_okx_single_symbol_small_range_1m_to_1h_build_preview_after_download_validator_v1_latest.json": payload,
    }
...
def validate_written_artifacts() -> Dict[str, Any]:
    required = [
        "historical_okx_single_symbol_small_range_1m_to_1h_build_preview.json",
        "historical_okx_single_symbol_small_range_1m_to_1h_build_scope.json",
        "historical_okx_single_symbol_small_range_1m_to_1h_build_approval_record.json",
        "historical_okx_single_symbol_small_range_1m_to_1h_build_fail_closed_rules.json",
        "historical_okx_single_symbol_small_range_1m_to_1h_build_self_validator.json",
        "historical_okx_single_symbol_small_range_1m_to_1h_build_preview_bundle_summary.json",
    ]
    loaded = {name: load_json(OUT_DIR / name, name) for name in required}
    summary = loaded["historical_okx_single_symbol_small_range_1m_to_1h_build_preview_bundle_summary.json"]
    scope = loaded["historical_okx_single_symbol_small_range_1m_to_1h_build_scope.json"]
    approval = loaded["historical_okx_single_symbol_small_range_1m_to_1h_build_approval_record.json"]
    self_validator = loaded["historical_okx_single_symbol_small_range_1m_to_1h_build_self_validator.json"]
    return {
        "all_preview_artifacts_exist": True,
        "all_preview_artifacts_valid_json": True,
        "scope_limited": scope.get("future_build_scope_limited_to_7_files_1_symbol_7_days") is True,
        "approval_future_build_next": approval.get("approval_grants_future_small_range_build_next") is True,
...
def blocked_payload(exc: Exception) -> Dict[str, Any]:
    ...
    return {
        "module_name": MODULE_NAME,
        "generated_at_utc": utc_now(),
        "historical_data_acquisition_okx_single_symbol_small_range_1m_to_1h_build_preview_bundle_status": STATUS_BLOCKED_CONTEXT,
        "final_decision": STATUS_BLOCKED_CONTEXT,
        "next_action": "STOP_FAIL_CLOSED_NO_SMALL_RANGE_1M_TO_1H_BUILD_PREVIEW_NO_BUILD",
        ...
        "bounded_bundle_mode_used": True,
        "user_acceleration_decision_respected": True,
        "small_range_build_preview_created": False,
        "small_range_build_execution_approval_record_created": False,
        "small_range_build_preview_self_validated": False,
        "build_scope": BUILD_SCOPE,
        "target_symbol": TARGET_SYMBOL,
        "date_range_start": DATE_RANGE_START,
        "date_range_end": DATE_RANGE_END,
        ...
        "expected_range_days": EXPECTED_RANGE_DAYS,
        "approval_grants_build_now": False,
        "approval_grants_future_small_range_build_next": False,
        "approval_grants_new_download_now": False,
        "new_download_performed_now": False,
        "full_csv_read_performed_now": False,
        "data_build_performed": False,
...
def write_blocked_artifact(payload: Dict[str, Any]) -> None:
    write_json(
        OUT_DIR / "repo_only_historical_data_acquisition_okx_single_symbol_small_range_1m_to_1h_build_preview_after_download_validator_v1_latest.json",
        payload,
    )
    write_json(OUT_DIR / "historical_okx_single_symbol_small_range_1m_to_1h_build_preview_bundle_summary.json", payload)
...
def main() -> int:
    try:
        ...
        payload = build_payload(preflight, self_validator)
