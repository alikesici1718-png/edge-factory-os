...
MODULE = "edge_factory_os_research_direction_contract_builder_v4"
NOW = datetime.now(timezone.utc)

REPO_DIR = Path(__file__).resolve().parents[1]
BASE_DIR = REPO_DIR.parent

NON_IMPULSE_EVAL_LATEST = (
    BASE_DIR
    / "edge_factory_os_non_impulse_mean_reversion_archetype_evaluator_v1"
    / "non_impulse_mean_reversion_archetype_evaluator_latest.json"
)
...
RUN_DIR = OUT_ROOT / f"research_direction_contract_builder_v4_{NOW.strftime('%Y%m%d_%H%M%S')}"
LATEST_JSON = OUT_ROOT / "research_direction_contract_builder_v4_latest.json"
LATEST_MD = OUT_ROOT / "research_direction_contract_builder_v4_latest.md"

CONTRACT_ROOT = BASE_DIR / "edge_factory_os_research_direction_contracts"
CONTRACT_LATEST = CONTRACT_ROOT / "market_neutral_relative_value_archetype_contract_latest.json"
...
def select_market_neutral_direction(queue_v3: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    directions = queue_v3.get("directions")
    if not isinstance(directions, list):
        return None

    for d in directions:
        if isinstance(d, dict) and d.get("research_key") == "RD3_02_MARKET_NEUTRAL_RELATIVE_VALUE_ARCHETYPE_SEARCH":
            return d
...
def main() -> int:
    ...
    non_impulse_eval = load_json(NON_IMPULSE_EVAL_LATEST)
    ...
    if not isinstance(non_impulse_eval, dict):
        critical.append("non_impulse_evaluator_latest_missing")
        non_impulse_eval = {}
    ...
    if non_impulse_eval.get("evaluator_status") != "NON_IMPULSE_MEAN_REVERSION_ARCHETYPE_EVALUATOR_BRANCH_CLOSED":
        critical.append(f"non_impulse_branch_not_closed:{non_impulse_eval.get('evaluator_status')}")

    if safe_get(non_impulse_eval, ["release_gate_feed", "RESEARCH_BRANCH_CLOSED"]) is not True:
        critical.append("non_impulse_release_feed_branch_not_closed")

    if safe_get(non_impulse_eval, ["release_gate_feed", "STRICT_12_OF_12_PREVIEW_FOUND"]) is not False:
        critical.append("non_impulse_eval_did_not_confirm_zero_strict_preview")

    if queue_v3.get("queue_status") != "NEW_RESEARCH_DIRECTIONS_V3_QUEUED_BRANCH_ROTATION":
        critical.append(f"unexpected_queue_v3_status:{queue_v3.get('queue_status')}")
    ...
    direction = select_market_neutral_direction(queue_v3)
    if not isinstance(direction, dict):
        critical.append("market_neutral_direction_not_found_in_queue_v3")
        direction = {}
    ...
    if critical:
        builder_status = "RESEARCH_DIRECTION_CONTRACT_BUILDER_V4_CRITICAL_BLOCKED"
        severity = "CRITICAL"
        allowed_scope = "READ_ONLY_REVIEW"
        next_action = "FIX_INPUTS_BEFORE_MARKET_NEUTRAL_CONTRACT"
        reason = "; ".join(critical)

    else:
        contract_core = {
            "research_key": "MARKET_NEUTRAL_RELATIVE_VALUE_ARCHETYPE_SEARCH_V1",
        ...
        contract_id = f"MARKET_NEUTRAL_RELATIVE_VALUE_ARCHETYPE_CONTRACT_V1_{contract_hash}_{NOW.strftime('%Y%m%d_%H%M%S')}"

        contract = {
            "contract_schema": "edge_factory_os_research_direction_contract_v4",
            ...
            "purpose": (
                "Start market-neutral relative-value / spread-style archetype search after non-impulse mean reversion also "
                "failed canonical strict 12/12. This branch attempts to reduce market regime dependency by ranking symbols "
                "cross-sectionally and testing paired/relative long-short templates. It is read-only/offline research only."
            ),

            "why_new_branch": {
                "prior_branch_closed": True,
                "closed_branch_source": str(NON_IMPULSE_EVAL_LATEST),
                "do_not_repeat_prior_failures": [
                    "impulse continuation route",
                    "ret3 threshold hold route",
                    ...
                    "non-impulse mean reversion archetype v1",
            ...
            "archetype_search_scope": {
                "archetype_family": "market_neutral_relative_value",
                ...
                "allowed_signal_families": [
                    "cross_sectional_rank_spread",
                    "winner_loser_pair_reversion",
                    "relative_strength_neutral_spread",
                    "liquidity_adjusted_rank_spread",
                    "volatility_adjusted_rank_spread",
                    "market_beta_neutral_rank_spread",
                    "sectorless_coin_cluster_spread",
                    "breadth_neutral_extreme_spread",
                ],
                "explicitly_disallowed_signal_families": [
                    "single-leg directional impulse continuation",
                    "single-leg ret3 threshold hold repair",
                    ...
                    "any template that does not evaluate canonical 12 policy months",
                ],
            },

            "required_runner_behavior": {
                "future_module": "edge_factory_os_market_neutral_relative_value_archetype_runner_v1.py",
            ...
            "expected_next_module": {
                "module_name": "edge_factory_os_market_neutral_relative_value_archetype_runner_v1.py",
                "purpose": (
                    "Search market-neutral relative-value templates on full 1Y universe under canonical 12/12 policy. "
        ...
        builder_status = "RESEARCH_DIRECTION_CONTRACT_V4_READY"
        severity = "ATTENTION"
        allowed_scope = "READ_ONLY_RESEARCH"
        next_action = "BUILD_MARKET_NEUTRAL_RELATIVE_VALUE_ARCHETYPE_RUNNER"
        reason = (
    ...
    result = {
        ...
        "release_gate_feed": {
            "RESEARCH_DIRECTION_CONTRACT_V4_BUILT": contract_written,
            "RESEARCH_KEY": "MARKET_NEUTRAL_RELATIVE_VALUE_ARCHETYPE_SEARCH_V1" if contract_written else None,
        ...
        "decision": {
            ...
            "next_module": "edge_factory_os_market_neutral_relative_value_archetype_runner_v1.py" if contract_written else None,
        },
    ...
    out_json = RUN_DIR / "research_direction_contract_builder_v4_state.json"
    out_md = RUN_DIR / "research_direction_contract_builder_v4_report.md"

    dump_json(out_json, result)
    dump_json(LATEST_JSON, result)

    md = f"""# EDGE FACTORY OS RESEARCH DIRECTION CONTRACT BUILDER v4
...
"""
    ...
    print("=" * 100)
    print("EDGE FACTORY OS RESEARCH DIRECTION CONTRACT BUILDER v4")
    print("=" * 100)
