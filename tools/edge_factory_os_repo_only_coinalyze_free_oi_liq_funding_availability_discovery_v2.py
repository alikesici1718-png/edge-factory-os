#!/usr/bin/env python3
"""Coinalyze free OI/liquidation/funding availability discovery V2.

Data availability discovery only. No strategy execution, no signals, no PnL,
no candidate generation, no edge claim, and no runtime/live/capital permission.
The Coinalyze API key is read only from COINALYZE_API_KEY and is never written.
"""
...
MODULE = "edge_factory_os_repo_only_coinalyze_free_oi_liq_funding_availability_discovery_v2"
ARTIFACT_KIND = "COINALYZE_FREE_OI_LIQ_FUNDING_AVAILABILITY_DISCOVERY_V2"
PASS_STATUS = "PASS_REPO_ONLY_COINALYZE_FREE_OI_LIQ_FUNDING_AVAILABILITY_DISCOVERY_V2_CREATED"
BLOCKED_MISSING_KEY_STATUS = "BLOCKED_COINALYZE_API_KEY_MISSING_AGAIN"
BLOCKED_API_STATUS = "BLOCKED_COINALYZE_API_OR_RATE_LIMIT_V2"
REPO_ROOT = Path(__file__).resolve().parents[1]
TOOL_PATH = REPO_ROOT / "tools" / "edge_factory_os_repo_only_coinalyze_free_oi_liq_funding_availability_discovery_v2.py"
ARTIFACT_PATH = REPO_ROOT / "artifacts" / "data_discovery" / "coinalyze_free_oi_liq_funding_availability_discovery_v2.json"
PRIOR_BLOCKED_ARTIFACT = REPO_ROOT / "artifacts" / "data_discovery" / "coinalyze_free_oi_liq_funding_availability_discovery_v1.json"
PRIOR_BLOCKED_COMMIT = "0c00fd0459ff1aaaa0939e7c2f0b24116f46c704"
...
def prior_blocked_artifact_modified() -> bool:
    for _status, path in status_entries():
        if path == str(PRIOR_BLOCKED_ARTIFACT.relative_to(REPO_ROOT)).replace("\\", "/"):
            return True
    return False
...
def stable_perp_or_swap(market: dict[str, Any]) -> bool:
    ...
    perp = any(token in symbol_text or token in type_text for token in ("PERP", "PERPETUAL", "SWAP"))
    return stable and (perp or market.get("is_perpetual") is True)
...
def coinalyze_get(endpoint: str, params: dict[str, Any], api_key: str) -> tuple[bool, int | None, Any, str | None]:
    ...
    request = urllib.request.Request(
        ...
        headers={"User-Agent": "edge-factory-os-coinalyze-v2-discovery/1.0"},
        method="GET",
    )
    try:
        ...
        try:
            ...
            body = exc.reason if isinstance(exc.reason, str) else "HTTPError"
        return False, exc.code, None, body
...
def base_payload(status: str, replacement_checks_all_true: bool) -> dict[str, Any]:
    ...
    prior_not_modified = not prior_blocked_artifact_modified()
    validation_checks = {
        "repo_clean_before_run": clean,
        "api_key_loaded_from_env_only": False,
        "api_key_not_written_to_artifact": True,
        "prior_blocked_artifact_not_modified": prior_not_modified,
    ...
    return {
        ...
        "prior_blocked_attempt": {
            ...
            "api_network_call_made": False,
            "artifact": str(PRIOR_BLOCKED_ARTIFACT.relative_to(REPO_ROOT)).replace("\\", "/"),
        },
        "api_key_handling": {
        ...
        "coinalyze_constraints": {
            ...
            "conservative_rate_limit_sleep_seconds": 1.25,
        ...
        "result_classification": "COINALYZE_FREE_DATA_BLOCKED_API_OR_RATE_LIMIT",
        "next_allowed_step": "COINALYZE_API_KEY_ENV_SETUP_REVIEW_V2",
        "next_module": "COINALYZE_API_KEY_ENV_SETUP_REVIEW_V2",
        "limitations": [],
        "safety_permissions": {
...
def classify_results(results: list[dict[str, Any]]) -> tuple[str, str, dict[str, Any]]:
    ...
    for result in results:
        ...
        if result.get("request_ok") and int(result.get("row_count", 0)) > 0:
            ...
            target = str(result.get("target_symbol"))
            interval = str(result.get("interval"))
            usable_by_endpoint.setdefault(endpoint, set()).add(target)
            interval_counts[interval] = interval_counts.get(interval, 0) + 1

    oi = usable_by_endpoint.get("open-interest-history", set())
    liq = usable_by_endpoint.get("liquidation-history", set())
    funding = usable_by_endpoint.get("funding-rate-history", set())
    best_interval = max(interval_counts.items(), key=lambda item: item[1])[0] if interval_counts else None
    summary = {
        "usable_oi_symbol_count": len(oi),
        "usable_liquidation_symbol_count": len(liq),
        "usable_funding_symbol_count": len(funding),
        "usable_oi_symbols": sorted(oi),
        "usable_liquidation_symbols": sorted(liq),
        "usable_funding_symbols": sorted(funding),
        "usable_interval_probe_counts": interval_counts,
        "best_usable_interval": best_interval,
    }

    major_overlap = {"BTCUSDT", "ETHUSDT", "SOLUSDT"} & oi & liq & funding
    if blocked_count and not (oi or liq or funding):
        return "COINALYZE_FREE_DATA_BLOCKED_API_OR_RATE_LIMIT", "COINALYZE_API_KEY_OR_RATE_LIMIT_REVIEW_V2", summary
    if len(major_overlap) >= 2 and (interval_counts.get("15min", 0) > 0 or interval_counts.get("1hour", 0) > 0):
...
def missing_key_payload() -> dict[str, Any]:
    ...
    payload["limitations"] = [
        "COINALYZE_API_KEY is still missing from the environment visible to this process.",
        "No Coinalyze API request or network call was made.",
        "The prior V1 blocked artifact was not modified or overwritten.",
    ]
...
def discover(api_key: str) -> dict[str, Any]:
    ...
    for market in markets:
        ...
        if exchange and stable_perp_or_swap(market):
            item = dict(market)
    ...
    payload["market_discovery_summary"] = {
        ...
        "selected_stable_perp_or_swap_count": len(selected),
    ...
    if classification == "COINALYZE_FREE_DATA_BLOCKED_API_OR_RATE_LIMIT":
        ...
        payload["limitations"].append("Coverage is for recent-data prototyping only; no strategy, signal, or PnL was computed.")
    ...
    payload["validation_checks"]["prior_blocked_artifact_not_modified"] = prior_not_modified
    if not clean or not prior_not_modified:
        payload["status"] = "BLOCKED_REPO_STATE_OR_PRIOR_ARTIFACT_MODIFIED"
        payload["result_classification"] = "COINALYZE_FREE_DATA_BLOCKED_API_OR_RATE_LIMIT"
        payload["next_allowed_step"] = "REPO_STATE_OR_PRIOR_ARTIFACT_REVIEW_V1"
        payload["next_module"] = "REPO_STATE_OR_PRIOR_ARTIFACT_REVIEW_V1"
        payload["replacement_checks_all_true"] = False
        payload["validation_checks"]["replacement_checks_all_true"] = False
        payload["limitations"].append("Repo state validation failed outside expected V2 files or prior blocked artifact was modified.")
    return payload
