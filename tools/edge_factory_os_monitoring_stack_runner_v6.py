...
OUT_ROOT = WORKSPACE / "edge_factory_os_monitoring_stack_runner_v6"
...
TOOLS = [
    ...
    {
        "step_key": "family_exposure_trend",
        "cmd": [sys.executable, "-u", r".\tools\edge_factory_os_family_exposure_trend_watcher_v1.py"],
        "required_marker_any": ["EDGE FACTORY OS FAMILY EXPOSURE TREND WATCHER v1", "trend_status:"],
        "pass_marker": "trend_status:",
        "dirty_expected_fail": False,
    },
]
...
LATEST_PATHS = {
    ...
    "family_exposure_trend": WORKSPACE / "edge_factory_os_family_exposure_trend_watcher_v1" / "family_exposure_trend_watcher_latest.json",
}
...
def summarize_latest() -> dict:
    ...
    trend = read_json(LATEST_PATHS["family_exposure_trend"])
    ...
    return {
        ...
        "family_exposure_trend_status": trend.get("trend_status"),
        "family_exposure_trend_attention_count": as_int(trend.get("attention_family_count")),
        "family_exposure_trend_info_count": as_int(trend.get("info_family_count")),
        "family_exposure_trend_next_action": trend.get("next_action"),
    }
...
def build_alerts(summary: dict, step_rows: list[dict], git_dirty: bool) -> list[dict]:
    ...
    if summary.get("family_exposure_trend_status") == "FAMILY_EXPOSURE_TREND_ATTENTION":
        add(
            6750,
            "ATTENTION",
            "FAMILY_EXPOSURE_TREND_ATTENTION",
            f"trend_attention_family_count={summary.get('family_exposure_trend_attention_count')}",
            summary.get("family_exposure_trend_next_action") or "CONTINUE_MONITORING_FAMILY_SAMPLE_BALANCE",
        )

    if summary.get("family_exposure_trend_status") == "FAMILY_EXPOSURE_TREND_INFO":
        add(
            4550,
            "INFO",
            "FAMILY_EXPOSURE_TREND_INFO",
            f"trend_info_family_count={summary.get('family_exposure_trend_info_count')}",
            summary.get("family_exposure_trend_next_action") or "KEEP_RUNNING_COLLECT_SAMPLE_DO_NOT_TOUCH",
        )
...
def main() -> int:
    out_dir = OUT_ROOT / f"monitoring_stack_runner_v6_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    state_path = out_dir / "monitoring_stack_runner_v6_state.json"
    latest_path = OUT_ROOT / "monitoring_stack_latest.json"
    steps_csv = out_dir / "monitoring_stack_runner_v6_steps.csv"
    alerts_csv = out_dir / "monitoring_stack_runner_v6_alerts.csv"
    report_path = out_dir / "monitoring_stack_runner_v6_report.md"
    ...
    print("EDGE FACTORY OS MONITORING STACK RUNNER v6")
    print("=" * 100)
