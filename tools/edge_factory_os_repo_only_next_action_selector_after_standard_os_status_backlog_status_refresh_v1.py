...
MODULE_NAME = "edge_factory_os_repo_only_next_action_selector_after_standard_os_status_backlog_status_refresh_v1"
OUT_DIR = LAB_ROOT / "edge_factory_os_repo_only_next_action_selector_after_standard_os_status_backlog_status_refresh_v1"
OUT_DIR.mkdir(parents=True, exist_ok=True)

EXPECTED_HEAD_PREFIX = "6288883"
CURRENT_TOOL_REL = "tools/edge_factory_os_repo_only_next_action_selector_after_standard_os_status_backlog_status_refresh_v1.py"

STATUS_JSON = LAB_ROOT / "edge_factory_os_repo_only_standard_os_status_refresh_after_standard_os_status_backlog_refresh_v1" / "repo_only_standard_os_status_refresh_after_standard_os_status_backlog_refresh_v1_latest.json"
STATUS_POST_JSON = LAB_ROOT / "edge_factory_os_repo_only_standard_os_status_refresh_after_standard_os_status_backlog_refresh_post_commit_check" / "repo_only_standard_os_status_refresh_after_standard_os_status_backlog_refresh_post_commit_check_latest.json"
...
SELECTED_NEXT_MODULE = "edge_factory_os_repo_only_development_queue_selector_after_standard_os_status_backlog_status_refresh_v1.py"
...
SAFETY_FLAGS: Dict[str, bool] = {
    "repo_only_next_action_selector_after_standard_os_status_backlog_status_refresh": True,
    "read_only_next_action_selection_only": True,
    "standard_os_status_refresh_after_standard_os_status_backlog_refresh_required": True,
    "development_queue_selector_after_standard_os_status_backlog_status_refresh_allowed_next": True,
...
def first_status(obj: Dict[str, Any]) -> Optional[str]:
    for key in [
        "standard_os_status_refresh_after_standard_os_status_backlog_refresh_status",
        "audit_status",
...
def validate_inputs() -> Dict[str, Any]:
    ...
    if subject != "Add repo-only standard OS status refresh after standard OS status backlog refresh":
        errors.append(f"unexpected HEAD commit subject: {subject}")

    paths = commit_paths("HEAD")
    checks["head_commit_paths"] = paths
    if paths != ["tools/edge_factory_os_repo_only_standard_os_status_refresh_after_standard_os_status_backlog_refresh_v1.py"]:
    ...
    inputs = {
        ...
        "development_queue_selector_after_backlog_refresh": (
            QUEUE_JSON,
            "REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_V1_READY",
            None,
        ),
        "development_queue_selector_after_backlog_refresh_post_check": (
            QUEUE_POST_JSON,
...
def build_selector(validation: Dict[str, Any]) -> Dict[str, Any]:
    ...
    refresh = status_obj.get("standard_os_status_refresh_after_standard_os_status_backlog_refresh", {})
    standard_status = refresh.get("standard_os_status", {})
    selected_next_step = refresh.get("selected_next_step", {})
    context = refresh.get("context_summary", {})
    effective_schema_state = context.get("effective_schema_state", {})
    effective_approval_state = context.get("effective_approval_state", {})

    queue_obj = validation["loaded"]["development_queue_selector_after_backlog_refresh"]
    ...
    prerequisites = {
        "standard_status_ready": status_obj.get("standard_os_status_refresh_after_standard_os_status_backlog_refresh_status") == "REPO_ONLY_STANDARD_OS_STATUS_REFRESH_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_V1_READY",
        "standard_status_selected_this_selector": status_obj.get("next_module") == "edge_factory_os_repo_only_next_action_selector_after_standard_os_status_backlog_status_refresh_v1.py",
        "standard_status_selected_next_step_this_selector": selected_next_step.get("module") == "edge_factory_os_repo_only_next_action_selector_after_standard_os_status_backlog_status_refresh_v1.py",
        "general_repo_only_flow_active": standard_status.get("general_repo_only_flow_active") is True,
        "backlog_status_chain_active": standard_status.get("standard_os_status_backlog_refresh_chain_active") is True,
        ...
        "queue_selected_standard_status_refresh": selected_queue_task.get("module") == "edge_factory_os_repo_only_standard_os_status_refresh_after_standard_os_status_backlog_refresh_v1.py",
        "manual_approval_absent": standard_status.get("manual_approval_present") is False and effective_approval_state.get("manual_approval_present") is False,
        "manual_approval_invalid": standard_status.get("manual_approval_valid") is False and effective_approval_state.get("manual_approval_valid") is False,
        "approval_granted_false": standard_status.get("approval_granted_now") is False and effective_approval_state.get("approval_granted_now") is False,
        "schema_apply_allowed_false": standard_status.get("schema_registry_apply_allowed_now") is False and effective_schema_state.get("schema_apply_allowed_now") is False,
        "schema_file_creation_allowed_false": standard_status.get("schema_file_creation_allowed_now") is False and effective_schema_state.get("schema_file_creation_allowed_now") is False,
        "schema_file_edit_allowed_false": standard_status.get("schema_file_edit_allowed_now") is False and effective_schema_state.get("schema_file_edit_allowed_now") is False,
    ...
    candidate_actions = [
        {
            "rank": 1,
            "key": "DEVELOPMENT_QUEUE_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_STATUS_REFRESH",
            "module": SELECTED_NEXT_MODULE,
            "scope": "REPO_ONLY_OS_INTELLIGENCE",
            "allowed": True,
            "reason": "Standard OS backlog status refresh is clean; select next repo-only development queue task.",
            "blocked_actions": [
    ...
    return {
        "selector_status": "NEXT_ACTION_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_STATUS_REFRESH_ACTIVE" if selected else "NEXT_ACTION_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_STATUS_REFRESH_BLOCKED",
        "candidate_actions": candidate_actions,
        "candidate_action_count": len(candidate_actions),
        "selected_action": selected,
        "selected_next_module": selected["module"] if selected else None,
        "selected_next_action": "BUILD_REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_STATUS_REFRESH_V1" if selected else None,
        ...
        "invariants": {
            ...
            "selected_module_is_development_queue_selector_after_standard_os_status_backlog_status_refresh": selected is not None and selected["module"] == SELECTED_NEXT_MODULE,
...
def main() -> int:
    ...
    payload = {
        "module": MODULE_NAME,
        "next_action_selector_after_standard_os_status_backlog_status_refresh_status": "REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_STATUS_REFRESH_V1_READY" if passed else "REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_STATUS_REFRESH_V1_BLOCKED",
        "severity": "OK" if passed else "BLOCKED",
        "allowed_scope": "REPO_ONLY_OS_INTELLIGENCE",
        "final_decision": "NEXT_ACTION_SELECTED_DEVELOPMENT_QUEUE_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_STATUS_REFRESH" if passed else "KEEP_FREEZE_REVIEW_NEXT_ACTION_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_STATUS_REFRESH_ERRORS",
        "next_action": "BUILD_REPO_ONLY_DEVELOPMENT_QUEUE_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_STATUS_REFRESH_V1" if passed else "REVIEW_NEXT_ACTION_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_STATUS_REFRESH_ERRORS",
        "next_module": SELECTED_NEXT_MODULE if passed else None,
        "reason": "Selected repo-only development queue selector after standard OS status backlog status refresh; schema apply/create/edit remain blocked and physically absent." if passed else "Next-action selector after standard OS status backlog status refresh validation failed.",
        ...
        "next_action_selector_after_standard_os_status_backlog_status_refresh": selector,
    ...
    latest_json = OUT_DIR / "repo_only_next_action_selector_after_standard_os_status_backlog_status_refresh_v1_latest.json"
    timestamped_json = OUT_DIR / f"repo_only_next_action_selector_after_standard_os_status_backlog_status_refresh_v1_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json"
    latest_txt = OUT_DIR / "repo_only_next_action_selector_after_standard_os_status_backlog_status_refresh_v1_latest.txt"

    latest_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    timestamped_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")

    lines = [
        "EDGE FACTORY OS REPO-ONLY NEXT ACTION SELECTOR AFTER STANDARD OS STATUS BACKLOG STATUS REFRESH v1",
        "=" * 100,
        f"next_action_selector_after_standard_os_status_backlog_status_refresh_status: {payload['next_action_selector_after_standard_os_status_backlog_status_refresh_status']}",
