...
PREVIEW_JSON = (
    REPO_ROOT
    / "edge_factory_os_bom_syntax_repair_remaining_preview"
    / "bom_syntax_repair_remaining_preview_latest.json"
)

OUT_DIR = REPO_ROOT / "edge_factory_os_bom_syntax_repair_remaining_apply"
OUT_JSON = OUT_DIR / "bom_syntax_repair_remaining_apply_latest.json"
OUT_TXT = OUT_DIR / "bom_syntax_repair_remaining_apply_latest.txt"

REQUIRED_PREVIEW_STATUS = "BOM_SYNTAX_REPAIR_REMAINING_PREVIEW_READY_NO_APPLY"
REQUIRED_PREVIEW_FINAL_DECISION = "BUILD_REMAINING_BOM_SYNTAX_REPAIR_APPLY_NEXT_IF_PREVIEW_SAFE"
...
def validate_preview(preview: Optional[Dict[str, Any]]) -> Tuple[bool, List[str], List[str], Dict[str, Any]]:
    ...
    for item in files_safe:
        ...
        norm = item.replace("\\", "/")
        if not (norm.startswith("src/") or norm.startswith("tools/")):
            errors.append(f"UNSAFE_PATH_SCOPE:{norm}")
            continue
        if not norm.endswith(".py"):
            errors.append(f"UNSAFE_PATH_EXTENSION:{norm}")
            continue
...
def write_outputs(result: Dict[str, Any]) -> None:
    ...
    lines: List[str] = []
    lines.append("EDGE FACTORY OS REMAINING BOM SYNTAX REPAIR APPLY v1")
    lines.append("=" * 100)
    ...
    lines.append(f"planned_file_count: {result['planned_file_count']}")
    lines.append("")
...
def main() -> None:
    ...
    apply_status = "BOM_SYNTAX_REPAIR_REMAINING_APPLY_BLOCKED"
    allowed_scope = "REPO_ONLY_OS_INTELLIGENCE"
    final_decision = "STOP_REVIEW_REMAINING_BOM_PREVIEW_OR_FILE_DRIFT"
    next_action = "STOP_NO_FILE_MUTATION_REVIEW_PREVIEW"
    next_module = "edge_factory_os_bom_syntax_repair_remaining_preview_v1.py"
    ...
    if not preview_ok:
        ...
        if not block_reasons:
            try:
                ...
                apply_status = "BOM_SYNTAX_REPAIR_REMAINING_APPLIED"
    ...
    result: Dict[str, Any] = {
        "module": "edge_factory_os_bom_syntax_repair_remaining_apply_v1.py",
        ...
        "changed_file_count": len(changed_records) if apply_status == "BOM_SYNTAX_REPAIR_REMAINING_APPLIED" else 0,
        ...
        "changed_files": [rec.to_dict() for rec in changed_records] if apply_status == "BOM_SYNTAX_REPAIR_REMAINING_APPLIED" else [],
    ...
    print(f"planned_file_count={result['planned_file_count']}")
