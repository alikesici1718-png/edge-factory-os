...
MODULE_NAME = "edge_factory_os_repo_only_development_backlog_prioritization_after_evidence_chain_rollout_v1"
CURRENT_TOOL_REL = "tools/edge_factory_os_repo_only_development_backlog_prioritization_after_evidence_chain_rollout_v1.py"
OUT_DIR = LAB_ROOT / MODULE_NAME

EXPECTED_HEAD = "2607c6c"
EXPECTED_PREVIOUS_TRACKED_PYTHON_COUNT = 584
NEXT_MODULE = "edge_factory_os_repo_only_final_form_gap_analysis_contract_v1.py"
POST_CHECK_STATUS = "REPO_ONLY_DEVELOPMENT_BACKLOG_PRIORITIZATION_AFTER_EVIDENCE_CHAIN_ROLLOUT_POST_COMMIT_CHECK_PASS"
...
QUEUE_SELECTOR_ARTIFACT = LAB_ROOT / "edge_factory_os_repo_only_development_queue_selector_after_evidence_chain_rollout_status_review_v1" / "repo_only_development_queue_selector_after_evidence_chain_rollout_status_review_v1_latest.json"
...
def git_state() -> Dict[str, Any]:
    ...
    return {
        ...
        "repo_clean_for_backlog_prioritization": len(dirty_tracked) == 0 and len(unexpected_untracked) == 0,
    }
...
def artifact_policy_ok(artifact: Dict[str, Any]) -> bool:
    return (
...
def selected_item() -> Dict[str, Any]:
    return {
        "module": NEXT_MODULE,
        "purpose": "Define a repo-only final-form gap analysis contract for current Edge Factory OS against intended self-governing final form.",
        "material_work": "turn the post-rollout OS status into a concrete contract for final-form self-inspection and gap analysis",
        "repo_only": True,
        ...
        "does_not_touch_schema_config_runtime_launcher_capital_holdout_candidate_family_live_order_strategy": True,
...
def build_payload() -> Dict[str, Any]:
    ...
    queue_selector = load_json(QUEUE_SELECTOR_ARTIFACT)
    item = selected_item()
    ...
    prior_selector_ok = (
        queue_selector.get("development_queue_selector_status") in {"READY", "PASS"}
        and queue_selector.get("next_module") == "edge_factory_os_repo_only_development_backlog_prioritization_after_evidence_chain_rollout_v1.py"
        and queue_selector.get("selected_next_item_is_material") is True
        and queue_selector.get("selected_next_item_is_repo_only") is True
        and queue_selector.get("selected_next_item_does_not_reopen_generic_governance_loop") is True
        and queue_selector.get("selected_next_item_does_not_approve_generic_runner") is True
        and artifact_policy_ok(queue_selector)
    )

    replacement_checks = {
        "expected_head_observed": git["head"] == EXPECTED_HEAD,
        "repo_clean_for_backlog_prioritization": git["repo_clean_for_backlog_prioritization"],
        ...
        "status_refresh_policy_ok": bool(status_refresh) and artifact_policy_ok(status_refresh),
        "status_review_policy_ok": bool(status_review) and artifact_policy_ok(status_review),
        "queue_selector_policy_ok": prior_selector_ok,
        ...
        "next_module_expected": NEXT_MODULE == "edge_factory_os_repo_only_final_form_gap_analysis_contract_v1.py",
    }
    ...
    payload: Dict[str, Any] = {
        ...
        "development_backlog_prioritization_status": "PASS" if ready else "FAIL_CLOSED",
        "post_check_status": POST_CHECK_STATUS if ready else "REPO_ONLY_DEVELOPMENT_BACKLOG_PRIORITIZATION_AFTER_EVIDENCE_CHAIN_ROLLOUT_POST_COMMIT_CHECK_FAIL_CLOSED",
        "final_decision": "DEVELOPMENT_BACKLOG_PRIORITIZED_FINAL_FORM_GAP_ANALYSIS_CONTRACT" if ready else "DEVELOPMENT_BACKLOG_PRIORITIZATION_FAIL_CLOSED",
        "next_action": "BUILD_FINAL_FORM_GAP_ANALYSIS_CONTRACT_V1" if ready else "REVIEW_DEVELOPMENT_BACKLOG_PRIORITIZATION_INPUTS",
        ...
        "selected_next_item_does_not_reopen_generic_governance_loop": selected_next_item_does_not_reopen_generic_governance_loop,
        "selected_next_item_does_not_approve_generic_runner": selected_next_item_does_not_approve_generic_runner,
        "old_mechanical_selector_backlog_loop_reentry_prevented": True,
        "repeated_name_growth_is_not_progress": True,
        ...
        "derived_live_repo_post_check_reason": "Backlog prioritization uses standard OS status refresh/review, development queue selector artifacts, and live repo replacement checks; evidence remains DERIVED_OVERUSED_ATTENTION and weaker than primary artifact verification.",
        ...
        "validation": {
            ...
            "queue_selector_loaded": bool(queue_selector),
        },
        "safety_flags": {
            "development_backlog_prioritization_after_evidence_chain_rollout": True,
            "repo_only": True,
            "auto_fix_performed": False,
            "direct_apply_performed": False,
            "repair_preview_required": False,
...
def write_outputs(payload: Dict[str, Any]) -> Dict[str, str]:
    ...
    latest_json = OUT_DIR / "repo_only_development_backlog_prioritization_after_evidence_chain_rollout_v1_latest.json"
    timestamped_json = OUT_DIR / f"repo_only_development_backlog_prioritization_after_evidence_chain_rollout_v1_{stamp}.json"
    latest_txt = OUT_DIR / "repo_only_development_backlog_prioritization_after_evidence_chain_rollout_v1_latest.txt"
...
def main() -> int:
    ...
    return 0 if payload["development_backlog_prioritization_status"] in {"READY", "PASS"} else 3
