...
MODULE_NAME = "edge_factory_os_repo_only_source_panel_pyarrow_schema_access_repair_approval_after_result_review_v1"
CURRENT_TOOL_REL = "tools/edge_factory_os_repo_only_source_panel_pyarrow_schema_access_repair_approval_after_result_review_v1.py"
OUT_DIR = LAB_ROOT / MODULE_NAME

EXPECTED_START_HEAD = "0e4c7e6"
EXPECTED_PREVIOUS_TRACKED_PYTHON_COUNT = 630
EXPECTED_TRACKED_PYTHON_COUNT = 631
...
PRIOR_REPAIR_PREVIEW_DIR = (
    LAB_ROOT / "edge_factory_os_repo_only_source_panel_parquet_schema_access_repair_preview_after_result_review_v1"
)
PRIOR_REPAIR_PREVIEW_LATEST_ARTIFACT = (
    PRIOR_REPAIR_PREVIEW_DIR
    / "repo_only_source_panel_parquet_schema_access_repair_preview_after_result_review_v1_latest.json"
)

NEXT_MODULE_APPLY_PREVIEW = (
    "edge_factory_os_repo_only_source_panel_pyarrow_schema_access_repair_apply_preview_after_result_review_v1.py"
)
NEXT_MODULE_BLOCKED = (
...
POST_CHECK_STATUS_BLOCKED = (
    "REPO_ONLY_SOURCE_PANEL_PYARROW_SCHEMA_ACCESS_REPAIR_APPROVAL_AFTER_RESULT_REVIEW_POST_COMMIT_CHECK_BLOCKED"
)
APPROVAL_STATUS_PASS = "PASS_APPROVAL_RECORD_CREATED_APPLY_PREVIEW_NEXT"
APPROVAL_STATUS_FAIL = "FAIL_CLOSED_APPROVAL_RECORD_INVALID"
...
def build_payload() -> Dict[str, Any]:
    ...
    prior, prior_valid_json, prior_json_error = load_json_checked(PRIOR_REPAIR_PREVIEW_LATEST_ARTIFACT)
    ...
    prior_repair_preview_respected = (
        prior_valid_json
        and prior.get("source_panel_parquet_schema_access_repair_preview_status") == "PASS_APPROVAL_REQUIRED"
        ...
        and prior.get("repair_preview_completed") is True
        ...
        and prior.get("preferred_repair_path") == "PYARROW_SCHEMA_ACCESS_REPAIR_APPROVAL_PREVIEW"
        and prior.get("preferred_repair_allowed_now") is False
    ...
    approval_scope = {
        "approval_grants_pyarrow_repair_approval_record_only": True,
    ...
    replacement_checks = {
        ...
        "tracked_python_count_increases_from_630_to_631": py["tracked_python_count"] == EXPECTED_TRACKED_PYTHON_COUNT,
        ...
        "prior_repair_preview_respected": prior_repair_preview_respected,
        ...
        "approval_record_only": approval_scope["approval_grants_pyarrow_repair_approval_record_only"] is True,
        "no_dependency_install_now": approval_scope["approval_grants_dependency_install_now"] is False
        and boundary["dependency_install_attempted"] is False,
        "no_environment_modification_now": approval_scope["approval_grants_environment_modification_now"] is False
        and boundary["environment_modified"] is False,
        "no_schema_config_creation_now": approval_scope["approval_grants_schema_or_config_creation_now"] is False
        and boundary["schema_or_config_created"] is False,
        "no_full_scan_or_row_read_now": approval_scope["approval_grants_full_parquet_scan_now"] is False
        and approval_scope["approval_grants_parquet_row_read_now"] is False
        and boundary["full_parquet_scan_performed"] is False
        and boundary["parquet_rows_read_now"] is False,
        "no_source_panel_rerun_now": approval_scope["approval_grants_source_panel_rerun_now"] is False
        and boundary["source_panel_rerun_performed"] is False,
        ...
        "next_module_is_apply_preview_not_install_or_apply": NEXT_MODULE_APPLY_PREVIEW.endswith("_apply_preview_after_result_review_v1.py"),
        "loop_remains_closed": True,
    }
    ready = all(value is True for value in replacement_checks.values())

    next_module = NEXT_MODULE_APPLY_PREVIEW if ready else NEXT_MODULE_BLOCKED
    status = APPROVAL_STATUS_PASS if ready else APPROVAL_STATUS_FAIL
    final_decision = (
        "PYARROW_SCHEMA_ACCESS_REPAIR_APPROVAL_RECORD_CREATED_APPLY_PREVIEW_NEXT"
        if ready
        else "PYARROW_SCHEMA_ACCESS_REPAIR_APPROVAL_RECORD_FAIL_CLOSED"
    )
    next_action = (
        "BUILD_SOURCE_PANEL_PYARROW_SCHEMA_ACCESS_REPAIR_APPLY_PREVIEW_AFTER_RESULT_REVIEW"
        if ready
    ...
    approval_context = {
        ...
        "schema_not_obtained_because_pyarrow_fastparquet_duckdb_parquet_tools_unavailable": schema_readers_unavailable,
        "source_panel_results_remain_useful_but_partial": True,
        "p1_limitation_remains_unresolved": True,
        "preferred_path": "PYARROW_SCHEMA_ACCESS_REPAIR_APPROVAL_PREVIEW",
    ...
    evidence_state = {
        ...
        "no_upgrade_until_schema_access_repaired_and_validated": True,
    }

    payload: Dict[str, Any] = {
        "module": MODULE_NAME,
        "created_at_utc": now_utc(),
        "source_panel_pyarrow_schema_access_repair_approval_status": status,
        "post_check_status": POST_CHECK_STATUS_PASS if ready else POST_CHECK_STATUS_BLOCKED,
        "final_decision": final_decision,
        "next_action": next_action,
        "next_module": next_module,
        "prior_repair_preview_respected": prior_repair_preview_respected,
        "pyarrow_schema_access_repair_approval_record_created": ready,
        "user_pyarrow_repair_approval_present": True,
        "user_pyarrow_repair_approval_scope": "APPROVAL_RECORD_ONLY_NO_INSTALL_NO_APPLY_NO_SCAN_NO_ROW_READ_NO_RERUN",
        **approval_scope,
        ...
        "pyarrow_installed_now": availability["pyarrow_installed_now"],
        "pyarrow_repair_apply_preview_allowed_next": ready,
        **boundary,
        ...
        "derived_live_repo_post_check_reason": (
            "This approval module creates only a repo-only pyarrow schema access repair approval record. "
        ...
        "next_module_decision": {
            ...
            "next_module_if_valid": NEXT_MODULE_APPLY_PREVIEW,
            "next_module_if_unsafe_or_invalid": NEXT_MODULE_BLOCKED,
        ...
        "prior_repair_preview_snapshot": {
            "artifact_path": str(PRIOR_REPAIR_PREVIEW_LATEST_ARTIFACT),
            "artifact_valid_json": prior_valid_json,
            "artifact_json_error": prior_json_error,
            "status": prior.get("source_panel_parquet_schema_access_repair_preview_status"),
            "post_check_status": prior.get("post_check_status"),
            "repair_preview_completed": prior.get("repair_preview_completed"),
...
def write_outputs(payload: Dict[str, Any]) -> Dict[str, str]:
    ...
    latest_json = OUT_DIR / "repo_only_source_panel_pyarrow_schema_access_repair_approval_after_result_review_v1_latest.json"
    timestamped_json = (
        OUT_DIR / f"repo_only_source_panel_pyarrow_schema_access_repair_approval_after_result_review_v1_{stamp}.json"
    )
    latest_txt = OUT_DIR / "repo_only_source_panel_pyarrow_schema_access_repair_approval_after_result_review_v1_latest.txt"
...
def main() -> int:
    ...
    print(rendered)
    return 0 if payload["source_panel_pyarrow_schema_access_repair_approval_status"] == APPROVAL_STATUS_PASS else 3
