...
MODULE_PATH = "tools/edge_factory_os_repo_only_ema9_pivot_15m_v2_closure_v1.py"
ARTIFACT_PATH = "artifacts/strategy_closures/ema9_pivot_15m_v2_closure_v1.json"
EVALUATOR_PATH = "artifacts/strategy_evaluations/ema9_pivot_15m_v2_evaluator_v1.json"

STATUS = "PASS_REPO_ONLY_EMA9_PIVOT_15M_V2_CLOSURE_CREATED"
ARTIFACT_KIND = "EMA9_PIVOT_15M_V2_CLOSURE"
EVALUATOR_STATUS = "PASS_REPO_ONLY_EMA9_PIVOT_15M_V2_EVALUATED"
...
EXPECTED_PRE_CLOSURE_HEAD = "9516123e858e6746ec7c6947c2e24b23bfeebaec"
EXPECTED_PRE_CLOSURE_TRACKED_PYTHON_COUNT = 939
...
def build_payload() -> Dict[str, Any]:
    ...
    if unexpected_dirty_paths:
        raise RuntimeError(f"unexpected dirty paths during V2 closure: {unexpected_dirty_paths}")
    if actual_head != EXPECTED_PRE_CLOSURE_HEAD:
        raise RuntimeError(f"HEAD moved before V2 closure: {actual_head} != {EXPECTED_PRE_CLOSURE_HEAD}")
    if actual_tracked_python_count != EXPECTED_PRE_CLOSURE_TRACKED_PYTHON_COUNT:
        raise RuntimeError(
            "tracked Python count mismatch before V2 closure: "
    ...
    payload: Dict[str, Any] = {
        ...
        "closure_reason": (
            "Route closed after evaluator. The evaluator result is preserved exactly; this V2 structural "
            "repair test grants no candidate, edge claim, family release, runtime, live, or capital permission."
        ),
        "permissions": {
    ...
    if not all(payload["validation_checks"].values()):
        raise RuntimeError(f"V2 closure validation checks failed: {payload['validation_checks']}")
