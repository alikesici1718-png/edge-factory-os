...
MODULE_NAME = "edge_factory_os_repo_only_future_module_evidence_chain_contract_enforcement_gate_v1"
CURRENT_TOOL_REL = "tools/edge_factory_os_repo_only_future_module_evidence_chain_contract_enforcement_gate_v1.py"
OUT_DIR = LAB_ROOT / MODULE_NAME

EXPECTED_HEAD = "a1a0f2d"
EXPECTED_PREVIOUS_TRACKED_PYTHON_COUNT = 576
NEXT_MODULE = "edge_factory_os_repo_only_future_module_evidence_chain_contract_enforcement_gate_validator_v1.py"
POST_CHECK_STATUS = "REPO_ONLY_FUTURE_MODULE_EVIDENCE_CHAIN_CONTRACT_ENFORCEMENT_GATE_POST_COMMIT_CHECK_PASS"
...
ENFORCEMENT_GATE_REQUIREMENTS = [
    "required evidence_chain_quality field",
    "required primary_artifact_verified field",
    "required exact_post_check_marker_verified field",
    "required derived_live_repo_post_check field",
    "required derived_live_repo_post_check_reason when derived=true",
    "required replacement_checks_all_true when derived=true",
    "required replacement_checks_are_not_equivalent_to_primary_artifact field",
    "required missing_primary_artifact_must_not_silently_pass field",
    "fail closed if evidence is missing",
    "classify plain PASS without full marker as ATTENTION",
    "preserve closed loop and blocked generic runner",
]
...
def git_state() -> Dict[str, Any]:
    ...
    return {
        ...
        "repo_clean_for_enforcement_gate": len(dirty_tracked) == 0 and len(unexpected_untracked) == 0,
    }
...
def build_payload() -> Dict[str, Any]:
    ...
    replacement_checks = {
        ...
        "repo_clean_for_enforcement_gate": git["repo_clean_for_enforcement_gate"],
        ...
        "next_module_expected": NEXT_MODULE == "edge_factory_os_repo_only_future_module_evidence_chain_contract_enforcement_gate_validator_v1.py",
    }
    ...
    payload: Dict[str, Any] = {
        ...
        "enforcement_gate_status": "READY" if ready else "FAIL_CLOSED",
        "post_check_status": POST_CHECK_STATUS if ready else "REPO_ONLY_FUTURE_MODULE_EVIDENCE_CHAIN_CONTRACT_ENFORCEMENT_GATE_POST_COMMIT_CHECK_FAIL_CLOSED",
        "final_decision": "FUTURE_MODULE_EVIDENCE_CHAIN_ENFORCEMENT_GATE_READY" if ready else "FUTURE_MODULE_EVIDENCE_CHAIN_ENFORCEMENT_GATE_FAIL_CLOSED",
        "next_action": "BUILD_FUTURE_MODULE_EVIDENCE_CHAIN_CONTRACT_ENFORCEMENT_GATE_VALIDATOR_V1" if ready else "REVIEW_FUTURE_MODULE_EVIDENCE_CHAIN_ENFORCEMENT_GATE_INPUTS",
        "next_module": NEXT_MODULE if ready else None,
        "evidence_chain_enforcement_gate_ready": ready,
        "enforcement_gate_requirements": ENFORCEMENT_GATE_REQUIREMENTS,
        "required_future_module_fields": REQUIRED_FUTURE_MODULE_FIELDS,
        ...
        "primary_artifact_verified_required": True,
        "exact_post_check_marker_verified_required": True,
        "derived_live_repo_post_check_required": True,
        "derived_live_repo_post_check_reason_required_when_derived_true": True,
        "replacement_checks_all_true_required_when_derived_true": True,
        "missing_primary_artifact_must_not_silently_pass": True,
        "fail_closed_if_evidence_missing": True,
        "plain_pass_without_full_marker_classified_attention": True,
        "derived_checks_weaker_than_primary_artifacts": True,
        ...
        "derived_live_repo_post_check_reason": "Enforcement gate uses the adoption record, contract artifact, and live repo replacement checks; evidence remains DERIVED_OVERUSED_ATTENTION and weaker than primary artifact verification.",
        "replacement_checks_all_true": ready,
        "derived_live_repo_post_check_replacement_checks": replacement_checks,
        "validation": {"git_state": git, "tracked_python_validation": py, "adoption_loaded": bool(adoption), "contract_loaded": bool(contract)},
        "safety_flags": {
            "future_module_evidence_chain_contract_enforcement_gate": True,
            "repo_only": True,
            "fail_closed_enforcement_gate": True,
            "generic_runner_approval_granted": False,
            "generic_runner_implementation_remains_blocked": True,
            "ordinary_selector_backlog_loop_reentry_allowed": False,
            "loop_remains_closed": True,
...
def write_outputs(payload: Dict[str, Any]) -> Dict[str, str]:
    ...
    latest_json = OUT_DIR / "repo_only_future_module_evidence_chain_contract_enforcement_gate_v1_latest.json"
    timestamped_json = OUT_DIR / f"repo_only_future_module_evidence_chain_contract_enforcement_gate_v1_{stamp}.json"
    latest_txt = OUT_DIR / "repo_only_future_module_evidence_chain_contract_enforcement_gate_v1_latest.txt"
...
def main() -> int:
    ...
    return 0 if payload["enforcement_gate_status"] == "READY" else 3
