...
MODULE_NAME = "edge_factory_os_repo_only_development_backlog_refresh_after_standard_os_status_refresh_v1"
OUT_DIR = LAB_ROOT / "edge_factory_os_repo_only_development_backlog_refresh_after_standard_os_status_refresh_v1"
OUT_DIR.mkdir(parents=True, exist_ok=True)

EXPECTED_HEAD_PREFIX = "c227878"
CURRENT_TOOL_REL = "tools/edge_factory_os_repo_only_development_backlog_refresh_after_standard_os_status_refresh_v1.py"
...
SELECTOR_JSON = LAB_ROOT / "edge_factory_os_repo_only_next_action_selector_after_standard_os_status_refresh_v1" / "repo_only_next_action_selector_after_standard_os_status_refresh_v1_latest.json"
SELECTOR_POST_JSON = LAB_ROOT / "edge_factory_os_repo_only_next_action_selector_after_standard_os_status_refresh_post_commit_check" / "repo_only_next_action_selector_after_standard_os_status_refresh_post_commit_check_latest.json"

SELECTED_NEXT_MODULE = "edge_factory_os_repo_only_next_action_selector_after_standard_os_status_backlog_refresh_v1.py"
...
SAFETY_FLAGS: Dict[str, bool] = {
    "repo_only_development_backlog_refresh_after_standard_os_status_refresh": True,
    "read_only_backlog_refresh_only": True,
    "development_queue_selector_after_standard_os_status_refresh_required": True,
    "next_action_selector_after_standard_os_status_backlog_refresh_allowed_next": True,
...
def first_status(obj: Dict[str, Any]) -> Optional[str]:
    for key in [
        ...
        "audit_status",
        "next_action_selector_after_standard_os_status_refresh_status",
    ]:
        value = obj.get(key)
        if value:
            return str(value)
...
def validate_inputs() -> Dict[str, Any]:
    ...
    if subject != "Add repo-only development queue selector after standard OS status refresh":
        errors.append(f"unexpected HEAD commit subject: {subject}")

    paths = commit_paths("HEAD")
    checks["head_commit_paths"] = paths
    if paths != ["tools/edge_factory_os_repo_only_development_queue_selector_after_standard_os_status_refresh_v1.py"]:
    ...
    inputs = {
        "development_queue_selector": (
            ...
            "edge_factory_os_repo_only_development_backlog_refresh_after_standard_os_status_refresh_v1.py",
        ),
        ...
        "next_action_selector": (
            SELECTOR_JSON,
            "REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_STANDARD_OS_STATUS_REFRESH_V1_READY",
            None,
        ),
        "next_action_selector_post_check": (
            SELECTOR_POST_JSON,
            "REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_STANDARD_OS_STATUS_REFRESH_POST_COMMIT_CHECK_PASS",
            None,
        ),
    }
...
def build_backlog(validation: Dict[str, Any]) -> Dict[str, Any]:
    ...
    selector = selector_obj.get("next_action_selector_after_standard_os_status_refresh", {})
    ...
    prerequisites = {
        ...
        "queue_selected_this_backlog_refresh": selected_task.get("module") == "edge_factory_os_repo_only_development_backlog_refresh_after_standard_os_status_refresh_v1.py",
        "queue_selected_repo_only_scope": selected_task.get("scope") == "REPO_ONLY_OS_INTELLIGENCE",
        "selector_ready": selector_obj.get("next_action_selector_after_standard_os_status_refresh_status") == "REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_STANDARD_OS_STATUS_REFRESH_V1_READY",
        "selector_selected_queue": selected_action.get("module") == "edge_factory_os_repo_only_development_queue_selector_after_standard_os_status_refresh_v1.py",
    ...
    backlog_items = [
        {
            "rank": 1,
            "key": "NEXT_ACTION_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH",
            "module": SELECTED_NEXT_MODULE,
            "scope": "REPO_ONLY_OS_INTELLIGENCE",
            "allowed": True,
            "reason": "Backlog refreshed after standard OS status refresh; select next repo-only development task.",
            "blocked_actions": [
    ...
    return {
        "backlog_refresh_status": "DEVELOPMENT_BACKLOG_AFTER_STANDARD_OS_STATUS_REFRESH_READY" if selected else "DEVELOPMENT_BACKLOG_AFTER_STANDARD_OS_STATUS_REFRESH_BLOCKED",
        ...
        "selected_next_action": "BUILD_REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_V1" if selected else None,
        ...
        "invariants": {
            ...
            "selected_module_is_next_action_selector_after_standard_os_status_backlog_refresh": selected is not None and selected["module"] == SELECTED_NEXT_MODULE,
...
def main() -> int:
    ...
    payload = {
        "module": MODULE_NAME,
        "development_backlog_refresh_after_standard_os_status_refresh_status": "REPO_ONLY_DEVELOPMENT_BACKLOG_REFRESH_AFTER_STANDARD_OS_STATUS_REFRESH_V1_READY" if passed else "REPO_ONLY_DEVELOPMENT_BACKLOG_REFRESH_AFTER_STANDARD_OS_STATUS_REFRESH_V1_BLOCKED",
        "severity": "OK" if passed else "BLOCKED",
        "allowed_scope": "REPO_ONLY_OS_INTELLIGENCE",
        "final_decision": "DEVELOPMENT_BACKLOG_REFRESH_AFTER_STANDARD_OS_STATUS_REFRESH_READY_FOR_NEXT_ACTION_SELECTOR" if passed else "KEEP_FREEZE_REVIEW_DEVELOPMENT_BACKLOG_AFTER_STANDARD_OS_STATUS_REFRESH_ERRORS",
        "next_action": "BUILD_REPO_ONLY_NEXT_ACTION_SELECTOR_AFTER_STANDARD_OS_STATUS_BACKLOG_REFRESH_V1" if passed else "REVIEW_DEVELOPMENT_BACKLOG_AFTER_STANDARD_OS_STATUS_REFRESH_ERRORS",
        "next_module": SELECTED_NEXT_MODULE if passed else None,
        "reason": "Development backlog refreshed after standard OS status refresh; schema apply/create/edit remain blocked and physically absent." if passed else "Development backlog refresh after standard OS status refresh validation failed.",
        ...
        "development_backlog_refresh_after_standard_os_status_refresh": backlog,
    ...
    latest_json = OUT_DIR / "repo_only_development_backlog_refresh_after_standard_os_status_refresh_v1_latest.json"
    timestamped_json = OUT_DIR / f"repo_only_development_backlog_refresh_after_standard_os_status_refresh_v1_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json"
    latest_txt = OUT_DIR / "repo_only_development_backlog_refresh_after_standard_os_status_refresh_v1_latest.txt"

    latest_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    timestamped_json.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")

    lines = [
        "EDGE FACTORY OS REPO-ONLY DEVELOPMENT BACKLOG REFRESH AFTER STANDARD OS STATUS REFRESH v1",
        "=" * 100,
        f"development_backlog_refresh_after_standard_os_status_refresh_status: {payload['development_backlog_refresh_after_standard_os_status_refresh_status']}",
