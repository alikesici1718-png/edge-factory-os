...
NEXT_MODULE_BLOCKED = (
    "edge_factory_os_repo_only_historical_data_acquisition_okx_10_symbol_pilot_"
    "policy_clean_build_execution_blocked_or_anomaly_record_after_eth_material_policy_v1.py"
)
...
EXPECTED_NEW_SYMBOL_SOURCE_ROWS_BEFORE_POLICY = 13_647_208
...
POLICY_SUMMARY = POLICY_DIR / "historical_okx_10_symbol_pilot_eth_material_duplicate_conflict_policy_summary.json"
ETH_MATERIAL_POLICY = POLICY_DIR / "historical_okx_10_symbol_pilot_eth_material_duplicate_conflict_policy.json"
ETH_POLICY_COUNTS = POLICY_DIR / "historical_okx_10_symbol_pilot_eth_duplicate_group_policy_counts.json"
ETH_QUARANTINE_POLICY = POLICY_DIR / "historical_okx_10_symbol_pilot_eth_material_conflict_quarantine_policy.json"
POLICY_CLEAN_BUILD_PREVIEW = POLICY_DIR / "historical_okx_10_symbol_pilot_eth_policy_clean_build_preview.json"
POLICY_CLEAN_BUILD_APPROVAL = POLICY_DIR / "historical_okx_10_symbol_pilot_eth_material_policy_clean_build_approval_record.json"
...
def validate_preflight(
    ...
    require(build_preview.get("future_build_must_reuse_btc_policy_clean_output_after_revalidation") is True, "build preview BTC reuse requirement missing")
    require(build_preview.get("future_build_must_apply_eth_duplicate_policy_from_this_module") is True, "build preview ETH material policy requirement missing")
    require(build_preview.get("future_build_must_fail_closed_on_new_sol_xrp_doge_ada_avax_link_ltc_dot_anomalies") is True, "build preview new-symbol fail-closed rule missing")
    require(build_approval.get("approval_grants_future_10_symbol_eth_material_policy_clean_build_next") is True, "build approval missing")
    for key in [
...
def process_new_symbol(
    ...
    expected_before_policy = ETH_RAW_ROWS_BEFORE_POLICY if symbol == "ETH-USDT-SWAP" else NOMINAL_SOURCE_ROWS_PER_SYMBOL
    expected_clean_source_rows = ETH_CLEAN_SOURCE_ROWS_AFTER_POLICY if symbol == "ETH-USDT-SWAP" else NOMINAL_SOURCE_ROWS_PER_SYMBOL
    expected_complete_rows = ETH_COMPLETE_1H_ROWS_AFTER_POLICY if symbol == "ETH-USDT-SWAP" else NOMINAL_OUTPUT_ROWS_PER_SYMBOL
...
def main() -> None:
    ...
    for symbol in NEW_SYMBOLS:
        result, files, schemas, drops, quarantines = process_new_symbol(symbol, by_symbol[symbol], eth_policy_maps)
