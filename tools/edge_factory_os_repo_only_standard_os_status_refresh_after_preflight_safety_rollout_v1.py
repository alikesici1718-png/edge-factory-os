...
MODULE_NAME = "edge_factory_os_repo_only_standard_os_status_refresh_after_preflight_safety_rollout_v1"
CURRENT_TOOL_REL = "tools/edge_factory_os_repo_only_standard_os_status_refresh_after_preflight_safety_rollout_v1.py"
OUT_DIR = LAB_ROOT / MODULE_NAME
EXPECTED_PARENT_HEAD = "4e8d22f"
EXPECTED_PREVIOUS_TRACKED_PYTHON_COUNT = 603
EXPECTED_TRACKED_PYTHON_COUNT = 604
NEXT_MODULE = "edge_factory_os_repo_only_standard_os_status_review_after_preflight_safety_rollout_v1.py"
POST_CHECK_STATUS = "REPO_ONLY_STANDARD_OS_STATUS_REFRESH_AFTER_PREFLIGHT_SAFETY_ROLLOUT_POST_COMMIT_CHECK_PASS"
...
ARTIFACTS = {
    "preflight_audit_review": LAB_ROOT / "edge_factory_os_repo_only_preflight_safety_readiness_post_rollout_audit_review_v1" / "repo_only_preflight_safety_readiness_post_rollout_audit_review_v1_latest.json",
    "preflight_rollout": LAB_ROOT / "edge_factory_os_repo_only_preflight_safety_readiness_rollout_status_record_v1" / "repo_only_preflight_safety_readiness_rollout_status_record_v1_latest.json",
    "preflight_adoption": LAB_ROOT / "edge_factory_os_repo_only_preflight_safety_readiness_adoption_record_v1" / "repo_only_preflight_safety_readiness_adoption_record_v1_latest.json",
    ...
    "evidence_chain_rollout": LAB_ROOT / "edge_factory_os_repo_only_future_module_evidence_chain_rollout_status_record_v1" / "repo_only_future_module_evidence_chain_rollout_status_record_v1_latest.json",
    "generic_loop_closure": LAB_ROOT / "edge_factory_os_repo_only_generic_governance_blocked_status_loop_closure_v1" / "repo_only_generic_governance_blocked_status_loop_closure_v1_latest.json",
}
...
def policy_ok(payload: Dict[str, Any]) -> bool:
    return (
...
def build_payload() -> Dict[str, Any]:
    ...
    evidence_chain_rollout_respected = bool(loaded["evidence_chain_rollout"]) and policy_ok(loaded["evidence_chain_rollout"])
    kill_switch_readiness_adoption_respected = loaded["kill_switch_adoption"].get("kill_switch_readiness_adoption_record_status") in {"READY", "PASS"} and loaded["kill_switch_adoption"].get("future_runtime_or_live_requires_kill_switch_readiness") is True
    preflight_safety_readiness_rollout_respected = loaded["preflight_audit_review"].get("post_rollout_audit_review_status") in {"READY", "PASS", "PASS_CONTEXTUAL", "PASS_WITH_ATTENTION"} and loaded["preflight_audit_review"].get("preflight_safety_readiness_rollout_respected") is True and policy_ok(loaded["preflight_audit_review"])
    generic_loop_closure_respected = loaded["preflight_audit_review"].get("loop_remains_closed") is True and loaded["preflight_audit_review"].get("generic_runner_implementation_remains_blocked") is True
    ...
    replacement_checks = {
        ...
        "generic_governance_loop_closure_respected": generic_loop_closure_respected,
        "evidence_chain_rollout_respected": evidence_chain_rollout_respected,
        "kill_switch_readiness_adoption_respected": kill_switch_readiness_adoption_respected,
        "preflight_safety_readiness_rollout_respected": preflight_safety_readiness_rollout_respected,
        "next_module_expected": NEXT_MODULE == "edge_factory_os_repo_only_standard_os_status_review_after_preflight_safety_rollout_v1.py",
    }
    ...
    payload: Dict[str, Any] = {
        ...
        "standard_os_status_refresh_status": "READY" if ready else "FAIL_CLOSED",
        "post_check_status": POST_CHECK_STATUS if ready else "REPO_ONLY_STANDARD_OS_STATUS_REFRESH_AFTER_PREFLIGHT_SAFETY_ROLLOUT_POST_COMMIT_CHECK_FAIL_CLOSED",
        "final_decision": "STANDARD_OS_STATUS_REFRESH_AFTER_PREFLIGHT_SAFETY_ROLLOUT_READY" if ready else "STANDARD_OS_STATUS_REFRESH_AFTER_PREFLIGHT_SAFETY_ROLLOUT_FAIL_CLOSED",
        "next_module": NEXT_MODULE if ready else None,
        "os_state_after_generic_governance_loop_closure": generic_loop_closure_respected,
        "evidence_chain_rollout_respected": evidence_chain_rollout_respected,
        "kill_switch_readiness_adoption_respected": kill_switch_readiness_adoption_respected,
        "preflight_safety_readiness_rollout_respected": preflight_safety_readiness_rollout_respected,
        ...
        "derived_live_repo_post_check_reason": "Standard OS status refresh uses generic governance closure, evidence-chain rollout, kill-switch adoption, preflight safety rollout/audit artifacts, and post-commit live repo replacement checks; evidence remains DERIVED_OVERUSED_ATTENTION and weaker than primary artifact verification.",
...
def write_outputs(payload: Dict[str, Any]) -> Dict[str, str]:
    ...
    latest_json = OUT_DIR / "repo_only_standard_os_status_refresh_after_preflight_safety_rollout_v1_latest.json"
    timestamped_json = OUT_DIR / f"repo_only_standard_os_status_refresh_after_preflight_safety_rollout_v1_{stamp}.json"
    latest_txt = OUT_DIR / "repo_only_standard_os_status_refresh_after_preflight_safety_rollout_v1_latest.txt"
...
def main() -> int:
    ...
    return 0 if payload["standard_os_status_refresh_status"] in {"READY", "PASS"} else 3
