...
# -*- coding: utf-8 -*-
r"""
EDGE FACTORY OS CONTROL TOWER v3
================================

Integrated top-level command center for the self-improving Edge Factory OS.

v3 adds Paper Runtime Observer to Control Tower v2.
...
It reads, but does not run, the latest manual approval record.
...
It DOES NOT:
    ...
    - record approval by itself

Run:
    python "C:\Users\alike\edge_factory_os_control_tower_v3.py"

Read-only:
    python "C:\Users\alike\edge_factory_os_control_tower_v3.py" --read_only

Outputs:
    <workspace>\edge_factory_os_control_tower_v3\control_tower_v3_YYYYMMDD_HHMMSS\
        os_control_tower_v3_report.md
        os_control_tower_v3_state.json
        os_control_tower_v3_actions.json
        os_control_tower_v3_run_log.csv
        os_control_tower_v3_evidence.csv
"""
...
ARTIFACTS = {
    ...
    "control_tower_v2": ("edge_factory_os_control_tower_v2", "control_tower_v2_", "os_control_tower_v2_state.json"),
}
...
def derive_state(logs: List[RunLog], data: Dict[str, Dict[str, Any]]) -> Tuple[Dict[str, Any], List[TowerAction]]:
    ...
    live_allowed = bool(gate_decision.get("live_allowed", False)) or bool(runtime_decision.get("live_allowed", False)) or bool(autopilot_state.get("live_allowed", False))
    ...
    if failed:
        ...
        notes.append("paper runtime has closed trade evidence; drift monitor is next")
        ...
        notes.append("paper runtime evidence exists but drift sample is not ready")
        ...
        notes.append(f"combined state needs review: autopilot={autopilot_mode}, gate={gate_status}, runtime={runtime_status}")

    actions = [build_action(x) for x in autopilot_actions if isinstance(x, dict)]

    if runtime_status == "PAPER_NOT_STARTED" and gate_status == "PAPER_START_BLOCKED_NO_MANUAL_APPROVAL":
    ...
    elif runtime_status == "PAPER_NOT_STARTED" and gate_status == "PAPER_START_REFERENCE_ALLOWED_MANUAL_ONLY":
        actions.insert(0, TowerAction(
            priority=5,
            action_key="manual_paper_reference_allowed_not_started",
            status="MANUAL_REFERENCE_ONLY",
            category="PAPER_GATE",
            title="Paper reference is allowed but paper has not started",
            reason="A human may manually start supervised paper; OS did not execute launcher.",
            command=None,
        ...
        actions.insert(0, TowerAction(
            priority=5,
            action_key="run_drift_monitor_after_paper_sample",
            status="READY_FOR_VALIDATION",
            category="PAPER_RUNTIME",
            title="Run live-vs-backtest drift monitor on paper sample",
            reason="Runtime observer detected closed paper trades. Live remains blocked.",
            command='python "C:\\Users\\alike\\edge_factory_live_vs_backtest_drift_monitor.py"',
            blocked_by=["manual review after drift"],
            safe_offline=True,
            starts_paper_or_live=False,
        ))
    elif runtime_status == "PAPER_RUNNING_WAITING_FOR_CLOSED_TRADES":
        actions.insert(0, TowerAction(
            priority=5,
            action_key="wait_for_closed_paper_trades",
            status="WAITING_FOR_PAPER_SAMPLE",
            category="PAPER_RUNTIME",
            title="Keep observing until closed paper trades exist",
            reason="Paper runtime exists but closed trade sample is not ready.",
            command=None,
    ...
    if not any(a.action_key == "live_remains_blocked" for a in actions):
        actions.append(TowerAction(
            ...
            reason="Control Tower v3 cannot approve live. Live requires paper drift validation and manual live review.",
            command=None,
    ...
    state = {
        ...
        "tower_notes": notes,
        "hard_rules": [
            "Control Tower v3 never starts paper/live automatically.",
            "Control Tower v3 never executes --apply.",
            "Control Tower v3 does not record manual approval by itself.",
            ...
            "Paper runtime observer only observes logs; it does not run/stop processes.",
        ],
    }
...
def write_report(path: Path, state: Dict[str, Any], logs: List[RunLog], actions: List[TowerAction], evidence: Dict[str, Evidence]) -> None:
    lines: List[str] = []
    lines.append("# Edge Factory OS Control Tower v3 Report")
    lines.append("")
    ...
    lines.append("")
    lines.append("Control Tower v3 is the integrated safe command center with runtime awareness. It sees the control plane, state transition, approval packet, approval record, paper-start gate, and paper runtime observer. It does not start paper/live or mutate active configuration.")
    lines.append("")
    path.write_text("\n".join(lines), encoding="utf-8")


def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Edge Factory OS Control Tower v3")
    ...
    p.add_argument("--timeout_sec", type=int, default=480)
    return p
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    out_root = Path(args.out_dir) if args.out_dir else workspace / "edge_factory_os_control_tower_v3"
    out_dir = out_root / f"control_tower_v3_{now_stamp()}"
    ...
    write_json(out_dir / "os_control_tower_v3_state.json", result)
    write_json(out_dir / "os_control_tower_v3_actions.json", [asdict(x) for x in actions])
    run_log_df(logs).to_csv(out_dir / "os_control_tower_v3_run_log.csv", index=False)
    action_df(actions).to_csv(out_dir / "os_control_tower_v3_actions.csv", index=False)
    evidence_df(evidence).to_csv(out_dir / "os_control_tower_v3_evidence.csv", index=False)
    write_report(out_dir / "os_control_tower_v3_report.md", state, logs, actions, evidence)

    print("EDGE FACTORY OS CONTROL TOWER v3")
    print("=" * 100)
    ...
    print("")
    print(f"Report : {out_dir / 'os_control_tower_v3_report.md'}")
    print(f"State  : {out_dir / 'os_control_tower_v3_state.json'}")
    print(f"Actions: {out_dir / 'os_control_tower_v3_actions.json'}")
