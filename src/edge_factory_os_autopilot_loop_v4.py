...
PROCESS_WATCHDOG = USERDIR / "edge_factory_os_process_watchdog_v1.py"
...
OUT_ROOT = WORKSPACE / "edge_factory_os_autopilot_loop_v4"
...
def load_latest_state(root_name: str, prefix: str, state_name: str) -> dict[str, Any]:
    d = latest_dir(WORKSPACE / root_name, prefix)
    if not d:
        return {}
    state = read_json(d / state_name)
...
def decide(
    ...
    process_status = process_state.get("status", "UNKNOWN")
    missing_processes = process_state.get("missing_components", []) or []
    ...
    if process_status != "PROCESS_WATCHDOG_PASS":
        severity = "ATTENTION"
        next_actions.append("PROCESS_WATCHDOG_NOT_PASS_INSPECT_MISSING_COMPONENTS")
        blocked.append("PROCESS_WATCHDOG_NOT_PASS")
        if missing_processes:
            blocked.append("MISSING_PROCESS_" + "_".join(str(x) for x in missing_processes))
    ...
    if severity == "OK":
        ...
        if closed_n >= 20:
            ...
            safe_actions_to_run.append(("drift_monitor_planner", [sys.executable, str(DRIFT_MONITOR)], 240))
        else:
        ...
        if closed_n >= 50:
            ...
            safe_actions_to_run.append(("capital_governor_review", [sys.executable, str(CAPITAL_GOVERNOR)], 240))
        else:
        ...
        if closed_n >= 20:
            ...
            safe_actions_to_run.append(("batch_family_pipeline_planner", [sys.executable, str(BATCH_PLANNER)], 240))
        else:
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Edge Factory OS Autopilot Loop v4.")
    ...
    run_dir = OUT_ROOT / f"os_autopilot_loop_v4_{stamp()}"
    run_dir.mkdir(parents=True, exist_ok=True)

    latest_state_path = OUT_ROOT / "edge_factory_os_autopilot_v4_latest_state.json"
    latest_status_path = OUT_ROOT / "edge_factory_os_autopilot_v4_latest_status.txt"
    history_jsonl = OUT_ROOT / "edge_factory_os_autopilot_v4_history.jsonl"
    history_csv = OUT_ROOT / "edge_factory_os_autopilot_v4_history.csv"

    print("EDGE FACTORY OS AUTOPILOT LOOP v4")
    print("=" * 100)
    ...
    print("mode: READ_ONLY_SUPERVISION_WITH_PROCESS_WATCHDOG")
    ...
    while True:
        ...
        command_center_result = run_cmd("command_center", [sys.executable, str(COMMAND_CENTER)], timeout=240)
        invariant_result = run_cmd("invariant_guard", [sys.executable, str(INVARIANT_GUARD)], timeout=240)
        candidate_registry_result = run_cmd("candidate_lifecycle_registry", [sys.executable, str(CANDIDATE_REGISTRY)], timeout=240)
        process_watchdog_result = run_cmd("process_watchdog", [sys.executable, str(PROCESS_WATCHDOG)], timeout=240)

        cc_state = load_latest_state(
            "edge_factory_os_command_center_v1",
            "os_command_center_v1_",
            "edge_factory_os_command_center_v1_state.json",
        )
        invariant_state = load_latest_state(
            "edge_factory_os_invariant_guard_v1",
            "os_invariant_guard_v1_",
            "edge_factory_os_invariant_guard_v1_state.json",
        )
        candidate_state = load_latest_state(
            "edge_factory_candidate_lifecycle_registry_v1",
            "candidate_lifecycle_registry_v1_",
            "candidate_lifecycle_registry_v1_state.json",
        )
        process_state = load_latest_state(
            "edge_factory_os_process_watchdog_v1",
            "os_process_watchdog_v1_",
            "edge_factory_os_process_watchdog_v1_state.json",
        )
        ...
        for name, result, severity_if_fail in [
            ("COMMAND_CENTER_FAILED", command_center_result, "CRITICAL"),
            ("INVARIANT_GUARD_FAILED_TO_RUN", invariant_result, "CRITICAL"),
            ("CANDIDATE_REGISTRY_FAILED_TO_RUN", candidate_registry_result, "ATTENTION"),
            ("PROCESS_WATCHDOG_FAILED_TO_RUN", process_watchdog_result, "ATTENTION"),
        ]:
            if not result["ok"]:
                if severity_if_fail == "CRITICAL":
                    decision["severity"] = "CRITICAL"
                elif decision["severity"] != "CRITICAL":
                    decision["severity"] = "ATTENTION"
                decision["next_actions"].insert(0, name)
                decision["safe_actions_blocked"].append(name)
        ...
        record = {
            ...
            "process_watchdog": process_watchdog_result,
            ...
            "process_watchdog_state_dir": process_state.get("_state_dir"),
            "decision": decision,
            ...
            "hard_rules": [
                ...
                "Only runs read-only planner/monitor modules when safe_execute is enabled and all guards pass.",
            ],
        }

        latest_state_path.write_text(json.dumps(record, indent=2, ensure_ascii=False, default=str), encoding="utf-8")
        ...
        status_line = (
            ...
            f"invariant={decision['invariant_status']} | process={decision['process_status']} | "
