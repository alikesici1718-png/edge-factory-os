...
def normalize_positions(df: pd.DataFrame, family_key: str, source_folder: Path, kind: str) -> pd.DataFrame:
    ...
    if "signal_id" not in x.columns:
        if kind == "pending":
            base_time = x.get("signal_time", pd.Series([""] * len(x))).astype(str)
...
class DynamicGlobalRiskManager:
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        raw_max = str(args.max_per_family_json or "").strip()
        # PowerShell nested Start-Process can pass surrounding quotes as literal characters.
        # Strip them so '{"a":1}' and "{"a":1}" both parse correctly.
        if (raw_max.startswith("'") and raw_max.endswith("'")) or (raw_max.startswith('"') and raw_max.endswith('"')):
            raw_max = raw_max[1:-1].strip()
        try:
            self.max_per_family: dict[str, int] = json.loads(raw_max) if raw_max else {}
        except Exception as e:
            self.max_per_family = {}
            print(f"[WARN] max_per_family_json parse failed: {e}; raw={raw_max!r}")
    ...
    def make_gate_decisions(self, open_all: pd.DataFrame, pending_all: pd.DataFrame) -> pd.DataFrame:
        ...
        if "signal_vol_quote" in x.columns:
            x["_vol_sort"] = -pd.to_numeric(x["signal_vol_quote"], errors="coerce").fillna(0.0)
        else:
            x["_vol_sort"] = 0.0

        x = x.sort_values(["_target_sort", "_vol_sort", "family_key", "coin"]).reset_index(drop=True)
    ...
    def run_forever(self) -> None:
        print("=" * 100)
        print("DYNAMIC GLOBAL PAPER RISK MANAGER v2")
        print("=" * 100)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Dynamic global paper risk manager v2.")
    ...
    ap.add_argument("--pending_grace_minutes", type=int, default=150)
    ap.add_argument("--poll_seconds", type=float, default=60.0)
    ap.add_argument("--once", action="store_true")
    args = ap.parse_args()

    mgr = DynamicGlobalRiskManager(args)
    mgr.run_forever()
