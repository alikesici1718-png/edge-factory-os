#!/usr/bin/env python
"""Validate compact non-overlap absorption event validation outputs."""
...
VALIDATOR_JSON = OUTPUTS_DIR / "orderbook_um_81_absorption_non_overlap_event_validation_validator_report.json"
VALIDATOR_MD = OUTPUTS_DIR / "orderbook_um_81_absorption_non_overlap_event_validation_validator_report.md"

SCANNER_PATH = REPO_ROOT / "src" / "edge_factory_orderbook_um_81_absorption_non_overlap_event_validation_v1.py"
VALIDATOR_PATH = REPO_ROOT / "src" / "edge_factory_orderbook_um_81_absorption_non_overlap_event_validation_validator_v1.py"
RUNNER_PATH = REPO_ROOT / "run_edge_factory_orderbook_um_81_absorption_non_overlap_event_validation_v1.ps1"
VALIDATOR_RUNNER_PATH = REPO_ROOT / "run_edge_factory_orderbook_um_81_absorption_non_overlap_event_validation_validator_v1.ps1"
DOCS_PATH = REPO_ROOT / "docs" / "edge_factory_orderbook_um_81_absorption_non_overlap_event_validation_v1.md"

COMPACT_OUTPUTS = [
    SUMMARY_JSON,
    SUMMARY_MD,
    CANDIDATE_COOLDOWN_CSV,
...
def row_level_or_parquet_outputs() -> list[str]:
    ...
    matches: list[str] = []
    for path in OUTPUTS_DIR.glob("*absorption_non_overlap_event_validation*"):
...
def validate() -> dict[str, Any]:
    ...
    candidate_rows = read_csv_rows(CANDIDATE_COOLDOWN_CSV)
    ...
    cooldowns = {str(row.get("cooldown_seconds", "")) for row in candidate_rows}
    latest_rows = [row for row in candidate_rows if str(row.get("latest_event_count", ""))]
    holdout_rows = [row for row in candidate_rows if str(row.get("holdout_event_count", ""))]
    ...
    checks = {
        ...
        "candidate_cooldown_has_twelve_rows": len(candidate_rows) == len(EXPECTED_CANDIDATES) * len(EXPECTED_COOLDOWNS),
        "candidate_cooldown_matches_fixed_candidates": candidate_keys == EXPECTED_CANDIDATES,
        "candidate_cooldown_matches_cooldowns": cooldowns == EXPECTED_COOLDOWNS,
        "candidate_rows_have_latest_and_holdout_metrics": len(latest_rows) == len(candidate_rows) and len(holdout_rows) == len(candidate_rows),
    ...
    status = (
        "PASS_ORDERBOOK_UM_81_ABSORPTION_NON_OVERLAP_EVENT_VALIDATOR"
        if replacement_checks_all_true
        else "BLOCKED_OR_PARTIAL_ORDERBOOK_UM_81_ABSORPTION_NON_OVERLAP_EVENT_VALIDATION"
    )
    report = {
        "status": status,
        ...
        "survived_non_overlap_candidate_cooldowns": summary.get("survived_non_overlap_candidate_cooldowns", []),
        "failed_non_overlap_candidate_cooldowns": summary.get("failed_non_overlap_candidate_cooldowns", []),
        "candidate_level_summary": summary.get("candidate_level_summary", []),
...
def write_validator_md(report: dict[str, Any]) -> None:
    lines = [
        "# Orderbook UM 81 absorption non-overlap event validator v1",
        "",
    ...
    lines.extend(["", "## Candidate Cooldown Results"])
    for row in report.get("candidate_cooldown_comparison", []):
        lines.append(
            "- "
            f"{row.get('category')} @ {row.get('horizon_seconds')}s cooldown={row.get('cooldown_seconds')}s: "
            f"{row.get('non_overlap_status')} latest={row.get('latest_price_effect_size_vs_null')} "
