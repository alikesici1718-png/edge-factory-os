#!/usr/bin/env python
"""Download bookDepth-matching Binance USD-M aggTrades ZIPs with checksum gates."""
...
from edge_factory_orderbook_um_81_full_matching_aggtrades_manifest_validator_v1 import (
    DOWNLOAD_MANIFEST_CSV as MANIFEST_CSV,
    EXPECTED_SYMBOL_COUNT,
    OUTPUTS_DIR,
    REPORT_JSON as MANIFEST_VALIDATION_JSON,
    REPO_ROOT,
    assert_public_aggtrades_url,
    bool_text,
    data_root,
...
DOWNLOAD_SUMMARY_JSON = OUTPUTS_DIR / "orderbook_um_81_full_matching_aggtrades_download_summary.json"
DOWNLOAD_SUMMARY_MD = OUTPUTS_DIR / "orderbook_um_81_full_matching_aggtrades_download_summary.md"
FILE_STATUS_CSV = OUTPUTS_DIR / "orderbook_um_81_full_matching_aggtrades_file_status.csv"
SYMBOL_COVERAGE_CSV = OUTPUTS_DIR / "orderbook_um_81_full_matching_aggtrades_symbol_coverage.csv"
BLOCKED_NOT_ACK_MD = OUTPUTS_DIR / "orderbook_um_81_full_matching_aggtrades_BLOCKED_DOWNLOAD_NOT_ACKNOWLEDGED.md"

REQUEST_SLEEP_SECONDS = 0.08
MAX_CHECKSUM_REDOWNLOADS = 2
USER_AGENT = "edge-factory-orderbook-um-81-full-matching-aggtrades-downloader-v1"
ACK_ENV = "AGGTRADES_81_FULL_MATCHING_DOWNLOAD"
WORKERS_ENV = "AGGTRADES_DOWNLOAD_WORKERS"
CONNECT_TIMEOUT_ENV = "AGGTRADES_DOWNLOAD_CONNECT_TIMEOUT"
READ_TIMEOUT_ENV = "AGGTRADES_DOWNLOAD_READ_TIMEOUT"
MAX_RETRIES_ENV = "AGGTRADES_DOWNLOAD_MAX_RETRIES"
DEFAULT_WORKERS = 8
MIN_WORKERS = 1
MAX_WORKERS = 32
...
class DownloadBlocked(RuntimeError):
    """Raised when the full matching aggTrades download must stop closed."""


class InsufficientDiskBlocked(DownloadBlocked):
    """Raised when the verified manifest says the target disk is too small."""
...
def external_paths() -> dict[str, Path]:
    ...
    return {
        ...
        "raw": raw,
        "logs": logs,
        "progress_json": logs / "progress" / "orderbook_um_81_full_matching_aggtrades_progress.json",
        "external_status_csv": logs / "orderbook_um_81_full_matching_aggtrades_file_status.csv",
        "external_status_jsonl": logs / "orderbook_um_81_full_matching_aggtrades_file_status.jsonl",
    }
...
def request_bytes(url: str, settings: DownloadSettings, max_bytes: int = 8192) -> bytes:
    assert_public_aggtrades_url(url.removesuffix(".CHECKSUM"))
    last_error = ""
...
def download_to_part(
    ...
    assert_public_aggtrades_url(url)
...
def write_blocked_not_acknowledged() -> int:
    BLOCKED_NOT_ACK_MD.write_text(
        "\n".join(
            [
                ...
                "",
                "The full matching 81-symbol aggTrades archive download was not started because the acknowledgement environment variable was not set to YES.",
    ...
    print("next_module: ORDERBOOK_UM_81_FULL_MATCHING_AGGTRADES_APPROVAL_OR_BLOCKER_REVIEW")
    return 2


def validate_prerequisites(rows: list[dict[str, str]], validation: dict[str, Any]) -> None:
    if validation.get("status") == "BLOCKED_INSUFFICIENT_DISK_FOR_FULL_MATCHING_AGGTRADES":
        disk = validation.get("disk", {})
        raise InsufficientDiskBlocked(
            "insufficient disk for full matching aggTrades "
            f"(free_gb={disk.get('free_gb')}, required_free_gb={disk.get('required_free_gb')})"
        )
    if validation.get("status") != "PASS_ORDERBOOK_UM_81_FULL_MATCHING_AGGTRADES_MANIFEST_VALIDATED":
    ...
    disk_checks = validation.get("validation_checks", {})
    if disk_checks.get("free_disk_space_ge_remaining_times_1_25") is not True:
        raise InsufficientDiskBlocked("manifest validation did not pass the free disk check")
...
def aggregate_symbol_coverage(rows: list[dict[str, str]], statuses: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for symbol in sorted(by_symbol_expected):
        ...
        dates = [row["file_date"] for row in expected_rows]
...
def compact_progress_payload(
    ...
    return {
        "status": "RUNNING_81_FULL_MATCHING_AGGTRADES_DOWNLOAD",
...
def final_status(summary: dict[str, Any]) -> str:
    if summary["failed_count"] > 0:
        return "FAIL_81_FULL_MATCHING_AGGTRADES_CHECKSUM_OR_DOWNLOAD"
    if summary["checksum_missing_count"] > 0:
        return "PARTIAL_81_FULL_MATCHING_AGGTRADES_DOWNLOAD_RETRY_REQUIRED"
    if summary["checksum_verified_count"] == summary["expected_file_count"]:
        return "PASS_81_FULL_MATCHING_AGGTRADES_DOWNLOAD_VERIFIED"
    return "PARTIAL_81_FULL_MATCHING_AGGTRADES_DOWNLOAD_RETRY_REQUIRED"


def write_summary_md(summary: dict[str, Any]) -> None:
    lines = [
        "# Orderbook UM 81 full matching aggTrades download summary v1",
        "",
        ...
        f"earliest_global_date: {summary.get('earliest_global_date', '')}",
        f"latest_global_date: {summary.get('latest_global_date', '')}",
        ...
        f"2. Re-run run_orderbook_um_81_full_matching_aggtrades_downloader_v1.ps1 with {ACK_ENV}=YES.",
        ...
        "",
        "## Scope",
        "- Public Binance Data Vision USD-M daily aggTrades ZIPs only.",
        "- Only symbol-days that match verified bookDepth coverage are included.",
        "- No older extra aggTrades history outside verified bookDepth coverage is downloaded.",
        "- No private endpoints, account endpoints, order execution, recommendations, or trading logic.",
        "",
        "## Warnings",
    ]
...
def write_failure_summary(
    reason: str,
    partial_statuses: list[dict[str, Any]],
    dirs: dict[str, Path],
    status: str = "FAIL_81_FULL_MATCHING_AGGTRADES_CHECKSUM_OR_DOWNLOAD",
) -> int:
    summary = {
        "status": status,
        ...
        "total_verified_gb": round(
            sum(int(item.get("observed_size_bytes") or 0) for item in partial_statuses if item.get("checksum_verified") is True)
            / 1_000_000_000,
            6,
        ),
        ...
        "failed_urls": [],
        "replacement_checks_all_true": False,
        "next_module": "ORDERBOOK_UM_81_FULL_MATCHING_AGGTRADES_DOWNLOAD_BLOCKER_REVIEW",
    }
...
def main() -> int:
    ...
    try:
        ...
        summary = {
            ...
            "restart_resume_instructions": [
                ...
                "Re-run run_orderbook_um_81_full_matching_aggtrades_downloader_v1.ps1 from the repo root.",
                "Existing ZIPs are skipped only after checksum verification.",
            ],
            "replacement_checks_all_true": False,
            "next_module": "ORDERBOOK_UM_81_FULL_MATCHING_AGGTRADES_DOWNLOAD_VALIDATOR_V1",
        }
        summary["status"] = final_status(summary)
        summary["replacement_checks_all_true"] = summary["status"] == "PASS_81_FULL_MATCHING_AGGTRADES_DOWNLOAD_VERIFIED"
        if summary["status"] != "PASS_81_FULL_MATCHING_AGGTRADES_DOWNLOAD_VERIFIED":
            summary["next_module"] = "ORDERBOOK_UM_81_FULL_MATCHING_AGGTRADES_DOWNLOAD_BLOCKER_REVIEW"
        DOWNLOAD_SUMMARY_JSON.write_text(json.dumps(summary, indent=2, sort_keys=True, ensure_ascii=True) + "\n", encoding="utf-8")
    ...
    except InsufficientDiskBlocked as exc:
        return write_failure_summary(str(exc), statuses, dirs, "BLOCKED_INSUFFICIENT_DISK_FOR_FULL_MATCHING_AGGTRADES")
    except ChecksumMismatchBlocked as exc:
