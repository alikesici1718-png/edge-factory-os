...
$LOG_DIR = "$RUN\startup_logs_v4_runnerfiles"
$RUNNER_DIR = "$RUN\startup_runners_v4"
...
function New-RunnerFile {
    ...
    $runner = "$RUNNER_DIR\$Name.runner.ps1"
...
Write-Host ''
Write-Host 'PROCESS EXITED. ExitCode=' `$code -ForegroundColor Red
...
function Start-VisibleComponent {
    ...
    $runner = New-RunnerFile -Name $Name -Script $Script -ComponentArgs $ComponentArgs

    Write-Host "[START-RUNNER] $Name" -ForegroundColor Green
...
Write-Host ""
Write-Host "Started MASTER_UPPER_SYSTEM v4 runner-file launcher." -ForegroundColor Green
Write-Host "Each component has its own runner .ps1 and visible PowerShell window." -ForegroundColor Yellow
