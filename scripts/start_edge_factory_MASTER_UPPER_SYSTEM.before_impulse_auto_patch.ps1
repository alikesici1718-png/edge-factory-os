# EDGE FACTORY MASTER UPPER SYSTEM STARTER
#
# This launcher passes the sizing contract to all active family loggers.
# Use only after logger sizing audit passes.
#
# Run:
#   powershell -ExecutionPolicy Bypass -File "C:\Users\alike\start_edge_factory_MASTER_UPPER_SYSTEM.ps1"

$PY = "python"
$USERDIR = "C:\Users\alike"
$BASE = "C:\Users\alike\OneDrive\Desktop\edge_lab_new"
$RUN = "$BASE\paper_run_gate_MASTER_UPPER_SYSTEM"
$GATE = "$RUN\global_risk_manager\global_gate_decisions.csv"
...
$mainObj = [ordered]@{
  run_dir = $RUN
  global_out_dir = "$RUN\global_risk_manager"
...
Start-LoggerWindow "GLOBAL RISK MANAGER MASTER_UPPER_SYSTEM" "$PY `"$USERDIR\global_paper_risk_manager_v4_config.py`" --config `"$MAIN_CONFIG`""
...
Start-LoggerWindow "IMPULSE LONG MASTER_UPPER_SYSTEM" "$PY `"$USERDIR\impulse_long_gate_aware_live_paper_logger.py`" --out_dir `"$RUN\live_impulse_event_long_paper`" --global_gate_path `"$GATE`" --require_global_gate --sizing_contract `"$SIZING_CONTRACT`""
...
Write-Host "python `"C:\Users\alike\edge_factory_live_decision_dashboard.py`" --base_dir `"$RUN`""
